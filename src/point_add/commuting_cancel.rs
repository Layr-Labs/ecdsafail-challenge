...
// Include the classical condition so a conditioned gate never matches its
// unconditioned twin. Conditioned gates are ordinary X-family/diagonal ops;
// they are not stream-wide barriers.
            ...
            if let Some(&previous) = last_same.get(&gate_key) {
                ...
                if blocker.is_none_or(|i| i < previous) {
                    ...
                    if matches!(op.kind, K::CCX | K::CCZ) {
                        if depth == 0 && op.c_condition == crate::circuit::NO_BIT {
