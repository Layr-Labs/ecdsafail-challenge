...
/// Unpinned probe census: prints opportunity counters for candidate levers
/// without asserting exact counts. Never a production gate.
#[test]
#[ignore = "q834-kimi experiment probe"]
fn q834_kimi_probe() {
    let data = zstd::stream::decode_all(COMPRESSED_HIR).unwrap();
    let g = parse_graph(&data);
    shared_product::validate_phase_child(&g);
    ...
    let conditional_t = 512*s.predicate_blocks - s.carry_selected
        - s.endpoint_blocks - s.endpoint_outer;
    let expected_2t = 2*static_t - conditional_t;
    let (rmin, rmax, rtotal) = s.proof.retained_stats();
    println!("PROBE ops={} static_T={} conditional_T={} expected_2T={} evictions={} collections={} retained_min={} retained_max={} retained_avg={}",
        s.output_ops + 4*257 - 2, static_t, conditional_t, expected_2t,
        s.proof.evictions(), s.proof.collections(), rmin, rmax,
        if s.proof.collections() > 0 { rtotal / s.proof.collections() } else { 0 });
    println!("PROBE_AFFINE queries={} counts={:?} clean={} one={} residual={:?} support=[{},{},{}] shared_unknown_pool={} retention_fallback={} prefix_unknown={}",
        s.proof.affine_queries, s.proof.affine_counts, s.proof.rewrites, s.one_cleanups,
        s.residual_counts, s.proof.support_dead, s.proof.support_x, s.proof.support_cx,
        s.shared_unknown_pool, s.retention.fallback, s.prefix_unknown);
}
