//! Exact cancellation of identical self-inverse quantum operations, capped at 256 passes.
//!
...
//! fixpoint or 256 productive passes. Qubit 0 (the constant outer control eliminated later) is never a
...
pub(super) fn cancel_identical_pairs(ops: &mut Vec<Op>) -> PeepholeStats {
    ...
    for op in ops.iter() {
        for q in wires(op) {
            if q > 834 { return cancel_identical_pairs_hashed32(ops); }
            nq = nq.max(q as usize + 1);
        }
        if op.c_condition != NO_BIT {
            if op.c_condition.0 > 1281 { return cancel_identical_pairs_hashed32(ops); }
            nb = nb.max(op.c_condition.0 as usize + 1);
        }
        if matches!(op.kind, K::BitInvert | K::BitStore0 | K::BitStore1 | K::Hmr) {
            if op.c_target == NO_BIT || op.c_target.0 > 1281 { return cancel_identical_pairs_hashed32(ops); }
    ...
    loop {
        ...
        eprintln!("PEEPHOLE_PASS pass={} removed={}", stats.passes, removed);
        let mut at = 0;
        ops.retain(|_| {
            let keep = alive[at];
            at += 1;
...
fn cancel_identical_pairs_hashed32(ops: &mut Vec<Op>) -> PeepholeStats {
    let mut stats = PeepholeStats::default();
    let mut alive = vec![true; ops.len()];
    loop {
        stats.passes += 1;
