...
fn configured_chunk(c: &mut Circuit, n: usize) -> Option<usize> {
    ...
    let cap = std::env::var("MIDQ_CHUNK_COMPARE_QCAP")
        ...
        .unwrap_or(QCAP)
        .min(crate::point_add::trailmix_port::inversion::shrunken_pz_state_machine::hybrid_profile::hard_ceiling());
...
fn emit(
    ...
    let cap = std::env::var("MIDQ_CHUNK_COMPARE_QCAP").ok()
        .and_then(|v| v.parse::<usize>().ok()).unwrap_or(QCAP).min(crate::point_add::trailmix_port::inversion::shrunken_pz_state_machine::hybrid_profile::hard_ceiling());
