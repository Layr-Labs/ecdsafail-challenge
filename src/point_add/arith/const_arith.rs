...
fn dead_low_carry_run(c: U256, last: usize, first_carry_is_zero: bool) -> usize {
    // Diagnostic A/B switch. The optimization remains default-on.
    if std::env::var("SUB4_EXACT_DEAD_LOW").ok().as_deref() == Some("0") {
        return 0;
    }
...
/// `cadd_nbit_const_direct_trunc_fast` with provably-dead low carries elided.
...
/// `csub_nbit_const_direct_trunc_fast` with provably-dead low borrows elided.
