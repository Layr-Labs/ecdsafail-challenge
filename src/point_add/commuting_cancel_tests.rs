...
#[test]
fn disjoint_conditioned_ops_are_not_stream_barriers() {
    let pair = gate(K::CCX, &[0, 1, 2]);
    let mut cond_x = gate(K::X, &[5]);
    cond_x.c_condition = BitId(0);
    let (after, stats) = transformed(&[pair, cond_x, pair]);
    assert_eq!(after, vec![cond_x]);
    assert_eq!(stats.removed_ops, 2);
    assert_eq!(stats.toffoli_pairs_unconditional, 1);

    let mut cond_pair = pair;
    cond_pair.c_condition = BitId(0);
    let disjoint = gate(K::X, &[5]);
    let (after, stats) = transformed(&[cond_pair, disjoint, cond_pair]);
    assert_eq!(after, vec![disjoint]);
    assert_eq!(stats.removed_ops, 2);
    assert_eq!(stats.toffoli_pairs_conditioned, 1);

    // Same wires, different conditions, must not match.
    let mut other = pair;
    other.c_condition = BitId(1);
    let before = [cond_pair, disjoint, other];
    let (after, stats) = transformed(&before);
    assert_eq!(after, before);
    assert_eq!(stats.removed_ops, 0);
}
...
fn enclosing_conditions_allow_only_internal_pairs() {
    ...
    // X on the Toffoli target commutes with the pair, condition or not.
    let mut direct = pair;
    direct.c_condition = BitId(0);
    let (after, stats) = transformed(&[direct, middle, direct]);
    assert_eq!(after, vec![middle]);
    assert_eq!(stats.removed_ops, 2);
    assert_eq!(stats.toffoli_pairs_conditioned, 1);
