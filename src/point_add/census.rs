...
fn mirrored_run(
    ...
    flags: &mut [u8],
    counts: &mut [Counts],
    mut dirty_trace: Option<(&[crate::point_add::OpSite], &mut DirtyTrace)>,
) -> (Vec<u64>, Vec<u64>, u64) {
        ...
        match op.kind {
            ...
            OperationType::Hmr | OperationType::R => {
                ...
                if op.kind == OperationType::R {
                    if let Some((sites, trace)) = dirty_trace.as_mut() {
                    let dirty = qubits[op.q_target.0 as usize] & cond;
                    trace.record(sites[op_idx], op_idx, dirty);
                    }
                }
...
pub(crate) fn run(ops: &[Op], sites: Option<&[crate::point_add::OpSite]>) {
    ...
    report(ops, &merged, &totals, lanes, sites);
}

#[derive(Clone, Copy, Default)]
struct SiteStats {
    dirty_lanes: u64,
    first_dirty_lanes: u64,
    first_before_classical: u64,
    first_before_phase: u64,
}

struct DirtyTrace {
    by_site: HashMap<crate::point_add::OpSite, SiteStats>,
    first: [Option<(crate::point_add::OpSite, usize)>; 64],
}
...
impl DirtyTrace {
    ...
    fn record(&mut self, site: crate::point_add::OpSite, op_idx: usize, dirty: u64) {
        if dirty == 0 {
            return;
        }
        self.by_site.entry(site).or_default().dirty_lanes += dirty.count_ones() as u64;
        let mut lanes = dirty;
        while lanes != 0 {
            let lane = lanes.trailing_zeros() as usize;
            lanes &= lanes - 1;
            if self.first[lane].is_none() {
                self.first[lane] = Some((site, op_idx));
                self.by_site.entry(site).or_default().first_dirty_lanes += 1;
            }
        }
    }
}

fn classical_failure_mask(
    regs: &[Vec<QubitOrBit>],
    qubits: &[u64],
    bits: &[u64],
    expected: &[(U256, U256)],
    ...
    for (shot, want) in expected.iter().enumerate() {
        let got = (
            read_register(&regs[0], qubits, bits, shot),
            read_register(&regs[1], qubits, bits, shot),
        );
        if got != *want {
            mask |= 1u64 << shot;
        }
...
/// Attribute the first dirty reset on each failing lane to its source emission site.
///
/// This runs on the constprop output, before later exact geometry rewrites, while the
/// `TRACE_OP_SITES` vector is still aligned one-to-one with the operation stream. It is
/// diagnostic-only and is never reached by an ordinary scoring build.
pub(crate) fn run_sites(ops: &[Op], sites: &[crate::point_add::OpSite]) {
    assert_eq!(sites.len(), ops.len(), "site trace must align with the operation stream");
    let rounds = std::env::var("TLM_SITE_CENSUS_ROUNDS")
        .ok()
        .and_then(|s| s.parse::<u64>().ok())
        .unwrap_or(64);
    let offset = std::env::var("TLM_CENSUS_ROUND_OFFSET")
        .ok()
        .and_then(|s| s.parse::<u64>().ok())
        .unwrap_or(0);
    let (num_q, num_b, _nregs, regs) = crate::circuit::analyze_ops(ops.iter());
    assert_eq!(regs.len(), 4, "site census expects the 4 harness registers");
    let (num_q, num_b) = (num_q as usize, num_b as usize);
    let ngates = count_gates(ops);
    let mut flags = vec![0u8; ngates];
    let mut counts = vec![Counts::default(); ngates];
    let mut totals: HashMap<crate::point_add::OpSite, SiteStats> = HashMap::new();
    let mut classical_total = 0u64;
    let mut phase_total = 0u64;
    let mut classical_without_dirty = 0u64;

    eprintln!(
        "SITE_CENSUS start: ops={} rounds={} lanes={} offset={}",
        ops.len(),
        rounds,
        rounds * 64,
        offset,
    ...
    for round in 0..rounds {
        let seed = round + offset;
        let mut seed_xof = measure_xof();
        let mut seeder = Simulator::new(num_q, num_b, &mut seed_xof);
        let expected = seed_lanes(&mut seeder, &regs, seed);
        let q0 = seeder.qubits.clone();
        let b0 = seeder.bits.clone();
        drop(seeder);

        let mut trace = DirtyTrace::new();
        let mut xof = measure_xof_seeded(seed + 0x9e37_79b9);
        let (fq, fb, phase) = mirrored_run(
            ops,
            &q0,
            &b0,
            &mut xof,
            &mut flags,
            &mut counts,
            Some((sites, &mut trace)),
        );
        let classical = classical_failure_mask(&regs, &fq, &fb, &expected);
        classical_total += classical.count_ones() as u64;
        phase_total += phase.count_ones() as u64;

        for (site, stats) in trace.by_site {
            let out = totals.entry(site).or_default();
            out.dirty_lanes += stats.dirty_lanes;
            out.first_dirty_lanes += stats.first_dirty_lanes;
        }
        for lane in 0..64 {
            let bit = 1u64 << lane;
            if let Some((site, _op_idx)) = trace.first[lane] {
                let out = totals.entry(site).or_default();
                if classical & bit != 0 {
                    out.first_before_classical += 1;
                }
                if phase & bit != 0 {
                    out.first_before_phase += 1;
                }
            } else if classical & bit != 0 {
                classical_without_dirty += 1;
            }
        }
    }
    ...
    rows.sort_by(|a, b| {
        b.1.first_before_classical
            .cmp(&a.1.first_before_classical)
            .then_with(|| b.1.first_dirty_lanes.cmp(&a.1.first_dirty_lanes))
            .then_with(|| a.0.cmp(&b.0))
    });
    eprintln!(
        "SITE_CENSUS faults: classical={} phase={} classical_without_prior_dirty={}",
        classical_total, phase_total, classical_without_dirty,
    );
    // Default 32 rows keeps the original behavior; Task 6's per-block LSBS search needs to
    // see past the handful of large-magnitude aggregate sites (the walk-end register frees,
    // which are not iteration-attributable -- see src/point_add/memory/06-census-and-lambda.md
    // and the Task 6 report) to the smaller-magnitude, iteration-attributed fold-ancilla sites
    // underneath them, so the cap is overridable rather than hardcoded.
    let top_n: usize = std::env::var("TLM_SITE_CENSUS_TOP")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(32);
    for ((file, line, context), stats) in rows.into_iter().take(top_n) {
        eprintln!(
            "SITE_CENSUS first_dirty classical={} phase={} first={} dirty={} site={}:{} context=0x{:08x}",
            stats.first_before_classical,
            stats.first_before_phase,
            stats.first_dirty_lanes,
            stats.dirty_lanes,
            file,
            line,
            context,
...
///
/// task-7 Step 5 ("reject shallow discoveries"): when `sites` is available and
/// `TLM_CENSUS_BASELINE_SITES=<path>` names a Step 2 baseline-ancestry CSV
/// (`deep_strip_sites::export_baseline_sites`'s output), a gate that looks dead or
/// downgradable in THIS sample only survives into `dead`/`down` if its `StableSiteKey`
/// (computed the same way, over THIS stream) is ALSO present in that baseline set --
/// i.e. it was independently certified dead/downgradable against the much larger original
/// census, at the exact same source call site. A gate that looks dead here but lacks that
/// ancestry is silently excluded, `rejected_no_baseline_ancestry` just counts it so the
/// filter's effect is visible in the log.
fn report(ops: &[Op], flags: &[u8], counts: &[Counts], lanes: u64, sites: Option<&[crate::point_add::OpSite]>) {
    ...
    let baseline_path = std::env::var_os("TLM_CENSUS_BASELINE_SITES");
    if baseline_path.is_some() {
        assert!(
            sites.is_some(),
            "TLM_CENSUS_BASELINE_SITES requires OpSite data (set TRACE_OP_SITES, which \
             point_add::build() does automatically whenever this env var is set)"
        );
    }
    let stable_keys: Option<Vec<Option<crate::point_add::deep_strip_sites::StableSiteKey>>> =
        sites.map(|s| crate::point_add::deep_strip_sites::compute_stable_site_keys(ops, s));
    let baseline_set: Option<std::collections::HashSet<(String, u32, u32, u8, u32)>> =
        baseline_path
            .as_ref()
            .map(|p| crate::point_add::deep_strip_sites::load_baseline_key_set(&p.to_string_lossy()));
    ...
    let mut g = 0usize;
    let mut rejected_no_baseline_ancestry = 0usize;
    for (i, op) in ops.iter().enumerate() {
        ...
        *o += 1;
        ...
        if let Some(set) = baseline_set.as_ref() {
            let has_ancestry = stable_keys
                .as_ref()
                .and_then(|sk| sk[i])
                .map(|(file, line, context, kind, ord)| {
                    set.contains(&(file.to_string(), line, context, kind, ord))
                })
                .unwrap_or(false);
            if !has_ancestry {
                // Only worth counting (and only reachable in a meaningful way) for a gate
                // that would otherwise have looked dead/downgradable in this sample --
                // this is exactly the "shallow discovery" the inner join must reject.
                if f & F_FIRED == 0 || f & F_C1_NOT_C2 == 0 || f & F_C2_NOT_C1 == 0 {
                    rejected_no_baseline_ancestry += 1;
                }
                continue;
            }
        }
    ...
    if baseline_set.is_some() {
        eprintln!(
            "CENSUS baseline-ancestry filter: rejected {} shallow (no-baseline-ancestry) \
             candidates that looked dead/downgradable in this sample",
            rejected_no_baseline_ancestry
        );
    }
