...
kernel void score(global const uint*in,global uint*out,uint jobs,uint start,uint count){
 ...
 uint lo=in[jobs+id],hi=in[2*jobs+id],n=in[3*jobs+id],split=in[6*jobs+id];
 uint old=0,cost=0,oldh=99,have=0,lastm=0,lastv=0,loads=0;
 for(uint i=1;i<n;i++){
  uint h=(i>=2&&i<=257)?(257-i)/64:99;
  if(h!=oldh){if(oldh!=99)old++;if(h!=99)old++;oldh=h;}
  if(i<2||i>257)continue;uint value=257-i;if(value<lo||value>=hi)continue;
  old+=tof(7);
  uint left=max(lo,h*64),right=min(hi,(h+1)*64),low=0;
  for(uint b=0;b<6;b++)if((left>>b)!=((right-1)>>b))low|=1u<<b;
  uint pm=(lo/64==(hi-1)/64)?0:192u;
  uint m=(low&~((1u<<split)-1))|pm,v=value&m;
  if(!have||m!=lastm||v!=lastv){if(have)cost+=tof(popcount(lastm));cost+=tof(popcount(m));lastm=m;lastv=v;have=1;loads++;}
  cost+=tof(1+popcount(low&((1u<<split)-1)));
 }
 if(oldh!=99)old++;if(have)cost+=tof(popcount(lastm));
 out[id*4]=old*2;out[id*4+1]=cost*2;out[id*4+2]=loads;out[id*4+3]=have;
}
)CLC";
struct Device{
...
int main(int argc,char**argv){
 assert(argc==3);std::ifstream f(argv[1]);unsigned jobs;f>>jobs;assert(jobs==203*7);std::vector<unsigned>input(jobs*8);for(unsigned j=0;j<jobs;j++)for(unsigned col=0;col<8;col++)f>>input[col*jobs+j];assert(f);
 ...
 std::cout<<"R00_LIFETIME_GPU_PASS jobs="<<jobs<<" both_B50=true CPU_scores_not_used_for_selection=true"<<std::endl;
}
