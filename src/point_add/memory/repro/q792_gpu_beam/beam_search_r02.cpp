...
kernel void assess(global const Job*jobs,global const ushort*plans,global uint*out,ulong start,ulong count,global const uint*cost){
 ulong lid=get_global_id(0);if(lid>=count)return;ulong gid=start+lid;Job j=jobs[gid];uint row=gid;uint len=rd(plans,row,0),pol=rd(plans,row,1);ulong a=j.truth;uint ft=0;
...
struct Device{
 ...
 Device(cl_device_id id,unsigned ix,const std::vector<Plan>&p):d(id),index(ix){cl_int e;cl_device_type type;ck(clGetDeviceInfo(d,CL_DEVICE_TYPE,sizeof(type),&type,nullptr),"type");assert(type==CL_DEVICE_TYPE_GPU);std::string name=info(d,CL_DEVICE_NAME);assert(name.find("B50")!=std::string::npos);std::cout<<"DEVICE "<<index<<" "<<name<<" driver="<<info(d,CL_DRIVER_VERSION)<<std::endl;ctx=clCreateContext(nullptr,1,&d,nullptr,nullptr,&e);ck(e,"context");q=clCreateCommandQueue(ctx,d,CL_QUEUE_PROFILING_ENABLE,&e);ck(e,"queue");
  ...
  for(unsigned l=0;l<2;l++){pr[l]=clCreateProgramWithSource(ctx,1,&kernel,nullptr,&e);ck(e,"program");std::string opts="-cl-std=CL1.2 -DLAYOUT="+std::to_string(l)+" -DPLAN_ROWS="+std::to_string(p.size());e=clBuildProgram(pr[l],1,&d,opts.c_str(),nullptr,nullptr);if(e){size_t n;clGetProgramBuildInfo(pr[l],d,CL_PROGRAM_BUILD_LOG,0,nullptr,&n);std::string log(n,' ');clGetProgramBuildInfo(pr[l],d,CL_PROGRAM_BUILD_LOG,n,log.data(),nullptr);std::cerr<<log<<std::endl;}ck(e,"build");kn[l]=clCreateKernel(pr[l],"assess",&e);ck(e,"kernel");pl[l]=clCreateBuffer(ctx,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,p.size()*STRIDE*2,l?static_cast<void*>(soa.data()):const_cast<Plan*>(p.data()),&e);ck(e,"plans");}
 }
 ~Device(){for(unsigned i=0;i<2;i++){clReleaseMemObject(pl[i]);clReleaseKernel(kn[i]);clReleaseProgram(pr[i]);}if(jobs)clReleaseMemObject(jobs);if(out)clReleaseMemObject(out);if(ct)clReleaseMemObject(ct);clReleaseCommandQueue(q);clReleaseContext(ctx);}
...
bool lessplan(const Plan&a,const Plan&b){if(a.len!=b.len)return a.len<b.len;if(a.pol!=b.pol)return a.pol<b.pol;return a.gate<b.gate;}
bool eqplan(const Plan&a,const Plan&b){return !lessplan(a,b)&&!lessplan(b,a);}
std::vector<std::string> split(std::string s,char c){std::vector<std::string>v;std::stringstream f(s);std::string x;while(std::getline(f,x,c))v.push_back(x);if(!s.empty()&&s.back()==c)v.emplace_back();return v;}
std::vector<Plan> neighbors(unsigned n,const std::vector<Plan>&beam){
 std::vector<unsigned>gates;for(unsigned t=0;t<n;t++){for(unsigned a=0;a<n;a++)if(a!=t)gates.push_back(a<<3|t);for(unsigned a=0;a<n;a++)for(unsigned b=a+1;b<n;b++)if(a!=t&&b!=t)gates.push_back(512|a<<6|b<<3|t);}
 std::vector<Plan>out;for(auto p:beam){out.push_back(p);for(unsigned b=0;b<n;b++){auto q=p;q.pol^=1<<b;out.push_back(q);}for(unsigned i=0;i<p.len;i++){auto q=p;q.len--;for(unsigned j=i;j<q.len;j++)q.gate[j]=q.gate[j+1];q.gate[q.len]=0;out.push_back(q);for(auto g:gates)if(g!=p.gate[i]){q=p;q.gate[i]=g;out.push_back(q);}}
 if(p.len<16)for(unsigned i=0;i<=p.len;i++)for(auto g:gates){auto q=p;q.len++;for(unsigned j=p.len;j>i;j--)q.gate[j]=q.gate[j-1];q.gate[i]=g;out.push_back(q);}}
 std::sort(out.begin(),out.end(),lessplan);out.erase(std::unique(out.begin(),out.end(),eqplan),out.end());return out;
}
int main(int argc,char**argv){assert(argc==4);std::string prefix=argv[1];load_tables(argv[3]);std::ifstream f(argv[2]);assert(f);std::string line;std::getline(f,line);std::vector<Job>base;std::vector<std::vector<Plan>>beam;std::vector<unsigned>original;
 while(std::getline(f,line)){auto v=split(line,'\t');assert(v.size()==10);Job j{std::stoull(v[2],nullptr,16),unsigned(std::stoul(v[0])),unsigned(std::stoul(v[1]))};if(base.empty()||base.back().n!=j.n||base.back().k!=j.k||base.back().truth!=j.truth){base.push_back(j);beam.emplace_back();original.push_back((std::stoul(v[4])<<16)|std::stoul(v[5]));}Plan p{};p.len=std::stoul(v[6]);p.pol=std::stoul(v[7]);auto gs=split(v[8],',');assert(gs.size()==p.len);for(unsigned i=0;i<p.len;i++)p.gate[i]=std::stoul(gs[i]);beam.back().push_back(p);}
 assert(base.size()==278);for(auto&b:beam)assert(b.size()==8);
 cl_uint np;ck(clGetPlatformIDs(0,nullptr,&np),"platforms");std::vector<cl_platform_id>ps(np);ck(clGetPlatformIDs(np,ps.data(),nullptr),"platform ids");std::vector<cl_device_id>ids;for(auto platform:ps){cl_uint n=0;cl_int e=clGetDeviceIDs(platform,CL_DEVICE_TYPE_GPU,0,nullptr,&n);if(e==CL_DEVICE_NOT_FOUND)continue;ck(e,"device count");std::vector<cl_device_id>v(n);ck(clGetDeviceIDs(platform,CL_DEVICE_TYPE_GPU,n,v.data(),nullptr),"devices");for(auto d:v)if(info(d,CL_DEVICE_NAME).find("B50")!=std::string::npos)ids.push_back(d);}assert(ids.size()==2&&ids[0]!=ids[1]);
 U total=0;unsigned checks=0;for(unsigned round=1;round<=6;round++){
  std::vector<Plan>plans;std::vector<Job>jobs;std::vector<size_t>offsets{0};for(unsigned i=0;i<base.size();i++){auto v=neighbors(base[i].n,beam[i]);plans.insert(plans.end(),v.begin(),v.end());jobs.insert(jobs.end(),v.size(),base[i]);offsets.push_back(plans.size());}assert(plans.size()<10000000);std::cout<<"ROUND_START round="<<round<<" candidates="<<plans.size()<<" host_data_bytes="<<(plans.size()*(sizeof(Plan)+sizeof(Job)+4))<<std::endl;
  std::vector<unsigned>out(plans.size());double wall=0,ms0=0,ms1=0;{Device a(ids[0],0,plans),b(ids[1],1,plans);size_t half=plans.size()/2;a.load(jobs,half);b.load(jobs,plans.size()-half);auto start=Clock::now();a.launch(1,256,0,half);b.launch(1,256,half,plans.size()-half);a.read(out.data(),half);b.read(out.data()+half,plans.size()-half);wall=std::chrono::duration<double,std::milli>(Clock::now()-start).count();ms0=a.lastms;ms1=b.lastms;}
  total+=plans.size();for(size_t i=0;i<plans.size();i+=4093){assert(out[i]==score(jobs[i],plans[i]));checks++;}
  unsigned changed=0,betterT=0;long gainT=0;std::ofstream result(prefix+"-round"+std::to_string(round)+"-best.tsv");assert(result);result<<"n\tk\ttruth\tcandidate\tT\tN\tdepth\tpolarity\tgates\tcubes\n";
  for(unsigned i=0;i<base.size();i++){std::vector<size_t>order(offsets[i+1]-offsets[i]);std::iota(order.begin(),order.end(),offsets[i]);std::partial_sort(order.begin(),order.begin()+8,order.end(),[&](size_t a,size_t b){return out[a]!=out[b]?out[a]<out[b]:lessplan(plans[a],plans[b]);});std::vector<Plan>next;for(unsigned z=0;z<8;z++){size_t ci=order[z];auto&p=plans[ci];auto&j=base[i];unsigned v=out[ci];assert(v==score(j,p));exact(j,p);checks++;next.push_back(p);result<<j.n<<'\t'<<j.k<<'\t'<<std::hex<<j.truth<<std::dec<<'\t'<<ci<<'\t'<<(v>>16)<<'\t'<<(v&65535)<<'\t'<<p.len<<'\t'<<p.pol<<'\t';for(unsigned k=0;k<p.len;k++){if(k)result<<',';result<<p.gate[k];}result<<'\t';bool first=true;for(auto[m,b]:selected(j,p)){if(!first)result<<',';first=false;result<<m<<':'<<b;}result<<'\n';}
   bool same=true;for(unsigned z=0;z<8;z++)same&=eqplan(beam[i][z],next[z]);changed+=!same;beam[i]=next;unsigned v=out[order[0]];assert(v<=original[i]);betterT+=(v>>16)<(original[i]>>16);gainT+=int(original[i]>>16)-int(v>>16);
  }
  std::cout<<"GPU_BEAM_ROUND_PASS round="<<round<<" candidates="<<plans.size()<<" total="<<total<<" wall_ms="<<wall<<" gpu0_ms="<<ms0<<" gpu1_ms="<<ms1<<" changed_beams="<<changed<<" better_T_jobs="<<betterT<<" unweighted_T_gain="<<gainT<<" CPU_checks_total="<<checks<<" all_selected_truths_exact=true"<<std::endl;if(changed==0)break;
 }std::cout<<"GPU_BEAM_SEARCH_PASS total_candidates="<<total<<" CPU_checks="<<checks<<" both_B50=true"<<std::endl;
}
