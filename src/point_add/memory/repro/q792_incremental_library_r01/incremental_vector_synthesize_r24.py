...
R=Path(__file__).resolve().parent;O=R/'incremental-vector-r24';F=Path(json.loads((R/'PLAN.json').read_text())['base'])/'gpu/fixtures-r01-whole-step';sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
...
def prove_synthesis(ops,n,delta):
 ...
 for i in range(n):assert state[i]==({1<<i}^set(delta.get(str(i),[]))),(i,ops,delta)
def linear_factor(rows,n):
 rows=rows[:];inv=[1<<i for i in range(n)];reduction=[]
 def cx(a,t):rows[t]^=rows[a];inv[t]^=inv[a];reduction.append([1,t,a,0])
 for i in range(n):
  pivot=next((j for j in range(i,n)if rows[j]>>i&1),None)
  if pivot is None:return None
  if pivot!=i:cx(i,pivot);cx(pivot,i);cx(i,pivot)
  for j in range(n):
   if j!=i and rows[j]>>i&1:cx(i,j)
 assert rows==[1<<i for i in range(n)]
 return inv,reduction[::-1]
for j in range(4):
 ...
 for ix,r in enumerate(rows):
  ...
  fs,lin=factor(q,n);assert len(fs)==cost
  u=r['direction'];L=[v^(lin if u>>i&1 else 0)for i,v in enumerate(r['linear'])];z=linear_factor(L,n)
  if z is None:stats['singular_linear_rejected']+=1;continue
  inverse,linear_ops=z;v=sum(((row&u).bit_count()%2)<<i for i,row in enumerate(inverse));new=[]
  if any((a&v).bit_count()%2 or(b&v).bit_count()%2 for a,b in fs):stats['nontransvection_rejected']+=1;continue
  for a,b in fs:new.extend(emit((a,b,v),n))
  new.extend(linear_ops);new.extend([0,i,0,0]for i in bits(r['constant']));new=cancel(new);nd=len(new)-2
  if nd>64:stats['N_cap_rejected']+=1;continue
  prove_synthesis(new,n,r['delta']);stats['CPU_full_state_proofs']+=1
  normalized=new;ids=r['ids'];new=[[k,ids[t],ids[a]if k else 0,ids[b]if k==2 else 0]for k,t,a,b in new]
  accepted.append({**r,'new':new,'normalized_new':normalized,'same_controls':False,'nonlinear_input_update':True,'new_T':cost,'saving_T':2-cost,'N_delta':nd,'proof':'Full vector correction propagated through every interior NCT including reads of initial target; exact symbolic full synthesis state; independent GPU/CPU quadratic rank; all inputs arbitrary'})
