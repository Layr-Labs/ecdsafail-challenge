...
| `07-lambda-is-the-only-gate.md` | `grind.rs` (λ + nonce grind along the real Fiat-Shamir path), the fault-channel split proving the phase channel is laundered classical error, and **corrections to three stale claims in 01–05** — read this before trusting `notes/02`'s λ budget |
...
The fourth, from `07`: **the phase channel is mostly laundered classical error** (an `R` on a dirtied ancilla
flips phase with p=½), so fixing the classical channel alone would take λ from 19.3 to ≈3. That is the only
remaining lever with real leverage — and `07` also corrects three claims in `01`–`05` that no longer hold.
