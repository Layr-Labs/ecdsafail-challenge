...
use std::sync::Arc;
use super::q792_local_pack_r01::Program;
...
fn fiat_shamir_seed(ops: &Program) -> sha3::Shake256Reader {
    ...
    ops.visit(|slice|{for op in slice {
...
fn run_tests(
    ...
    for batch in 0..num_batches {
        ...
        ops.visit(|slice|sim.apply_iter(slice.iter()));
...
pub(super) fn run() {
    ...
    let program=Program { blocks:b.q792_packed_blocks.take().unwrap(),len:b.counted_ops };
    assert!(b.compact_blocks.take().unwrap().is_empty());
    assert_eq!(program.blocks.iter().map(|x|x.len()).sum::<usize>(),program.len());
    let mut unique=std::collections::HashSet::new();let mut stored_bytes=0usize;
    for block in &program.blocks{if unique.insert(Arc::as_ptr(block)){stored_bytes+=block.bytes();}}
    eprintln!("COMPACT_BUILT expanded_ops={} blocks={} unique_blocks={} stored_bytes={} elapsed={:?}; local storage only, official raw ABI56",program.len(),program.blocks.len(),unique.len(),stored_bytes,started.elapsed());
    let(mut nq,mut nb,mut nr)=(0,0,0);let mut regs:Vec<Vec<QubitOrBit>>=Vec::new();
    program.visit(|slice|{let(q,b,r,local)=analyze_ops(slice.iter());nq=nq.max(q);nb=nb.max(b);nr=nr.max(r);regs.resize_with(regs.len().max(local.len()),Vec::new);for(i,reg)in local.into_iter().enumerate(){regs[i].extend(reg);}});
    assert_eq!(nq,792);assert_eq!(nr,4);assert_eq!(regs.len(),4);
