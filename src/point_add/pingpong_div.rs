...
fn rounds_for(direction: PingPongDirection) -> usize {
    match direction {
        ...
        PingPongDirection::Multiply => {
            ...
            tuned_window("SUB4_PP_ROUNDS_MUL", &SLOT, 720)
        }
    }
}
...
fn rounds() -> usize {
    ...
    tuned_window("SUB4_PP_ROUNDS", &SLOT, 720)
}
...
fn replay_chunk() -> usize {
    static SLOT: std::sync::OnceLock<usize> = std::sync::OnceLock::new();
    tuned_window("SUB4_PP_REPLAY_CHUNK", &SLOT, 256)
}
...
fn replay_chunk_compare() -> usize {
    ...
    tuned_window("SUB4_PP_REPLAY_CHUNK_COMPARE", &SLOT, 256)
}

fn replay_fold_window() -> usize {
    static SLOT: std::sync::OnceLock<usize> = std::sync::OnceLock::new();
    tuned_window("SUB4_PP_REPLAY_FOLD_WINDOW", &SLOT, 256)
}
...
fn endpoint_fold_window() -> usize {
    static SLOT: std::sync::OnceLock<usize> = std::sync::OnceLock::new();
    tuned_window("SUB4_PP_ENDPOINT_FOLD_WINDOW", &SLOT, 230)
}

fn replay_flag_compare() -> usize {
    static SLOT: std::sync::OnceLock<usize> = std::sync::OnceLock::new();
    tuned_window("SUB4_PP_REPLAY_FLAG_COMPARE", &SLOT, 256)
}
...
fn sched_bias() -> i32 {
    ...
    *SLOT.get_or_init(|| {
        std::env::var("SUB4_PP_SCHED_BIAS").ok().and_then(|v| v.parse().ok()).unwrap_or(251)
    })
}
...
fn plan(rounds: usize) -> Option<Plan> {
    if std::env::var_os("SUB4_PP_NO_INTERLEAVE")
        .map(|v| v != "0")
        .unwrap_or(true)
    {
        return None;
