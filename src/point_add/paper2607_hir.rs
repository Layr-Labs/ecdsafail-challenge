...
impl State {
    ...
    fn assert_accounting(&self, source_ops: usize, declarations: usize) {
        assert_eq!(self.input_ops, source_ops + 3 * self.predicate_blocks - 35 * self.prefix_rewrites);
        let expected = (self.input_ops as isize + self.proof.rewrites as isize - self.proof.support_dead as isize + self.selected_delta) as usize;
...
pub(super) fn build() -> Vec<Op> {
    ...
    eprintln!("MOTIF_REFINEMENT: matched={:?} selected={:?} t_saved={} output_delta={} skipped_cleanups={} skipped_support={:?}",state.motif_counts,state.selected_counts,state.selected_t_saving,state.selected_delta,state.skipped_cleanups,state.skipped_support);
    eprintln!("SUPPORT_LOWERING: dead={} x={} cx={}",state.proof.support_dead,state.proof.support_x,state.proof.support_cx);
    eprintln!(
        "CLEAN_AND_MBU: semantic_rewrites={} emitted_rewrites={} extra_bits=1 cache_evictions={}",
...
#[cfg(test)]
#[test]
#[ignore = "source-bound count census: run only under the research compute gate"]
fn motif_source_census() {
    ...
    state.motif_enabled = true;
    state.motif_exclusions = motif::exclusions(&graph);
    expand_node(&graph, &mut state, graph.root, 0, graph.root_qubits, 0, graph.root_bits);
    state.flush_raw();
    assert!(state.out.is_empty() && state.proof.balanced());
    ...
    let expected_t = static_t - 512 * state.predicate_blocks / 2;
    assert_eq!(expected_t, 63_795_307);
    assert_eq!(state.selected_counts, [170, 340, 11701]);
    assert_eq!(state.selected_t_saving, 12302);
    assert_eq!(state.selected_delta, -47903);
    assert_eq!(state.input_ops, 401952311);
    assert_eq!(state.output_ops, 403693090);
    assert_eq!(state.proof.rewrites, 1790307);
    assert_eq!(state.skipped_cleanups, 11610);
    assert_eq!(state.skipped_support, [0, 0, 0]);
    assert_eq!(state.predicate_blocks, 1284);
    assert_eq!(state.prefix_rewrites, 7421);
    assert_eq!(state.shared_rewrites, 4229758);
    ...
    eprintln!("MOTIF_CENSUS expected_t={expected_t} static_t={static_t} input={} output={} selected={:?} delta={} semantic_trace={:016x}{:016x} fingerprint={}", state.input_ops,state.output_ops,state.selected_counts,state.selected_delta,state.trace[0],state.trace[1],state.proof.diagnostic_fingerprint());
}
