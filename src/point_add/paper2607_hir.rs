...
#[cfg(test)]
mod combined_count_tests {
    use super::*;

    #[test]
    #[ignore = "root-gated sole combined-source census; count-only, no operation vector"]
    fn production_default_count_only_census() {
        assert!(std::env::var_os("CLEAN_AND_EXPERIMENT_CACHE_CAP").is_none());
        assert!(std::env::var_os("PAPER2607_KEEP_CONTROL").is_none());
        let destination = std::env::var("CLEAN_AND_CENSUS_PATH")
            .expect("explicit census output path required");
        ...
        let n = graph.root_bits / 5;
        assert_eq!(n, 256);
        assert_eq!(graph.root_qubits, 835);
        let summary = graph.summaries[graph.root];
        eprintln!("COMBINED_CENSUS_START pid={} hir_bytes={} nodes={}",
            std::process::id(), decoded.len(), graph.nodes.len());
        let qinputs: Vec<_> = (1..=2 * n).collect();
        let cinputs: Vec<_> = (0..2 * n).collect();
        let mut state = State::new(graph.root_qubits, graph.root_bits,
            &qinputs, &cinputs, false, 0);
        assert_eq!(state.proof.cache_cap(), 2_097_152);
        expand_node(&graph, &mut state, graph.root, 0, graph.root_qubits,
            0, graph.root_bits);
        state.flush_raw();
        assert!(matches!(state.pending_hmr, PendingHmr::None));
        assert!(state.proof.balanced());
        ...
        assert_eq!(state.predicate_blocks, 1_284);
        // Each validated predicate block emits 256 forward plus 256 inverse
        // CCX under its HMR bit. The proof's observed conditional CCX counter
        // must agree; these CCX are not eligible for clean-AND replacement.
        assert_eq!(512 * state.predicate_blocks, 657_408);
        assert_eq!(state.proof.conditional_ccx, 657_408);
        assert_eq!(state.input_ops, summary.output_ops + 3 * state.predicate_blocks);
        assert_eq!(state.output_ops, state.input_ops + state.proof.rewrites as usize);
        assert_eq!(state.input_hist.iter().sum::<usize>(), state.input_ops);
        assert_eq!(state.output_hist.iter().sum::<usize>(), state.output_ops);
        assert_eq!(state.outer_toggles, 2);
        let declarations = 4 * (n + 1);
        let candidate_ops = state.output_ops + declarations - state.outer_toggles;
        // Histograms observe the existing production stream before its final
        // outer-control specialization and exclude register declarations.
        // No executed/average Toffoli metric is inferred from this census.
        let report = format!(concat!(
            "{{\n  \"status\": \"count-only combined source; no full evaluator\",\n",
            "  \"cache_entry_cap\": {},\n  \"experiment_override_present\": false,\n",
            "  \"unconditional_ccx_rewrites\": {},\n  \"cache_evictions\": {},\n",
            "  \"predicate_blocks\": {},\n  \"conditional_predicate_ccx\": {},\n",
            "  \"candidate_qubits\": {},\n  \"candidate_classical_bits\": {},\n",
            "  \"original_hir_ops\": {},\n  \"input_ops_before_clean_and\": {},\n",
            "  \"output_ops_after_clean_and\": {},\n  \"candidate_ops\": {},\n",
            "  \"register_declaration_ops\": {},\n  \"outer_control_toggles_removed\": {},\n",
            "  \"outer_control_toffolis_lowered\": {},\n  \"raw_ccx_seen\": {},\n",
            "  \"input_kind_histogram\": {:?},\n  \"output_kind_histogram\": {:?},\n",
            "  \"histogram_scope\": \"after predicate transform; before outer specialization; excludes declarations\",\n",
            "  \"hir_decoded_bytes\": {},\n  \"hir_nodes\": {},\n",
            "  \"retained_operation_count\": {},\n  \"retained_operation_capacity\": {},\n",
            "  \"emission_vec_payload_bytes\": {},\n  \"seconds\": {:.6},\n",
            "  \"combined_average_toffoli\": null\n}}\n"),
            state.proof.cache_cap(), state.proof.rewrites, state.proof.evictions(),
            state.predicate_blocks, state.proof.conditional_ccx,
            graph.root_qubits - 1, graph.root_bits + 2, summary.output_ops,
            state.input_ops, state.output_ops, candidate_ops, declarations,
            state.outer_toggles, state.outer_lowered, state.proof.ccx_seen,
            state.input_hist, state.output_hist, decoded.len(), graph.nodes.len(),
            state.out.len(), state.out.capacity(), candidate_ops * std::mem::size_of::<Op>(),
