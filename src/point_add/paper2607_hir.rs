...
#[path="half_square_fold.rs"] mod half_square_fold;
...
#[cfg(test)]
#[path="half_square_arch_tests.rs"] mod half_square_arch_tests;
#[cfg(test)]
#[path="half_square_emit_tests.rs"] mod half_square_emit_tests;
#[cfg(test)]
#[path="half_square_extract_tests.rs"] mod half_square_extract_tests;
...
pub(super) fn build() -> Vec<Op> {
    ...
    let decoded = half_square_fold::maybe_replace(decoded);
    drop(original);
    ...
    state.extend_retention_nodes(graph.nodes.len());
    state.family24_locations.resize_with(graph.nodes.len(), Vec::new);
    state.family36_locations.resize_with(graph.nodes.len(), Vec::new);
    state.family63_locations.resize_with(graph.nodes.len(), Vec::new);
    ...
    assert_eq!(
        state.predicate_blocks,
        if graph.root == 2274 { 1028 } else { 1284 },
        "unexpected predicate-cleanup count for selected HIR root",
    );
