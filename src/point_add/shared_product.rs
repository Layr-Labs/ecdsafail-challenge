...
mod tests {
    ...
    fn shared_product_full_source_census() {
        ...
        assert_eq!(state.input_ops,graph.summaries[graph.root].output_ops+3*1284-35*state.prefix_rewrites);
        assert_eq!(state.output_ops,graph.summaries[graph.root].output_ops + state.proof.rewrites as usize + 3*state.predicate_blocks - 35*state.prefix_rewrites);
        ...
        let report=format!("{{\"status\":\"source census only; unchanged9024 replay pending\",\"hits\":{},\"shared_rewrites\":{},\"unknown_pool\":{},\"clean_and_rewrites\":{},\"predicate_blocks\":{},\"static_toffoli\":{static_t},\"expected_toffoli\":{expected_t},\"ops\":{},\"qubits\":834,\"seconds\":{}}}",state.shared_hits,state.shared_rewrites,state.shared_unknown_pool,state.proof.rewrites,state.predicate_blocks,state.output_ops+4*257-2,start.elapsed().as_secs_f64());
