...
fn configured_retain_budget(cap: usize) -> usize {
    // Experiment-only override; production always retains cap/2.
    #[cfg(test)]
    if let Some(raw) = std::env::var_os("CLEAN_AND_EXPERIMENT_RETAIN_BUDGET") {
        return raw
            .to_str()
            .expect("retain budget must be UTF-8")
            .parse::<usize>()
            .expect("retain budget must be an integer");
    }
    cap / 2
}

fn configured_collect_headroom() -> usize {
    // Experiment-only override; production always collects with 16 slots left.
    #[cfg(test)]
    if let Some(raw) = std::env::var_os("CLEAN_AND_EXPERIMENT_HEADROOM") {
        return raw
            .to_str()
            .expect("headroom must be UTF-8")
            .parse::<usize>()
            .expect("headroom must be an integer");
    }
    16
}

fn configured_affine_limit(name: &str, default: usize) -> usize {
    // Experiment-only override; production always uses the defaults.
    #[cfg(test)]
    if let Some(raw) = std::env::var_os(name) {
        return raw
            .to_str()
            .expect("affine limit must be UTF-8")
            .parse::<usize>()
            .expect("affine limit must be an integer");
    }
    default
}
...
impl Values {
    fn new() -> Self {
        let cache_cap = configured_cache_cap();
        Self {
            next: 2,
            by_expr: HashMap::new(),
            by_id: HashMap::new(),
            cache_cap,
            retain_budget: configured_retain_budget(cache_cap),
            collect_headroom: configured_collect_headroom(),
            evictions: 0,
            collections: 0,
            retained_min: u64::MAX,
            retained_max: 0,
            retained_total: 0,
        }
    }
    ...
    /// Keep a bounded breadth-first prefix of definitions reachable from live
    /// values. Every ID still denotes its original immutable Boolean function;
    /// forgetting a definition can lose a proof, but cannot create an equality.
    fn retain_live(&mut self, roots: impl Iterator<Item = u64>) {
        let budget = self.retain_budget;
        let mut keep = HashSet::with_capacity(budget);
        let mut queue = Vec::with_capacity(budget);
        for value in roots {
            let id = value & !1;
            if self.by_id.contains_key(&id) && keep.len() < budget && keep.insert(id) {
                queue.push(id);
            }
        }
        let mut cursor = 0;
        while cursor < queue.len() && keep.len() < budget {
            let id = queue[cursor];
            cursor += 1;
            let (Expr::Xor(a, b) | Expr::And(a, b)) = self.by_id[&id];
            for child in [a & !1, b & !1] {
                if self.by_id.contains_key(&child) && keep.len() < budget && keep.insert(child) {
                    queue.push(child);
                }
            }
        }
        self.by_id.retain(|id, _| keep.contains(id));
        self.by_expr.retain(|_, id| keep.contains(id));
        self.collections += 1;
        let kept = keep.len() as u64;
        self.retained_min = self.retained_min.min(kept);
        self.retained_max = self.retained_max.max(kept);
        self.retained_total += kept;
    }
...
pub(super) struct Proof {
 pub residual_action:Option<(usize,bool)>,live:HashMap<u64,usize>, pub affine_allowed:bool,pub affine_counts:[[u64;2];9],pub affine_queries:u64,
    affine_visited_cap:usize,affine_depth_cap:usize,affine_width_cap:usize,
    values: Values,
    q: Vec<u64>,
    c: Vec<u64>,
    condition: u64,
...
impl Proof {
    pub fn new(nq: usize, nb: usize, qinputs: &[usize], cinputs: &[usize]) -> Self {
        let mut s = Self {
            residual_action:None,live:HashMap::new(),affine_allowed:true,affine_counts:[[0;2];9],affine_queries:0,
            affine_visited_cap:configured_affine_limit("CLEAN_AND_EXPERIMENT_AFFINE_VISITED",24),
            affine_depth_cap:configured_affine_limit("CLEAN_AND_EXPERIMENT_AFFINE_DEPTH",4),
            affine_width_cap:configured_affine_limit("CLEAN_AND_EXPERIMENT_AFFINE_WIDTH",4),
            values: Values::new(),
            q: vec![0; nq],
            c: vec![0; nb],
            condition: 1,
    ...
    pub fn collections(&self) -> u64 {
        self.values.collections
    }
    #[cfg(test)]
    pub fn retained_stats(&self) -> (u64, u64, u64) {
        (
            self.values.retained_min,
            self.values.retained_max,
            self.values.retained_total,
        )
    }
    #[cfg(test)]
    ...
    pub fn step(&mut self, op:&Op)->bool {
        // One original operation interns at most six expressions. Collect with
        // sixteen slots left; intern's conservative full-clear remains a backstop.
        if self.values.by_expr.len().saturating_add(self.values.collect_headroom) >= self.values.cache_cap {
            self.values.retain_live(self.q.iter().chain(&self.c)
                .chain(std::iter::once(&self.condition)).chain(&self.stack).copied());
        }
...
// Read-only test diagnostic, including bounded live-cache collection counters.
#[cfg(test)]
impl Proof {
 pub(super) fn diagnostic_fingerprint(&self)->String {
  ...
  for v in [self.values.next,self.values.evictions,self.values.collections,self.values.cache_cap as u64,self.condition,self.rewrites,self.ccx_seen,self.conditional_ccx,self.support_dead,self.support_x,self.support_cx]{h.update(v.to_le_bytes());}
  ...
  format!("{{\"sha3_256\":\"{:x}\",\"next\":{},\"evictions\":{},\"collections\":{},\"entries\":{},\"cap\":{}}}",h.finalize(),self.values.next,self.values.evictions,self.values.collections,self.values.by_id.len(),self.values.cache_cap)
 }
}
...
impl Proof {
 fn affine(&self,value:u64,target:usize)->Option<(Vec<usize>,bool)>{
  ...
  if out.len()>self.affine_width_cap.min(8){return None;}Some((out,parity))
 }
 fn affine_visit(&self,value:u64,target:usize,depth:usize,visited:&mut usize,terms:&mut Vec<usize>,parity:&mut bool)->Option<()> {
  *visited+=1;if *visited>self.affine_visited_cap{return None;}
  *parity^=value&1!=0;let id=value&!1;if id==0{return Some(());}
  ...
  if depth>=self.affine_depth_cap{return None;}
...
mod live_cache_tests {
    ...
    #[test]
    fn retained_ids_are_exact_truth_tables_across_collections() {
        let mut values = Values::new();
        values.cache_cap = 64;
        values.retain_budget = 32;
        let mut pool = vec![(0, 0u8), (1, 255)];
        for truth in [0xaa, 0xcc, 0xf0] { pool.push((values.fresh(), truth)); }
        let mut meanings = HashMap::new();
        let mut seed = 0x12345678u64;
        for _ in 0..20_000 {
            if values.by_expr.len() >= 48 {
                let next = values.next;
                values.retain_live(pool.iter().map(|x| x.0));
                assert_eq!(values.next, next);
                assert!(values.by_id.len() <= 32);
                assert_eq!(values.by_expr.len(), values.by_id.len());
                for (expr, id) in &values.by_expr { assert!(values.by_id.get(id) == Some(expr)); }
            }
            seed = seed.wrapping_mul(6364136223846793005).wrapping_add(1);
            pool.push((values.fresh(), (seed >> 17) as u8));
            let a = pool[seed as usize % pool.len()];
            ...
            let (id, truth) = if seed & 1 == 0 {
                (values.and(a.0, b.0), a.1 & b.1)
            } else { (values.xor(a.0, b.0), a.1 ^ b.1) };
            for (key, truth) in [(id, truth), (id ^ 1, !truth)] {
                if let Some(old) = meanings.insert(key, truth) { assert_eq!(old, truth); }
            }
            pool.push((id, truth));
            while pool.len() > 20 { pool.remove(5); }
        }
        assert!(values.collections > 100);
    }

    #[test]
    fn collection_roots_include_classical_and_saved_conditions() {
        let mut p = Proof::new(8, 2, &[1, 2, 3], &[0, 1]);
        p.values.cache_cap = 64;
        let a = p.values.and(p.q[1], p.q[2]);
        let b = p.values.xor(a, p.q[3]);
        let c = p.values.and(b, p.c[0]);
        let original_c1 = p.c[1];
        let d = p.values.xor(c, original_c1);
        p.q[4] = a;
        p.c[1] = b;
        p.condition = c;
        p.stack.push(d);
        while p.values.by_expr.len() < 48 {
            let x = p.values.fresh();
            let y = p.values.fresh();
            p.values.and(x, y);
        }
        let next = p.values.next;
        p.step(&Op::empty()); // Neg: read-only phase record, triggers collection.
        assert_eq!(p.values.collections, 1);
        assert_eq!(p.values.evictions, 0);
        assert_eq!(p.values.next, next);
        for root in [a, b, c, d] { assert!(p.values.by_id.contains_key(&(root & !1))); }
        assert_eq!(p.values.and(p.q[1], p.q[2]), a);
        assert_eq!(p.values.xor(a, p.q[3]), b);
        assert_eq!(p.values.and(b, p.c[0]), c);
        assert_eq!(p.values.xor(c, original_c1), d);
    }
}
