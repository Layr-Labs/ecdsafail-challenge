...
#[test]
fn quadratic_distributive_identity_is_used() {
    let f = Fixture {
        name: "distributive",
        ops: vec![
            cc(0, 1, 3),
    ...
    let original = with_abi(&f);
    let output = simplify(original.clone());
    assert!(
        output.contains(&cx(3, 7)),
        "missing quadratic distributive equality rewrite"
    );
    assert!(legal_ccx_edits(&original, &output));
    assert_eq!(compare(&f, &original, &output, 0..8), (16, 0));
}

// The small R29 cap=2 fixtures run here at the actual production cap32.
// Separate boundary tests below exercise real cap32 overflow.
fn quadratic_fixtures() -> Vec<Fixture> {
    use OperationType::*;
    vec![
        Fixture {
            name: "cap-loss",
            ...
            qi: vec![0, 1, 2],
            bi: vec![],
            required: vec!["EqualControls", "ZeroControl"],
        },
        Fixture {
            name: "distributive-refactor",
            ops: vec![
                cc(0, 1, 3),
                cc(0, 2, 4),
                cx(1, 5),
            ...
            qi: vec![0, 1, 2],
            bi: vec![],
            required: vec!["EqualControls"],
        },
        Fixture {
            name: "square-free-idempotency",
            ops: vec![
                cx(0, 3),
                cx(1, 3),
                cx(0, 4),
        ...
        Fixture {
            ...
            qi: vec![0, 1, 2],
            bi: vec![],
            required: vec!["ZeroControl"],
        },
        Fixture {
            name: "cubic-must-stay-opaque",
            ops: vec![cc(0, 1, 3), cc(2, 3, 4), cc(0, 4, 5), h(5, 0, None)],
            qi: vec![0, 1, 2],
            bi: vec![],
            required: vec![],
        },
        Fixture {
            name: "distinct-opaque-products",
            ops: vec![
                cc(0, 1, 4),
                cc(2, 4, 5),
                cc(3, 4, 6),
        ...
        Fixture {
            name: "review-quadratic-snapshot-Hmr-overwrite",
            ops: vec![
                h(0, 0, None),
                h(1, 1, None),
                push(0),
                ...
                pop(),
                cc(3, 4, 6),
                op(CZ, Some(6), Some(2), None, None, None),
                h(6, 2, None),
            ],
            qi: vec![0, 1, 2],
            bi: vec![],
        ...
        Fixture {
            name: "review-cubic-guard-reset-copy",
            ops: vec![
                push(0),
                push(1),
                push(2),
        ...
        Fixture {
            name: "review-nonlinear-false-guard",
            ops: vec![
                push(0),
                push(1),
                op(BitStore1, None, None, None, Some(0), None),
            ...
            qi: vec![0, 1],
            bi: vec![0, 1],
            required: vec!["FalseGuard"],
        },
        Fixture {
            name: "review-cap-guard-snapshot",
            ops: vec![
                cx(0, 3),
                cx(1, 3),
                cx(2, 3),
...
#[test]
fn reviewed_quadratic_and_guard_fixtures_preserve_full_state() {
    let mut total = 0;
    for f in quadratic_fixtures() {
        let original = with_abi(&f);
        let output = simplify(original.clone());
        assert!(
            legal_ccx_edits(&original, &output),
            "{} illegal edit",
            f.name
        );
        let count = |ops: &[Op]| ops.iter().filter(|o| o.kind == OperationType::CCX).count();
        assert!(
            count(&original) - count(&output) >= f.required.len(),
            "{} missing identities",
            f.name
        );
        let (cases, bad) = compare(
            &f,
            &original,
            &output,
            0..1usize << (f.qi.len() + f.bi.len()),
        );
        assert_eq!(bad, 0, "{} semantic mismatch", f.name);
        total += cases;
        eprintln!("{}: {cases} exhaustive cases", f.name);
    }
    assert_eq!(total, 384);
}
#[test]
fn union_retains_affine_only_equality_after_quadratic_cap_loss() {
    use super::{AffineSupport, Rewrite, Support};
    let mut gates: Vec<_> = (1..33).map(|q| cx(q, 35)).collect();
    gates.extend([
        cc(0, 35, 36),
        cx(36, 37),
        cc(33, 34, 38),
    ...
    let original = with_abi(&f);
    let mut a = AffineSupport::new(&original);
    let mut q = Support::new(&original);
    let mut last_a = Rewrite::Keep;
    let mut last_q = Rewrite::Keep;
    for op in &original {
        last_a = a.step(op);
        last_q = q.step(op);
    }
    assert!(matches!(last_a, Rewrite::Cx(QubitId(36))));
    assert!(
        matches!(last_q, Rewrite::Keep),
        "fixture must exercise a genuinely affine-only proof"
    );
    let output = simplify(original.clone());
    assert_eq!(output.last(), Some(&cx(36, 39)));
    assert!(legal_ccx_edits(&original, &output));
    let inputs = std::iter::once(0)
        .chain((0..35).map(|i| 1usize << i))
        .chain([3, 3 | (1usize << 33) | (1usize << 34), (1usize << 35) - 1]);
    assert_eq!(compare(&f, &original, &output, inputs), (39, 0));
}
#[test]
fn polynomial_algebra_is_exact_or_whole_fresh_and_capped() {
    use super::{Quadratic, Support};
    let mut s = Support::new(&[]);
    for _ in 0..3 {
        s.fresh();
    }
    let monomials = vec![(0, 0), (1, 1), (2, 2), (0, 1), (0, 2), (1, 2)];
    let polys: Vec<_> = (0..128)
        .map(|bits| {
            let mut atoms: Vec<_> = monomials
                .iter()
                .enumerate()
                .filter(|(i, _)| bits >> (i + 1) & 1 != 0)
                .map(|(_, &x)| x)
                .collect();
            atoms.sort_unstable();
            Quadratic {
        ...
        .collect();
    let eval = |p: &Quadratic, x: u64| {
        p.atoms.iter().fold(p.constant, |v, &(a, b)| {
            v ^ (((x >> a) & (x >> b) & 1) != 0)
        })
    };
    for a in &polys {
        for b in &polys {
            for multiply in [false, true] {
                let next = s.next_atom;
                let result = if multiply { s.and(a, b) } else { s.xor(a, b) };
                if s.next_atom == next {
                    for x in 0..8 {
                        assert_eq!(
                            eval(&result, x),
                    ...
                    assert_eq!(s.next_atom, next + 1);
                    assert!(!result.constant);
                    assert_eq!(result.atoms, vec![(next, next)]);
                }
            }
        }
    }
    // Real production cap: 32 distinct monomials retained, 33/34 forgotten.
    let mut s = Support::new(&[]);
    for _ in 0..40 {
        s.fresh();
    }
    ...
    let b = Quadratic {
        constant: false,
        atoms: (2..18).map(|i| (i, i)).collect(),
    };
    assert_eq!(s.and(&a, &b).atoms.len(), 32);
    let b = Quadratic {
        constant: false,
        atoms: (2..19).map(|i| (i, i)).collect(),
    };
    let n = s.next_atom;
    let r = s.and(&a, &b);
    assert_eq!(r.atoms, vec![(n, n)]);
    assert_eq!(s.next_atom, n + 1);
    let a = Quadratic {
        constant: true,
        atoms: (0..32).map(|i| (i, i)).collect(),
    };
    let b = Quadratic {
        constant: false,
        atoms: vec![(32, 32)],
    ...
    let r1 = s.and(&ab, &c);
    let r2 = s.and(&ab, &c);
    assert!(r1 != r2, "distinct opaque evaluations must stay fresh");
}
#[test]
fn cubic_drop_and_opaque_alias_mutants_are_semantically_detected() {
    for name in ["cubic-must-stay-opaque", "distinct-opaque-products"] {
        let f = quadratic_fixtures()
            .into_iter()
            .find(|f| f.name == name)
            .unwrap();
        let original = with_abi(&f);
        ...
        let last = mutant
            .iter()
            .rposition(|o| o.kind == OperationType::CCX)
            .unwrap();
        if name == "cubic-must-stay-opaque" {
            mutant.remove(last);
        } else {
            let o = mutant[last];
            mutant[last] = cx(o.q_control1.0, o.q_target.0);
        }
        let (_, bad) = compare(&f, &original, &mutant, 0..1usize << f.qi.len());
        assert!(bad > 0, "{name} mutant escaped");
        eprintln!("{name} mutant mismatches={bad}");
    }
}

#[test]
fn unknown_controls_after_conditional_reset_and_hmr_stay_unknown() {
    for f in [
        Fixture {
            name: "guarded-reset-unknown-controls",
            ops: vec![reset(0, Some(0)), cc(0, 1, 2), h(2, 1, None)],
            qi: vec![0, 1],
            bi: vec![0],
            required: vec![],
        },
        Fixture {
            name: "hmr-unknown-control",
            ops: vec![
                h(0, 0, None),
                op(OperationType::X, Some(2), None, None, None, Some(0)),
                cc(1, 2, 3),
                h(3, 1, None),
            ],
            qi: vec![0, 1],
        ...
        assert_eq!(bad, 0, "{} mismatch", f.name);
        eprintln!("{}: {cases} cases", f.name);
    }
}
