...
fn retention_tests(){
 ...
 for (shape,&(data,re,end,t,a,b))in fixtures.iter().enumerate(){let ops=test_decode(data);assert_eq!(ops.len(),end);let mut locals=vec![t,a,b];for o in &ops{for &q in &o.q{if !locals.contains(&q){locals.push(q);}}}let n=locals.len();assert!(n<64);
  for context in 0..5{
   let build=|enabled,retain|{let inputs:Vec<_>=(1..=n).filter(|q|context==3||*q!=1).collect();let mut s=State::new(n+1,2,&inputs,&[0,1],retain,0);s.comparator_capture=Some(Vec::new());s.retention.enabled=enabled;s.retention.locations=vec![vec![RetentionLocation{h:0,re,end,start_byte:0,end_byte:data.len(),t,a,b,c:0}]];s.qmap=vec![QubitId(0);*locals.iter().max().unwrap()+1];for(j,&q)in locals.iter().enumerate(){s.qmap[q]=QubitId((j+1)as u64);}
    ...
    let mut ix=0;let mut active=None;for(j,o)in ops.iter().enumerate(){s.retention_boundary(0,j,o.start,0,0,&mut ix,&mut active);test_emit(&mut s,o);}s.retention_boundary(0,end,data.len(),0,0,&mut ix,&mut active);if context==1||context==2||context==4{s.raw(K::PopCondition);}s.flush_raw();assert!(s.proof.balanced());let captured=s.comparator_capture.take().unwrap();assert_eq!(captured.len(),s.output_ops);for op in captured{s.deliver(op);}s};
