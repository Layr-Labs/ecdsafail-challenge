...
pub fn build() -> Vec<Op> {
    ...
    // Exact pair-cancellation window over the per-step NCT stream (shared_optimize.rs cancel_nct);
    // 16384 recovers more nested inverse pairs than 4096; 65536 finds nothing further.
    set_default_env("Q795_CORE_CANCEL","16384");
    // Reversed control-ladder order in mixed_mcx exposes shared metadata-prefix cancellations;
    // control factors commute, no predicate changes (length_recompute.rs).
    ...
    for name in ["Q794_T10_COMBINED_LOAN","Q794_T10_PAIRED_LOW","Q794_T10_ENDPOINT_LOCAL","Q794_T10_ENDPOINT_CACHE","Q794_T10_CACHE_PAIRED","Q794_SIGN_COMBINED_TOP","Q794_SIGN_FACTOR_LOW","Q794_SIGN_PAIRED_LOW","Q794_R01_SEED_FACTOR","Q794_TFACTOR"] {set_default_env(name,"1");}
    ...
    // Q794 resumed campaign: closed exact affine metadata frames.
    set_default_env("Q794_R00_PARTIAL_MAJ","1");
    set_default_env("Q794_SIGN_MAJ_HISTORY","1");
    set_default_env("Q794_R01_S_AFFINE","1");
    // Integration-v4 qualification defaults; release requires complete tests.
    for name in ["Q794_SIGN_RANK_FOLD","Q794_R01_SUM_XOR","Q794_MBU_FWD_SIGN","Q794_MBU_REV_R00"] {set_default_env(name,"1");}
    for name in ["Q794_T10_C1_PREFIX","Q794_T10_KNOWN_MASK","Q794_R00_ENDPOINTS","Q794_R00_S_DOMAIN","Q794_R01_ODD_CACHE"] {set_default_env(name,"1");}
    for name in ["Q794_T10_RANK_PREFIX","Q794_AUPDATE_ACTIVE_AFFINE","Q794_TRANSFER_ACTIVE_AFFINE"] {set_default_env(name,"1");}
    for name in ["Q794_AUPDATE_PREFIX_AFFINE","Q794_TRANSFER_PREFIX_AFFINE"] {set_default_env(name,"1");}
    set_default_env("Q794_R01_SEED_POLARITY","1");
    set_default_env("Q794_R01_NUMERIC","1");
    set_default_env("Q794_R01_NUMERIC_PARTIAL","1");
    set_default_env("Q794_R01_NUMERIC_ROUTE","2");
    set_default_env("Q794_R01_NUMERIC_SEED","1");
    set_default_env("Q794_SIGN_PREPARE_RIPPLE","1");
    set_default_env("Q794_SIGN_SUM_MAJ","1");
    set_default_env("Q794_SIGN_PREDICATES","1");
    set_default_env("Q794_SIGN_PREPARE_CLEAN","1");
    set_default_env("Q794_SIGN_SUM_PARITY","1");
    set_default_env("Q794_ENTRY_NUMERIC","1");
    set_default_env("Q794_R01_A7_PREFIX","1");
    set_default_env("Q794_T10_NUMERIC_QUOTIENT","1");
    set_default_env("Q794_T10_C1_DECISION","1");
    set_default_env("Q794_T10_POLISH","1");
    set_default_env("Q794_R01_ENDPOINT_FACTOR","1");
    set_default_env("Q794_COUNTER_REFLECTION","1");
    for name in ["Q794_COUNTER_AFFINE","Q794_ENTRY_UNPACK_AFFINE","Q794_ENTRY_PACK_AFFINE","Q794_EXIT_UNPACK_AFFINE","Q794_EXIT_PACK_AFFINE"] {set_default_env(name,"1");}
    if let Ok(mode) = std::env::var("LOWQ_Q794_NATIVE_MODE") {
        match mode.as_str() {
            "affine-boundaries" => trailmix_port::inversion::q794_affine_frames::run(),
            "fold-helpers" => trailmix_port::inversion::q794_rank_fold::verification::run(),
            "sum-xor" => trailmix_port::inversion::q794_sum_xor::check(),
            ...
            "whole-count" | "whole-stream" | "whole-default-count" | "whole-experimental-count" | "whole-experimental-stream" => {
                ...
                if mode=="whole-stream" || mode=="whole-experimental-stream"{
                    ...
                    std::env::set_var("SPRINT_STREAM_SEED","q794-numseed2-predeclared-independent-seed-20260910");
                }
    ...
    if std::env::var("POINT_ADD_DIALOG_ROUTE").ok().as_deref() != Some("1") {
        if trailmix_port::inversion::q794_mbu::enabled(){
            assert!(trailmix_port::inversion::paper2607_eea::shared_length_enabled()&&trailmix_port::inversion::paper2607_eea::step_optimizer_enabled(),"MBU requires measurement-safe per-template optimization; global conditional-CCX cancellation is not permitted");
        }
