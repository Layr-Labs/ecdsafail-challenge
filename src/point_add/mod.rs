...
pub fn build() -> Vec<Op> {
    // Low-width routes emit billion-record streams. Level 1 preserves the
    // decompressed operation stream while keeping server serialization below
    // the runner time envelope; callers can still request another level.
    set_default_env("ZSTD_LEVEL", "1");
