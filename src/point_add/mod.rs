...
/// Identity-keyed deep strip (DELETION column) for the q=824 trailmix stream,
/// backed by an embedded binary side table. See deep_strip_table.rs.
mod deep_strip_table;
...
impl OpStreamWriter {
    ...
    fn new(path: &Path) -> std::io::Result<Self> {
        ...
        Ok(Self {
            ...
            temporary_path,
            count: 0,
            cancel_adjacent_ccx: std::env::var("CANCEL_ADJACENT_CCX").ok().as_deref() == Some("1"),
            pending_ccx: Vec::new(),
            removed_ccx: 0,
        })
    }

    fn write_raw_op(&mut self, op: &Op) -> std::io::Result<()> {
    ...
    fn flush_pending_ccx(&mut self) -> std::io::Result<()> {
        let pending = std::mem::take(&mut self.pending_ccx);
        for op in pending {
            self.write_raw_op(&op)?;
        }
        Ok(())
    }

    fn write_op(&mut self, op: &Op) -> std::io::Result<()> {
        if self.cancel_adjacent_ccx && op.kind == OperationType::CCX {
            if self.pending_ccx.last() == Some(op) {
                self.pending_ccx.pop();
                self.removed_ccx += 2;
            } else {
                self.pending_ccx.push(*op);
            }
            return Ok(());
        }
        self.flush_pending_ccx()?;
        self.write_raw_op(op)
    }

    fn finish(mut self) -> std::io::Result<u64> {
        self.flush_pending_ccx()?;
        if self.removed_ccx != 0 {
            eprintln!("CANCEL_ADJACENT_CCX removed={}", self.removed_ccx);
        }
...
impl B {
    ...
    pub(crate) fn finish_stream_writer(&mut self) {
        ...
        if std::env::var("CANCEL_ADJACENT_CCX").ok().as_deref() == Some("1") {
            assert!(actual <= expected, "optimized streamed operation count");
        } else {
    ...
    fn cancel_adjacent_ccx_in_memory(ops: &mut Vec<Op>) -> usize {
        ...
        for read in 0..ops.len() {
            let op = ops[read];
            if op.kind == OperationType::CCX && write != 0 && ops[write - 1] == op {
                write -= 1;
                removed += 2;
            } else {
                ops[write] = op;
    ...
    fn current_ops_len(&self) -> usize {
        if self.count_only || self.is_streaming() {
            self.counted_ops
        } else {
            self.ops.len()
        }
...
pub fn build() -> Vec<Op> {
    ...
    if std::env::var("POINT_ADD_DIALOG_ROUTE").ok().as_deref() != Some("1") {
        set_default_env("CANCEL_ADJACENT_CCX", "1");
        let builder = trailmix_port::build_builder();
        let mut ops = builder.ops;
        if !ops.is_empty() {
            let removed = B::cancel_adjacent_ccx_in_memory(&mut ops);
            eprintln!("CANCEL_ADJACENT_CCX removed={removed}");
        }
        // The census was taken on the POST-cancel stream (441,967,961 ops /
        // 192,216,787 CCX+CCZ), so the strip keys address this point exactly.
        return deep_strip_table::apply(ops);
    }
