...
mod q792_local_pack_r01;
mod q792_compact_check_r01;
...
pub struct B {
    ...
    pub(crate) q792_packed_blocks: Option<Vec<std::sync::Arc<q792_local_pack_r01::Block>>>,
    pub count_only: bool,
...
impl B {
    ...
    pub(crate) fn new_with_ops_capacity(ops_capacity: usize) -> Self {
        ...
        if std::env::var("LOWQ_WHOLE_COMPACT_SELFTEST").ok().as_deref()==Some("1") {
            ...
            if trailmix_port::inversion::q792_lifecycle_r01::enabled(){b.q792_packed_blocks=Some(Vec::new());}
    ...
    pub(crate) fn flush_compact_block(&mut self) {
        if self.compact_blocks.is_some() {
            ...
            if !self.ops.is_empty() {
                if let Some(blocks)=&mut self.q792_packed_blocks{blocks.push(std::sync::Arc::new(q792_local_pack_r01::Block::raw(std::mem::take(&mut self.ops))));}else{self.compact_blocks.as_mut().unwrap().push(std::sync::Arc::new(std::mem::take(&mut self.ops)));}
            }
        }
    }
...
pub fn build() -> Vec<Op> {
    ...
    // Own folded20 metadata integration, whole peak792 and exact stage46 counts.
    ...
    if let Ok(mode) = std::env::var("LOWQ_Q793_NATIVE_MODE") {
        match mode.as_str() {
            ...
            "whole-default-count" => {
                assert!(trailmix_port::inversion::q793_lifecycle_r03::candidate_configuration(),"source-default Q793 configuration required");
            ...
            "whole-9024" => {
                assert!(trailmix_port::inversion::q793_lifecycle_r03::candidate_configuration(),"canonical9024 requires source-default Q793");
                std::env::set_var("LOWQ_WHOLE_COMPACT_SELFTEST","1");
                std::env::remove_var("POINT_ADD_COUNT_ONLY");
                q793_compact_check_r01::run();
            },
            "fold20-affine-memory" => trailmix_port::inversion::q792_affine_memory_r01::check(),
            "fold20-incremental-library" => trailmix_port::inversion::q792_incremental_library_r01::check(),
            "fold20-proved-library" => trailmix_port::inversion::q792_proved_library_r01::check(),
            "fold20-bank-factor" => trailmix_port::inversion::q792_bank_factor_r01::check(),
            "fold20-star-factor" => trailmix_port::inversion::q792_star_factor_r01::check(),
            "fold20-step-export" => {std::env::set_var("FOLD20_STEP_FULLWIDTH","1");assert!(std::env::var_os("STEP_GPU_EXPORT_DIR").is_some());trailmix_port::inversion::q792_step_r01::run();},
            "fold20-conditional-mcx" => trailmix_port::inversion::conditional_mcx::verification::run(),
            "fold20-predicates" => trailmix_port::inversion::q792_fold20_predicate_r01::run(),
            "fold20-szero-chart" => trailmix_port::inversion::q792_szero_chart_r01::run(),
            "fold20-szero-exit" | "fold20-szero-exit-offguard" => trailmix_port::inversion::q792_szero_exit_r01::run(),
            "fold20-address-loan" => {trailmix_port::inversion::q792_fold20_address_r01::run_loan();std::process::exit(0);}
            "fold20-address" => trailmix_port::inversion::q792_fold20_address_r01::run(),
            "fold20-czero-chart" => trailmix_port::inversion::q792_czero_chart_r01::run(),
            "fold20-r00" | "fold20-r00-all" | "fold20-r00-final-empty" => trailmix_port::inversion::q792_r00_r01::run(),
            "rank4-r01-lower" => trailmix_port::inversion::q792_rank4_lower_r01::run(),
            "fold20-asmall-chart" => trailmix_port::inversion::q792_asmall_chart_r01::run(),
            "fold20-sign-phaselease" | "fold20-sign-memo-export" => trailmix_port::inversion::q792_sign_phaselease_r01::verification::run(),
            "fold20-entry-head" => trailmix_port::inversion::q792_entry_head_r01::run(),
            "fold20-entry-boundary" => trailmix_port::inversion::q792_entry_boundary_r01::run(),
            "fold20-step" => trailmix_port::inversion::q792_step_r01::run(),
            "fold20-r01-sumchart" => trailmix_port::inversion::q792_r01_sumchart_r01::run(),
            "fold20-local-pack" => q792_local_pack_r01::check(),
            "fold20-whole-9024" => {
                assert!(trailmix_port::inversion::q792_lifecycle_r01::candidate_configuration(),"canonical9024 requires frozen source-default Q792");
                std::env::set_var("LOWQ_WHOLE_COMPACT_SELFTEST","1");std::env::remove_var("POINT_ADD_COUNT_ONLY");q792_compact_check_r01::run();
            },
            "fold20-r01-supported-route" => trailmix_port::inversion::q792_r01_nohs_normal_r01::run_supported_route(),
            "fold20-supported-address" => trailmix_port::inversion::q792_fold20_address_r01::run_supported(),
            "fold20-table-factor" => trailmix_port::inversion::q792_fold20_r01::run_factor(),
            "fold20-gpu-affine" => trailmix_port::inversion::q792_gpu_affine_r01::run(),
            "fold20-rotation-fusion" => trailmix_port::inversion::q792_rotation_r01::run(),
            "fold20-trace-cancel" => trailmix_port::inversion::q792_trace_cancel_r01::run(),
            "fold20-output-basis" => trailmix_port::inversion::q792_output_basis_r01::run(),
            "fold20-gpu-beam" => trailmix_port::inversion::q792_gpu_beam_r01::run(),
            "fold20-lifetime" => trailmix_port::inversion::q792_lifetime_cache_r01::run(),
            "fold20-gpu-nonlinear" => trailmix_port::inversion::q792_gpu_nonlinear_r01::run(),
            "fold20-esop" => trailmix_port::inversion::q792_esop_r01::run(),
            "fold20-halve" => trailmix_port::inversion::q792_halve_check_r01::run(),
            "fold20-lifecycle" => trailmix_port::inversion::q792_lifecycle_r01::check::run(),
            "fold20-lifecycle-boundaries" => trailmix_port::inversion::q792_lifecycle_r01::check::run_boundaries(),
            "fold20-allocation-audit" => {
                std::env::set_var("LOWQ_Q792_EEA","1");std::env::set_var("POINT_ADD_COUNT_ONLY","1");std::env::set_var("TRACE_NAMED_PEAK_TARGETS","792,793");
                let b=trailmix_port::build_builder();assert!(b.count_only&&b.ops.is_empty());eprintln!("FOLD20_ALLOCATION_AUDIT peak={} phase={}; schedules omitted, NOT whole count or correctness",b.peak_qubits,b.peak_phase);
            },
            "fold20-c1-codec-census" | "fold20-gpu-census" | "fold20-whole-count" | "fold20-whole64" => {
                if mode=="fold20-gpu-census"{std::env::set_var("FOLD20_GPU_CAPTURE","1");}
                std::env::set_var("LOWQ_Q792_EEA","1");std::env::set_var("POINT_ADD_COUNT_ONLY","1");
                if mode=="fold20-whole64"{std::env::set_var("SPRINT_STREAM_CHECK","1");std::env::set_var("SPRINT_STREAM_SEED","q792-integrate-r01-predeclared-independent-seed-v1");}
                let b=trailmix_port::build_builder();assert!(b.count_only&&b.ops.is_empty());eprintln!("Q792_WHOLE_DIAGNOSTIC_PASS mode={mode} peak={} ops={}; NOT canonical9024 or official acceptance",b.peak_qubits,b.current_ops_len());
            },
            "fold20-sign-nop2" => trailmix_port::inversion::q792_sign_nop2_r01::verification::run(),
            "fold20-t10-a-chart" => {trailmix_port::inversion::q792_t10_a_chart_r01::run();std::process::exit(0);}
            "fold20-t10-phaselease" => trailmix_port::inversion::q792_t10_phaselease_r01::run(),
            "t-only-full" => trailmix_port::inversion::q792_t_only_full_r03::run(),
            "t-only-full-r04" => trailmix_port::inversion::q792_t_only_full_r04::run(),
            "t-only-selection" => trailmix_port::inversion::q792_t_only_selection_check_r01::run(),
            "fold20-t10-nop2" => trailmix_port::inversion::q792_t10_nop2_check_r01::run(),
            "fold20-r01-phaselease" | "fold20-r01-memo-export" => trailmix_port::inversion::q792_r01_phaselease_r01::run(),
            "fold20-r01-nohs" => trailmix_port::inversion::q793_r01_dynamic_timefix_r01::run_nohs(),
            "fold20-phase-conjugacy" => trailmix_port::inversion::q792_phase_conjugacy_r01::run(),
            "fold20-phase-union" => trailmix_port::inversion::q792_phase_union_r01::run(),
            "fold20-phase-routing-export" => trailmix_port::inversion::q792_t10_phaselease_r01::run(),
            "fold20-memory-coordinate" => trailmix_port::inversion::q792_memory_coordinate_r01::run(),
            "fold20-memory-budget" => trailmix_port::inversion::q792_memory_coordinate_r01::budget(),
            "fold20-unfold-lease" => trailmix_port::inversion::q792_unfold_lease_r01::run(),
            "fold20-r01-gap" => trailmix_port::inversion::q793_r01_dynamic_timefix_r01::run_gap(),
            "fold20-r01" => trailmix_port::inversion::q792_r01_r01::run(),
            "rank4-r01-clean" => trailmix_port::inversion::q792_rank4_lower_r01::run_clean(),
            "fold20-class-charts" => trailmix_port::inversion::q792_class_chart_r01::run(),
            "fold20-terminal" => trailmix_port::inversion::q792_terminal_r01::run(),
            "fold20-counter-r02" => trailmix_port::inversion::q792_fold20_counter_r02::run(),
            "fold20-counter" => trailmix_port::inversion::q792_fold20_counter_r01::run(),
            "fold20-rank" => trailmix_port::inversion::q792_fold20_rank_r01::run(),
            "fold20-readout" => trailmix_port::inversion::q792_fold20_r01::run(),
