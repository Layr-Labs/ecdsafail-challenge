...
pub fn build() -> Vec<Op> {
    if std::env::var_os("Q813_EXPAND_CENSUS").is_some() {
        trailmix_port::inversion::paper2607_eea::trace_expanded_conjugation_census();
        return Vec::new();
    }
    ...
    if std::env::var("POINT_ADD_DIALOG_ROUTE").ok().as_deref() != Some("1") {
        ...
        if !ops.is_empty() {
            ...
            let removed2 = B::cancel_commuting_ccx_in_memory(&mut ops, w);
            eprintln!("CANCEL adjacent={removed} commuting(window={w})={removed2}");
        }
        return ops;
    }
