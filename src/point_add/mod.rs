...
mod trailmix_port;
mod exact_boolean;
mod symbolic_clean_and;
mod clean_chunk_plan;
mod chunked_controlled_add;
mod zero_scratch_neg;
...
fn cancel_adjacent_toffoli(mut ops: Vec<Op>) -> Vec<Op> {
    ...
    ops.truncate(write);
    eprintln!("CANCEL_ADJACENT_TOFFOLI removed={removed}");
    ops
}

/// Cancel identical self-inverse gates separated only by operations on
/// disjoint qubits. Removing Clifford pairs can expose additional Toffoli
/// pairs. Classical-control and non-unitary operations are barriers, keeping
/// this pass deliberately more conservative than a general commutation
/// optimizer.
fn cancel_disjoint_commuting_toffoli(mut ops: Vec<Op>) -> Vec<Op> {
    const NONE: usize = usize::MAX;
    const BLOCKED: usize = usize::MAX - 1;
    const MAX_BACKSCAN: usize = 16_384;
    ...
    fn touches(op: &Op, q: QubitId) -> bool {
        q != NO_QUBIT
            && (op.q_control2 == q || op.q_control1 == q || op.q_target == q)
    }

    let max_qubit = ops
        .iter()
        .flat_map(|op| qubits(op))
        .filter(|&q| q != NO_QUBIT)
        .map(|q| q.0 as usize)
        .max()
        .unwrap_or(0);
    let mut last_touch = vec![NONE; max_qubit + 1];
    ...
    for read in 0..ops.len() {
        let op = ops[read];
        if is_barrier(&op) {
            last_touch.fill(BLOCKED);
            continue;
        }

        let touched = qubits(&op);
        let is_self_inverse = matches!(
            op.kind,
            OperationType::X
                | OperationType::Z
                | OperationType::CX
        ...
        let previous = touched
            ...
            .copied()
            .filter(|&q| q != NO_QUBIT)
            .map(|q| last_touch[q.0 as usize])
            .reduce(|a, b| if a == b { a } else { BLOCKED })
            .unwrap_or(BLOCKED);
            ...
            && ops[previous] == op
        {
            ...
            for q in touched.into_iter().filter(|&q| q != NO_QUBIT) {
                let mut restored = NONE;
                let lower = previous.saturating_sub(MAX_BACKSCAN);
                for probe in (lower..previous).rev() {
                    if cancelled[probe] {
                        continue;
                    }
                    if is_barrier(&ops[probe]) {
                        restored = BLOCKED;
                        break;
                    }
                    if touches(&ops[probe], q) {
                        restored = probe;
                        break;
                    }
                }
                if lower != 0 && restored == NONE {
                    restored = BLOCKED;
                }
                last_touch[q.0 as usize] = restored;
            }
            continue;
        }

        for q in touched.into_iter().filter(|&q| q != NO_QUBIT) {
            last_touch[q.0 as usize] = read;
        }
    }
    ...
    ops.truncate(write);
    eprintln!(
        "CANCEL_DISJOINT_COMMUTING removed_ops={removed} removed_toffoli={removed_toffoli}"
    );
    ops
}

/// Extend the conservative cancellation pass across overlapping X-family and
/// diagonal gates that provably commute. Removing Clifford pairs can expose
/// further Toffoli pairs. Non-unitary and classically conditioned operations
/// remain global barriers.
fn cancel_xfamily_commuting_toffoli(mut ops: Vec<Op>) -> Vec<Op> {
    ...
    let max_qubit = ops
        ...
        .max()
        .unwrap_or(0);
    let mut last_write = vec![NONE; max_qubit + 1];
    let mut last_control_read = vec![NONE; max_qubit + 1];
    let mut last_phase_touch = vec![NONE; max_qubit + 1];
    let mut last_swap = vec![NONE; max_qubit + 1];
    let mut last_same = HashMap::<(u8, u64, u64, u64), usize>::new();
    let mut cancelled = vec![false; ops.len()];
    ...
    for read in 0..ops.len() {
        ...
        let xfamily_controls: Option<&[QubitId]> = match op.kind {
            OperationType::X => Some(&[]),
            OperationType::CX => Some(std::slice::from_ref(&op.q_control1)),
            OperationType::CCX => Some(&[op.q_control2, op.q_control1]),
            _ => None,
        };
        let diagonal_support: Option<&[QubitId]> = match op.kind {
            OperationType::Z => Some(std::slice::from_ref(&op.q_target)),
            OperationType::CZ => Some(&[op.q_control1, op.q_target]),
            OperationType::CCZ => Some(&[op.q_control2, op.q_control1, op.q_target]),
            _ => None,
        };
        if xfamily_controls.is_some() || diagonal_support.is_some() {
            let gate_key = key(&op);
            if let Some(&previous) = last_same.get(&gate_key) {
                let after_barrier = barrier == NONE || previous > barrier;
                let blocker = if let Some(controls) = xfamily_controls {
                    controls
                        .iter()
                        .copied()
                    ...
                    diagonal_support
                        .expect("one commuting family is selected")
                        .iter()
                        .copied()
                ...
                if after_barrier && blocker.is_none_or(|index| index < previous) {
                    cancelled[previous] = true;
                    cancelled[read] = true;
        ...
        match op.kind {
            OperationType::X => last_write[op.q_target.0 as usize] = read,
            OperationType::CX => {
            ...
            OperationType::Z => last_phase_touch[op.q_target.0 as usize] = read,
            OperationType::CZ => {
    ...
    ops.truncate(write);
    eprintln!(
        "CANCEL_XFAMILY_COMMUTING removed_ops={removed} removed_toffoli={removed_toffoli}"
    );
    ops
}

fn exact_cancellation_selftest() {
    fn op(kind: OperationType, qs: &[u64]) -> Op {
        let mut value = Op::empty();
        value.kind = kind;
        value.q_target = QubitId(qs[0]);
        if qs.len() > 1 { value.q_control1 = QubitId(qs[1]); }
        if qs.len() > 2 { value.q_control2 = QubitId(qs[2]); }
        value
    }
    // These gates permute basis states and attach signs. Checking every basis
    // state and its phase also checks the action on arbitrary superpositions.
    fn action(ops: &[Op], mut bits: u64) -> (u64, bool) {
        let mut phase = false;
        for gate in ops {
            let bit = |q: QubitId| ((bits >> q.0) & 1) != 0;
            let target = 1 << gate.q_target.0;
            match gate.kind {
                OperationType::X => bits ^= target,
                OperationType::CX if bit(gate.q_control1) => bits ^= target,
                OperationType::CCX if bit(gate.q_control1) && bit(gate.q_control2) => bits ^= target,
                OperationType::Z => phase ^= bit(gate.q_target),
                OperationType::CZ => phase ^= bit(gate.q_target) && bit(gate.q_control1),
                OperationType::CCZ => phase ^= bit(gate.q_target) && bit(gate.q_control1) && bit(gate.q_control2),
                OperationType::Swap => {
                    if bit(gate.q_target) != bit(gate.q_control1) {
                        bits ^= target | (1 << gate.q_control1.0);
                    }
                }
                OperationType::CX | OperationType::CCX => {}
    ...
    for a in 0..4 {
        for kind in [OperationType::X, OperationType::Z] {
            gates.push(op(kind, &[a]));
        }
        for b in 0..4 {
            if b == a { continue; }
            for kind in [OperationType::CX, OperationType::CZ, OperationType::Swap] {
                gates.push(op(kind, &[a,b]));
            }
            for c in 0..4 {
                if c == a || c == b { continue; }
                for kind in [OperationType::CCX, OperationType::CCZ] {
                    gates.push(op(kind, &[a,b,c]));
                }
            }
        }
    }
    let check = |input: Vec<Op>| {
        let reduced = cancel_xfamily_commuting_toffoli(
            cancel_disjoint_commuting_toffoli(input.clone()));
        for bits in 0..16 {
            assert_eq!(action(&input, bits), action(&reduced, bits),
                "cancellation changed basis state {bits}: {input:?}");
        }
    };
    let mut checked = 0;
    for &first in &gates {
        for &middle in &gates {
            check(vec![first, middle, first]);
            checked += 1;
        }
    }
    let mut state = 0x713ce5a809fbd624u64;
    for _ in 0..512 {
        let mut input = Vec::new();
        for _ in 0..96 {
            state ^= state << 13; state ^= state >> 7; state ^= state << 17;
            input.push(gates[state as usize % gates.len()]);
        }
        check(input);
        checked += 1;
    }
    let gate = op(OperationType::CCX, &[0,1,2]);
    for kind in [OperationType::Hmr, OperationType::R, OperationType::PushCondition,
        OperationType::PopCondition, OperationType::BitStore0] {
        let mut barrier = Op::empty();
        barrier.kind = kind;
        let input = vec![gate, barrier, gate];
        assert_eq!(cancel_disjoint_commuting_toffoli(input.clone()), input);
        assert_eq!(cancel_xfamily_commuting_toffoli(input.clone()), input);
    }
    eprintln!("EXACT_CANCELLATION_SELFTEST PASS: {checked} circuits, 16 basis states plus phase each");
}

pub fn build() -> Vec<Op> {
    if std::env::var_os("MIDQ_D_POSITION_CACHE_SELFTEST").is_some() || std::env::var_os("MIDQ_D_POSITION_CACHE_LEDGER").is_some() {
        trailmix_port::build_builder();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_D_ALIGN_ITERATION_SELFTEST").is_some() {
        trailmix_port::build_builder();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_D_ALIGN_COMMON_WINDOW_SELFTEST").is_some() {
        trailmix_port::build_builder();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_D_ALIGNMENT_LEDGER").is_some() {
        trailmix_port::build_builder();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_QRETAIN_SELFTEST").is_some() {
        trailmix_port::build_builder();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_HYBRID_MARGIN_SELFTEST").is_some() {
        trailmix_port::build_builder();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_GENERIC_QUOTIENT_CODE_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::midq_generic_quotient_code_selftest();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_HYBRID_PROFILE_SELFTEST").is_some()
        || std::env::var_os("MIDQ_HYBRID_PROFILE_DUMP").is_some() {
        trailmix_port::build_builder();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_CHECKPOINT_CAP_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::checkpoint_cap_aware::selftest();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_CHECKPOINT_CAP_PROFILE").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::checkpoint_cap_aware::profile();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_ZERO_SCRATCH_NEG_SELFTEST").is_some() {
        zero_scratch_neg::selftest();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_CHUNKED_CONTROLLED_ADD_SELFTEST").is_some() {
        chunked_controlled_add::selftest();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_ALL_CONST_FOLDS_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::cell_folds::all_const_selftest::run();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_PREFIX_POPCOUNT_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::midq_prefix_popcount_selftest();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_PREFIX_POPCOUNT_PROFILE").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::midq_prefix_popcount_profile();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_PREFIX_NO_TERMINAL_PROFILE").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::midq_prefix_no_terminal_profile();
        std::process::exit(0);
    }
    if std::env::var_os("MIDQ_PREFIX_NO_TERMINAL_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::midq_prefix_no_terminal_selftest();
        std::process::exit(0);
    }
    if std::env::var_os("MIDQ_CELL_FOLDS_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::cell_folds::selftest::run();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_TAIL_METADATA_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::tail_metadata_codec::run();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_NARROW_COEFFICIENT_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::midq_narrow_coefficient_selftest();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_CHUNKED_PREFIX_SELFTEST").is_some() {
        clean_chunk_plan::selftest();
        trailmix_port::inversion::shrunken_pz_state_machine::chunked_bitlength::selftest();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_MEASURED_DEMUX_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::midq_measured_demux_selftest();
        return Vec::new();
    }
    if std::env::var("MIDQ_EXACT_BOOLEAN_SELFTEST").ok().as_deref() == Some("1") {
        exact_boolean::selftest();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_COUNTER_TAPE_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::counter_tape::selftest();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_QUOTIENT_CODE_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::midq_quotient_code_selftest();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_PREDICATE_CLEAR_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::midq_predicate_clear_selftest();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_OUTER_VENT_SELFTEST").is_some() {
        trailmix_port::rfold_mbu::exact_selftest::run_vents();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_OUTER_PHASE_SELFTEST").is_some() {
        trailmix_port::rfold_mbu::exact_selftest::run_phase();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_OUTER_EXACT_SELFTEST").is_some() {
        trailmix_port::rfold_mbu::exact_selftest::run();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_ODD_VALUES_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::midq_odd_values::selftest();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_TAIL_CHECKPOINT_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::midq_tail_checkpoint::selftest();
        return Vec::new();
    }
    if std::env::var_os("MIDQ_CLZ_PARITY_SELFTEST").is_some()
        || std::env::var_os("MIDQ_RETAIN_DIV_LENGTHS_SELFTEST").is_some()
        || std::env::var_os("MIDQ_MEASURE_COMPARE_SELFTEST").is_some()
        || std::env::var_os("MIDQ_GATE_COMPARE_SELFTEST").is_some()
        || std::env::var_os("MIDQ_DIRTY_CONST_SELFTEST").is_some() {
        trailmix_port::build_builder();
        return Vec::new();
    }
    if std::env::var_os("EXACT_CANCELLATION_SELFTEST").is_some() {
        exact_cancellation_selftest();
        return Vec::new();
    }
    if std::env::var("POINT_ADD_DIALOG_ROUTE").ok().as_deref() != Some("1") {
        let mut ops = trailmix_port::build_builder().ops;
        if std::env::var_os("POINT_ADD_COUNT_ONLY").is_some() {
            // Count-only mode stores no ops; the nonce-tail split and the
            // commuting-cancellation passes need them. The count-only total
            // is pre-cancellation.
            return ops;
        }
        // The final 48 X;X pairs select the Fiat-Shamir nonce. Optimize the
        // arithmetic prefix and preserve this fixed-length identity suffix.
        let tail = if std::env::var_os("TRAILMIX_TAIL_NONCE").is_some() {
            let start = ops.len().checked_sub(96).expect("48-bit nonce tail");
            assert!(ops[start..].chunks_exact(2).all(|pair|
                pair[0].kind == OperationType::X && pair[0] == pair[1]));
            ops.split_off(start)
        } else {
            Vec::new()
        };
        let ops = cancel_adjacent_toffoli(ops);
        let ops = if std::env::var_os("CANCEL_DISJOINT_COMMUTING_TOFFOLI").is_some() {
            cancel_disjoint_commuting_toffoli(ops)
        } else {
            ops
        };
        let ops = if std::env::var_os("CANCEL_XFAMILY_COMMUTING_TOFFOLI").is_some() {
            cancel_xfamily_commuting_toffoli(ops)
        } else {
            ops
        };
        let ops = if std::env::var("MIDQ_EXACT_BOOLEAN").ok().as_deref() == Some("1") {
            exact_boolean::simplify(ops)
        } else {
            ops
        };
        // Q834's symbolic engine runs after the incumbent pass and before the
        // detached nonce tail is restored. Disabled mode preserves source bytes.
        let mut ops = if std::env::var("MIDQ_SYMBOLIC_CLEAN_AND").ok().as_deref() == Some("1") {
            symbolic_clean_and::simplify(ops)
        } else {
            ops
        };
        ops.extend(tail);
        trailmix_port::search_optimized_tail_nonce(&ops);
        return ops;
    }
