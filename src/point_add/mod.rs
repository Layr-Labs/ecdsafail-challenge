...
pub fn build() -> Vec<Op> {
    ...
    if let Ok(mode) = std::env::var("LOWQ_Q794_NATIVE_MODE") {
        match mode.as_str() {
            ...
            "whole-count" | "whole-stream" | "whole-default-count" => {
                if mode!="whole-default-count"{std::env::set_var("LOWQ_Q794_EEA","1");}
                assert!(trailmix_port::inversion::q794_lifecycle::candidate_configuration(),"source-default Q794 configuration required");
                std::env::set_var("Q794_MOD4","1");
                std::env::set_var("POINT_ADD_COUNT_ONLY","1");
                if mode=="whole-stream"{
                    std::env::set_var("SPRINT_STREAM_CHECK","1");
                    std::env::set_var("SPRINT_STREAM_SEED","q794-c08-predeclared-independent-seed-v1");
                }
