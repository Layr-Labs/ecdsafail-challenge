...
pub fn build() -> Vec<Op> {
    ...
    // W46 (lane-kimi-w46): research-lane A16+A12 consumer-window leaf cut on
    // the T10 v10 route digit trees (q793_t10_routes_v10). Cap the tree address
    // at the proved T10-consumer bound min(2*A_hi-7,255)==min(4b+3,255) and
    // floor it at the block A support, both read off circ.q797_a_support.
    // Dropped leaves sit off the live address, where the routing unitary
    // cancels with its literal inverse; =0 restores the W45 bitstream exactly.
    set_default_env("Q793_A16_V10","1");
    set_default_env("Q793_A12_V12","1");
    // W47 (lane-kimi-w47): research-lane A17+A9a consumer-window leaf cut on
    // the T10 v12 quotient pop/mask route trees (q793_t10_routes_v12
    // exchanges). Cap the tree address at min(2*A_hi-7,hi_shipped) and floor
    // it at max(1,A_lo), both read off circ.q797_a_support. Dropped leaves sit
    // off the live address, where the routing unitary cancels with its
    // literal inverse; =0 restores the W46 bitstream exactly.
    set_default_env("Q793_A17_V12","1");
    set_default_env("Q793_A9A_V12","1");
    // W48 lever 1 (A11 `sum_low_cut`, lane-grok-a11 reference): lower C+S
    // address cut 256-A_hi inside gather_span (entry codec A+C+S in
    // [255,257]). Numerically identical to the A2 t_top floor on all 202
    // blocks; gathers that already pass min>=floor are unaffected, the
    // direct core gathers (min=2) drop the dead low leaves. =0 restores
    // the W47 bitstream exactly.
    set_default_env("Q793_A11_SUMLOWCUT","1");
    // W48 lever 2 (A26 SHORT-FLAG-SUM1, research lane tick-16 packet): the
    // short_flag value list was filtered on the UPPER C+S bound only; the
    // numerically identical t_top lower floor (gather_sum, two screens down)
    // makes [C+S==1] identically false on blocks 0..124, so the (h=0,[1])
    // group is dead there. b125 has floor==1 and keeps value=1 (off-by-one
    // boundary). =0 keeps floor=0 and restores the W47 bitstream exactly.
    set_default_env("Q793_SHORT_FLAG_LOW_CUT","1");
    // A19-sm0 (level-2 mask-ladder prefix on the idle v12 carry sm[0],
    // q793_t10_fused_v3 Range::equality) is implemented and same-vector
    // proven (research/w47/diff-sm0.log) but stays sweep-NACK'd
    // (sweep-20260913T051615Z kill list): parked OFF by default; =1
    // re-enables for A/B measurement.
    ...
    if let Ok(mode) = std::env::var("LOWQ_Q793_NATIVE_MODE") {
        match mode.as_str() {
            ...
            "t10-full-v13-w47-diff" => trailmix_port::inversion::q793_t10_full_check_v13::run_w47_differential(),
            "t10-v12-exchanges-w47-diff" => trailmix_port::inversion::q793_t10_full_check_v13::run_w47_exchanges_diff(),
