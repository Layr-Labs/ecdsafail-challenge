...
pub fn build() -> Vec<Op> {
    ...
    if std::env::var_os("MIDQ_PREFIX_TERMINAL_WIDE_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::midq_prefix_terminal_wide_selftest();
        return Vec::new();
    }
    ...
    if std::env::var_os("MIDQ_PACKED_SELFTEST").is_some() {
        trailmix_port::inversion::shrunken_pz_state_machine::midq_packed_selftest();
        return Vec::new();
    }
    ...
    if std::env::var("POINT_ADD_DIALOG_ROUTE").ok().as_deref() != Some("1") {
        let built = trailmix_port::build_builder();
        // MIDQ_EVAL_SHOTS (debug): keep the raw pre-pass stream for
        // trailmix_port::eval_shots below.
        let raw_for_eval = std::env::var_os("MIDQ_EVAL_SHOTS").map(|_| built.ops.clone());
        let mut ops = built.ops;
        ...
        if let Some(raw) = raw_for_eval {
            trailmix_port::eval_shots(&raw, &ops);
        }
        return ops;
    }
