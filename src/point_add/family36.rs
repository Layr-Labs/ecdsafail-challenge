...
pub(super) fn try_apply(
    ...
    if count-ix<10||r.data[r.at]!=5||s.family36_locations[node].binary_search(&ix).is_err(){return None;}
    ...
    if !plain(v[0],5,3)||!plain(v[1],3,2)||!plain(v[4],3,2){return None;}
    let [a,b,c]=v[0].q;let d=v[1].q[0];let e=v[1].q[1];let f=v[4].q[0];let g=v[4].q[1];
    if !cx(v[2],d,c)||!pair(v[3],c,e,d)||!cx(v[5],d,g)||!pair(v[6],c,e,d)||!cx(v[7],d,c)||!cx(v[8],c,e)||!pair(v[9],a,b,c){return None;}
    let wires=[a,b,c,d,e,f,g];let mut mapped=Vec::new();for q in wires{if q>=qlen{return None;}let p=s.qmap[qs+q];if p==QubitId(0)||mapped.contains(&p){return None;}mapped.push(p);}
    ...
    emit(5,&[0,1,2]);emit(3,&[3,4]);emit(3,&[3,2]);emit(3,&[5,6]);emit(3,&[3,6]);emit(5,&[2,4,6]);emit(3,&[3,2]);emit(3,&[2,4]);emit(5,&[0,1,2]);
    s.flush_raw();
    ...
    s.family36_counts[0] += 1;
    r.at = rr.at;
    Some(len)
}

pub(super) fn locations()->Vec<Vec<usize>>{let mut v=vec![Vec::new();2270];for l in include_str!("family36_locations.tsv").lines(){let r:Vec<usize>=l.split_whitespace().map(|s|s.parse().unwrap()).collect();v[r[1]].push(r[2]);}v}
#[cfg(test)]include!("family36_tests.rs");
