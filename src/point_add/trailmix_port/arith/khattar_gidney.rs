...
impl<'a> KgPrefixAnd<'a> {
    ...
    pub fn new_compact_refs(q: &[&'a QReg], anc_owned: &'a [QReg]) -> Self {
        let anc: Vec<&QReg> = anc_owned.iter().collect();
        Self::new_compact_borrowed(q, &anc)
    }

    /// Compact plan with caller-owned clean wires, including borrowed scratch.
    pub fn new_compact_borrowed(q: &[&'a QReg], anc_owned: &[&'a QReg]) -> Self {
        let n = q.len();
        let full = kg_prefix_ancilla_count(n);
        ...
        if compact == full {
            let anc_refs: Vec<&QReg> = anc_owned.iter().take(full).copied().collect();
...
/// Apply `(-1)^AND(bits)` and restore all inputs and prefix scratch.
/// The caller may condition the whole producer/phase/reverse on an HMR bit.
/// Unlike computing a fresh predicate and uncomputing it, this traverses the
/// prefix producer only once in each direction, with a Z/CZ endpoint.
pub(crate) fn phase_and_of_khattar_gidney_refs(circ: &mut Circuit, bits: &[&QReg]) {
    match bits.len() {
        0 => {
            let mut op = crate::circuit::Op::empty();
            op.kind = crate::circuit::OperationType::Neg;
            circ.b.push_op(op);
            return;
        }
        1 => {
        ...
        2 => {
            circ.cz(bits[0], bits[1]);
            return;
        }
        _ => {}
    }
    let anc_owned = circ.alloc_qreg_bits("kg_phase_anc", kg_prefix_ancilla_count(bits.len()));
    let anc_refs: Vec<&QReg> = anc_owned.iter().collect();
    let layers = kg_get_layers_for_prefix_and(bits, &anc_refs);
    let used = &layers[..=bits.len()];
    for layer in used {
        for &op in &layer.ops {
            op.emit(circ);
        }
    }
    match used.last().expect("full prefix layer").ctrls.as_slice() {
        [a] => circ.z(a),
        [a, b] => circ.cz(a, b),
        ctrls => panic!("KG phase endpoint needs 1 or 2 controls, got {}", ctrls.len()),
    }
    for layer in used.iter().rev() {
        for &op in layer.ops.iter().rev() {
            op.emit(circ);
        }
    }
