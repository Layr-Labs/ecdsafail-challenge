...
pub fn mcx_dirty_ladder(
    ...
    // Q795 route: while the step's dedicated always-clean scratch is installed, every k>=3 ladder
    // uses the exact one-clean-scratch 2k-3 form instead of the 4k-8 dirty echo. toggle restores
    // the scratch and every control, so callers' restore/cancel arguments are unchanged.
    if let Some(scratch) = crate::point_add::trailmix_port::inversion::length_recompute::clean_scratch() {
        let cs: Vec<(&QReg, bool)> = ctrls.iter().map(|&q| (q, true)).collect();
        crate::point_add::trailmix_port::inversion::paired_clean_mcx::toggle(circ, &cs, target, &scratch);
        return;
    }
