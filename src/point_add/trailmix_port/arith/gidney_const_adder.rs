...
#[path = "compact_const_carry.rs"]
mod compact_const_carry;
...
pub fn controlled_add_const_gidney_refs(
    ...
    assert!(dirty.len() >= n - 1, "need n-1 borrowed dirty bits");
    if std::env::var("MIDQ_COMPACT_CONST_CARRY").ok().as_deref() == Some("1")
        && !a.iter().chain(dirty).any(|bit| bit.id() == ctrl.id())
    {
        compact_const_carry::add(circ, ctrl, a, c, dirty);
        return;
    }
