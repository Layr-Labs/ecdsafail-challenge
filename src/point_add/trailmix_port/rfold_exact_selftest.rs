...
fn production_components(vent_cap: Option<usize>) -> usize {
    let mut seed = Shake256::default();
    ...
    for (kind, nb, alias) in variants {
        ...
        let old = compile(kind, vent_cap.is_some(), nb, alias);
...
pub(crate) fn run() {
    ...
    let full = production_components(None);
    match saved {
...
pub(crate) fn run_vents() {
    ...
    let full = production_components(Some(1024));
