...
#[path = "rfold_exact_selftest.rs"]
pub(crate) mod exact_selftest;
...
/// Same exact window permutation as the run-based fold, with idle high bits
/// borrowed as dirty carries. The donor is restored before any shift or MBU.
fn controlled_rfold_window(circ: &mut Circuit, ctrl: &QReg, a: &[QReg], r: &[u8]) {
    if std::env::var("MIDQ_OUTER_DIRTY_CONST").ok().as_deref() == Some("1") {
        crate::point_add::trailmix_port::arith::gidney_const_adder::controlled_add_const_gidney(
            circ, ctrl, &a[..RFOLD_WINDOW], r, &a[RFOLD_WINDOW..2 * RFOLD_WINDOW - 1],
        );
    } else {
        crate::point_add::trailmix_port::arith::const_add::controlled_add_const_runs_forced(
            circ, ctrl, &a[..RFOLD_WINDOW], r,
        );
    }
}
...
/// Spend only compile-time qubit headroom on exact carry measurement vents.
/// A source-aliased control needs a stable copy while the hybrid adder uses b
/// for carries; reserve that qubit before computing the available vent budget.
fn controlled_outer_int_add(circ: &mut Circuit, ctrl: &QReg, a: &[QReg], b: &[QReg]) {
    let cap = std::env::var("MIDQ_OUTER_VENT_QCAP").ok()
    ...
    assert!(!a.iter().any(|q| q.id() == ctrl.id()), "control aliases target");
    circ.flush_pending_frees();
    let aliases_b = b.iter().any(|q| q.id() == ctrl.id());
    let live = circ.b.active_qubits as usize + usize::from(aliases_b);
    let vents = cap.saturating_sub(live).min(a.len().saturating_sub(1));
    if vents == 0 {
        crate::point_add::trailmix_port::arith::ripple_add::controlled_add(circ, ctrl, a, b);
        return;
    }
    let copy = aliases_b.then(|| {
        let copy = circ.alloc_qreg("outer_cadd_ctrl");
        circ.cx(ctrl, &copy);
        copy
    });
    ...
    crate::point_add::trailmix_port::arith::gidney_const_adder::controlled_hybrid_add_refs(
        circ, copy.as_ref().unwrap_or(ctrl), &a_refs, &b_refs, vents,
    );
...
pub fn controlled_mod_add_rfold_mbu(circ: &mut Circuit, ctrl: &QReg, a: &[QReg], b: &[QReg]) {
    ...
    if nb == 257 {
        controlled_outer_int_add(circ, ctrl, a, b);
    } else {
    ...
    controlled_rfold_window(circ, &a[256], a, &r);
...
pub fn mod_double_rfold_mbu(circ: &mut Circuit, a: &[QReg]) {
    ...
    // on the window. The borrowed high bits are restored before the MBU.
    controlled_rfold_window(circ, &a[256], a, &r);
