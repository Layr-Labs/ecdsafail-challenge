...
impl Circuit {
    ...
    fn maybe_trace_live_names(&mut self) {
        ...
        let phase_filter = std::env::var("TRAILMIX_TRACE_LIVE_PHASE")
            .unwrap_or_else(|_| "p.bitlen".to_string());
        if !self.b.phase.contains(&phase_filter) {
            return;
        }
        if !self
            .live_trace_emitted
    ...
    /// Temporarily recycle zero storage whose owning handles are suspended.
    /// The body must not access those handles or let a new allocation escape.
    /// Reset checks validate both zero boundaries; the same wire IDs return.
    pub(crate) fn with_recycled_zero(&mut self, wires: &[&QReg], body: impl FnOnce(&mut Self)) {
        self.flush_pending_frees();
        let mut names = BTreeMap::new();
        for q in wires {
            assert!(!names.contains_key(&q.id), "duplicate recycled wire");
            let name = self.live_names.remove(&q.id).expect("recycled wire must be owned and live");
            names.insert(q.id, name);
            self.b.free(QubitId(q.id.into()));
        }
        body(self);
        self.flush_pending_frees();
        for (id, name) in names {
            self.b.reacquire(QubitId(id.into()));
            self.b.r(QubitId(id.into()));
            assert!(self.live_names.insert(id, name).is_none(), "recycled allocation escaped");
        }
    }
