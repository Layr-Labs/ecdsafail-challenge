...
pub fn mod_halve_canonical_mbu(circ: &mut Circuit, acc: &[QReg]) {
    ...
    if super::inversion::q792_lifecycle_r01::enabled() {
        // p = 2^256-R. A complemented positive sparse increment subtracts
        // R with only five prefix ancillas, without retaining a run chain.
        circ.cx(&input_was_odd,&acc[256]);
        for q in acc {circ.x(q);}
        super::arith::const_add::controlled_add_const_sparse(circ,&input_was_odd,acc,&secp256k1_r_le());
        for q in acc {circ.x(q);}
    } else {controlled_add_p_secp256k1(circ, &input_was_odd, acc);}
