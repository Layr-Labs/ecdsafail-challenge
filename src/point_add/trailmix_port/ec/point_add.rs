...
/// `MIDQ_EVAL_SHOTS` checkpoint of `tx` / `ty` at a top-level section boundary.
fn eval_mark_xy(circ: &Circuit, name: &str, tx: &[QReg], ty: &[QReg]) {
    if !crate::point_add::trailmix_port::eval_shots_enabled() {
        return;
    }
    let mut ids: Vec<u64> = tx.iter().map(|q| u64::from(q.id())).collect();
    ids.extend(ty.iter().map(|q| u64::from(q.id())));
    crate::point_add::trailmix_port::eval_mark(circ, name.to_string(), ids, None);
}
...
pub fn ec_add_inplace_shrunken_pz(
    ...
    eval_mark_xy(circ, "ec3.dy_build", tx, ty);
    ...
    eval_mark_xy(circ, "ec3.dx_build", tx, ty);
    if low_pressure_creg_qload_enabled() {
    ...
    eval_mark_xy(circ, "ec3.inv_fwd", tx, ty);
    ...
    *ty = dy_vec;
    ...
    if zero_dy_newdx_route_enabled() {
        ...
        eval_mark_xy(circ, "ec3.dy_zero", tx, ty);
        eval_mark_xy(circ, "ec3.dy_zero.lambda", &lambda, &[]);
    ...
    eval_mark_xy(circ, "ec3.alt.cancel", tx, ty);
    eval_mark_xy(circ, "ec3.alt.cancel.lambda", &lambda, &[]);
    ...
    *ty = ndy_vec;
    ...
    eval_mark_xy(circ, "ec3.alt.new_x_restore", tx, ty);
    if low_pressure_creg_qload_enabled() {
    ...
    eval_mark_xy(circ, "ec3.alt.new_y_restore", tx, ty);
    if low_pressure_creg_qload_enabled() {
    ...
    eval_mark_xy(circ, "ec3.unpad", tx, ty);
