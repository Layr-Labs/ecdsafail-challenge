...
fn configure_sub1000_trailmix_route() {
    ...
    set_default_env("MIDQ_PAYLOAD_VALUE_REPLAY_START", &inversion::shrunken_pz_state_machine::hybrid_profile::default_payload_replay_start().to_string());
    ...
    set_default_env("MIDQ_OUTER_VENT_QCAP", "1009");
    set_default_env("MIDQ_PZ_VENT_QCAP", "1009");
    set_default_env("MIDQ_COMPACT_CONST_CARRY", "1");
    set_default_env("MIDQ_QUOTIENT_CODE", "1");
    set_default_env("MIDQ_CHUNKED_PREFIX", "1");
    set_default_env("MIDQ_PREFIX_QCAP", "1009");
    set_default_env("MIDQ_CHUNK_COMPARE", "1");
    set_default_env("MIDQ_CHUNK_COMPARE_QCAP", "1009");
    ...
    set_default_env("MIDQ_ZERO_SCRATCH_QCAP", "1009");
    ...
    set_default_env("TRAILMIX_TAIL_NONCE", "16");
    set_default_env("HYBRID_QRETAIN", "32");
    set_default_env("HYBRID_QCAP", "1100");
    set_default_env("HYBRID_MARGIN_SUPPORT", "exact-positive-l31");
    set_default_env("MIDQ_D_ALIGN_DRIVER", "1");
    set_default_env("MIDQ_D_ALIGN_COMMON_WINDOW", "1");
    set_default_env("MIDQ_D_POSITION_CACHE", "1");
    inversion::shrunken_pz_state_machine::hybrid_profile::configure_runtime_caps();
}
...
pub fn build_builder() -> crate::point_add::B {
    ...
    if std::env::var_os("MIDQ_D_POSITION_CACHE_SELFTEST").is_some() || std::env::var_os("MIDQ_D_POSITION_CACHE_LEDGER").is_some() {
        inversion::shrunken_pz_state_machine::d_position_cache::diagnostic();
        return crate::point_add::B::new();
    }
    if std::env::var_os("MIDQ_D_ALIGN_ITERATION_SELFTEST").is_some() {
        inversion::shrunken_pz_state_machine::d_alignment_iteration::selftest();
        return crate::point_add::B::new();
    }
    if std::env::var_os("MIDQ_D_ALIGN_COMMON_WINDOW_SELFTEST").is_some() {
        inversion::shrunken_pz_state_machine::d_alignment_prep::common_window_selftest();
        return crate::point_add::B::new();
    }
    if std::env::var_os("MIDQ_D_ALIGNMENT_LEDGER").is_some() {
        inversion::shrunken_pz_state_machine::d_alignment_ledger::run();
        return crate::point_add::B::new();
    }
    if std::env::var_os("MIDQ_QRETAIN_SELFTEST").is_some() {
        inversion::shrunken_pz_state_machine::qretain::selftest();
        return crate::point_add::B::new();
    }
    if std::env::var_os("MIDQ_HYBRID_MARGIN_SELFTEST").is_some() {
        inversion::shrunken_pz_state_machine::hybrid_margin::selftest();
        return crate::point_add::B::new();
    }
    if std::env::var_os("MIDQ_HYBRID_PROFILE_SELFTEST").is_some() {
        inversion::shrunken_pz_state_machine::hybrid_profile::selftest();
        return crate::point_add::B::new();
    }
    if std::env::var_os("MIDQ_HYBRID_PROFILE_DUMP").is_some() {
        inversion::shrunken_pz_state_machine::hybrid_profile::dump_runtime_schedule();
        return crate::point_add::B::new();
    }
    inversion::shrunken_pz_state_machine::hybrid_profile::report();
    inversion::shrunken_pz_state_machine::hybrid_margin::report();
