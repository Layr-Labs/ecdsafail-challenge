...
pub fn build_builder() -> crate::point_add::B {
    ...
    // lanes are allocated before point addition and held across the Q816 peak,
    // so N padding lanes produce an exact Q(816 + N) circuit.  Each lane is
    ...
    const NOVELTY_PAD_DEFAULT: usize = 183;
    ...
    let ops_capacity = if inversion::paper2607_eea::enabled() {
        ...
        // Exact Q816 Aux4 whole-circuit count. Reserving the final length
        // avoids geometric Vec growth above the 64 GiB evaluator envelope.
        909_537_025 + 3 * novelty_pad_qubits
    } else {
        751_661_003
    };
    ...
    if inversion::paper2607_eea::enabled() {
        let structural_t = builder.counted_kind_ops[OperationType::CCX as usize]
            + builder.counted_kind_ops[OperationType::CCZ as usize];
        assert_eq!(
            builder.peak_qubits,
            816 + u32::try_from(novelty_pad_qubits).expect("novelty padding must fit u32"),
        ...
        assert_eq!(structural_t, 692_379_243, "paper2607 Toffoli drift");
    }
