...
pub mod inversion {
    ...
    pub mod q794_r01;
    pub mod q794_r01_odd_cache;
    pub mod q794_exit_frames;
    pub mod q794_exit_prefix;
    pub mod q794_sum_xor;
    pub mod q794_rank_fold;
    pub mod q794_sign_fold;
    pub mod q794_sign_fold_maj;
    #[path="sign_phase_native_check.rs"] pub(crate) mod q794_mbu_native;
    pub(crate) mod q794_mbu_checkpoint;
    pub mod q794_t10;
    pub mod q794_t10_fused;
    pub mod q794_t10_quotient;
    pub mod q794_t10_check;
    pub mod q794_sign;
    pub mod q794_sign_maj_history;
    pub mod q795_r01_s_affine;
...
pub fn build_builder() -> crate::point_add::B {
    ...
    // Reserve the measured source-baked stream exactly. Q794 requires
    // 69,581,397,312 bytes of Op payload, below the128GB submission budget.
    // Unmeasured Q794 overrides are diagnostic-only: never grow a stale Vec.
    let ops_capacity = if inversion::q794_lifecycle::enabled() {
    ...
    if inversion::q794_lifecycle::enabled() {
        if inversion::q794_mbu::enabled(){let (ct,packets)=inversion::q794_mbu::totals();let static_t=builder.counted_kind_ops[OperationType::CCX as usize]+builder.counted_kind_ops[OperationType::CCZ as usize];eprintln!("Q794_MBU_WHOLE peak={} operations={} static_T={} conditional_MBU_T={} expected_T_times2={} packets={} classical_bits={}; expected from exact balanced measurements, actual replay separate",builder.peak_qubits,builder.current_ops_len(),static_t,ct,2*static_t as u64-ct,packets,builder.next_bit);}
        ...
        assert!(builder.peak_qubits<=807+u32::try_from(novelty_pad_qubits).unwrap(),"Q807 prefix-plus-consumer-cache whole cap exceeded");
        ...
        if inversion::q794_lifecycle::candidate_configuration(){
            // Retain the verified upstream allocation capacity during candidate
            // qualification; actual resource counts above are never replaced
            // by these ceilings. Width and strict 128GB checks remain in force.
            assert!(builder.current_ops_len()<=inversion::q794_lifecycle::CANDIDATE_OPS+3*novelty_pad_qubits,"Q794 operation capacity exceeded");
            assert!(builder.counted_kind_ops[OperationType::CCX as usize]+builder.counted_kind_ops[OperationType::CCZ as usize]<=inversion::q794_lifecycle::CANDIDATE_TOFFOLI,"Q794 Toffoli ceiling exceeded");
        }
    } else if inversion::rank5_lifecycle::enabled() {
