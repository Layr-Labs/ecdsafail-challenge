...
pub fn build_builder() -> crate::point_add::B {
    ...
    let ops_capacity = if inversion::paper2607_eea::enabled() {
        ...
        989_862_278 + 3 * novelty_pad_qubits
    } else {
        751_661_003
    };
    ...
    if inversion::paper2607_eea::enabled() {
        ...
        assert_eq!(
            builder.peak_qubits,
            // tight-decode EEA carries a +9 clean TAnc decode pool (TIGHT_ANC_WIDTH)
            822 + u32::try_from(novelty_pad_qubits).expect("novelty padding must fit u32"),
            "paper2607 peak-qubit drift"
        );
        assert_eq!(
            builder.current_ops_len(),
            549_041_686 + 3 * novelty_pad_qubits,
            "paper2607 emitted-op drift"
        );
        assert_eq!(structural_t, 264_696_587, "paper2607 Toffoli drift");
    }
