...
pub fn build_builder() -> crate::point_add::B {
    ...
    let ops_capacity = if inversion::paper2607_eea::enabled() {
        ...
        // Exact Q819 Aux7 Phase2-loan whole-circuit count. Reserving the final length
    ...
    if inversion::paper2607_eea::enabled() {
        let structural_t = builder.counted_kind_ops[OperationType::CCX as usize]
            + builder.counted_kind_ops[OperationType::CCZ as usize];
        assert_eq!(builder.peak_qubits, 819, "paper2607 peak-qubit drift");
        assert_eq!(
            builder.current_ops_len(),
            1_025_185_268,
            "paper2607 emitted-op drift"
        );
        assert_eq!(structural_t, 861_065_959, "paper2607 Toffoli drift");
    }
