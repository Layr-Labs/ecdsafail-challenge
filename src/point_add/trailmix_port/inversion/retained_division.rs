...
pub(super) fn add_update(c: &mut Circuit, gate: &QReg, a: &[&QReg], b: &[&QReg], subtract: bool) {
...
fn ctz_shift(
    ...
    // Forward enters with shift=active*ctz_sentinel(q), inverse with shift=0.
    // A direct XOR deposit implements both boundaries without a temporary word.
    if direct_ctz::try_apply(c, q, shift, active) { return; }
...
pub(super) mod tests {
    ...
    pub(crate) fn retained_lengths_match_original() {
        if std::env::var_os("MIDQ_DIRECT_CTZ_SELFTEST").is_some() {
            direct_ctz::selftest();
        }
