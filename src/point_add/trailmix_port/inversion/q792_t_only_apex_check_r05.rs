//! A253/254 apex T10 physical test, scalar ADD/pop oracle and packed low3 chart.
...
pub fn run(){
    ...
    for block in 0..1{for j in 0..4{
        ...
        let n=259;circ.q797_a_support=Some((0,256));
        let mut built=Circuit::new();built.b.count_only=false;built.b.fiat_hash=None;built.q797_a_support=Some((0,256));let physical=built.alloc_qreg_bits("fold20-physical",564);
        super::emit(&mut built,&physical[..20],&physical[20],&physical[21],&physical[23..282],&physical[282..541],&physical[541..],n,j);assert_eq!(built.b.next_qubit,564);let b=built.into_builder();
        let pack=|old:&Vec<u64>|{let mut out=vec![0u64;564];out[20..].copy_from_slice(&old[21..]);for lane in 0..64{let rd=|at:usize,n:usize|(0..n).fold(0usize,|v,i|v|(((old[at+i]>>lane)&1)as usize)<<i);let rank=rd(0,5);let a=rd(5,6);let cl=rd(11,6);let sm=rd(17,4);let code=if rank==29&&a==63{assert!(sm<4);47|(cl<<10)|(sm<<16)}else{super::super::q792_fold20_rank_r01::encode(rank,a,cl,sm,&ts)};for i in 0..20{out[i]|=(((code>>i)&1)as u64)<<lane;}}out};
        for op in &b.ops{op.validate();assert!(matches!(op.kind,K::X|K::CX|K::CCX));for i in [256,257,258]{let q=physical[23+i].id()as u64;assert!(op.q_target.0!=q&&op.q_control1.0!=q&&op.q_control2.0!=q,"T10 touches omitted W1[{i}]");}}
        ...
        let mut export=std::env::var_os("PHASE_ROUTING_EXPORT_DIR").map(|dir|{
            ...
            for op in &b.ops{let kind=match op.kind{K::X=>0u32,K::CX=>1,K::CCX=>2,_=>unreachable!()};for x in [kind,op.q_target.0 as u32,op.q_control1.0 as u32,op.q_control2.0 as u32]{out.write_all(&x.to_le_bytes()).unwrap();}}
            std::fs::File::create_new(root.join(format!("j{j}-cases.bin"))).unwrap()
        });
        let rows:Vec<_>=data[12..].chunks_exact(138).filter(|r|{let time=u16::from_le_bytes(r[..2].try_into().unwrap())as usize;(time-1)%4==j}).collect();if rows.is_empty(){continue;}
        let mut f=Fixed;let mut sim=Simulator::new(564,0,&mut f);
        for pattern in 0..6{for batch in 0..rows.len().div_ceil(64){
            ...
            for lane in 0..64{
                ...
                let rk=bits(r,0,5);let av=64*ts[rk][0]+bits(r,5,6);let cv=64*ts[rk][1]+bits(r,11,6);let ph1=bit(r,21)as usize;let ph2=bit(r,22)as usize;let phase=2*ph1+ph2;
                let mut sv=64*ts[rk][2]+4*bits(r,17,4)+(j&1)+2*((j>>1)^(ph1&(j&1))^((ph1^ph2)&(cv&1)));
                let tag=phase==3&&bit(r,23)&&av==0&&cv==1&&sv==0;if tag{sv=256;}let on=phase==2&&(av==253||av==254);
                if on{
                ...
                for(w,output)in[(&mut before,false),(&mut after,true)]{
                    ...
                    let(second_site,second_base)=match phase{0=>(&w2[av+2],false),1=>(&w2[av+1],false),2 if cv==1=>(&w1[av+2],!(output&&on)),2=>(&w2[av+3],false),_=>(&w1[av+if tag{3}else{2}],false)};
            ...
            if let Some(out)=export.as_mut(){
                use std::io::Write;
                if batch%31==0||batch+1==rows.len().div_ceil(64){for lane in 0..64{for state in [&before,&after]{for word in 0..9{let mut packed=0u64;for bit in 0..64{let index=word*64+bit;if index<564{packed|=((state[index]>>lane)&1)<<bit;}}out.write_all(&packed.to_le_bytes()).unwrap();}}}}
                continue;
            }
            sim.qubits.copy_from_slice(&before);sim.phase=0x937c1546a123b9ff;sim.apply_iter(b.ops.iter());
            if sim.qubits!=after{let diffs:Vec<_>=sim.qubits.iter().zip(&after).enumerate().filter(|(_, (x,y))|x!=y).map(|(i,(x,y))|(i,format!("{:016x}",x^y))).collect();panic!("T-only apex T block{block} j{j} pattern{pattern} batch{batch} diffs={diffs:?}");}
        ...
        // Canonical terminal history is excluded even if phase rails are
        // arbitrary. Keep its address255 zero lender, vary every other word.
        if export.is_none(){for first in (0..1024).step_by(64){
            let mut seed=0x792a253254u64^(first as u64)^((j as u64)<<32);
            let mut before:Vec<_>=(0..564).map(|_|rnd(&mut seed)).collect();
            for lane in 0..64{let key=first+lane;let history=key/4;let phase=key%4;let code=47|(history<<10);
                for i in 0..20{let mask=1u64<<lane;before[i]=(before[i]&!mask)|(((code>>i&1)as u64)<<lane);}
                for(i,value)in[(20,phase>>1),(21,phase&1),(540,0)]{let mask=1u64<<lane;before[i]=(before[i]&!mask)|((value as u64)<<lane);}
            }
            sim.qubits.copy_from_slice(&before);sim.phase=0x931512ab7d2253ff;sim.apply_iter(b.ops.iter());assert_eq!(sim.qubits,before,"apex terminal offguard j{j} first{first}");assert_eq!(sim.phase,0x931512ab7d2253ff);total+=64;
        }}
        if export.is_some(){eprintln!("PHASE_ROUTING_EXPORTED_CLOCK j={j} ops={} T={t}",b.ops.len());}else{eprintln!("T_ONLY_APEX_R05_BLOCK block={block} j={j} ops={} T={t} PASS",b.ops.len());}
    }}
    if std::env::var_os("PHASE_ROUTING_EXPORT_DIR").is_some(){eprintln!("PHASE_ROUTING_EXPORT_PASS scalar_references_only=true native_simulation_executed=false");return;}
    eprintln!("T_ONLY_APEX_R05_PASS lanes={total} active={active} C1={c1_count} M255={m255} M256={m256} shortA={short} A253+={extreme} terminal={terminal} C256={c256} weighted_forward_ops={weighted_ops} weighted_forward_T={weighted_t}; all saved geometries/all4passengerpairs+2random/packed3holes/dirty/phase/literalinverse; NOT fullstep or wholeQ793");
}
