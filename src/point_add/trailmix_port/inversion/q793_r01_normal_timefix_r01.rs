...
thread_local!{static SUPPORT_END:std::cell::Cell<Option<usize>>=const{std::cell::Cell::new(None)};}
fn set_support_end(v:Option<usize>)->Option<usize>{SUPPORT_END.with(|s|s.replace(v))}
fn numeric_route<'a>(circ:&mut Circuit,m:&[QReg],word:&'a[QReg],offset:usize)->&'a QReg{
    assert_eq!(m.len(),8);assert!(offset<=1);
    let mut lo=circ.q797_a_support.map_or(0,|(lo,_)|lo.min(253));
    // A6a slack-3: live scan covers word [rfirst,255]; reference writes [M+3,255]
    // iff M >= rfirst-3. Slack-2 HARD_NACK (prunes a supported leaf).
    if let Some(se)=SUPPORT_END.with(|s|s.get()){
        lo=lo.max((259usize.saturating_sub(se)).saturating_sub(3)).min(253);
    }
...
fn seed(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],g:&QReg,mask:&QReg,ha:&QReg,
    ...
    for a_class in [0,1,2,252,253]{
        if circ.q797_a_support.is_some_and(|(lo,hi)|!(lo<=a_class&&a_class<hi)){continue;}
    ...
    fn low_update_original(&self,circ:&mut Circuit,w1:&[QReg],w2:&[QReg],decision:&QReg){
        for shift in 0..3{if (shift+1)%2!=self.j%2{continue;}
            ...
            if shift==0&&allow_a1{
                ...
                super::q793_r01_a1normalize_timefix_r01::q0_xor(circ,self.rank,self.c,w1,&base,v[2],self.dirty,self.support_end);
            ...
            let mut change=|target:usize,extra:&[(&QReg,bool)]|{
                ...
                if circ.q797_a_support.map_or(true,|(lo,hi)|lo<=0&&0<hi){
                ...
                for ac in [252,253]{let keep=256usize.saturating_sub(ac+shift).min(3);
                    if extra.iter().any(|&(q,_)|(keep..3).any(|i|q.id()==v[i].id())){
                        if circ.q797_a_support.map_or(true,|(lo,hi)|lo<=ac&&ac<hi){
