...
pub fn run(){
    ...
    let support_hi=std::env::var("Q792_T_TEST_A_HI").ok().map(|v|v.parse::<usize>().unwrap()).unwrap_or(256);assert!((4..=256).contains(&support_hi));eprintln!("T_ONLY_UNIFIED_R01_INTERFACE_SUPPORT A_hi={support_hi} M_max={}",(2*support_hi).saturating_sub(7));
    ...
    for block in 0..1{for j in 0..4{
        ...
        let n=259;circ.q797_a_support=Some((0,support_hi));
        let mut built=Circuit::new();built.b.count_only=false;built.b.fiat_hash=None;built.q797_a_support=Some((0,support_hi));let physical=built.alloc_qreg_bits("fold20-physical",564);
        ...
        let rows:Vec<_>=data[12..].chunks_exact(138).filter(|row|{
            let time=u16::from_le_bytes(row[..2].try_into().unwrap())as usize;if (time-1)%4!=j{return false;}
            let r=&row[2..70];let rk=bits(r,0,5);let av=64*ts[rk][0]+bits(r,5,6);
            // The retained global address tree always includes the terminal255
            // alias. All other phases obey the declared public A support.
            if av==255{return true;}if av>=support_hi{return false;}
            if bit(r,21)&&!bit(r,22){let cv=64*ts[rk][1]+bits(r,11,6);let semantic_c=if cv==0{256}else{cv};if av+semantic_c>(2*support_hi).saturating_sub(7){return false;}}
            true
        }).collect();if rows.is_empty(){continue;}
        ...
        for pattern in 0..6{for batch in 0..rows.len().div_ceil(64){
            ...
            for lane in 0..64{
                ...
                let tag=phase==3&&bit(r,23)&&av==0&&cv==1&&sv==0;if tag{sv=256;}let on=phase==2&&av<=252;
                if on{
            ...
            if sim.qubits!=after{let diffs:Vec<_>=sim.qubits.iter().zip(&after).enumerate().filter(|(_, (x,y))|x!=y).map(|(i,(x,y))|(i,format!("{:016x}",x^y))).collect();panic!("T-only full block{block} j{j} pattern{pattern} batch{batch} diffs={diffs:?}");}
        ...
        if export.is_some(){eprintln!("PHASE_ROUTING_EXPORTED_CLOCK j={j} ops={} T={t}",b.ops.len());}else{eprintln!("T_ONLY_UNIFIED_R01_BLOCK block={block} j={j} ops={} T={t} PASS",b.ops.len());}
    }}
    ...
    eprintln!("T_ONLY_UNIFIED_R01_PASS lanes={total} active={active} C1={c1_count} M255={m255} M256={m256} shortA={short} A253+={extreme} terminal={terminal} C256={c256} weighted_forward_ops={weighted_ops} weighted_forward_T={weighted_t}; all saved geometries/all4passengerpairs+2random/packed3holes/dirty/phase/literalinverse; NOT fullstep or wholeQ793");
}
