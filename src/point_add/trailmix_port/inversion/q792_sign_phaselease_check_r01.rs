...
fn export_case(out:&mut std::fs::File,before:&[u64],after:&[u64]){
 use std::io::Write;for lane in 0..64{for state in [before,after]{for word in 0..9{let mut packed=0u64;for bit in 0..64{let i=word*64+bit;if i<state.len(){packed|=((state[i]>>lane)&1)<<bit;}}out.write_all(&packed.to_le_bytes()).unwrap();}}}
}
pub fn run(){
    ...
    for j in 0..4{
        ...
        let mut circ=Circuit::new();let m=circ.alloc_qreg_bits("folded",20);
        ...
        emit(&mut circ,&m,&p1,&p2,&sign,&w1,&w2,&helpers,j,256);assert_eq!(circ.b.next_qubit,owned);let builder=circ.into_builder();
        ...
        eprintln!("FOLD20_SIGN_PHASELEASE_BUILT j={j} rows={} ops={} T={}",rows.len(),builder.ops.len(),builder.ops.iter().filter(|o|o.kind==K::CCX).count());
        if count_only{continue;}
        let mut export=std::env::var_os("SIGN_MEMO_EXPORT_DIR").map(|dir|{
            use std::io::Write;let root=std::path::PathBuf::from(dir);let mut out=std::fs::File::create_new(root.join(format!("j{j}-ops.bin"))).unwrap();
        ...
        for pattern in 0..11{for batch in 0..rows.len().div_ceil(64){
            ...
            for lane in 0..64{let r=rows[(64*batch+lane)%rows.len()];let av=r[128]as usize;let cv=r[129]as usize;let sv=r[130]as usize;let phase=if pattern<8{3}else{pattern-8};
                ...
                for w in [&mut before,&mut after]{
                    let encoded=super::super::q792_fold20_rank_r01::encode(rk,av&63,cv&63,(sv>>2)&15,&ts);for i in 0..20{put(w,&m[i],lane,encoded>>i&1!=0);}
            ...
            if sim.qubits!=after{let bad:Vec<_>=sim.qubits.iter().zip(&after).enumerate().filter(|(_, (x,y))|x!=y).map(|(i,(x,y))|(i,format!("{:016x}",x^y))).collect();
                let lanes:u64=sim.qubits.iter().zip(&after).map(|(x,y)|x^y).fold(0,|x,y|x|y);let cases:Vec<_>=(0..64).filter(|&l|lanes>>l&1!=0).map(|l|{let r=rows[(64*batch+l)%rows.len()];(l,r[128],r[129],r[130],r[131])}).collect();panic!("Sign20 phaselease j={j} pattern={pattern} batch={batch} diffs={bad:?} cases={cases:?}");}
            assert_eq!(sim.phase,0);sim.apply_iter(builder.ops.iter().rev());assert_eq!(sim.qubits,before);assert_eq!(sim.phase,0);total+=64;
        }}
    }
    if std::env::var_os("SIGN_MEMO_EXPORT_DIR").is_some(){eprintln!("SIGN_MEMO_EXPORT_PASS scalar_references_only=true native_simulation_executed=false");return;}
    if count_only{eprintln!("FOLD20_SIGN_PHASELEASE_COUNT_ONLY all4templates; no native PASS claim");return;}
    eprintln!("FOLD20_SIGN_PHASELEASE_PASS lanes={total} active={active} short={short} S0={s0} A254={a254} scalar_rows={rowcount}; complete Sign wrapper, one funded mask, no clean carry, all three absent rails, all eight cargo patterns and three off phases, exact phase/inverse/allwire restoration; no whole-step claim");
}
