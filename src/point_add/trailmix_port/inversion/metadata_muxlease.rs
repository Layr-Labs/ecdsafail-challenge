...
pub(super) fn emit_chart_compute(circ:&mut Circuit,rank:&[&QReg],plan:&SwapPlan,d:&QReg,rest:&[QReg]){
    if let SwapPlan::Esop(sh,cubes)=plan{
        if let &Some((i,j))=sh{circ.cx(rank[j],rank[i]);}
        for &(m,v) in cubes{let cs:Vec<(&QReg,bool)>=(0..rank.len()).filter(|&b|m>>b&1!=0).map(|b|(rank[b],v>>b&1!=0)).collect();match cs.len(){0=>circ.x(d),_=>mixed_mcx(circ,&cs,d,rest)}}
        if let &Some((i,j))=sh{circ.cx(rank[j],rank[i]);}
        return;
    }
    let (frame,polarity,terms):(Vec<(usize,usize)>,usize,&Vec<usize>)=match plan{SwapPlan::Anf(p,t)=>(Vec::new(),*p,t),SwapPlan::Framed(f,p,t)=>(f.clone(),*p,t),SwapPlan::Esop(..)=>unreachable!()};
...
impl SwapPlan{
    /// Toffoli count of the rank chart alone (frame/shear CXs are free), E=0.
    pub(super) fn chart_t(&self)->usize{match self{SwapPlan::Anf(_,ts)|SwapPlan::Framed(_,_,ts)=>ts.iter().map(|&m|nccx(m.count_ones()as usize)).sum(),SwapPlan::Esop(_,cubes)=>cubes.iter().map(|&(m,_)|nccx(m.count_ones()as usize)).sum()}}
}
...
/// research/w5 scorer output, or — W45e joint arbitration — a shear-ESOP
/// winner (optional single input shear x_i^=x_j + ESOP cubes over mixed
/// literals, bulengerk 792 lineage via lane-kimi-w5b). `Framed` emits
/// f(x)=ANF(G*x^p) as [frame CXs][X(p)][skeleton][X(p)][reverse frame]; the
/// elimination network implements G^-1, so its reverse implements G at
/// identical length (the scorer's cx_len convention, bank_common::w5_cx_len).
/// `Esop` emits [shear][skeleton][cubes][skeleton][shear]; negative literals
/// are X-wrapped by mixed_mcx at emission.
pub(super) enum SwapPlan{Anf(usize,Vec<usize>),Framed(Vec<(usize,usize)>,usize,Vec<usize>),Esop(Option<(usize,usize)>,Vec<(usize,usize)>)}
...
pub(super) fn swap_plan(truth:Vec<bool>,width:usize,ext:usize,model:McxModel)->SwapPlan{
    ...
    let mut best=SwapPlan::Anf(polarity,terms);
    let (mut bt,mut bo)=(old_t,old_ops);
    let mut affine:Option<(usize,usize)>=None;
    if std::env::var("Q793_TABLE_AFFINE").ok().as_deref()!=Some("0"){
        let mask=truth.iter().enumerate().fold(0u64,|a,(i,&b)|a|if b{1u64<<i}else{0});
        if let Some(e)=super::q793_table_affine::lookup(width,mask){
            ...
            if new_t<old_t&&new_ops<=old_ops{best=SwapPlan::Framed(frame,p,new_terms);bt=new_t;bo=new_ops;affine=Some((new_t,new_ops));}
        }
    }
    // W45e joint arbitration: the shear-ESOP challenger competes against the
    // W45 winner (baked affine frame where it took, else polarity-ANF) under
    // the tree's site-exact cost model. Accept only a strictly smaller T
    // actual with ops not up; ties keep the incumbent (deterministic).
    // Q793_TABLE_ESOP=0 drops the offer and reproduces the W45 frozen stream.
    let mut verdict=if affine.is_some(){"affine"}else{"anf"};
    match esop_offer(&truth,width,ext,model){
        Some((plan,t,ops))=>{
            if t<bt&&ops<=bo{best=plan;verdict="esop";}
            arbitration_log(width,ext,model,&truth,(old_t,old_ops),affine,Some((t,ops)),verdict);
        }
        None=>arbitration_log(width,ext,model,&truth,(old_t,old_ops),affine,None,verdict),
    }
    let _=(bt,bo);
    best
}
/// Shear-ESOP offer for the joint arbitration (lane-kimi-w5b port of the
/// bulengerk 792 ESOP lineage): identity plus the n(n-1) single shears, each
/// solved by q793_table_esop_r01::plan and scored site-exactly with the tree's
/// k=popcount+ext 0/1/4k-8 model, per-cube negative-literal X-wraps
/// 2*popcount(m^v), +2 for the skeleton pair and +2 for the shear wrap. Every
/// candidate is exhaustively re-verified as the COMPOSED shear+cube emission
/// on every address. Dirty sites only: clean-scratch route tables keep the
/// affine/ANF verdict, exactly as in the W5b port. Cached per (truth,ext).
fn esop_offer(truth:&[bool],width:usize,ext:usize,model:McxModel)->Option<(SwapPlan,usize,usize)>{
    esop_challenger(truth,width,ext,model).map(|(sh,cubes,t,ops)|(SwapPlan::Esop(sh,cubes),t,ops))
}
/// The shear-ESOP challenger itself: best (shear,cubes,t,ops) over identity
/// plus the n(n-1) single shears, or None when gated off or no plan exists.
/// `ops` includes the swap-shaped skeleton pair (+2); bespoke sites with a
/// different emission shape re-score ops themselves from (shear,cubes).
pub(super) fn esop_challenger(truth:&[bool],width:usize,ext:usize,model:McxModel)->Option<(Option<(usize,usize)>,Vec<(usize,usize)>,usize,usize)>{
    if std::env::var("Q793_TABLE_ESOP").ok().as_deref()==Some("0")||std::env::var("Q796_MUX_POLARITY").ok().as_deref()==Some("0"){return None;}
    // Layered kill switches: Q793_TABLE_AFFINE=0 restores the W4 stream, so
    // the ESOP offer (a layer above the W45 affine winners) is dropped too.
    if std::env::var("Q793_TABLE_AFFINE").ok().as_deref()==Some("0"){return None;}
    if !matches!(model,McxModel::Dirty)||!(4..=6).contains(&width)||ext+width>22{return None;}
    use std::collections::HashMap;use std::sync::{Mutex,OnceLock};
    static CACHE:OnceLock<Mutex<HashMap<(Vec<bool>,usize),Option<(Option<(usize,usize)>,Vec<(usize,usize)>,usize,usize)>>>>=OnceLock::new();
    let mut cache=CACHE.get_or_init(||Mutex::new(HashMap::new())).lock().unwrap();
    let key=(truth.to_vec(),ext);
    if let Some(found)=cache.get(&key){return found.clone();}
    let truth=&key.0;
    let mut best:Option<(Option<(usize,usize)>,Vec<(usize,usize)>,usize,usize)>=None;
    for shear in 0..=(width*(width-1)){
        let(g,sh)=if shear==0{(truth.clone(),None)}else{let mut k=shear-1;let i=k/(width-1);k%=width-1;let j=if k<i{k}else{k+1};((0..truth.len()).map(|x|truth[x^(((x>>j)&1)<<i)]).collect(),Some((i,j)))};
        let Some(cubes)=super::q793_table_esop_r01::plan(width,&g,ext)else{continue};
        let t:usize=cubes.iter().map(|&(m,_)|nccx(ext+m.count_ones()as usize)).sum();
        let ops:usize=2+cubes.iter().map(|&(m,v)|nccx(ext+m.count_ones()as usize).max(1)+2*(m^v).count_ones()as usize).sum::<usize>()+2*sh.is_some()as usize;
        for x in 0..truth.len(){let sx=match sh{None=>x,Some((i,j))=>x^(((x>>j)&1)<<i)};let got=cubes.iter().fold(false,|v,&(m,b)|v^(sx&m==b));assert_eq!(got,truth[x],"shear-ESOP synthesis changed truth table");}
        if best.as_ref().is_none_or(|b|(t,ops)<(b.2,b.3)){best=Some((sh,cubes,t,ops));}
    }
    cache.insert(key,best.clone());
    best
}
/// Structural T of one n-control dirty-ladder mixed_mcx (4n-8 for n>=3),
/// exposed for bespoke-site ops models.
pub(super) fn mcx_t(n:usize)->usize{nccx(n)}
/// Joint-arbitration verdict log (diagnostic only): with
/// LOWQ_W45E_ARBITRATION_LOG=1, one line per distinct (truth,width,ext)
/// recording the polarity-ANF, baked-affine and shear-ESOP offers as
/// t/ops pairs and the verdict. No op-stream effect, on or off.
fn arbitration_log(width:usize,ext:usize,model:McxModel,truth:&[bool],anf:(usize,usize),affine:Option<(usize,usize)>,esop:Option<(usize,usize)>,verdict:&str){
    if std::env::var("LOWQ_W45E_ARBITRATION_LOG").ok().as_deref()!=Some("1"){return;}
    use std::collections::HashSet;use std::sync::{Mutex,OnceLock};
    static SEEN:OnceLock<Mutex<HashSet<(Vec<bool>,usize,usize,bool)>>>=OnceLock::new();
    let mut seen=SEEN.get_or_init(||Mutex::new(HashSet::new())).lock().unwrap();
    if !seen.insert((truth.to_vec(),width,ext,matches!(model,McxModel::Clean))){return;}
    let mask=truth.iter().enumerate().fold(0u64,|a,(i,&b)|a|if b{1u64<<i}else{0});
    let fmt=|c:Option<(usize,usize)>|c.map(|(t,o)|format!("{t}/{o}")).unwrap_or_else(||"-".to_string());
    let m=match model{McxModel::Dirty=>"dirty",McxModel::Clean=>"clean"};
    eprintln!("W45E_ARBITRATION width={width} ext={ext} model={m} truth={mask:#018x} anf={} affine={} esop={} verdict={verdict}",fmt(Some(anf)),fmt(affine),fmt(esop));
}
...
pub(super) fn emit_plan(circ:&mut Circuit,controls:&[&QReg],plan:SwapPlan,left:&QReg,right:&QReg,skel_pre:impl Fn(&mut Circuit),skel_post:impl Fn(&mut Circuit),mut toggle:impl FnMut(&mut Circuit,Vec<(&QReg,bool)>,&QReg)){
    if let SwapPlan::Esop(sh,cubes)=&plan{
        // Shear wrap outside the skeleton; literal values ride the controls
        // (mixed_mcx X-wraps the negative ones; the ops were charged above).
        if let Some((i,j))=sh{circ.cx(controls[*j],controls[*i]);}
        skel_pre(circ);
        for &(m,v) in cubes{let mut cs=vec![(left,true)];cs.extend((0..controls.len()).filter(|&i|m>>i&1!=0).map(|i|(controls[i],v>>i&1!=0)));toggle(circ,cs,right);}
        skel_post(circ);
        if let Some((i,j))=sh{circ.cx(controls[*j],controls[*i]);}
        return;
    }
    let (frame,polarity,terms)=match plan{SwapPlan::Anf(p,t)=>(Vec::new(),p,t),SwapPlan::Framed(f,p,t)=>(f,p,t),SwapPlan::Esop(..)=>unreachable!()};
