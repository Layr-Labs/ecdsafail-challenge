...
pub(super) fn mcx_t(n:usize)->usize{match n{0|1=>0,2=>1,_=>4*n-8}}
/// Davio/ESOP candidate for one table under the site's own cost model: `shear`
/// is an optional single input shear x_i ^= x_j (self-inverse CX wrap), and the
/// cubes are (mask, literal values) over the sheared inputs. Ported from
/// bulengerk's public 792 lineage (q792_esop_r01); `paired` stays dead.
pub(super) struct EsopSwap{pub shear:Option<(usize,usize)>,pub cubes:Vec<(usize,usize)>,pub t:usize,pub ops:usize}
impl Clone for EsopSwap{fn clone(&self)->Self{EsopSwap{shear:self.shear,cubes:self.cubes.clone(),t:self.t,ops:self.ops}}}
pub(super) struct TableSwap{pub polarity:usize,pub terms:Vec<usize>,pub anf_t:usize,pub anf_ops:usize,pub esop:Option<EsopSwap>}
impl TableSwap{
    /// The 792 table() rule: ESOP replaces polarity-ANF only by_t with ops not up.
    pub(super) fn winner(&self)->Option<&EsopSwap>{self.esop.as_ref().filter(|e|e.ops<=self.anf_ops&&(e.t,e.ops)<(self.anf_t,self.anf_ops))}
    /// Chart T of the winning synthesis (rank-echo / zero-factor cost models).
    pub(super) fn chart_t(&self)->usize{self.winner().map(|e|e.t).unwrap_or(self.anf_t)}
}
/// One cached offer per (truth, extra): the polarity-ANF winner plus the best
/// shear+ESOP candidate (identity plus the n(n-1) single shears, no Möbius
/// frames) scored with the tree's k=popcount+extra 4k-8 model. Every candidate
/// is exhaustively re-verified as the COMPOSED emission on every address.
/// Q793_TABLE_ESOP=0 drops the offer; emission is then bit-identical.
pub(super) fn swap_offer(truth:Vec<bool>,width:usize,extra:usize)->TableSwap{
    use std::collections::HashMap;use std::sync::{Mutex,OnceLock};
    let(polarity,terms)=swap_terms(truth.clone(),width);
    let anf_t:usize=terms.iter().map(|&m|mcx_t(extra+m.count_ones()as usize)).sum();
    let anf_ops:usize=terms.iter().map(|&m|mcx_t(extra+m.count_ones()as usize).max(1)).sum::<usize>()+2*polarity.count_ones()as usize;
    if std::env::var("Q793_TABLE_ESOP").ok().as_deref()==Some("0")||std::env::var("Q796_MUX_POLARITY").ok().as_deref()==Some("0")||!(4..=6).contains(&width)||extra+width>22{
        return TableSwap{polarity,terms,anf_t,anf_ops,esop:None};
    }
    static CACHE:OnceLock<Mutex<HashMap<(Vec<bool>,usize),Option<EsopSwap>>>>=OnceLock::new();
    let mut cache=CACHE.get_or_init(||Mutex::new(HashMap::new())).lock().unwrap();
    let key=(truth,extra);
    if let Some(found)=cache.get(&key){return TableSwap{polarity,terms,anf_t,anf_ops,esop:found.clone()};}
    let truth=&key.0;
    let mut best:Option<EsopSwap>=None;
    for shear in 0..=(width*(width-1)){
        ...
        let Some(cubes)=super::q793_table_esop_r01::plan(width,&g,extra)else{continue};
        let t:usize=cubes.iter().map(|&(m,_)|mcx_t(extra+m.count_ones()as usize)).sum();
        let ops:usize=cubes.iter().map(|&(m,v)|mcx_t(extra+m.count_ones()as usize).max(1)+2*(m^v).count_ones()as usize).sum::<usize>()+2*sh.is_some()as usize;
        ...
        let cand=EsopSwap{shear:sh,cubes,t,ops};
        if best.as_ref().is_none_or(|b|(cand.t,cand.ops)<(b.t,b.ops)){best=Some(cand);}
    }
    cache.insert(key,best.clone());
    TableSwap{polarity,terms,anf_t,anf_ops,esop:best}
}
/// Emit the external-prefix-free chart of a swap offer into `target`. The ANF
/// branch reproduces the rank-echo compute closures op-for-op.
pub(super) fn chart_toggle(circ:&mut Circuit,word:&[QReg],swap:&TableSwap,target:&QReg,helpers:&[QReg]){
    if let Some(e)=swap.winner(){
        if let Some((i,j))=e.shear{circ.cx(&word[j],&word[i]);}
        for &(m,v) in &e.cubes{let cs:Vec<(&QReg,bool)>=(0..word.len()).filter(|&b|m>>b&1!=0).map(|b|(&word[b],v>>b&1!=0)).collect();match cs.len(){0=>circ.x(target),_=>mixed_mcx(circ,&cs,target,helpers)}}
        if let Some((i,j))=e.shear{circ.cx(&word[j],&word[i]);}
        return;
    }
    for i in 0..word.len(){if swap.polarity>>i&1!=0{circ.x(&word[i]);}}
    for &m in &swap.terms{let cs:Vec<(&QReg,bool)>=(0..word.len()).filter(|&b|m>>b&1!=0).map(|b|(&word[b],true)).collect();match cs.len(){0=>circ.x(target),1=>circ.cx(cs[0].0,target),_=>mixed_mcx(circ,&cs,target,helpers)}}
    for i in (0..word.len()).rev(){if swap.polarity>>i&1!=0{circ.x(&word[i]);}}
}
pub(super) fn truth_swap(circ:&mut Circuit,controls:&[&QReg],truth:Vec<bool>,left:&QReg,right:&QReg,helpers:&[QReg]) {
    let swap=swap_offer(truth,controls.len(),1);
    if let Some(e)=swap.winner(){
        if let Some((i,j))=e.shear{circ.cx(controls[j],controls[i]);}
        circ.cx(right,left);
        for &(m,v) in &e.cubes {let mut cs=vec![(left,true)];cs.extend((0..controls.len()).filter(|&i|m>>i&1!=0).map(|i|(controls[i],v>>i&1!=0)));mixed_mcx(circ,&cs,right,helpers);}
        circ.cx(right,left);
        if let Some((i,j))=e.shear{circ.cx(controls[j],controls[i]);}
        return;
    }
    let(polarity,terms)=(swap.polarity,&swap.terms);
...
fn skip_zero_factored(circ:&mut Circuit,rank:&[QReg],low:&[QReg],axis:usize,guard:Option<&QReg>,root:&QReg,passenger:&QReg,helpers:&[QReg])->bool{
    ...
    let compute=|circ:&mut Circuit|{chart_toggle(circ,rank,&swap,d,rest);};
