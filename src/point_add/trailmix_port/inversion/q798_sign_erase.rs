...
pub(super) fn emit(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,sign:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,n:usize){
    ...
    let fuse_top=super::metadata_muxlease::active("Q795_SIGN_TOP_LOAN_FUSION");
    if fuse_top{assert!(dual&&super::metadata_muxlease::active("Q795_SIGN_TWO_PASS"));}
    if dual{
    ...
    if dual{
        if fuse_top{super::q795_t11_top::top_flag_loan(circ,rank,c,sm,p1,p2,mask,&dirty[0],w1,&dirty[1..],j,false);}
        else{super::q795_t11_top::top_flag(circ,rank,c,sm,p1,p2,mask,w1,dirty,j);}
    ...
    if super::metadata_muxlease::active("Q795_SIGN_TWO_PASS") {
        ...
        if fuse_top{super::q795_sign_two_pass::emit_funded(circ,rank,c,sm,p1,p2,mask,sign,w1,w2,dirty,j,n);}
        else{super::q795_sign_two_pass::emit(circ,rank,c,sm,p1,p2,mask,sign,w1,w2,dirty,j,n);}
    } else {
    ...
    if dual{
        ...
        if fuse_top{super::q795_t11_top::top_flag_loan(circ,rank,c,sm,p1,p2,mask,&dirty[0],w1,&dirty[1..],j,true);}
        else{super::q795_t11_top::top_flag(circ,rank,c,sm,p1,p2,mask,w1,dirty,j);}
    }
