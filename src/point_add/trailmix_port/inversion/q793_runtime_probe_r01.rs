...
pub fn run(){
 ...
 for block in 0..26{for j in 0..4{let at=Instant::now();let ops=super::q793_lifecycle_r03::template(block,j);template_secs+=at.elapsed().as_secs_f64();ops_count+=ops.len();let at=Instant::now();cache.insert((block,j),&ops);assert_eq!(cache.get((block,j)).unwrap(),ops);check_secs+=at.elapsed().as_secs_f64();}}
 let first_secs=started.elapsed().as_secs_f64();let at=Instant::now();for _ in 0..3{for block in 0..26{for j in 0..4{let ops=cache.get((block,j)).unwrap();std::hint::black_box(ops);}}}let reuse_secs=at.elapsed().as_secs_f64();
