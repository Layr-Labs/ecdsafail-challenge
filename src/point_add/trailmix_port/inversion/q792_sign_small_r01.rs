...
fn flag_charted(circ:&mut Circuit,m:&[QReg],rank:&[QReg],g:&QReg,out:&QReg,dirty:&[QReg],j:usize){
    let c=&m[10..16];let sm=&m[16..20];let carry=&dirty[0];let rest=&dirty[1..];let ts=triples();
