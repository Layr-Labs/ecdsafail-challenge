...
fn permutation_inner(circ:&mut Circuit,word:&[&QReg],guard:&QReg,helpers:&[QReg],swaps:&[(usize,usize)],scratch:Option<&[QReg]>) {
    for &(left,right) in swaps {
        if super::metadata_muxlease::active("Q794_TRANSFER_ACTIVE_AFFINE"){
            super::q794_affine_frames::swap(circ,word,guard,&[],helpers,left,right,scratch);continue;
        }
...
pub(super) fn exit_clean_toggle(circ:&mut Circuit,cs:&[(&QReg,bool)],out:&QReg,scratch:&[QReg]) {
    ...
    let update=|circ:&mut Circuit,i:usize| {
        ...
        if let Some((sm,bits))=exit_cache{
            let key=value>>(8-bits);if cache_key.get()!=Some(key){
                if let Some(old)=cache_key.get().filter(|_|super::metadata_muxlease::active("Q794_TRANSFER_PREFIX_AFFINE")){
                    // Symmetric pure-XOR transitions retrace the exact mirrored
                    // key walk, restoring arbitrary offguard S scratch/cache.
                    super::q794_exit_prefix::transition(circ,&boundary[8-bits..],old,key,&sm[0],Some(&sm[1..]),helpers);
