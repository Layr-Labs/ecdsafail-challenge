#!/usr/bin/env python3
"""Regenerate and certify the Q822 Aux10 paper2607 primitive stream."""
...
def main() -> None:
    ...
    candidate_generator = data / "eea_circuit_s835_exactwidth_dirty12.py"
    ...
    generator_hash = sha256(candidate_generator)
    if generator_hash != PINNED_GENERATOR_SHA256:
        raise SystemExit(
            f"candidate generator hash {generator_hash}, "
            f"expected {PINNED_GENERATOR_SHA256}"
        )
    ...
    aggregate_data["source_identity"] = {
        "candidate_generator_sha256": generator_hash,
        "support_commit": commit,
        "support_source_sha256": support_hash,
    }
