...
def _toggle_raw_controls_dirty(qc: QuantumCircuit, controls: Sequence[Qubit],
    ...
    if len(controls) == 1:
        ...
        _mcx_dirty_ladder(qc, controls, target, dirty)
...
def compact_lc_swap_gate(*, k: int, K: int,
    ...
    # The Cuccaro carry is clean again immediately after the affine l_q add.
    # Its next use is the matching subtract after the unary scan, so its
    # lifetime does not overlap the decoder path.  For trees of depth at most
    # eight, let the deepest path level reuse that physical lane and recover
    # the Q819 two-raw-level decoder.  Depth-nine rows still need seven live
    # path lanes, so they retain the proven three-raw-level fallback.
    use_carry_as_path = depth <= 8
    path_depth = max(0, depth - (2 if use_carry_as_path else 3))
    ...
    _add_const_dirty(qc, l_q, 3, list(Dirty) + list(Work1[:2]))

    def leaf(j: int, controls: Sequence[Qubit]) -> None:
        # The two direction branches select exactly one adjacent Work1 lane.
        # Select that lane into the upper slot, apply one common equality-
        # controlled swap with Sign, then undo the selection.  This is exactly
        # equivalent to two direction-controlled swaps, but removes Direction
        # from the expensive raw-control product and evaluates it only with
        # the two surrounding Fredkins.
        low = Work1[j - k]
        high = Work1[j - k + 1]
...
def compact_prefix_add_midtail_gate(*, n: int, k: int, K: int,
    ...
    dirty map, ten dirty passengers, endpoint registers, and all seven clean
    ...
    Dirty = QuantumRegister(DIRTY_PASSENGER_SIZE, "DirtyPassenger")
    Scratch = QuantumRegister(7, "Scratch")
    qc = _e._block_circuit(
    ...
    _upper_zero_map_midpoint_seven_dirty(
...
def append_one_step_T(qc: QuantumCircuit, *, T: int, n: int, len_width: int, shift_width: int,
    ...
    _e._append_with_optional_clbits(
        ...
        + list(l_t) + list(l_q) + list(l_s) + list(Dirty) + scratch[:2],
    )
    _toggle_live_r_phase(qc, phase1=Phase1[0], l_rp=l_rp, out=ctrl, dirty=Dirty)
    ...
    _decrement_by_dirty_carry(qc, l_q, Dirty, ctrl)
    _make_condition(qc, [(Phase1[0], 1), (Phase2[0], 0)], ctrl, scratch)
    _make_condition(qc, [(Phase1[0], 0), (Phase2[0], 1)], ctrl, scratch)
    _increment_by_dirty_carry(qc, l_q, Dirty, ctrl)
    _make_condition(qc, [(Phase1[0], 0), (Phase2[0], 1)], ctrl, scratch)
    ...
    if t_addsub_reachable:
        ...
        _e._append_with_optional_clbits(qc, tsub, [ctrl, Sign[0]] + list(Work1[k3-1:K3]) + list(Work2[k3-1:K3])
                                        ...
                                        + (scratch + [Phase2[0]])[:tsub.num_qubits-(2+2*(K3-k3+1)+LT_WIDTH+3)])
    _borrow_phase2_for_tadd(
        qc, phase1=Phase1[0], phase2=Phase2[0], l_q=l_q,
    ...
    if T % 4 == 0:
        ...
        _toggle_terminal_endpoint_raw(
            ...
            dirty=Dirty, scratch=scratch, extra_lenders=[Iter[0]],
        )
        ...
        # nor l_q, so Phase2 can supply its sixth clean path lane here too.
        _borrow_phase2_for_tadd(
            qc, phase1=Phase1[0], phase2=Phase2[0], l_q=l_q,
            dirty=Dirty,
        )
