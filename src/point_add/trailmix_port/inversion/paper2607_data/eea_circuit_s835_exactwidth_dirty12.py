...
def _terminal_aux7_decoder_alias(labels: Sequence[int], endpoint: int) -> int:
    """Classify an off-support endpoint through the pinned unary tree."""
    labels = list(sorted(set(labels)))
    if not labels:
        raise ValueError("terminal Aux7 alias needs a nonempty low support")
    while len(labels) > 1:
        bit = _e._split_bit(labels)
...
def _terminal_aux7_direct_inversion_mask(
    labels: Sequence[int], leaf: int,
) -> int:
    """Return index-bit X conjugations live at a dirty-octet leaf callback."""
    labels = list(sorted(set(labels)))
    if leaf not in labels:
        raise ValueError("terminal Aux7 leaf is outside the low support")
    while _tight_unary_depth_for_labels(labels) > 3:
        bit = _e._split_bit(labels)
        branch = (leaf >> bit) & 1
        labels = [
            label for label in labels
...
def _range_scan_tight_dirty_octet_aux7_terminal(
    qc: QuantumCircuit,
    *,
    leq: bool,
    boundary: Sequence[Qubit],
    ...
    """Exact six-clean terminal scan for labels through 259.
    ...
    """
    if k > K:
        raise ValueError("terminal Aux7 scan needs k <= K")
    if K > 259:
        raise ValueError("terminal Aux7 scan supports labels at most 259")
    if not (k <= 255 < K):
        _range_scan_tight_dirty_octet(
            qc, leq=leq, boundary=boundary, k=k, K=K, ctrl=ctrl,
            scratch=scratch, borrowed=borrowed, leaf_fn=leaf_fn, order=order,
        ...
        return
    if len(boundary) != 9:
        raise ValueError("terminal Aux7 crossing scan needs a 9-bit endpoint")
    if len(scratch) < 6:
        raise ValueError("terminal Aux7 crossing scan needs six clean lanes")
    if len(borrowed) < 8:
        raise ValueError("terminal Aux7 crossing scan needs eight dirty lenders")

    main = list(range(k, 256))
    depth = _tight_unary_depth_for_labels(main)
    range_acc = scratch[max(0, depth - 3)]
    ...
    aliases = {
        endpoint: _terminal_aux7_decoder_alias(main, endpoint)
    ...
    direct_masks = {
        label: _terminal_aux7_direct_inversion_mask(main, label)
    ...
    def low_scan(*, toggle_before_leaf: bool) -> None:
        unary_range_iteration_dirty_octet(
            ...
            range_acc=range_acc, ancillas=scratch[:max(0, depth - 3)],
    ...
    if leq and order == "inc":
        ...
        raise ValueError("bad terminal Aux7 scan mode/order")
...
def _upper_zero_map_borrowed(qc: QuantumCircuit, *, ctrl: Qubit,
    ...
    depth = _tight_unary_depth_for_labels(list(range(k, K + 1)))
    if len(scratch) < min(6, max(1, depth - 2)):
    ...
    _range_scan_tight_dirty_octet_aux7_terminal(
...
def _toggle_terminal_endpoint_raw(
    ...
    *,
    ...
    lenders = list(dirty) + list(scratch[:6])
...
def append_one_step_T(qc: QuantumCircuit, *, T: int, n: int, len_width: int, shift_width: int,
    ...
    _e._append_with_optional_clbits(qc, tsub, [ctrl, Sign[0]] + list(Work1[k3-1:K3]) + list(Work2[k3-1:K3])
                                    ...
                                    + scratch[:tsub.num_qubits-(2+2*(K3-k3+1)+LT_WIDTH+3)])
    _make_condition(qc, [(Phase2[0], 0), (Sign[0], 1)], tmp, scratch[1:])
    _make_condition(qc, [(Phase1[0], 1), (tmp, 0)], ctrl, scratch[1:])
    _make_condition(qc, [(Phase2[0], 0), (Sign[0], 1)], tmp, scratch[1:])
    qc.cx(Phase1[0], Sign[0])
    # At the T-add boundary Phase2 is redundant with the exact phase/l_q
    # domain.  Move D into the otherwise-unreachable l_q=256 marker and recover
    # B from l_q!=0, making Phase2 a seventh clean scratch lane.  The inverse
    # sequence below restores both registers exactly.
