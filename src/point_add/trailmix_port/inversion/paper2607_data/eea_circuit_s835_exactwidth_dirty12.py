...
def compact_lc_swap_gate(*, k: int, K: int,
    ...
    if depth > 9:
        raise ValueError("four-clean LC swap supports decoder depth at most nine")
    ...
    # its lifetime as the deepest decoder-path lane and expose the final five
    # decoder levels directly.  Every certified quotient-swap window has
    # depth at most nine, so four shared clean lanes suffice.
...
def compact_r_subrestore_fused_gate(*, n: int, k: int, K: int,
    ...
    l_s = QuantumRegister(LS_WIDTH, "l_s")
    Dirty = QuantumRegister(DIRTY_PASSENGER_SIZE, "DirtyPassenger")
    Scratch = QuantumRegister(3, "Scratch")
    qc = _e._block_circuit(
    ...
    def leaf_first(
        ...
        _toggle_raw_controls_dirty(
            qc, s_controls, acc, Dirty, clean_helper=clean_helper,
        )
        _toggle_raw_controls_dirty(
            qc, q_controls, acc, Dirty, clean_helper=clean_helper,
        )
        _toggle_raw_controls_dirty(
...
def compact_prefix_add_midtail_gate(*, n: int, k: int, K: int,
    ...
    dirty map, ten dirty passengers, endpoint registers, and all five clean
    ...
    if encoded_labels:
        unary_range_iteration_dirty_hexadecet(
            qc, index_reg=l_t, labels=encoded_labels, ctrl=Ctrl[0],
            range_acc=acc, ancillas=path, borrowed=Dirty[4:7],
    ...
    _upper_zero_map_midpoint_five_dirty(
...
def append_one_step_T(qc: QuantumCircuit, *, T: int, n: int, len_width: int, shift_width: int,
    ...
    _e._append_with_optional_clbits(
        ...
        + list(l_t) + list(l_q) + list(l_s) + list(Dirty) + scratch[:3],
    )
    _toggle_live_r_phase(qc, phase1=Phase1[0], l_rp=l_rp, out=ctrl, dirty=Dirty)
    ...
    # can hold that temporary while all five clean scratch lanes remain
    # available.
    ...
    if T % 4 == 0:
        ...
        _toggle_terminal_endpoint_raw(
            ...
            extra_lenders=[Iter[0], Sign[0], Phase1[0]],
        )
