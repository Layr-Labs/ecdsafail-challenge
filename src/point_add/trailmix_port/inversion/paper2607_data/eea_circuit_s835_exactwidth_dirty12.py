...
def compact_lc_swap_gate(*, k: int, K: int,
    ...
    Dirty = QuantumRegister(6, "DirtyPassenger")
    depth = _tight_unary_depth_for_labels(list(range(k, K + 1)))
    if depth > 9:
        raise ValueError("three-clean LC swap supports decoder depth at most nine")
    ...
    # its lifetime as the deepest decoder-path lane and expose the final six
    # decoder levels directly.  Every certified quotient-swap window has
    # depth at most nine, so three shared clean lanes suffice.
...
def compact_r_subrestore_fused_gate(*, n: int, k: int, K: int,
    ...
    carry = Scratch[0]
    acc = Scratch[1]
    # The equality accumulator is clean and idle throughout affine setup and
    # teardown.  Lend it to the width-nine maps, then return it before either
    # arithmetic scan starts.
    affine_helper = acc
    conditional_helper = Mode[0]
    ...
    _swap_zero_259_uncontrolled_dirty(
        qc, l_s, extension, Dirty, clean_helper=affine_helper,
    )
    ...
    def leaf_first(
        ...
        _apply_cell_raw_conditionally_clean(
            qc, "sub", "first", s_controls, addend, target, carry, Dirty,
            clean_helper=conditional_helper,
        )
        _toggle_raw_controls_conditionally_clean(
            qc, s_controls, acc, Dirty, conditional_helper,
        )
        _toggle_raw_controls_conditionally_clean(
            qc, q_controls, acc, Dirty, conditional_helper,
        )
        _toggle_raw_controls_conditionally_clean(
    ...
    # Keep Mode/Phase1 at zero on live-R states so it remains the conditional
    # helper during the second scan.  Implement the old restore predicate
    # 1 xor (Phase2 & Sign) directly inside each second-cell kernel.
    ...
    def leaf_second(
        ...
        if j < K:
            ...
            _apply_r_fused_second_cell_implicit_mode_borrowed(
                qc, phase2=Phase2[0], sign=Sign[0], ctrl=acc,
                addend=addend, target=target, carry=next_addend,
                borrowed=Dirty[0],
            )
        _apply_r_fused_second_cell_implicit_mode_conditionally_clean(
            qc, phase2=Phase2[0], sign=Sign[0], controls=s_controls,
            addend=addend,
...
def compact_prefix_addsub_gate(*, k: int, K: int,
    ...
    Work2 = QuantumRegister(M, "Work2")
    l_t = QuantumRegister(LT_WIDTH, "l_t")
    Borrowed = QuantumRegister(4, "Borrowed")
...
def compact_prefix_add_midtail_gate(*, n: int, k: int, K: int,
    ...
    if encoded_labels:
        unary_range_iteration_dirty_32raw(
            qc, index_reg=l_t, labels=encoded_labels, ctrl=Ctrl[0],
            range_acc=acc, ancillas=path, borrowed=Dirty[4:8],
    ...
    def copy_selected_carry() -> None:
        def leaf(encoded: int, controls: Sequence[Qubit]) -> None:
            _toggle_raw_controls_dirty(
                qc, list(controls) + [Work1[encoded + 1]], acc, Dirty[4:],
            )

        unary_iteration_dirty_32raw(
            qc, index_reg=l_t, labels=encoded_labels,
            ctrl=Ctrl[0], ancillas=Scratch[:3],
    ...
    def selected_carry_sign_toggle() -> None:
        ...
        unary_iteration_dirty_32raw(
            ...
            ancillas=Scratch[:3], leaf_fn=leaf, order="inc",
        )
    ...
    if encoded_labels:
        ...
        _upper_zero_map_midpoint_three_dirty(
            qc, ctrl=Ctrl[0], boundary_B=l_s, bits=Work2,
            dirty_map=Work1, dirty=Dirty, scratch=Scratch[:3],
        )
        selected_carry_sign_toggle()
        _upper_zero_map_midpoint_three_dirty(
            qc, ctrl=Ctrl[0], boundary_B=l_s, bits=Work2,
...
def _range_scan_tight_dirty_hexadecet_aux6_terminal(
    ...
    *,
    ...
    """Exact four/five/six-clean terminal scan for labels through 259.
    ...
    if len(scratch) < 4:
        raise ValueError("terminal crossing scan needs four clean lanes")
    if len(borrowed) < 8:
    ...
    path_reduction = 3 if use_six_clean else (4 if use_five_clean else 5)
...
def append_one_step_T(qc: QuantumCircuit, *, T: int, n: int, len_width: int, shift_width: int,
    ...
    if t_addsub_reachable:
        ...
        _e._append_with_optional_clbits(qc, tsub, [ctrl, Sign[0]] + list(Work1[k3-1:K3]) + list(Work2[k3-1:K3])
                                        + list(l_t) + [Dirty[3], Dirty[6], Dirty[0], Dirty[1]]
                                        + (scratch + [Phase2[0], Iter[0]])[:tsub.num_qubits-(2+2*(K3-k3+1)+LT_WIDTH+4)])
    _borrow_iter_for_tadd(
        qc, phase1=Phase1[0], iteration=Iter[0], l_q=l_q,
    ...
    if T % 4 == 0:
        ...
        _toggle_terminal_endpoint_raw(
            ...
            extra_lenders=[Iter[0], Sign[0], Phase1[0], Phase2[0]],
        )
