...
def compact_lc_swap_gate(*, k: int, K: int,
    ...
    if depth > 9:
        raise ValueError("two-clean LC swap supports decoder depth at most nine")
    ...
    # its lifetime as the deepest decoder-path lane and expose the final seven
    # decoder levels directly.  Every certified quotient-swap window has
    # depth at most nine, so two shared clean lanes suffice.
...
def compact_r_subrestore_fused_gate(*, n: int, k: int, K: int,
    ...
    def leaf_first(
        ...
        _toggle_raw_controls_dirty(
            qc, s_controls, acc, Dirty,
        )
        _toggle_raw_controls_dirty(
            qc, q_controls, acc, Dirty,
        )
        _toggle_raw_controls_dirty(
    ...
    def leaf_second(
        ...
        _apply_r_fused_second_cell_implicit_mode_raw(
            qc, phase2=Mode[0], sign=Sign[0], controls=s_controls,
            addend=addend,
...
def compact_prefix_add_midtail_gate(*, n: int, k: int, K: int,
    ...
    if encoded_labels:
        unary_range_iteration_dirty_64raw(
            qc, index_reg=l_t, labels=encoded_labels, ctrl=Ctrl[0],
            range_acc=acc, ancillas=path, borrowed=Dirty[4:9],
    ...
    def copy_selected_carry() -> None:
        ...
        unary_iteration_dirty_64raw(
            ...
            ctrl=Ctrl[0], ancillas=Scratch[:2],
    ...
    def selected_carry_sign_toggle() -> None:
        ...
        unary_iteration_dirty_64raw(
            ...
            ancillas=Scratch[:2], leaf_fn=leaf, order="inc",
        )
...
def _upper_zero_map_borrowed(qc: QuantumCircuit, *, ctrl: Qubit,
    ...
    depth = _tight_unary_depth_for_labels(list(range(k, K + 1)))
    if len(scratch) < min(3, max(1, depth - 5)):
...
def append_one_step_T(qc: QuantumCircuit, *, T: int, n: int, len_width: int, shift_width: int,
    ...
    # The exact four-phase/l_q domain lets Phase2 be packed into l_q and
    # cleared.  R reconstructs the live phase-B bit between its two scans, so
    # the cleared physical lane can replace the removed second Aux qubit.
    ...
    if t_addsub_reachable:
        ...
        _e._append_with_optional_clbits(qc, tsub, [ctrl, Sign[0]] + list(Work1[k3-1:K3]) + list(Work2[k3-1:K3])
                                        ...
                                        + (scratch + [Phase2[0], Iter[0]])[:tsub.num_qubits-(2+2*(K3-k3+1)+LT_WIDTH+5)])
    _borrow_iter_for_tadd(
        qc, phase1=Phase1[0], iteration=Iter[0], l_q=l_q,
        dirty=Dirty, inverse=True,
    )
    ...
    if T % 4 == 0:
        ...
        _toggle_terminal_endpoint_raw(
            ...
            extra_lenders=[Iter[0], Sign[0], Phase1[0], Phase2[0], l_t[0]],
        )
