...
def main() -> None:
    ...
    # Git may check these JSON files out with CRLF on Windows.  The circuit
    # loader pins the canonical LF byte stream, so the independent verifier
    # must use the same cross-platform identity instead of hashing checkout
    # line endings.
    encoded = args.table.read_bytes().replace(b"\r\n", b"\n")
    ...
    if table["certificate"]["sha256"] != hashlib.sha256(
        (here / "certificate.json").read_bytes().replace(b"\r\n", b"\n")
    ).hexdigest():
