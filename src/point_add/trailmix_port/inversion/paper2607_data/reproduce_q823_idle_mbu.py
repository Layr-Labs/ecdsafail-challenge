#!/usr/bin/env python3
"""Portable, source-pinned Q823 retained-carry T-add stream reproduction. Attribution: gpt-5.

Builds real Qiskit circuits and uses the original production flatten/pack code.
...
SOURCE_PINS = {
    'eea_circuit_s835_exactwidth_dirty12.py': 'cf926e6782c8c4606627c299733646db8ee7359d62d8ffe4e6c1ea23a91cd216',
...
KINDS = {'x': 1, 'cx': 2, 'ccx': 3, 'z': 4, 'cz': 5, 'swap': 6, 'clean_c3x_mbu': 7}
ARITIES = {'x': 1, 'cx': 2, 'ccx': 3, 'z': 1, 'cz': 2, 'swap': 2, 'clean_c3x_mbu': 5}
...
def worker(task):
    ...
    flat = load_module('q823_original_production_flatten', HERE / 'generate_eea_blob.py')
    ...
    with path.open('xb') as handle:
        with zstandard.ZstdCompressor(level=12).stream_writer(handle, closefd=False) as stream:
            stream.write(b'P26EEA2\0' + struct.pack('<IIII', 256, 577, start, end))
            for step in range(start, end + 1):
                ...
                step_records = 0
                for kind, positions in flat.flatten(circuit):
                    ...
                    record = flat.pack_record(kind, positions)
                    stream.write(record)
                ...
                per_step.append({'step': step, 'counts': dict(counts), 'records': step_records,
                                 ...
                                 'raw_record_sha256': step_digest.hexdigest()})
                total.update(counts)
    ...
    signal.alarm(0)
    report = {'schema': 'paper2607-eea-primitive-stream-v3', 'source_module': 'q823_mbu_generation_source',
              ...
              'executed_toffoli': total['ccx'] + 2 * total['clean_c3x_mbu'],
...
def main():
    ...
    try:
        ...
        aggregate = {'schema': 'q823-idle-mbu-generated-stream-v1', 'status': 'COMPLETE_GENERATION_UNVALIDATED',
