#!/usr/bin/env python3
"""Differential basis-state checks for the Q819 T-add Phase2 loan."""
...
def build_wrapper(module, *, upper: int, loan: bool) -> QuantumCircuit:
    ...
    qc = QuantumCircuit(
        ...
        l_rp, Dirty, Scratch, name=f"TADD_WRAPPER_{'LOAN' if loan else 'OLD'}",
    )
    if loan:
...
def phase_cases(cases: int, rng: random.Random) -> tuple[list[int], list[int], list[int]]:
    ...
    for case in range(cases):
        ...
        if phase == 0:
            ...
            logical = rng.randrange(0, weight)
            p1, p2, lq = 1, 0, (logical - 1) & 511
        else:
            p1, p2, lq = 1, 1, 511
        phase1 |= p1 << case
        phase2 |= p2 << case
...
def check_pair(original, candidate, *, upper: int, cases: int, seed: int) -> None:
    ...
    encoded_labels = [rng.randrange(0, max(1, upper - 1)) for _ in range(cases)]
    values = {
        "Phase1": p1,
        "Phase2": p2,
        ...
        "l_t": words_from_values(encoded_labels, 8),
        "l_q": lq,
    ...
    if any(diff.get_register(new, new_state, "Scratch")):
        raise AssertionError(f"upper={upper}: loan scratch not clean")
    ...
    print(
        f"PASS q819_tadd_phase2_loan upper={upper} cases={cases} "
...
def main() -> None:
    ...
    old = load_module("q819_tadd_old", args.old_generator.resolve())
    candidate = load_module(
        "q819_tadd_candidate", HERE / "eea_circuit_s835_exactwidth_dirty12.py"
    )
    for index, upper in enumerate((2, 17, 128, 256, 257)):
        check_pair(
            old, candidate, upper=upper, cases=args.cases,
            seed=0x8197AD00 + index,
        )
