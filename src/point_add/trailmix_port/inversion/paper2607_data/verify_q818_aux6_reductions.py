#!/usr/bin/env python3
"""Differential basis-state checks for the Q818 six-clean-aux route."""
...
def check_terminal_endpoint(original, candidate) -> None:
    ...
    scratch_o = QuantumRegister(6, "Scratch")
    old = QuantumCircuit(lq_o, ls_o, out_o, dirty_o, scratch_o)
    original._toggle_terminal_endpoint_raw(
        old, l_q=lq_o, l_s=ls_o, out=out_o[0], dirty=dirty_o, scratch=scratch_o,
    )
    ...
    out_n = QuantumRegister(1, "Out")
    dirty_n = QuantumRegister(10, "DirtyPassenger")
    scratch_n = QuantumRegister(5, "Scratch")
    iteration_n = QuantumRegister(1, "Iter")
    new = QuantumCircuit(lq_n, ls_n, out_n, dirty_n, scratch_n, iteration_n)
    candidate._toggle_terminal_endpoint_raw(
        new, l_q=lq_n, l_s=ls_n, out=out_n[0], dirty=dirty_n,
        scratch=scratch_n, extra_lenders=iteration_n,
    )
    ...
    for name, values in common.items():
        diff.set_register(old, old_state, name, values)
        diff.set_register(new, new_state, name, values)
    diff.set_register(new, new_state, "Iter", diff.random_words(1, cases, rng))
    old_initial, new_initial = old_state.copy(), new_state.copy()
    diff.apply(old, old_state, case_mask)
    diff.apply(new, new_state, case_mask)
    for name in common:
...
def check_tsub(original, candidate, cases: int) -> None:
    ...
    for upper in (1, 130, 200, 257):
        ...
        compare_gate_positional(
            ...
            new_private_fields=[("Borrowed", 5)],
            old_private={"Borrowed": first_three},
            new_private={"Borrowed": first_three + diff.random_words(2, cases, rng)},
        )
...
def check_step_one(original, candidate, cases: int) -> None:
    rng = random.Random(0x81857E9)
    case_mask = (1 << cases) - 1
    old = original.build_step_circuit(
        256, 1, T_max=1616, aux_size=7, measurement_uncompute=False,
    )
...
def check_all_construct(candidate) -> None:
    ...
    for step in range(1, 1617):
        circuit = candidate.build_step_circuit(
            256, step, T_max=1616, aux_size=6, measurement_uncompute=False,
        )
    ...
    receipt = {
        "schema": "paper2607-q818-aux6-schedule-count-v1",
    ...
    print("PASS all-steps constructed=1616 local_width=572 clean_aux=6", flush=True)
...
def main() -> None:
    ...
    original = diff.load_module("paper2607_q819_reference", args.original.resolve())
    candidate = diff.load_module("paper2607_q818_candidate", args.candidate.resolve())
    check_live_predicate(original, candidate)
    ...
    print("PASS Q818 Aux6 differential suite", flush=True)
