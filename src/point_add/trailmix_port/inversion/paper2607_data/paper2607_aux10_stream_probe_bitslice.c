...
static unsigned dirty_bit(unsigned case_index, unsigned bit) {
    return (unsigned)((case_index * 13U + bit * 7U + 1U) & 1U);
}
...
static unsigned char *decode(const char *path, size_t *size_out) {
    ...
    rewind(f);
    unsigned char *decoded = malloc((size_t)raw_size);
    assert(decoded);
    assert(fread(decoded, 1, (size_t)raw_size, f) == (size_t)raw_size);
    fclose(f);
    *size_out = (size_t)raw_size;
    return decoded;
}
...
static void initialize_case(lane_t initial[WIDTH], unsigned case_index, const char *x_hex) {
    ...
    for (unsigned i = 0; i < LQ_WIDTH; i++) initial[LQ_START + i] |= mask;
    unsigned shift_zero = SHIFT_MODULUS - 1;
    for (unsigned i = 0; i < LS_WIDTH; i++)
        if ((shift_zero >> i) & 1U) initial[LS_START + i] |= mask;
    unsigned encoded = bitlen - 1;
    for (unsigned i = 0; i < LRP_WIDTH; i++)
        if ((encoded >> i) & 1) initial[LRP_START + i] |= mask;
    for (unsigned i = 0; i < DIRTY_WIDTH; i++)
        if (dirty_bit(case_index, i)) initial[DIRTY_START + i] |= mask;
}
...
static void inspect_case(const lane_t state[WIDTH], unsigned case_index, const char *x_hex) {
    ...
    for (unsigned i = 0; i < AUX_WIDTH; i++)
        assert(lane_bit(state, AUX_START + i, case_index) == 0);
    for (unsigned i = 0; i < DIRTY_WIDTH; i++)
        assert(lane_bit(state, DIRTY_START + i, case_index) ==
               dirty_bit(case_index, i));
    assert(reg_value(state, LT_START, LT_WIDTH, case_index) == 255);
    assert(reg_value(state, LQ_START, LQ_WIDTH, case_index) == 511);
    assert(reg_value(state, LRP_START, LRP_WIDTH, case_index) == 255);

    unsigned padding =
        (reg_value(state, LS_START, LS_WIDTH, case_index) + 1) % SHIFT_MODULUS;
