#!/usr/bin/env python3
"""Install a verified Q815 Aux3 stream and synchronize Rust constants."""
...
def replace_constant(source: str, name: str, value: int) -> str:
    pattern = re.compile(
        rf"^(const {re.escape(name)}: usize = )[0-9_]+;$", re.MULTILINE
    )
...
def main() -> None:
    ...
    reduction_manifest = args.stream / "reduction_manifest.json"
    if reduction_manifest.is_file():
        shutil.copy2(
            reduction_manifest,
            destination / "reduction_manifest.json",
        )

    verifier_source = Path(__file__).resolve().parent
    shutil.copy2(
        verifier_source / "verify_q815_stream.py",
        destination / "verify_exactwidth_stream.py",
    )
    shutil.copy2(
        verifier_source / "verify_q818_stream.py",
        destination / "verify_q818_stream.py",
    )
    shutil.copy2(
        verifier_source / "verify_q819_stream.py",
        destination / "verify_q819_stream.py",
    )
    ...
    values = {
        ...
        "STREAM_RECORDS_PER_TRAVERSAL": int(aggregate["records_per_traversal"]),
    ...
    print(
        "PASS installed Q815 Aux3 stream "
