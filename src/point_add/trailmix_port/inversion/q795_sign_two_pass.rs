...
pub(super) fn emit(circ:&mut Circuit,rank:&[QReg],c:&[QReg],sm:&[QReg],guard:&QReg,cache:&QReg,mask:&QReg,sign:&QReg,source:&[QReg],target:&[QReg],helpers:&[QReg],j:usize,n:usize) {
    emit_impl(circ,rank,c,sm,guard,cache,mask,sign,source,target,helpers,j,n,false);
}
/// The enclosing supported top-flag frame already funded helpers[0].
/// On guard0 the entire core is identity for arbitrary incoming carry/data.
pub(super) fn emit_funded(circ:&mut Circuit,rank:&[QReg],c:&[QReg],sm:&[QReg],guard:&QReg,cache:&QReg,mask:&QReg,sign:&QReg,source:&[QReg],target:&[QReg],helpers:&[QReg],j:usize,n:usize) {
    emit_impl(circ,rank,c,sm,guard,cache,mask,sign,source,target,helpers,j,n,true);
}
fn emit_impl(circ:&mut Circuit,rank:&[QReg],c:&[QReg],sm:&[QReg],guard:&QReg,cache:&QReg,mask:&QReg,sign:&QReg,source:&[QReg],target:&[QReg],helpers:&[QReg],j:usize,n:usize,funded:bool) {
    ...
    if !funded{top_loan(circ,rank,c,sm,guard,cache,carry,source,dirty,j);}
