...
pub(super) fn rotate(circ:&mut Circuit,m:&[QReg],p1:&QReg,p2:&QReg,word:&[QReg],helpers:&[QReg],post:bool) {
    ...
    // Q833 directional fusion adapted to our signed one-position rotation:
    // p1*(2*p2-1) = p2 - (p1 XOR p2). Each single-control rotation is exact
    // on arbitrary words; P1 is restored before leaving this local frame.
    circ.cx(p2,p1);
    for i in 0..word.len()-1{circ.cx(&word[i+1],&word[i]);circ.ccx(p1,&word[i],&word[i+1]);circ.cx(&word[i+1],&word[i]);}
    circ.cx(p2,p1);
    for i in (0..word.len()-1).rev(){circ.cx(&word[i+1],&word[i]);circ.ccx(p2,&word[i],&word[i+1]);circ.cx(&word[i+1],&word[i]);}
