...
fn build(sched: &StepWidths, wq: usize) -> Harness {
    build_variant(sched, wq, false, false)
}

fn build_variant(sched: &StepWidths, wq: usize, inverse: bool, supported: bool) -> Harness {
    ...
    if supported {
        super::multiply_supported(&mut c, &r1, &r2, &ex1, &ex2, &q, &s_rot, &off, &gate, sched.step, sched, inverse);
    } else if inverse {
        super::multiply_backward_marked(&mut c, &r1, &r2, &ex1, &ex2, &q, &s_rot, &off, &gate, sched.step, sched, &mut |c, name| {
...
fn check_supported(fwd: &Harness, inv: &Harness, shots: &[Shot], label: &str, cnt: &mut Counts) {
    let data: Vec<Vec<bool>> = shots.iter().map(|s| s.data.clone()).collect();
    let expected: Vec<Vec<bool>> = shots.iter().map(|s| s.expect.clone()).collect();
    assert_eq!(simulate(fwd, &data, &format!("{label}-supported-fwd")), expected);
    assert_eq!(simulate(inv, &expected, &format!("{label}-supported-inv")), data);
    cnt.supported_checked += shots.len();
}
...
pub(super) fn run() {
    ...
    for step in 0..NSTEPS {
        ...
        let sfwd = build_variant(&sched, wq, false, true);
        let sinv = build_variant(&sched, wq, true, true);
        assert!(sfwd.t <= h.t && sfwd.scratch <= h.scratch);
        ...
        check_supported(&sfwd, &sinv, &shots, &format!("mul-s{step}"), &mut cnt);
        if !shots0.is_empty() {
            run_batch(&h, &shots0, &format!("mul0-s{step}"));
            check_supported(&sfwd, &sinv, &shots0, &format!("mul0-s{step}"), &mut cnt);
        }
        cnt.steps_run += 1;
    ...
    for &step in &[1usize, 5, 30, 100, 200, 250, 300, 378, 440, 480, 500, 520, 529] {
        ...
        for (name, spec) in specs_for(&sched, wq) {
            ...
            if let Some(row) = got {
                ...
                check_supported(&sfwd, &sinv, &shots, &label, &mut cnt);
                made.push(name);
            } else {
                missing.push(name);
            }
    ...
    assert_eq!(cnt.supported_checked, cnt.active + cnt.gate0 + cnt.filler);
    eprintln!("PACKED_MULTIPLY_SUPPORTED PASS {} rows, both directions/phase/every reset; generic tests unchanged", cnt.supported_checked);
}
