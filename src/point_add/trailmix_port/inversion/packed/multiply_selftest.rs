...
fn shot(row: &MulRow, s: &StepWidths, wq: usize, gate: bool, cnt: &mut Counts) -> Shot {
    ...
    let ca_new = row.ca_new();
    let carry = row.carry();
    let data = state(&row.a, &row.b, &row.ca_old, &row.cb, &row.q, wq, gate);
    let expect = if gate {
        let q_new = row.q ^ (one() << row.s2);
        state(&row.a, &row.b, &ca_new, &row.cb, &q_new, wq, true)
    } else {
        data.clone()
    };
    cnt.mul_rows += 1;
...
fn filler(pre: &Pre, wq: usize, cnt: &mut Counts) -> Shot {
    let data = state(&pre.a, &pre.b, &pre.ca, &pre.cb, &pre.q, wq, false);
    cnt.filler += 1;
    cnt.shots += 1;
        ...
        for t in &traces {
            match &t.mul[step] {
                ...
                Some(row) => {
                    ...
                    shots.push(filler(&t.pre[step], wq, &mut cnt));
                }
                None => shots.push(filler(&t.pre[step], wq, &mut cnt)),
            }
        }
