...
use crate::point_add::trailmix_port::inversion::shrunken_pz_schedule::{
    packed_q_width, reg_widths, SHRUNKEN_PZ_NSTEPS,
};
...
use super::sched::{exp_base, rebase, EXP_BITS};
...
/// The q register width of row `i`: the thin envelope clamped to
/// `TRAILMIX_Q_CAP` (`shrunken_pz_schedule::packed_q_width`, the same
/// function the support model mirrors: the q-clamp lever of design section
/// 5; `bl(q) > min(w_q, cap)` is the `width_q` miss).
...
fn exp_bases(i: usize) -> (usize, usize) {
    ...
    (exp_base(wa.max(wb)), exp_base(wca.max(wcb)))
}
