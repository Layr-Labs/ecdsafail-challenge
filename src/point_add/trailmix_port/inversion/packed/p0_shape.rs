...
/// `s += bl(src)` (dec: `s -= bl(src)`) by the plain KG ladder of
/// `bit_length_lean_middle` with the `+1` folded into X-brackets
/// (`s -= pos + 1` is `s += ~pos`; `s += pos + 1` is `~s += ~pos`), so the
/// scratch is pos 9 + KG ancillae only (no `inc_khattar_gidney` ancillae).
/// Every packed-prefix exponent scan (S_0, teardown, unpack/repack) uses it.
fn exp_scan(c: &mut Circuit, src: &[&QReg], s: &[QReg], dec: bool, carry: Option<&QReg>) {
    let n = src.len();
    if n == 0 { return; }
    let sec = c.push_section("p.bitlen");
    let pos = c.alloc_qreg_bits("bll.pos", s.len());
    xor_const(c, &pos, n);
...
/// e_B = bl(B), e_ca = 0, e_cb = 1, q = 0, counter = 0, parity = 1.
fn enter_s0(c: &mut Circuit, dx: Vec<QReg>, passenger_carry: Option<&QReg>, tag: &str) -> Packed {
    ...
    xor_const(c, &ex1[..EXP_BITS], 256); // e_A = 256
    xor_const(c, &ex1[EXP_BITS..], 1); // e_cb = 1
    ...
    // e_B = bl(B): one ungated 256-wide scan (B < p < 2^256, wire 256 is 0).
    {
        ...
        exp_scan(c, &src, &ex2[..EXP_BITS], false, passenger_carry);
    }
    c.pop_section(&sec);
    Packed {
        r1,
...
fn exit_s0(c: &mut Circuit, mut p: Packed, passenger_carry: Option<&QReg>) -> Vec<QReg> {
    let sec = c.push_section("pk.s0.exit");
    {
        ...
        exp_scan(c, &src, &p.ex2[..EXP_BITS], true, passenger_carry);
    }
    let par = sign_storage::owned(&mut p.parity);
    c.x(&par);
    c.zero_and_free(par);
    xor_const(c, &p.ex1[..EXP_BITS], 256);
    xor_const(c, &p.ex1[EXP_BITS..], 1);
    c.x(&p.r1[256]);
    load_p(c, &p.r1[..256]);
...
fn teardown_530(c: &mut Circuit, p: &mut Packed, passenger_carry: Option<&QReg>) -> Vec<QReg> {
    ...
    // e_cb := 0 by one ungated 256-wide scan of the reversed R1 (cb < 2^256).
    {
        ...
        exp_scan(c, &src, &p.ex1[EXP_BITS..], true, passenger_carry);
    }
    xor_const(c, &p.ex2[..EXP_BITS], 1); // e_B: 1 -> 0
    xor_const(c, &p.ex2[EXP_BITS..], 256); // e_ca: 256 -> 0
...
fn recreate_530(c: &mut Circuit, p: &mut Packed, cb: Vec<QReg>, passenger_carry: Option<&QReg>, tag: &str) {
    ...
    xor_const(c, &p.ex2[..EXP_BITS], 1);
    xor_const(c, &p.ex2[EXP_BITS..], 256);
    {
        let src: Vec<&QReg> = p.r1[1..].iter().rev().collect();
        exp_scan(c, &src, &p.ex1[EXP_BITS..], false, passenger_carry);
    }
    c.pop_section(&sec);
}
...
fn handoff_forward(c: &mut Circuit, p: &mut Packed, passenger_carry_slot: &mut Option<QReg>) -> TailHandoff {
    ...
    let a = unpack_value(c, &p.r1, "midq.a", w0);
    {
        ...
        exp_scan(c, &src, &e_a, true, passenger_carry);
    ...
    let b = unpack_value(c, &p.r2, "midq.b", w0);
    {
        ...
        exp_scan(c, &src, &e_b, true, passenger_carry);
...
fn handoff_backward(c: &mut Circuit, p: &mut Packed, h: TailHandoff, passenger_carry_slot: &mut Option<QReg>, tag: &str) {
    ...
    {
        let src: Vec<&QReg> = b.iter().collect();
        exp_scan(c, &src, &p.ex2, false, passenger_carry);
    }
    ...
    {
        let src: Vec<&QReg> = a.iter().collect();
        exp_scan(c, &src, &p.ex1, false, passenger_carry);
    }
