...
use super::capture_compare::{capture_clear, capture_compare};
...
pub(crate) fn multiply_forward(
    ...
    multiply_step(c, r1, r2, ex1, ex2, q, s_rot, off, gate_mul, step, sched, false, false, &mut |_, _| {});
}
...
pub(crate) fn multiply_backward(
    ...
    multiply_step(c, r1, r2, ex1, ex2, q, s_rot, off, gate_mul, step, sched, true, false, &mut |_, _| {});
}

/// Multiply on the module's supported states, with off and s_rot zero at
/// both boundaries. The inverse requires a supported forward POST state.
/// Only forward M10 erases its known predicate; generic dirty-state callers
/// must keep using multiply_forward/multiply_backward.
#[allow(clippy::too_many_arguments)]
pub(crate) fn multiply_supported(
    c: &mut Circuit, r1: &[QReg], r2: &[QReg], ex1: &[QReg], ex2: &[QReg],
    q: &[QReg], s_rot: &[QReg], off: &QReg, gate_mul: &QReg,
    step: usize, sched: &StepWidths, inverse: bool,
) {
    multiply_step(c, r1, r2, ex1, ex2, q, s_rot, off, gate_mul, step, sched, inverse, true, &mut |_, _| {});
}
...
pub(crate) fn multiply_forward_marked(
    ...
    multiply_step(c, r1, r2, ex1, ex2, q, s_rot, off, gate_mul, step, sched, false, false, mark);
}
...
pub(crate) fn multiply_backward_marked(
    ...
    multiply_step(c, r1, r2, ex1, ex2, q, s_rot, off, gate_mul, step, sched, true, false, mark);
}
...
fn multiply_step(
    ...
    let m1 = |c: &mut Circuit, inv: bool| {
        aligned_scan_bottom(c, r2, e_b, e_ca, gate_mul, sched.w_b, sched.m1_lo_b(), base_v, base_c, inv, Some(off));
    };
    ...
    let m10 = |c: &mut Circuit| {
        ...
        if supported && !inverse { capture_clear(c, gate_mul, &v, &u, e_cb, sched.m10_capture_window(), off); }
        else { capture_compare(c, gate_mul, &v, &u, e_cb, sched.m10_capture_window(), off); }
    };
