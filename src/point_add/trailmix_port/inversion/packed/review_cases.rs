...
fn sub_state(st: &St, wq: usize, gate: bool) -> Vec<bool> {
...
fn drv_expected(st: &St, wq: usize, nctr: usize, parity: bool, counter: usize) -> Vec<bool> {
    let mut v = ring_lsb(&st.a, &st.cb);
    v.extend(ring_lsb(&st.b, &st.ca));
    v.extend(bits_of(bl(&st.a), EXP_BITS));
    v.extend(bits_of(bl(&st.cb), EXP_BITS));
...
fn route_env_defaults() {
    ...
    for (k, v) in [
        ...
        ("TRAILMIX_THIN_VALIDATE", "500000"), ("TRAILMIX_COUNTER_W", "8"), ("TRAILMIX_Q_CAP", "99"), ("TRAILMIX_Q_TARGET", "685"),
...
fn run_sub_case(case: &SubCase, t: &Trace, term_from: usize) -> (Kind, Result<(), &'static str>, usize, usize, String) {
    ...
    let data = sub_state(&row.pre, wq, true);
    // expected: the mid state (after the substep, before the swap), s_rot/off/term 0
    let mut expect = sub_state(&row.mid, wq, true);
    let _ = &mut expect;
...
fn driver_round_trip(inputs: &[U512], label: &str, term_from: usize) -> (usize, usize, Vec<usize>) {
    ...
    let s0_ids = drv_data_ids(&p);
    {
        ...
        for (j, t) in traces.iter().enumerate() {
            ...
            let bits = drv_expected(&st, p.q.len(), nctr, true, 0);
    ...
    for i in 0..NSTEPS {
        ...
        for (j, t) in traces.iter().enumerate() {
            ...
            let row = &t.rows[i];
            let want = drv_expected(&row.post, wq, nctr, row.parity, row.counter);
    ...
    for i in (0..NSTEPS).rev() {
        ...
        assert_eq!(&ids, want_ids, "{label} backward row {i}: register wires differ");
    ...
    for (j, t) in traces.iter().enumerate() {
        ...
        let st = St { a: prime(), b: t.x, ca: U512::ZERO, cb: one(), q: U512::ZERO };
        let want = drv_expected(&st, p.q.len(), nctr, true, 0);
...
fn probe(spec: &str, term_from: usize, pop: bool) {
    ...
    let div = row.kind == Kind::Div;
    let data = sub_state(&row.pre, wq, true);
    let expect = sub_state(&row.mid, wq, true);
