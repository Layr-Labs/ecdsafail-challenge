...
#[inline]
fn uma_masked(c: &mut Circuit, f: &QReg, ctrl: &QReg, carry: &QReg, a: &QReg, b: &QReg, t: &QReg, active_mask: bool) {
    c.ccx(f, b, t);
    c.ccx(a, t, carry);
    c.cx(carry, a);
    if active_mask {
        ...
        match &mut mask {
            ZoneMask::Flags(flags) => {
                ...
                for i in zone.clone().rev() {
                    uma_masked(c, flags[i - zone.start], ctrl, &carry, target[i], addend[i], &t, false);
                }
            }
...
fn masked_sweeps(
    ...
    driver(c, true, &mut |c, i, g| {
        ...
        next = i;
        uma_masked(c, &f, ctrl, carry, target[i], addend[i], t, active_mask);
        c.cx(g, &f); // f = [i - 1 < E]
    });
