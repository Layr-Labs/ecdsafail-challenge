...
//! per masked cell in total without captures, 13 with M5's two.
//!
...
#[inline]
fn maj_masked(c: &mut Circuit, f: &QReg, carry: &QReg, a: &QReg, b: &QReg, t: &QReg) {
    c.ccx(f, carry, b);
    c.ccx(f, b, t);
    c.cx(carry, a);
    c.ccx(a, t, carry);
...
#[inline]
fn uma_masked(c: &mut Circuit, f: &QReg, ctrl: &QReg, carry: &QReg, a: &QReg, b: &QReg, t: &QReg) {
    c.ccx(f, b, t);
    c.ccx(a, t, carry);
    c.cx(carry, a);
    c.ccx(ctrl, t, a);
    ...
    if z > 0 {
        ...
        match &mut mask {
            ZoneMask::Flags(flags) => {
                ...
                for i in zone.clone() {
                    maj_masked(c, flags[i - zone.start], &carry, target[i], addend[i], &t);
                }
                for i in zone.clone().rev() {
                    uma_masked(c, flags[i - zone.start], ctrl, &carry, target[i], addend[i], &t);
                }
            }
            ZoneMask::Exponent(exp) => {
                ...
                masked_sweeps(c, ctrl, target, addend, &zone, &carry, &t, captures, &mut driver);
            }
...
fn masked_sweeps(
    ...
    c.x(&f); // f = [zone.start - 1 < E] = 1 on the support
    ...
    driver(c, false, &mut |c, i, g| {
        ...
        next += 1;
        c.cx(g, &f); // f = [i < E]
        if let Some(w) = captures.overflow {
            c.ccx(g, carry, w);
        }
        if captures.absorb {
            c.ccx(g, carry, target[i]);
        }
        maj_masked(c, &f, carry, target[i], addend[i], t);
    });
    ...
    driver(c, true, &mut |c, i, g| {
        ...
        next = i;
        uma_masked(c, &f, ctrl, carry, target[i], addend[i], t);
        c.cx(g, &f); // f = [i - 1 < E]
    });
