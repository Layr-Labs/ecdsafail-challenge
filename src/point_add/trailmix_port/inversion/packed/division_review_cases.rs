...
fn miss_snapshot(cnt: &Counts) -> [usize; 10] {
    [cnt.miss_width_a, cnt.miss_width_q, cnt.miss_lo_b, cnt.miss_rot, cnt.miss_drop, cnt.miss_term_row, cnt.miss_term_window, cnt.miss_residue4, cnt.miss_residue9, cnt.miss_exp_range]
}
const MISS_NAMES: [&str; 10] = ["width_a", "width_q", "lo_b", "rot", "drop", "term_row", "term_window", "residue4", "residue9", "exp_range"];
...
fn shot_refined(row: &Row, sched: &StepWidths, wq: usize, with_term: bool, gate: bool, cnt: &mut Counts, lob: &mut BTreeMap<LoB, (usize, usize)>) -> (Shot, bool) {
    ...
    if gate {
        ...
        let only_lo_b = after[2] == before[2] + 1 && (0..10).all(|i| i == 2 || after[i] == before[i]);
...
pub(super) fn run(term_from: usize, cnt: &mut Counts, circuits: &mut usize) {
    ...
    // A shift of 24 needs at least 25 quotient bits, not 24.
    // With a smaller q clamp the s = 24/25 rows are
    // q-width misses under the static tables too (inverse-checked only).
    let q_capped = std::env::var("TRAILMIX_Q_CAP").ok().and_then(|s| s.parse::<usize>().ok()).is_some_and(|c| c < 25);
    ...
    // Wider envelopes move the boundary beyond the fixed adversarial inputs.
    // Require actual value-checked boundary rows from either test population.
    let checked = |name: &str| classes.get(name).map_or(0, |e| e.0) > 0;
    assert!(rc.adv_eawa_off1 > 0 || checked("eA=WA_off1_tight"));
    assert!(rc.adv_tight_eawa > 0 || checked("eA=WA_off0_tight") || checked("eA=WA_off1_tight"));
    for need in ["terminal", "b1_inexact"] {
        assert!(classes.get(need).map_or(0, |e| e.0) > 0, "class {need} never value-checked");
    }
    let q_capped = std::env::var("TRAILMIX_Q_CAP").ok().and_then(|s| s.parse::<usize>().ok()).is_some_and(|c| c < 24);
    if !thin && !q_capped {
        // class coverage is asserted on the static tables (the thin schedule's narrower
        // q / W_A envelopes, and the shipped q clamp, leave some boundary rows unbuildable)
        for need in [
