...
/// Gated decoder peak, including folded-root endpoint and transition scratch.
pub(super) fn gated_scratch(bits: usize) -> usize {
    if bits >= 7 { 4.max(bits - 3) }
...
pub(crate) fn onehot_stream_with(
    ...
    match gate {
        Some(g) if k >= 7 => eng.sixteenth_root(c, g, body),
        Some(g) if k >= 5 => eng.octant_root(c, g, body),
...
impl Engine<'_> {
    fn sixteenth_root(&self, c: &mut Circuit, g: &QReg, body: &mut dyn FnMut(&mut Circuit, usize, &QReg)) {
        let k = self.exp.len();
        let size = 1usize << (k - 4);
        let mut blocks: Vec<usize> = (0..16).filter(|&i| self.in_range(i * size, size)).collect();
        if self.descending { blocks.reverse(); }
        let root = c.alloc_qreg("onehot.sixteenth");
        let bits: Vec<&QReg> = self.exp[k-4..].iter().collect();
        let endpoint = |c: &mut Circuit, index: usize, erase: bool| {
            for j in 0..4 { if index >> j & 1 == 0 { c.x(bits[j]); } }
            let mut chain = Vec::new();
            for j in 0..3 {
                let q = c.alloc_qreg("onehot.endpoint");
                c.ccx(chain.last().unwrap_or(g), bits[j], &q);
                chain.push(q);
            }
            if erase { self.clear(c, &root, &chain[2], bits[3]); }
            else { c.ccx(&chain[2], bits[3], &root); }
            for j in (0..3).rev() {
                self.clear(c, &chain[j], if j == 0 { g } else { &chain[j-1] }, bits[j]);
            }
            for q in chain { c.zero_and_free(q); }
            for j in (0..4).rev() { if index >> j & 1 == 0 { c.x(bits[j]); } }
        };
        endpoint(c, blocks[0], false);
        for (j, &block) in blocks.iter().enumerate() {
            if j > 0 {
                let low = block.min(blocks[j-1]);
                assert_eq!(block.abs_diff(blocks[j-1]), 1);
                let pivot = (low ^ (low+1)).ilog2() as usize;
                // Adjacent equality flags differ by three affine factors:
                // unchanged upper bits and lower bits XOR the carry pivot.
                let mut factors = Vec::new();
                for bit in 0..4 {
                    if bit < pivot { c.cx(bits[pivot], bits[bit]); }
                    else if bit > pivot && low >> bit & 1 == 0 { c.x(bits[bit]); }
                    if bit != pivot { factors.push(bits[bit]); }
                }
                let a = c.alloc_qreg("onehot.move.a");
                let b = c.alloc_qreg("onehot.move.b");
                c.ccx(g, factors[0], &a);
                c.ccx(&a, factors[1], &b);
                c.ccx(&b, factors[2], &root);
                self.clear(c, &b, &a, factors[1]);
                self.clear(c, &a, g, factors[0]);
                c.zero_and_free(b);
                c.zero_and_free(a);
                for bit in (0..4).rev() {
                    if bit < pivot { c.cx(bits[pivot], bits[bit]); }
            ...
            self.visit(c, k-4, block * size, &root, body);
        }
        endpoint(c, *blocks.last().unwrap(), true);
        c.zero_and_free(root);
    }

    // Folding three address bits saves one live wire. Adjacent octant flags
    // differ by a product of two affine bits, so each move needs one temporary.
    fn octant_root(&self, c: &mut Circuit, g: &QReg, body: &mut dyn FnMut(&mut Circuit, usize, &QReg)) {
        let k = self.exp.len();
        let size = 1usize << (k - 3);
        let mut octants: Vec<usize> = (0..8).filter(|&i| self.in_range(i * size, size)).collect();
        if self.descending { octants.reverse(); }
        let root = c.alloc_qreg("onehot.octant");
        self.octant_endpoint(c, g, &root, octants[0], false);
        for (j, &octant) in octants.iter().enumerate() {
            if j != 0 { self.octant_move(c, g, &root, octants[j - 1], octant); }
            self.visit(c, k - 3, octant * size, &root, body);
        }
        self.octant_endpoint(c, g, &root, *octants.last().unwrap(), true);
        c.zero_and_free(root);
    }

    fn octant_endpoint(&self, c: &mut Circuit, g: &QReg, root: &QReg, index: usize, erase: bool) {
        let k = self.exp.len();
        let bits = [&self.exp[k - 3], &self.exp[k - 2], &self.exp[k - 1]];
        for j in 0..3 { if index & (1 << j) == 0 { c.x(bits[j]); } }
        let t = c.alloc_qreg("onehot.top");
        let u = c.alloc_qreg("onehot.middle");
        c.ccx(g, bits[2], &t);
        c.ccx(&t, bits[1], &u);
        if erase { self.clear(c, root, &u, bits[0]); }
        else { c.ccx(&u, bits[0], root); }
        self.clear(c, &u, &t, bits[1]);
        self.clear(c, &t, g, bits[2]);
        c.zero_and_free(u);
        c.zero_and_free(t);
        for j in (0..3).rev() { if index & (1 << j) == 0 { c.x(bits[j]); } }
    }

    fn octant_move(&self, c: &mut Circuit, g: &QReg, root: &QReg, from: usize, to: usize) {
        assert_eq!(from.abs_diff(to), 1);
        let k = self.exp.len();
        let (h, m, l) = (&self.exp[k - 1], &self.exp[k - 2], &self.exp[k - 3]);
        let lower = from.min(to);
        let t = c.alloc_qreg("onehot.move");
        if lower & 1 == 0 {
            if lower & 4 == 0 { c.x(h); }
            if lower & 2 == 0 { c.x(m); }
            c.ccx(g, h, &t);
