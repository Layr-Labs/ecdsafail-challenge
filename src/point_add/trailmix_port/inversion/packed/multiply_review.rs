...
fn pre_state(s: &StepWidths, row: &Row, wq: usize, gate: bool) -> Vec<bool> {
    state(s, &row.a, &row.b, &row.ca, &row.cb, &row.q, wq, gate)
}
fn post_state(s: &StepWidths, row: &Row, wq: usize, gate: bool) -> Vec<bool> {
    let s2 = row.s2.expect("multiply row");
    let q_new = row.q ^ (one() << s2);
    state(s, &row.a, &row.b, &row.ca_new(), &row.cb, &q_new, wq, gate)
}

fn shot(s: &StepWidths, row: &Row, wq: usize, dir: Dir, gate: bool) -> Shot {
    match (dir, gate) {
        (Dir::Fwd, true) => Shot { data: pre_state(s, row, wq, true), expect: post_state(s, row, wq, true) },
        (Dir::Bwd, true) => Shot { data: post_state(s, row, wq, true), expect: pre_state(s, row, wq, true) },
        (Dir::Round, true) => Shot { data: pre_state(s, row, wq, true), expect: pre_state(s, row, wq, true) },
        (Dir::Bwd, false) => Shot { data: post_state(s, row, wq, false), expect: post_state(s, row, wq, false) },
        (_, false) => Shot { data: pre_state(s, row, wq, false), expect: pre_state(s, row, wq, false) },
    }
}
...
fn run_step(traces: &[Trace], step: usize, cnt: &mut Counts, tag: &str, verbose: bool) -> usize {
    ...
    for (h, dir) in [(&hf, Dir::Fwd), (&hb, Dir::Bwd), (&hr, Dir::Round)] {
        for gate in [true, false] {
            let shots: Vec<Shot> = on.iter().map(|r| shot(&s, r, wq, dir, gate)).collect();
            if !shots.is_empty() {
                cnt.shots += shots.len();
    ...
    if !off.is_empty() {
        let shots: Vec<Shot> = off.iter().map(|r| shot(&s, r, wq, Dir::Round, true)).collect();
...
fn run_synth(rng: &mut impl XofReader, step: usize, cnt: &mut Counts, tag: &str) -> (Vec<&'static str>, Vec<&'static str>) {
    ...
    for (name, spec) in specs_for(&s, wq) {
        ...
        for (h, dir) in [(&hf, Dir::Fwd), (&hb, Dir::Bwd), (&hr, Dir::Round)] {
            let shots = [shot(&s, &row, wq, dir, true), shot(&s, &row, wq, dir, false)];
            cnt.shots += 2;
