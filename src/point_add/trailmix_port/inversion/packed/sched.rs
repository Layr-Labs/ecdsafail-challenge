...
//! (`MIDQ_PACKED_QCAP`, default 866 = design section 5); the direct-ctz
...
use crate::point_add::trailmix_port::inversion::shrunken_pz_schedule::{reg_widths, shift_bounds, thin_lo};

/// The 257-wire rings.
pub(crate) const RING: usize = 257;
/// Exponent register width (design: 9-bit bit-lengths 0..256).
pub(crate) const EXP_BITS: usize = 9;
/// The aligned scans' window width (design 2.4).
pub(crate) const WINDOW: usize = 32;
/// Packed peak the substeps budget their scratch against (design section 5).
pub(crate) const DEFAULT_QCAP: usize = 866;
...
pub(crate) struct StepWidths {
    ...
    pub w_c: usize,
    /// q's natural envelope width (the driver sizes q; informational here).
    pub w_q: usize,
...
impl StepWidths {
    ...
    pub(crate) fn from_schedule(step: usize) -> Self {
        ...
        Self::new(step, wa.max(wb), wb, wca.max(wcb), wq, lo(wa), lo(wb), lo(wca), lo(wcb), sdb, s2b)
    }
    ...
    pub(crate) fn assert_packed_step(&self) {
        ...
        assert!(self.w_a - self.div_ring_bottom() <= 127, "packed step {}: D11 ring {}..{} wider than 128", self.step, self.div_ring_bottom(), self.w_a);
    }
    ...
    /// `k = e_B - cascade_lo` cascade cells.
    pub(crate) fn div_capture_window(&self) -> CaptureWindow {
        ...
        CaptureWindow { lo: lo.min(self.w_a), hi: self.w_a, field: FieldEnd::AddrMinus(lo_c) }
    }
    ...
    pub(crate) fn mul_zone_start(&self) -> usize {
        (RING - self.w_a).min(self.lo_cb + 1).min(self.mul_window_cells())
    }
    ...
    pub(crate) fn m1_lo_b(&self) -> usize {
        if self.lo_b_free() { 0 } else { self.lo_b.min(RING - WINDOW).min(self.w_b) }
    }
    ...
    pub(crate) fn m10_capture_window(&self) -> CaptureWindow {
        CaptureWindow { lo: self.m10_lo_c() + 1, hi: self.w_c, field: FieldEnd::AddrMinus(self.m10_cascade_lo()) }
    }
    ...
    pub(crate) fn dump_line(&self) -> String {
        ...
        format!(
            "MIDQ_PACKED_SCHED step={} w_a={} w_b={} w_c={} w_q={} lo_a={} lo_b={} lo_ca={} lo_cb={} rb_div={} rb_mul={} s_div_bound={} s2_bound={} | D4/D9 cascade_lo={} addr=[{},{}] D7 zone_start={} D11 ring=[{},{}) n_w={} D0 n0={} | M1 lo_b={} M5 zone_start={} M10 lo_c={} cascade_lo={}",
            self.step, self.w_a, self.w_b, self.w_c, self.w_q, self.lo_a, self.lo_b, self.lo_ca, self.lo_cb, self.rb_div, self.rb_mul,
            self.s_div_bound, self.s2_bound, self.div_cascade_lo(), win.lo, win.hi, self.div_zone_start(), self.div_ring_bottom(), self.w_a,
