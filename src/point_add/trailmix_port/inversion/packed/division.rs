...
pub(crate) fn division_forward(
    ...
    division_step(c, r1, r2, ex1, ex2, q, s_rot, off, gate_div, term, step, sched, false, false);
}
...
pub(crate) fn division_backward(
    ...
    division_step(c, r1, r2, ex1, ex2, q, s_rot, off, gate_div, term, step, sched, true, false);
}

/// Division on the module's supported states, with off/s_rot/term zero at
/// entry and exit. The inverse requires a supported forward POST state.
/// D9 forward and D4 backward then erase known predicates. Unlike the generic
/// APIs, this does NOT promise a reversible extension to arbitrary dirty data.
#[allow(clippy::too_many_arguments)]
pub(crate) fn division_supported(
    c: &mut Circuit, r1: &[QReg], r2: &[QReg], ex1: &[QReg], ex2: &[QReg],
    q: &[QReg], s_rot: &[QReg], off: &QReg, gate_div: &QReg, term: Option<&QReg>,
    step: usize, sched: &StepWidths, inverse: bool,
) {
    division_step(c, r1, r2, ex1, ex2, q, s_rot, off, gate_div, term, step, sched, inverse, true);
}
...
fn division_step(
    ...
    let d4 = |c: &mut Circuit| {
        ...
        if supported && inverse { capture_clear(c, gate, &v, &u, e_b, window, off); }
        else { capture_compare(c, gate, &v, &u, e_b, window, off); }
    };
