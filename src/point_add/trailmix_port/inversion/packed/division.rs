...
//!
//! # Not merged: D10 with D11's rotate-back
//!
//! The design merges D10 into D11's rotate-back on late rows (`L = 0`); the
//! reviewed `aligned_scan_top` rotates back internally, so D10 is a separate
//! `rb`-layer rotation here: `+rb (W_A - 1)` T on late rows (375 at step 378).
//!
...
use super::aligned_scan::{aligned_scan_top, EXP_BITS};
use super::capture_compare::capture_compare;
use super::ctz::ctz_xor;
use super::sched::{scratch_room, StepWidths, RING};
use super::exponent_arith::{
    ctrl_add_const, ctrl_sub_reg, ctrl_add_reg, off_update, off_update_inverse, offset_apply,
    offset_apply_inverse, offset_release, offset_release_inverse, term_toggle, xor_diff_low,
};
...
fn division_step(
    ...
    let d0 = |c: &mut Circuit| {
        if let Some(t) = term {
            term_toggle(c, gate, e_b, s_rot, &r1[..n0], t);
        }
    };
    ...
    let d7 = |c: &mut Circuit, inv: bool| {
        ...
        masked_add_refs(c, gate, &target, &addend, zone.clone(), ZoneMask::Exponent(e_b), !inv, Captures::default());
    };
    ...
    let d11 = |c: &mut Circuit, inv: bool| {
        aligned_scan_top(c, r1, e_b, e_a, gate, w_a, sched.lo_b, inv, |c, pos| {
            if let Some(t) = term {
                if inv {
                    terminal_override_nw_inverse(c, t, e_a, pos, s_rot, n_w);
                } else {
                    terminal_override_nw(c, t, e_a, pos, s_rot, n_w);
                }
            }
        });
    };
    ...
    if !inverse {
        ...
        item(c, "D10", |c| d10(c, false));
        item(c, "D0c", |c| d0(c));
        item(c, "D12", |c| d12(c));
    } else {
        item(c, "D12", |c| d12(c));
        item(c, "D0c", |c| d0(c));
        item(c, "D10", |c| d10(c, true));
