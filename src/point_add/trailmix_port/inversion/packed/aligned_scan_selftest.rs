...
fn scan_motion_t(width: usize, window: usize) -> usize {
    if super::super::window_gather::enabled() {
        super::super::window_gather::toffoli(width, window, T7_BITS)
    } else {
        subring_rotate_toffoli(width, T7_BITS)
    }
}
...
fn build_top(w_a: usize, lo_b: usize, inverse: bool) -> Harness {
    ...
    aligned_scan_top(&mut c, &r1, &e_b, &e_a, &gate, w_a, lo_b, 0, inverse, None, None, false, |c, pos| {
...
fn build_bottom(w_b: usize, lo_b: usize, inverse: bool) -> Harness {
    ...
    let start = c.b.ops.len();
    aligned_scan_bottom(&mut c, &r2, &e_b, &e_ca, &gate, w_b, lo_b, 0, 0, inverse, None);
    let t = toffoli(&c.b.ops[start..]);
    let scratch = peak_scratch(&c.b.ops[start..], &ids);
    Harness { ops: c.b.ops.clone(), ids, t, scratch }
}
...
pub(super) fn run() {
    ...
    for &step in &steps {
        ...
        for (ci, &(w_a, lo_b)) in top_cfgs.iter().enumerate() {
            ...
            if ci == 0 {
                ...
                t_rows.push(format!("D11 step {step:3} W_A {w_a:3} lo_B {lo_b:3} L {l:3} S {s_ring:3} n_w {n_w:2}: T {t:5} (rot {})", 2 * scan_motion_t(s_ring, n_w)));
            }
        }
        let w_b = w.min(256);
        ...
        for (ci, &(w_b, lo_b)) in bot_cfgs.iter().enumerate() {
            ...
            if ci == 0 {
                let (top, s_ring) = bottom_geometry(w_b, lo_b);
                t_rows.push(format!("M1  step {step:3} W_B {w_b:3} lo_B {lo_b:3} T {top:3} S {s_ring:3}      : T {t:5} (rot {})", 2 * scan_motion_t(s_ring, WINDOW)));
            }
        }
    }
    ...
    for &(step, w_a) in &design {
        ...
        design_rows.push(format!(
            ...
            2 * scan_motion_t(s_top, n_w), top - 2 * scan_motion_t(s_top, n_w), top_h.scratch,
            2 * scan_motion_t(s_bot, WINDOW), bot - 2 * scan_motion_t(s_bot, WINDOW), bot_h.scratch));
    }
    ...
    let pieces = [
        piece("t7(7b)", &|c| {
            ...
            exp_offset(c, &t, &e, 76, true, false, None);
        }),
        piece("ladder32(3n)", &|c| {
        ...
        piece("u=257-e", &|c| {
            ...
            exp_to_k_minus(c, &er, 257, false);
        }),
        piece("u7=(257-bv-bc)-e (7b, k=128)", &|c| {
            let e = c.alloc_qreg_bits("e", 7);
            let er: Vec<&QReg> = e.iter().collect();
            exp_to_k_minus(c, &er, 128, false);
        }),
        piece("cmp9", &|c| {
