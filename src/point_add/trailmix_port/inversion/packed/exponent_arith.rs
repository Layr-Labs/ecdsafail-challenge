...
/// Leaf-by-leaf DFS one-hot engine on `addr` rooted at `root` (design 2.2 in
/// cursor form). Level `l` (1-based, from the MSB) holds
/// `root AND [addr[w-l..w) == the top l bits of the current leaf]`; the leaf
/// wire is level `w`. Ascending only: `advance` moves from leaf i to i+1 by
/// clearing the trailing-ones levels, one CX flip and re-pushing them; the
/// total is ~1 CCX per visited leaf. Inactive rows (`root = 0`) hold every
/// level at 0.
...
impl<'a> OnehotCursor<'a> {
    fn new(root: &'a QReg, addr: &'a [QReg]) -> Self {
        Self { root, addr, levels: Vec::new(), cur: None, coherent: onehot_coherent() }
    }
    ...
    fn push(&mut self, c: &mut Circuit, depth: usize, b: bool) {
        ...
        {
            let parent = if depth == 0 { self.root } else { &self.levels[depth - 1] };
            let bit = &self.addr[self.w() - 1 - depth];
            if !b {
                c.x(bit);
            }
    ...
    fn leaf(&self) -> &QReg {
        if self.w() == 0 { self.root } else { &self.levels[self.w() - 1] }
    }
    ...
    fn advance(&mut self, c: &mut Circuit) {
        ...
        match self.cur {
            ...
            Some(i) => {
                assert!(w > 0 && i + 1 < (1usize << w), "onehot cursor: past the last leaf");
                let t = i.trailing_ones() as usize;
                for depth in (w - t..w).rev() {
    ...
    /// Unwind every level (from the current leaf to the idle state).
    fn finish(&mut self, c: &mut Circuit) {
        if let Some(i) = self.cur.take() {
            let w = self.w();
            for depth in (0..w).rev() {
                let b = (i >> (w - 1 - depth)) & 1 == 1;
                self.pop(c, depth, b);
            }
        }
    }
...
/// pruned (never fire). Self-inverse; every ancilla (eq1, root, the KG
/// ancillae, the cursor levels, the capture flags) is returned to 0.
pub(crate) fn term_toggle(
    ...
    c.ccx(gate_div, &eq1, &root);
    if n == 0 {
        // A mod 1 == 0 always: term ^= root AND [shift == 0].
        let zero_bits = refs(shift);
        for q in &zero_bits {
            c.x(q);
        }
        let mut ctrls: Vec<&QReg> = vec![&root];
        ...
        let anc = c.alloc_qreg_bits("pk.term_kg", kg_prefix_compact_ancilla_count(n));
        let mut cursor = OnehotCursor::new(&root, shift);
        {
            let win = refs(a_win);
            let kg = KgPrefixAnd::new_compact_refs(&win, &anc);
