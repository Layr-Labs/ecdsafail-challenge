...
fn shot(row: &Row, sched: &StepWidths, wq: usize, with_term: bool, gate: bool, cnt: &mut Counts) -> Shot {
    ...
    data.extend(bits_of(e_a, EXP_BITS));
    data.extend(bits_of(e_cb, EXP_BITS));
    ...
    expect.extend(bits_of(e_a_new, EXP_BITS));
    expect.extend(bits_of(e_cb, EXP_BITS));
    expect.extend(bits_of(e_b, EXP_BITS));
    expect.extend(bits_of(e_ca, EXP_BITS));
...
pub(super) fn run() {
    ...
    assert!(cnt.ea_eq_wa_off0 > 0, "no e_A = W_A, off = 0 row");
