...
pub(crate) fn aligned_scan_top(
    ...
    let align = |c: &mut Circuit, up: bool| {
        ...
        exp_offset(c, &t7, e_b, w_a - base, true, false);
        subring_rotate(c, &ring, &t7, up);
        exp_offset(c, &t7, e_b, w_a - base, true, true);
        for q in t7 {
            c.zero_and_free(q);
        }
    };
...
pub(crate) fn aligned_scan_bottom(
    ...
    let align = |c: &mut Circuit, up: bool| {
        let t7 = c.alloc_qreg_bits("pas.t7", T7_BITS);
        exp_offset(c, &t7, e_b, k_align, false, false);
        subring_rotate(c, &ring, &t7, up);
        exp_offset(c, &t7, e_b, k_align, false, true);
        for q in t7 {
            c.zero_and_free(q);
        }
    };
