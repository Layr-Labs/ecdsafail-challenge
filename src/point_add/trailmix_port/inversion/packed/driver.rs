...
use super::super::*;
use super::capture_compare::{capture_clear, capture_compare, CaptureWindow, FieldEnd};
...
fn role_compute(c: &mut Circuit, p: &Packed, active: &QReg, role: &QReg, sched: &StepWidths, erase: bool) {
    ...
    if erase { capture_clear(c, active, &v, &u, p.e_cb(), window, role); }
    else { capture_compare(c, active, &v, &u, p.e_cb(), window, role); }
...
fn role_clear(c: &mut Circuit, p: &Packed, active: &QReg, role: &QReg, sched: &StepWidths, terminal_row: bool, erase: bool) {
    ...
    if erase { capture_clear(c, active, &v, &u, p.e_b(), window, role); }
    else { capture_compare(c, active, &v, &u, p.e_b(), window, role); }
...
pub(crate) fn pass_step(c: &mut Circuit, p: &Packed, i: usize, inverse: bool) -> StepMoments {
    ...
    // Both role predicates are rooted at active: role implies active.
    // Use role directly, and recover active from the unchanged counter only
    // at the boundaries. No separate gate or active wire spans either substep.
    if !inverse {
        let active = compute_active(c, active_counter);
        let role = c.alloc_qreg("cross.role");
        role_compute(c, p, &active, &role, &sched, false);
        if no_terminal && pop {
            prefix_popcount::update(c, &p.counter, &role, &active, false);
        }
        uncompute_active(c, active_counter, &active);
        c.zero_and_free(active);
        // Step boundaries carry zero off/s_rot/term and the packed support
        // invariants required by the promised-domain arithmetic interfaces.
        multiply_supported(c, &p.r1, &p.r2, &p.ex1, &p.ex2, &p.q, &p.s_rot, off, &role, i, &sched, false);
        let active = compute_active(c, active_counter);
        c.cx(&active, &role); // X(role) on active rows; a frozen row keeps role = 0
        uncompute_active(c, active_counter, &active);
        c.zero_and_free(active);
        let term = terminal_row.then(|| c.alloc_qreg("pk.term"));
        division_supported(c, &p.r1, &p.r2, &p.ex1, &p.ex2, &p.q, &p.s_rot, off, &role, term.as_ref(), i, &sched, false);
        if let Some(t) = term {
            c.zero_and_free(t);
        }
        let active = compute_active(c, active_counter);
        role_clear(c, p, &active, &role, &sched, terminal_row, true);
        c.zero_and_free(role);
        swap_and_done_forward(c, p, active, no_terminal);
    } else {
        let active = undo_done_and_swap(c, p, no_terminal);
        let role = c.alloc_qreg("cross.role");
        role_clear(c, p, &active, &role, &sched, terminal_row, false);
        uncompute_active(c, active_counter, &active);
        c.zero_and_free(active);
        let term = terminal_row.then(|| c.alloc_qreg("pk.term"));
        division_supported(c, &p.r1, &p.r2, &p.ex1, &p.ex2, &p.q, &p.s_rot, off, &role, term.as_ref(), i, &sched, true);
        if let Some(t) = term {
            c.zero_and_free(t);
        }
        let active = compute_active(c, active_counter);
        ...
        uncompute_active(c, active_counter, &active);
        c.zero_and_free(active);
        multiply_supported(c, &p.r1, &p.r2, &p.ex1, &p.ex2, &p.q, &p.s_rot, off, &role, i, &sched, true);
        let active = compute_active(c, active_counter);
        if no_terminal && pop {
            prefix_popcount::update(c, &p.counter, &role, &active, true);
        }
        role_compute(c, p, &active, &role, &sched, true);
        c.zero_and_free(role);
        uncompute_active(c, active_counter, &active);
        c.zero_and_free(active);
    }
