...
use super::division::division_supported;
use super::exponent_arith::{max_into_second, unmax_into_second};
use super::multiply::multiply_supported;
...
fn role_workspace<'a>(p: &'a Packed, sched: &StepWidths, coefficients: bool) -> Vec<&'a QReg> {
    let mut scratch: Vec<&QReg> = p.s_rot.iter().chain(p.off.iter()).collect();
    let (first, second, bound) = if coefficients {
        (p.e_ca(), p.e_cb(), sched.w_c - sched.base_c())
    } else {
        (p.e_a(), p.e_b(), sched.w_a - sched.base_v())
    };
    let used = (usize::BITS - bound.leading_zeros()) as usize;
    scratch.extend(first.iter().skip(used));
    scratch.extend(second.iter().skip(used));
    scratch
}

fn role_counter_workspace<'a>(p: &'a Packed, sched: &StepWidths, counter_parked: bool) -> Vec<&'a QReg> {
    if counter_parked { return Vec::new(); }
    // FROM clears the cache; before row i's done update the counter <= i-FROM.
    let counter_used = if sched.step < terminal_from() {
        if popcount_on(&p.counter) { (usize::BITS - (p.q.len() + 1).leading_zeros()) as usize } else { 0 }
    } else {
        (usize::BITS - (sched.step - terminal_from()).leading_zeros()) as usize
    };
    p.counter.iter().skip(counter_used).collect()
}

/// `role ^= active AND [A < B]` (exact; module doc).
fn role_compute(c: &mut Circuit, p: &Packed, active: &QReg, role: &QReg, sched: &StepWidths, erase: bool, counter_parked: bool) {
    let storage = role_counter_workspace(p, sched, counter_parked);
    c.with_recycled_zero(&storage, |c| {
    let sec = c.push_section("pk.role");
    let flag = c.alloc_qreg("pk.rc.c");
    max_into_second(c, p.e_ca(), p.e_cb(), &flag); // e_cb := m = max(e_ca, e_cb)
    let lo_c = sched.role_compute_lo_c();
    ...
    // All borrowed bits are globally zero here, including inactive rows.
    let scratch = role_workspace(p, sched, false);
    super::capture_compare::capture_with_scratch(c, active, &v, &u, p.e_cb(), window, role, erase, &scratch);
...
fn role_clear(c: &mut Circuit, p: &Packed, active: &QReg, role: &QReg, sched: &StepWidths, terminal_row: bool, erase: bool, counter_parked: bool) {
    let storage = role_counter_workspace(p, sched, counter_parked);
    c.with_recycled_zero(&storage, |c| {
    ...
    let scratch = role_workspace(p, sched, true);
    super::capture_compare::capture_with_scratch(c, active, &v, &u, p.e_b(), window, role, erase, &scratch);
...
// Before FROM the cache is popcount(q), possibly updated by +/-1 before the
// arithmetic. Its value is nonnegative and at most q.len()+1 on support.
fn early_workspace(p: &Packed, pop: bool) -> Vec<&QReg> {
    let used = if pop {
        (usize::BITS - (p.q.len() + 1).leading_zeros()) as usize
    } else { 0 };
    p.counter.iter().skip(used).collect()
}

// counter != 0 implies a frozen row with q = 0. On active rows counter = 0.
// Controlled exchange therefore makes ALL counter wires globally zero, while
// the frozen count sits in q. Every arithmetic operation is then disabled on
// that branch and restores arbitrary q. Repeat after the substeps to unpack.
fn park_frozen_counter(c: &mut Circuit, p: &Packed, active: &QReg) {
    c.x(active);
    for (counter, quotient) in p.counter.iter().zip(&p.q) {
        c.cswap(active, counter, quotient);
    }
    c.x(active);
}

fn arithmetic_pair(c: &mut Circuit, p: &Packed, role: &QReg, sched: &StepWidths, active: Option<&QReg>, inverse: bool) {
    let off = p.off.as_ref().expect("live offset");
    let mul = |c: &mut Circuit| multiply_supported(c, &p.r1, &p.r2, &p.ex1, &p.ex2,
        &p.q, &p.s_rot, off, role, sched.step, sched, inverse);
    let div = |c: &mut Circuit| {
        let term = (sched.step >= terminal_from()).then(|| c.alloc_qreg("pk.term"));
        division_supported(c, &p.r1, &p.r2, &p.ex1, &p.ex2, &p.q, &p.s_rot,
            off, role, term.as_ref(), sched.step, sched, inverse);
        if let Some(q) = term { c.zero_and_free(q); }
    };
    if inverse { div(c); } else { mul(c); }
    if let Some(active) = active { c.cx(active, role); } else { c.x(role); }
    if inverse { mul(c); } else { div(c); }
}
...
pub(crate) fn pass_step(c: &mut Circuit, p: &Packed, i: usize, inverse: bool) -> StepMoments {
    ...
    if terminal_row {
        let active = if inverse { undo_done_and_swap(c, p, false) }
            else { compute_active(c, active_counter) };
        let role = c.alloc_qreg("cross.role");
        park_frozen_counter(c, p, &active);
        // Generic test schedules can have q shorter than the counter. Park
        // and recycle only their common prefix; the other counter bits stay.
        let storage: Vec<&QReg> = p.counter.iter().take(p.q.len()).collect();
        c.with_recycled_zero(&storage, |c| {
            if inverse { role_clear(c, p, &active, &role, &sched, true, false, true); }
            else { role_compute(c, p, &active, &role, &sched, false, true); }
            arithmetic_pair(c, p, &role, &sched, Some(&active), inverse);
            if inverse { role_compute(c, p, &active, &role, &sched, true, true); }
            else { role_clear(c, p, &active, &role, &sched, true, true, true); }
        });
        park_frozen_counter(c, p, &active);
        ...
        } else { swap_and_done_forward(c, p, active, false); }
    } else if !inverse {
        let active = compute_active(c, active_counter);
        let role = c.alloc_qreg("cross.role");
        role_compute(c, p, &active, &role, &sched, false, false);
        if pop {
            prefix_popcount::update(c, &p.counter, &role, &active, false);
        }
        uncompute_active(c, active_counter, &active);
        c.zero_and_free(active);
        let storage = early_workspace(p, pop);
        c.with_recycled_zero(&storage, |c| arithmetic_pair(c, p, &role, &sched, None, false));
        let active = compute_active(c, active_counter);
        role_clear(c, p, &active, &role, &sched, false, true, false);
        c.zero_and_free(role);
        swap_and_done_forward(c, p, active, true);
    } else {
        let active = undo_done_and_swap(c, p, true);
        let role = c.alloc_qreg("cross.role");
        role_clear(c, p, &active, &role, &sched, false, false, false);
        uncompute_active(c, active_counter, &active);
        c.zero_and_free(active);
        let storage = early_workspace(p, pop);
        c.with_recycled_zero(&storage, |c| arithmetic_pair(c, p, &role, &sched, None, true));
        let active = compute_active(c, active_counter);
        if pop {
            prefix_popcount::update(c, &p.counter, &role, &active, true);
        }
        role_compute(c, p, &active, &role, &sched, true, false);
        c.zero_and_free(role);
        uncompute_active(c, active_counter, &active);
        c.zero_and_free(active);
    }
