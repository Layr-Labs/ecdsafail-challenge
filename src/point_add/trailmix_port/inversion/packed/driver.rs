...
pub(crate) fn pass_step(c: &mut Circuit, p: &Packed, i: usize, inverse: bool) -> StepMoments {
    ...
    if !inverse {
        ...
        multiply_forward(c, &p.r1, &p.r2, &p.ex1, &p.ex2, &p.q, &p.s_rot, off, &role, i, &sched);
        let active = compute_active(c, active_counter);
        c.cx(&active, &role); // X(role) on active rows; a frozen row keeps role = 0
        uncompute_active(c, active_counter, &active);
        c.zero_and_free(active);
        let term = terminal_row.then(|| c.alloc_qreg("pk.term"));
        division_forward(c, &p.r1, &p.r2, &p.ex1, &p.ex2, &p.q, &p.s_rot, off, &role, term.as_ref(), i, &sched);
        if let Some(t) = term {
            c.zero_and_free(t);
        }
        let active = compute_active(c, active_counter);
        ...
        c.zero_and_free(active);
        let term = terminal_row.then(|| c.alloc_qreg("pk.term"));
        division_backward(c, &p.r1, &p.r2, &p.ex1, &p.ex2, &p.q, &p.s_rot, off, &role, term.as_ref(), i, &sched);
        if let Some(t) = term {
            c.zero_and_free(t);
        }
        let active = compute_active(c, active_counter);
        ...
        uncompute_active(c, active_counter, &active);
        c.zero_and_free(active);
        multiply_backward(c, &p.r1, &p.r2, &p.ex1, &p.ex2, &p.q, &p.s_rot, off, &role, i, &sched);
        let active = compute_active(c, active_counter);
        if no_terminal && pop {
