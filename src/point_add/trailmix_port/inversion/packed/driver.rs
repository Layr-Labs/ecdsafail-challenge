...
//! the route to `MIDQ_PACKED_QCAP` (default 866) for the duration of one
...
fn role_compute(c: &mut Circuit, p: &Packed, active: &QReg, role: &QReg, sched: &StepWidths) {
    ...
    let window = CaptureWindow { lo: lo_c + 1, hi: sched.w_c.max(lo_c + 1), field: FieldEnd::BaseMinusAddr(RING) };
...
fn role_clear(c: &mut Circuit, p: &Packed, active: &QReg, role: &QReg, sched: &StepWidths, terminal_row: bool) {
    ...
    let window = CaptureWindow { lo: lo_v + 1, hi: sched.w_a.max(lo_v + 1), field: FieldEnd::BaseMinusAddr(RING) };
