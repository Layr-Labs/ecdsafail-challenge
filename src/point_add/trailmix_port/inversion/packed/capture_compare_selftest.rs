...
impl Case {
    fn build(n: usize, m: usize, window: CaptureWindow) -> Case {
        ...
        assert!(ops.iter().all(|op| !matches!(op.kind,
        ...
        let case = Case { ops, ids, is_data, n, m, window, nq, nb: nb as usize, toffoli, ancilla: nq - 2 * n - m - 2 };
    ...
    fn effective(&self) -> Option<(usize, usize)> {
        let hi_addr = (1usize << self.m) - 1;
        let mut eff: Option<(usize, usize)> = None;
        for a in self.window.lo..=self.window.hi.min(hi_addr) {
    ...
    fn t_model(&self) -> (usize, usize) {
        ...
        let (nodes, quarters) = engine_model(self.m, lo, hi, nofold);
        let engine = nodes + 3 * quarters;
        let t = 2 * self.n + z + engine * if coherent { 2 } else { 1 };
        (t, 2 * self.n + 2 * z)
    }

    fn describe(&self) -> String {
        let (_, design) = self.t_model();
        format!("n={} m={} z={} window=[{},{}] {:?} T={} (design 2n+2z={}) ancilla_wires={}",
            self.n, self.m, self.leaves(), self.window.lo, self.window.hi, self.window.field,
    ...
    fn apply(&self, sim: &mut Simulator<'_, Measurements>, live: u64) -> Vec<u64> {
        ...
        for (i, op) in self.ops.iter().enumerate() {
            if op.kind == OperationType::R {
                sim.apply_iter(self.ops[start..i].iter());
                assert_eq!(sim.qubit(op.q_target) & live, 0, "dirty reset at op {i}: {op:?} [{}]", self.describe());
                start = i;
            }
        }
        sim.apply_iter(self.ops[start..].iter());
...
fn random_batches(case: &Case, batches: usize, tie: bool, meas: &mut Measurements, rng: &mut Rng) -> usize {
    ...
    for _ in 0..batches {
        ...
        for s in 0..64 {
            let lo = case.window.lo.saturating_sub(2);
            let hi = (case.window.hi + 2).min((1 << case.m) - 1);
            let addr = lo + rng.below(hi - lo + 1);
            for i in 0..case.m {
                cols[case.idx_addr(i)] |= (((addr >> i) & 1) as u64) << s;
            }
...
fn synthetic_cases(checked: &mut usize, report: &mut Vec<String>, modes: &[usize]) {
    const N: usize = 257;
    let asc = Case::build(N, 9, CaptureWindow { lo: 1, hi: 257, field: FieldEnd::AddrMinus(0) });
    let desc = Case::build(N, 9, CaptureWindow { lo: 0, hi: 256, field: FieldEnd::BaseMinusAddr(257) });
    let clamp = Case::build(N, 9, CaptureWindow { lo: 301, hi: 1000, field: FieldEnd::AddrMinus(300) });
...
fn real_state_cases(checked: &mut usize, report: &mut Vec<String>, seeds: u64) {
    ...
    let asc = Case::build(RING, 9, CaptureWindow { lo: 1, hi: 257, field: FieldEnd::AddrMinus(0) });
    let desc = Case::build(RING, 9, CaptureWindow { lo: 0, hi: 256, field: FieldEnd::BaseMinusAddr(257) });
        ...
        for step in 0..NSTEPS {
            ...
            *checked += m10.flush(&asc, m);
            ...
            if w_a_div > 0 {
                ...
                let n = w_a_div - lo_a;
                let case = Case::build(n, 9, CaptureWindow { lo: lo_a + 1, hi: w_a_div, field: FieldEnd::AddrMinus(lo_a) });
                thin_cases += 1;
            ...
            if w_c_mul > 0 {
                ...
                let case = Case::build(n, 9, CaptureWindow { lo: lo_c + 1, hi: w_c_mul, field: FieldEnd::AddrMinus(lo_c) });
                thin_cases += 1;
            ...
            if w_c_pre > 0 {
                ...
                let case = Case::build(257 - lo_c, 9, CaptureWindow { lo: lo_c, hi: w_c_pre, field: FieldEnd::BaseMinusAddr(257) });
                thin_cases += 1;
            ...
            if w_a_end > 0 {
                ...
                let case = Case::build(257 - lo_a, 9, CaptureWindow { lo: lo_a, hi: w_a_end, field: FieldEnd::BaseMinusAddr(257) });
                thin_cases += 1;
...
pub(super) fn run() {
    ...
    for n in 8..=12usize {
        let m = 4;
        let case = Case::build(n, m, CaptureWindow { lo: 1, hi: n, field: FieldEnd::AddrMinus(0) });
    ...
    for n in 8..=9usize {
        let m = 4;
        let windows = [
            CaptureWindow { lo: 1, hi: n, field: FieldEnd::AddrMinus(0) },
            CaptureWindow { lo: 0, hi: n, field: FieldEnd::AddrMinus(0) },      // addr 0 -> k = 0 dropped
            CaptureWindow { lo: 3, hi: n - 1, field: FieldEnd::AddrMinus(0) },  // sub-window
            CaptureWindow { lo: 4, hi: n + 3, field: FieldEnd::AddrMinus(3) },  // offset cascade (D4 lo)
            CaptureWindow { lo: 0, hi: n, field: FieldEnd::BaseMinusAddr(n) },  // role shape, addr n -> k = 0
            CaptureWindow { lo: 3, hi: n + 1, field: FieldEnd::BaseMinusAddr(n + 2) },
        ];
    ...
    let wide = |n: usize, lo: usize, hi: usize, field: FieldEnd, batches: usize,
        ...
        let case = Case::build(n, 9, CaptureWindow { lo, hi, field });
    ...
    for (coherent, nofold) in [(true, false), (false, true), (true, true)] {
        ...
        let case = Case::build(8, 4, CaptureWindow { lo: 0, hi: 8, field: FieldEnd::AddrMinus(0) });
