...
pub(crate) fn capture_clear(
    ...
    if env_flag("MIDQ_ONEHOT_COHERENT")
        || std::env::var("MIDQ_CAPTURE_PHASE_ERASE").ok().as_deref() == Some("0") {
        capture_compare(c, root, v, u, addr, window, out);
        return;
    }
    ...
    c.hmr(out, phase);
    c.with_condition(phase, |c| capture_impl(c, root, v, u, addr, window, None));
    c.free_bit(phase);
}
...
fn capture_impl(
    ...
    let mut next_cell = k_lo;
    {
        let mut body = |c: &mut Circuit, g: &QReg, value: usize| {
            ...
            if let Some(out) = out { c.ccx(g, b[k - 1], out); }
            else { c.cz(g, b[k - 1]); }
            if k < n {
                cell(c, k);
            }
