...
/// and the address-to-cell map. `lo`, `hi` and the map are in TRUE address
/// units; the address register holds `addr - base` (the 7-bit rebased
/// exponents of `sched.rs`; `base = 0` for a plain register), so the sweep
/// runs over the register values `[lo - base, hi - base]`, clipped to the
/// representable addresses `[base, base + 2^m - 1]`.
...
pub(crate) struct CaptureWindow {
    ...
    pub hi: usize,
    pub field: FieldEnd,
    /// True address = register value + base.
    pub base: usize,
}
...
fn onehot_sweep(
    ...
    if !env_flag("MIDQ_ONEHOT_NOFOLD") && m >= 5 {
        super::onehot_stream::onehot_stream(c, addr, lo, hi+1, Some(root), !ascending,
            &mut |c, i, g| body(c, g, i));
        return;
    }
...
pub(crate) fn capture_compare(
    ...
    capture_impl(c, root, v, u, addr, window, Some(out), &[]);
}

/// Erase a known comparison result, correcting the HMR phase from unchanged
/// operands. Unlike capture_compare, PRE requires out == the predicate.
pub(crate) fn capture_clear(
    c: &mut Circuit,
    root: &QReg,
    v: &[&QReg],
    u: &[&QReg],
    ...
    capture_with_scratch(c, root, v, u, addr, window, out, true, &[]);
}

/// The same XOR/known-predicate erase with caller-owned CLEAN workspace.
/// Every scratch bit must start at zero and is returned to zero, not freed.
#[allow(clippy::too_many_arguments)]
pub(crate) fn capture_with_scratch(
    c: &mut Circuit, root: &QReg, v: &[&QReg], u: &[&QReg], addr: &[QReg],
    window: CaptureWindow, out: &QReg, erase: bool, scratch: &[&QReg],
) {
    let mut ids = std::collections::HashSet::new();
    assert!(scratch.iter().all(|q| ids.insert(q.id()) && q.id() != root.id()
        && q.id() != out.id() && v.iter().chain(u).all(|r| r.id() != q.id())
        && addr.iter().all(|r| r.id() != q.id())), "capture scratch aliases data");
    if !erase || env_flag("MIDQ_ONEHOT_COHERENT")
        || std::env::var("MIDQ_CAPTURE_PHASE_ERASE").ok().as_deref() == Some("0") {
        capture_impl(c, root, v, u, addr, window, Some(out), scratch);
        return;
    }
    assert!(v.iter().chain(u.iter()).all(|q| q.id() != out.id())
        && addr.iter().all(|q| q.id() != out.id()) && root.id() != out.id());
    let phase = c.alloc_bit();
    c.hmr(out, phase);
    // HMR has made out clean; lend it to its own phase correction.
    let phase_scratch: Vec<&QReg> = std::iter::once(out).chain(scratch.iter().copied()).collect();
    c.with_condition(phase, |c| capture_impl(c, root, v, u, addr, window, None, &phase_scratch));
    c.free_bit(phase);
}
...
fn capture_impl(
    ...
    debug_assert!(
        v.iter().chain(u.iter()).all(|q| out.is_none_or(|o| q.id() != o.id()) && q.id() != root.id())
            && addr.iter().all(|q| out.is_none_or(|o| q.id() != o.id()) && q.id() != root.id())
            && out.is_none_or(|o| o.id() != root.id()),
    ...
    // cells (values outside the register's address space [base, base + 2^m)
    // cannot occur: clamp).
    let base = window.base;
    let hi_addr = base + (1usize << addr.len()) - 1;
    let mut eff: Option<(usize, usize)> = None;
    for a in window.lo.max(base)..=window.hi.min(hi_addr) {
    ...
    let (cc, scratch) = match scratch.split_first() {
        Some((q, rest)) => (BorrowedQReg::Borrowed(*q), rest),
        None => (BorrowedQReg::Owned(c.alloc_qreg("bc.c")), scratch),
    };
    ...
    let fresh = !env_flag("MIDQ_ONEHOT_COHERENT")
        && std::env::var("MIDQ_CAPTURE_CHUNKS").ok().as_deref() != Some("0");
    let (prefix, sizes) = if fresh {
        let available = super::sched::scratch_room(c).saturating_sub(carry::engine_room(addr.len()))
            + scratch.len();
        let available = std::env::var("MIDQ_CAPTURE_CHUNK_ROOM").ok()
            .and_then(|v| v.parse::<usize>().ok()).map_or(available, |v| v.min(available));
        carry::plan(k_hi, available)
    } else { (n, Vec::new()) };
    // The fresh suffix reads unchanged operands. Its checkpoint phases are
    // corrected before the in-place prefix is reversed.
    for i in 0..prefix.min(k_lo) { cell(c, i); }
    let initial = if prefix == 0 { &cc } else { b[prefix - 1] };
    let mut carries = carry::Carries::new(a, b, initial, prefix, sizes, scratch);
    for i in prefix..k_lo { carries.advance(c, i); }
    let mut next_cell = k_lo;
    {
        let mut body = |c: &mut Circuit, g: &QReg, value: usize| {
            let k = window.cells(value + base).unwrap();
            assert_eq!(k, next_cell, "capture leaves must arrive in ascending cell order");
            // The same pure carry gives either an XOR result or its phase oracle.
            let borrow = if k <= prefix { b[k - 1] } else { carries.current() };
            if let Some(out) = out { c.ccx(g, borrow, out); }
            else { c.cz(g, borrow); }
            if k < prefix { cell(c, k); }
            else if k < k_hi { carries.advance(c, k); }
            next_cell = k + 1;
        };
        onehot_sweep(c, root, addr, lo - base, hi - base, ascending, &mut body);
    }
    debug_assert_eq!(next_cell, k_hi + 1);
    for i in (k_hi + 1)..prefix { cell(c, i); }
    carries.finish(c);
    // Reverse cascade: restore ~v, u and cc.
    for i in (1..prefix).rev() {
        c.ccx(b[i - 1], a[i], b[i]);
        c.cx(b[i], b[i - 1]);
        c.cx(b[i], a[i]);
    }
    ...
    if let BorrowedQReg::Owned(q) = cc { c.zero_and_free(q); }
    for q in v {
        c.x(q);
    }
