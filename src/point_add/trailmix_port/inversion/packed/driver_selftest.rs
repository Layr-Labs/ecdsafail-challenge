...
pub(crate) fn oracle_data_bits(x: U512, row: Option<usize>, wq: usize, nctr: usize, term_from: usize, pop: bool) -> Vec<bool> {
    ...
    match row {
        None => expected_lenient(&St { a: prime(), b: t.x, ca: U512::ZERO, cb: one(), q: U512::ZERO }, wq, nctr, true, 0),
        Some(i) => {
            let r = &t.rows[i];
            expected_lenient(&r.post, wq, nctr, r.parity, r.counter)
        }
    }
}
...
fn expected(st: &St, wq: usize, nctr: usize, parity: bool, counter: usize) -> Vec<bool> {
    assert!(bl(&st.q) <= wq, "q wider than its register");
    expected_lenient(st, wq, nctr, parity, counter)
}
...
/// departure anyway.
fn expected_lenient(st: &St, wq: usize, nctr: usize, parity: bool, counter: usize) -> Vec<bool> {
    let mut v = ring_lsb_lenient(&st.a, &st.cb);
    v.extend(ring_lsb_lenient(&st.b, &st.ca));
    v.extend(bits_of(bl(&st.a), EXP_BITS));
    v.extend(bits_of(bl(&st.cb), EXP_BITS));
    v.extend(bits_of(bl(&st.b), EXP_BITS));
    v.extend(bits_of(bl(&st.ca), EXP_BITS));
...
pub(super) fn run() {
    ...
    let s0_ids = data_ids(&p);
    {
        ...
        for (j, t) in traces.iter().enumerate() {
            ...
            let bits = expected(&st, p.q.len(), nctr, true, 0);
    ...
    for i in 0..steps {
        ...
        for j in 0..ninputs {
            ...
            let row = &t.rows[i];
            let want = expected(&row.post, wq, nctr, row.parity, row.counter);
    ...
    for i in (0..steps).rev() {
        ...
        assert_eq!(&ids, want_ids, "backward row {i}: register wires differ from the forward snapshot");
    ...
    for (j, t) in traces.iter().enumerate() {
        ...
        let st = St { a: prime(), b: t.x, ca: U512::ZERO, cb: one(), q: U512::ZERO };
        let want = expected(&st, p.q.len(), nctr, true, 0);
        let g = shot_bits(&got, j);
