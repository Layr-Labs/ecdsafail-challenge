...
fn parked_counter_test() {
    struct Measurements { mode: usize, rng: Box<dyn XofReader> }
    ...
    for width in [1usize, 4, 7, 8, 28] {
    let mut c = Circuit::new();
    let q = c.alloc_qreg_bits("park.q", width);
    let counter = c.alloc_qreg_bits("park.counter", 8);
    let active = c.alloc_qreg("park.active");
    let p = Packed { r1: vec![], r2: vec![], ex1: vec![], ex2: vec![], q,
        counter, s_rot: vec![], off: None, parity: None };
    let ids: Vec<QubitId> = p.q.iter().chain(&p.counter).chain([&active])
        .map(|q| QubitId(q.id().into())).collect();
    super::park_frozen_counter(&mut c, &p, &active);
    let storage: Vec<&QReg> = p.counter.iter().take(width).collect();
    c.with_recycled_zero(&storage, |c| {
        // Fill every recycled slot and one ordinary slot, then exercise
        // measured cleanup and both explicit/drop-based allocation returns.
        let scratch = c.alloc_qreg_bits("park.scratch", storage.len() + 1);
        let other = |i: usize| if width == 1 { &active } else { &p.q[(i + 1) % width] };
        for (i, bit) in scratch.iter().enumerate() { c.ccx(&p.q[i % width], other(i), bit); }
        for (i, bit) in scratch.iter().enumerate().rev() { c.clear_and(bit, &p.q[i % width], other(i)); }
        for (i, bit) in scratch.into_iter().enumerate() {
            if i % 2 == 0 { c.zero_and_free(bit); } else { drop(bit); }
        }
    });
    super::park_frozen_counter(&mut c, &p, &active);
    c.flush_pending_frees();
    assert_eq!(c.b.active_qubits as usize, width + 9);
    assert_eq!(c.b.peak_qubits as usize, width + 10);
    for mode in 0..4 {
        let mut seed = Shake256::default();
        seed.update(b"packed-counter-parking-v1");
        let mut rng = Measurements { mode, rng: Box::new(seed.finalize_xof()) };
        for start in (0..2048usize).step_by(64) {
            let mut sim = Simulator::new(width + 10, 64, &mut rng);
            for lane in 0..64 {
                let index = start + lane;
                let count = if index < 1024 { 0 } else { 1 + index % 255 };
                let quotient = if count == 0 { index.wrapping_mul(0x9e3779b9) & ((1 << width) - 1) } else { 0 };
                let values = (quotient as u64) | ((count as u64) << width) | (u64::from(count == 0) << (width + 8));
                for (i, &id) in ids.iter().enumerate() {
                    *sim.qubit_mut(id) |= ((values >> i) & 1) << lane;
                }
            }
            let before = snapshot(&sim, &ids);
            apply_chunk(&mut sim, &c.b.ops, &[], u64::MAX, "parked counter");
            assert_eq!(snapshot(&sim, &ids), before);
            assert_eq!(sim.phase, 0);
            assert_ancillae_clean(&sim, &ids, u64::MAX, "parked counter");
        }
    }
    }
    eprintln!("PACKED_COUNTER_PARK PASS40960: q1/4/7/8/28,all255frozen counts,1024active quotients,4measurement modes,all wires/phase/every reset,allocator spill/drop");
}
