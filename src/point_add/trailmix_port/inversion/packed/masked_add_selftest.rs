...
fn measured_cells() -> bool {
    std::env::var("MIDQ_MASKED_MEASURED").ok().as_deref() == Some("1")
}

fn cell_t(mask: MaskKind) -> usize {
    let base = if measured_cells() { 7 } else { 9 };
    base - 2 * usize::from(mask != MaskKind::Flags && super::active_mask_enabled())
}
...
impl Harness {
    ...
    fn check_batch<R: XofReader>(&mut self, batch: &[Case], rng: &mut R) {
        let v = self.fwd.v;
        // A measured stream is inverted by the independently built arithmetic
        // twin, not by reversing the order of its measurement instructions.
        let reversed = v.mask.coherent() && !measured_cells() && !self.skip_reversed;
...
fn report(b: &Built) {
    ...
    let cells = cell_t(v.mask) * z - cache_saving(b);
    let caps = if v.captures { z } else { 0 };
    match b.t_leaf {
        Some(leaf) => {
            assert_eq!(leaf, cells + caps, "leaf T per masked cell ({v:?})");
            let engine = b.t_total - plain - leaf;
            eprintln!(
                "PACKED_MASKED_ADD T n={} zone=[{},{}) subtract={} captures={} mask={}: total={} = plain {}x3 + masked {}x{} + captures {}x{} + engine (2 sweeps) {}; peak ancillae {} (c, t, f + engine)",
                v.n, v.zone_lo, v.n, v.subtract as u8, v.captures as u8, v.mask.name(), b.t_total,
                v.zone_lo, z, cell_t(v.mask), if v.captures { 1 } else { 0 }, z, engine, b.peak - b.data_wires
            );
        }
        None => {
            ...
            eprintln!(
                "PACKED_MASKED_ADD T n={} zone=[{},{}) subtract={} captures={} mask={}: total={} (plain {}x3, masked {}x{}, captures {}x{}, engine {}); peak ancillae {}",
                v.n, v.zone_lo, v.n, v.subtract as u8, v.captures as u8, v.mask.name(), b.t_total,
                v.zone_lo, z, cell_t(v.mask), if v.captures { 1 } else { 0 }, z,
...
fn check_miss_batch<R: XofReader>(h: &mut Harness, batch: &[Case], rng: &mut R) {
    ...
    if v.mask.coherent() && !measured_cells() {
...
/// 1 T for the joint pair of captures per leaf.
fn design_model(v: Variant) -> usize {
    let z = v.n - v.zone_lo;
    3 * v.n + 6 * z + if z > 0 { 2 * z } else { 0 } + if v.captures { z } else { 0 }
}

fn cache_saving(b: &Built) -> usize {
    if !matches!(b.v.mask, MaskKind::Exponent | MaskKind::Rebased) {
        return 0;
    }
    let saved = std::env::var_os("MIDQ_MASKED_PRODUCT_CACHE");
    std::env::set_var("MIDQ_MASKED_PRODUCT_CACHE", "0");
    let reference = build(b.v);
    match saved {
        Some(v) => std::env::set_var("MIDQ_MASKED_PRODUCT_CACHE", v),
        None => std::env::remove_var("MIDQ_MASKED_PRODUCT_CACHE"),
    }
    reference.t_total - b.t_total
}
...
fn report_design(b: &Built, label: &str) {
    ...
    let cells = 3 * v.zone_lo + cell_t(v.mask) * z + if v.captures { z } else { 0 } - cache_saving(b);
    ...
    if z > 0 && v.mask != MaskKind::Flags {
        ...
        assert!(engine as usize <= per_leaf * (z + 40), "engine T of {label}: {engine} for z={z}");
    }
}
...
fn run_product_cache<R: XofReader>(rng: &mut R) {
    let keys = ["MIDQ_MASKED_PRODUCT_CACHE", "MIDQ_PACKED_QCAP", "MIDQ_PACKED_CTZ_ROOM",
        "MIDQ_MASKED_MEASURED", "MIDQ_ONEHOT_COHERENT", "MIDQ_MASKED_ACTIVE_MASK"];
    let saved: Vec<_> = keys.iter().map(std::env::var_os).collect();
    std::env::remove_var("MIDQ_PACKED_CTZ_ROOM");
    let mut checked = 0;
    let mut variants = 0;
    for measured in [false, true] {
        std::env::set_var("MIDQ_MASKED_MEASURED", if measured { "1" } else { "0" });
        std::env::set_var("MIDQ_ONEHOT_COHERENT", if measured { "0" } else { "1" });
        for active in [false, true] {
            std::env::set_var("MIDQ_MASKED_ACTIVE_MASK", if active { "1" } else { "0" });
            for (n, zone_lo) in [(3, 0), (8, 3), (16, 5), (76, 1), (257, 181), (257, 0)] {
                for mask in [MaskKind::Exponent, MaskKind::Rebased] {
                    for subtract in [false, true] {
                        for captures in [false, true] {
                            let v = Variant { n, zone_lo, mask, subtract, captures };
                            let z = n - zone_lo;
                            std::env::set_var("MIDQ_MASKED_PRODUCT_CACHE", "0");
                            std::env::set_var("MIDQ_PACKED_QCAP", "847");
                            let reference = build(v);
                            let reserve = 3 + super::super::onehot_stream::gated_scratch(v.exp_bits());
                            for room in [0, 1, z / 2, z] {
                                let cap = reference.data_wires as usize + reserve + room;
                                std::env::set_var("MIDQ_PACKED_QCAP", cap.to_string());
                                std::env::set_var("MIDQ_MASKED_PRODUCT_CACHE", "1");
                                let mut h = Harness::new(v);
                                assert!(h.fwd.peak as usize <= cap, "cache exceeded cap: {v:?}");
                                assert_eq!(reference.t_total - h.fwd.t_total,
                                    room * if measured { 1 } else { 2 }, "cache T saving: {v:?}");
                                let cases = wide_cases(v, 128, 0xb930_46a7 ^ (n as u64) ^ room as u64);
                                h.check_all(cases.clone().into_iter(), rng);
                                if !measured {
                                    for batch in cases.chunks(64) {
                                        let out = run_batch(&h.fwd, &h.fwd.ops, batch, rng);
                                        let back = run_batch(&h.fwd, &h.rev, &out, rng);
                                        assert_eq!(back, batch, "cached coherent gate reverse: {v:?}");
                                    }
                                }
                                checked += h.checked;
                                variants += 1;
    ...
    eprintln!("PACKED_MASKED_PRODUCT_CACHE PASS {checked} cases, {variants} variants; zero/partial/full cache, exact T savings, cap, phase, reset, both inverses");
}
