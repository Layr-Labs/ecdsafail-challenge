...
fn build(v: Variant) -> Built {
    ...
    let exp = match v.mask {
        ...
        _ => c.alloc_qreg_bits("madd.test.e", addr_width(n)),
    };
...
fn run_batch<R: XofReader>(b: &Built, ops: &[Op], cases: &[Case], rng: &mut R) -> Vec<Case> {
    ...
    for (s, case) in cases.iter().enumerate() {
        ...
        if !b.exp.is_empty() {
            ...
            assert_eq!(e, case.e, "exponent changed ({:?})", b.v);
        }
        for (k, &id) in b.flags.iter().enumerate() {
...
fn report_design(b: &Built, label: &str) {
    ...
    if z > 0 && v.mask != MaskKind::Flags {
        ...
        // onehot_stream doc).
        let per_leaf = if v.mask == MaskKind::Exponent { 2 } else { 4 };
...
pub(super) fn run() {
    ...
    // (75 masked), subtract, no captures, 9-bit exponent.
    for mask in [MaskKind::ExponentCoherent, MaskKind::Exponent] {
