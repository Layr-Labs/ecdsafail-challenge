...
fn wide_roundtrip_cases(width: usize, cases: Vec<Case>) {
    let mut c = Circuit::new();
    let a = c.alloc_qreg_bits("test.a", 257);
    let b = c.alloc_qreg_bits("test.b", 257);
    let mut q = c.alloc_qreg_bits("test.q", width);
    ...
    restore(&mut c, &a, &b, &mut q, k, width);
    ...
    eprintln!("QCODE WIDE PASS width={width} tag_bits={} inputs={} Q={nq} compressed_live={compressed_live} flush_T={} compress_T={} restore_T={} inverse_flush_T={}",
        code_bits(width), cases.len(), tof(&c.b.ops[..after_flush]), tof(&c.b.ops[after_flush..compressed]),
        tof(&c.b.ops[compressed..restored]), tof(&c.b.ops[restored..]));
}

fn wide_roundtrip() { wide_roundtrip_cases(Q_BITS, wide_cases()); }
...
fn generic_cases(width: usize) -> Vec<Case> {
    let one=U512::from(1);let max=(one<<257)-one;
    let mask=(one<<width)-one;
    let mut cases=vec![[U512::ZERO,one,U512::ZERO],[max,one,U512::ZERO],
        [max,U512::ZERO,U512::ZERO],[max,max,U512::ZERO]];
    let mut random=rng();
    for k in 0..width {
        for sample in 0..4 {
            let mut bytes=[0u8;64];random.read(&mut bytes);let random_word=U512::from_le_bytes(bytes);
            let q=((((random_word & mask)>>k)|one)<<k)&mask;
            let d=((random_word << 131usize) | (random_word >> 381usize))&((one<<k)-one);
            let divisor_limit=max/(q+d);
            let b=match sample {0=>one,1=>divisor_limit,_=>random_word%divisor_limit+one};
            let residue_limit=(b-one).min(max-(q+d)*b);
            let residue=if sample&1==0 {U512::ZERO}else{residue_limit};
            let a=d*b+residue;
            assert!(a+q*b<=max && q.bit_len()<=width && q.trailing_zeros()==k);
            assert!((a/b)<(one<<k));
            cases.push([a,b,q]);
        }
    }
...
pub(crate) fn run_generic() {
    overflow_counterexample();
    for width in [1,2,17,18,19,30,32,38,64] {
        assert_eq!(code_bits(width),usize::BITS as usize-width.leading_zeros()as usize);
        wide_roundtrip_cases(width,generic_cases(width));
    }
    if std::env::var_os("MIDQ_GENERIC_QUOTIENT_CODE_EXTENDED").is_some() {
        for width in [128,257] {wide_roundtrip_cases(width,generic_cases(width));}
    }
    eprintln!("MIDQ_GENERIC_QUOTIENT_CODE_SELFTEST PASS widths1,2,17,18,19,30,32,38,64;257-bit coefficients; sentinel/zero-divisor/flush/restore/phase/pre-reset; extended128/257 only when requested");
}
