...
struct Range<'a>{rank:&'a[QReg],a:&'a[QReg],g:&'a QReg,cache:&'a QReg,mask:&'a QReg,dirty:&'a[QReg],group:isize,prefix:Option<super::q792_lifetime_cache_r01::C1Cache<'a>>}
impl Range<'_>{
    ...
    fn equality(&mut self,circ:&mut Circuit,value:usize){
        ...
        if !factor||lo/64!=(hi-1)/64 {
            if self.group!=h as isize{if let Some(prefix)=self.prefix.as_mut(){prefix.clear(circ);}self.high(circ,self.group);self.high(circ,h as isize);self.group=h as isize;}
            cs.push((self.cache,true));
        }
        if factor{if let Some(prefix)=self.prefix.as_mut(){prefix.equality(circ,value,lo/64!=(hi-1)/64);return;}}
...
pub(super) fn add_and_clear(circ:&mut Circuit,rank:&[QReg],source:&[QReg],target:&[QReg],a:&[QReg],g:&QReg,cache:&QReg,mask:&QReg,p1:&QReg,helpers:&[QReg],n:usize,c:&[QReg],sm:&[QReg],j:usize,c1:bool,low:bool){
    ...
    top_loan(circ,rank,a,g,carry,target,dirty);circ.cx(g,mask);
    let prefix=super::q792_lifetime_cache_r01::C1Cache::new(circ,a,cache,&c[4],&c[5],g,mask,n);
    let mut range=Range{rank,a,g,cache,mask,dirty,group:-1,prefix};let mut updates=Vec::new();
    for i in 0..n{
    ...
    assert!(updates.is_empty());circ.cx(g,mask);top_loan(circ,rank,a,g,carry,target,dirty);
