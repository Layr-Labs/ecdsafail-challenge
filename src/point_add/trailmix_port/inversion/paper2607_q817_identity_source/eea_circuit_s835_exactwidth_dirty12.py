...
def compact_lc_swap_gate(*, k: int, K: int,
    ...
    Dirty = QuantumRegister(5, "DirtyPassenger")
    ...
    # The Cuccaro carry is clean between the affine add and subtract.  Reuse
    # its lifetime as the deepest decoder-path lane whenever that reduces the
    # raw suffix from four levels to three.  Depth-nine windows still require
    # the proven four-raw-level fallback at Q817.
    use_carry_as_path = depth <= 8
    path_depth = max(0, depth - (3 if use_carry_as_path else 4))
    ...
    iteration = (
        ...
        if use_carry_as_path
        else unary_iteration_dirty_hexadecet_raw
    )
    iteration(
...
def compact_r_subrestore_fused_gate(*, n: int, k: int, K: int,
    ...
    _add_dirty_carry(
        qc, l_q, affine_addend, Dirty[9], clean_helper=clean_helper,
    )
    _add_const_dirty(qc, l_q, 4, Dirty, clean_helper=clean_helper)
    _const_minus_dirty(qc, l_s, n + 2, Dirty, clean_helper=clean_helper)
    _swap_zero_259_uncontrolled_dirty(
        qc, l_s, extension, Dirty, clean_helper=clean_helper,
    )
    ...
    def leaf_first(j, s_controls, q_controls):
        assert len(s_controls) == len(q_controls) == 10
        assert eq_s not in s_controls and eq_s not in q_controls
        addend, target = qpair(j)
        if j < K:
            next_addend, _ = qpair(j + 1)
            _apply_cell_clean_hmr(qc, 'sub', 'first', acc, addend, target,
                                  next_addend, clean_helper)
        _toggle_raw_controls_dirty(qc, s_controls, eq_s, Dirty, clean_helper=clean_helper)
        _apply_cell_clean_hmr(qc, 'sub', 'first', eq_s, addend, target,
                              carry, clean_helper)
        qc.cx(eq_s, acc)
        _toggle_raw_controls_dirty(qc, s_controls, eq_s, Dirty, clean_helper=clean_helper)
        _toggle_raw_controls_dirty(qc, q_controls, acc, Dirty, clean_helper=clean_helper)
        _toggle_raw_controls_dirty(qc, list(q_controls) + [addend], Sign[0], Dirty,
                                   clean_helper=clean_helper)
    ...
    def leaf_second(j, s_controls, q_controls):
        assert len(s_controls) == len(q_controls) == 10
        assert eq_s not in s_controls and eq_s not in q_controls
        addend, target = qpair(j)
        _toggle_raw_controls_dirty(qc, q_controls, acc, Dirty, clean_helper=clean_helper)
        ...
        if j < K:
            ...
            _apply_r_fused_second_cell_clean_hmr(
                qc, mode=Mode[0], ctrl=acc, addend=addend, target=target,
                carry=next_addend, clean_temp=clean_helper)
        _apply_r_fused_second_cell_clean_hmr(
            qc, mode=Mode[0], ctrl=eq_s, addend=addend, target=target,
            carry=carry, clean_temp=clean_helper)
        _toggle_raw_controls_dirty(qc, s_controls, eq_s, Dirty, clean_helper=clean_helper)
    ...
    _const_minus_dirty(qc, l_s, n + 2, Dirty, clean_helper=clean_helper)
    _sub_const_dirty(qc, l_q, 4, Dirty, clean_helper=clean_helper)
    _sub_dirty_carry(
        qc, l_q, affine_addend, Dirty[9], clean_helper=clean_helper,
    )
    return _e._finalize_block(qc)
...
def compact_prefix_addsub_gate(*, k: int, K: int,
    ...
    Work2 = QuantumRegister(M, "Work2")
    l_t = QuantumRegister(LT_WIDTH, "l_t")
    Borrowed = QuantumRegister(3, "Borrowed")
...
def _low256_range_scan_conditioned_dirty_five(
    ...
    *,
    ...
    """Scan 0..255 with four clean path lanes and restored dirty lenders."""
    if len(index_reg) != LS_WIDTH:
        raise ValueError("five-lane low decoder requires a 9-bit index")
    if len(ancillas) < 4:
        raise ValueError("five-lane low decoder requires four path lanes")
    if len(dirty) < 3:
        raise ValueError("five-lane low decoder requires three dirty lenders")
    path = list(ancillas[:4])
    high = index_reg[8]
    bit7 = index_reg[7]
    root = path[0]
...
def _range_scan_259_five_dirty(
    ...
    *,
    ...
    """Inclusive modulo-259 scan with five clean and restored dirty lanes."""
    if len(scratch) < 5:
        raise ValueError("dirty mod-259 range scan requires five clean lanes")
    path = list(scratch[:4])
    range_acc = scratch[4]
    if order == "inc":
        qc.cx(ctrl, range_acc)
...
def _upper_zero_map_midpoint_five_dirty(
    ...
    *,
    ...
    """Apply the 259-bit upper-zero map with five clean lanes."""
    if len(bits) != 259 or len(dirty_map) != 259:
        raise ValueError("five-lane midpoint map requires 259-bit registers")
    if len(scratch) < 5:
        raise ValueError("five-lane midpoint map requires five clean lanes")
...
def _range_scan_tight_dirty_hexadecet_aux6_terminal(
    ...
    *,
    ...
    """Exact five/six-clean terminal scan for labels through 259.
    ...
    use_six_clean = len(scratch) >= 6
    if not (k <= 255 < K):
        scan = (
            _range_scan_tight_dirty_octet
            if use_six_clean
            else _range_scan_tight_dirty_hexadecet
        )
        scan(
            qc, leq=leq, boundary=boundary, k=k, K=K, ctrl=ctrl,
            scratch=scratch, borrowed=borrowed, leaf_fn=leaf_fn, order=order,
    ...
    if len(scratch) < 5:
        raise ValueError("terminal Aux6 crossing scan needs five clean lanes")
    if len(borrowed) < 8:
    ...
    path_reduction = 3 if use_six_clean else 4
    ...
    direct_masks = {
        label: _terminal_aux6_direct_inversion_mask(
            main, label, 3 if use_six_clean else 4,
        )
...
def _upper_zero_map_borrowed(qc: QuantumCircuit, *, ctrl: Qubit,
    ...
    depth = _tight_unary_depth_for_labels(list(range(k, K + 1)))
    if len(scratch) < min(5, max(1, depth - 3)):
...
def compact_pre_shift_gate(*, work_size: int,
    ...
    both, inc_helper = Scratch

    qc.x(Phase1[0])
    for i in range(work_size - 1):
        _e.cswap_toffoli(qc, Phase1[0], Work2[i], Work2[i + 1])
    inc_mod259_1ctrl_dirty(
        qc, Phase1[0], l_s, Dirty, clean_helper=inc_helper,
    )

    qc.ccx(Phase1[0], Phase2[0], both)
    _e.controlled_rotate_right_by_two(qc, both, list(Work2))
    dec_mod259_1ctrl_dirty(qc, both, l_s, Dirty, clean_helper=inc_helper)
    dec_mod259_1ctrl_dirty(qc, both, l_s, Dirty, clean_helper=inc_helper)
    qc.ccx(Phase1[0], Phase2[0], both)
...
def append_one_step_T(qc: QuantumCircuit, *, T: int, n: int, len_width: int, shift_width: int,
    ...
    _e._append_with_optional_clbits(
        ...
        + list(l_t) + list(l_q) + list(l_s) + list(Dirty) + scratch[:4],
    )
    _toggle_live_r_phase(qc, phase1=Phase1[0], l_rp=l_rp, out=ctrl, dirty=Dirty)
    ...
    # can hold that temporary while all six clean scratch lanes remain
    # available.
    ...
    if t_addsub_reachable:
        ...
        _e._append_with_optional_clbits(qc, tsub, [ctrl, Sign[0]] + list(Work1[k3-1:K3]) + list(Work2[k3-1:K3])
                                        + list(l_t) + [Dirty[3], Dirty[6], Dirty[0]]
                                        + (scratch + [Phase2[0], Iter[0]])[:tsub.num_qubits-(2+2*(K3-k3+1)+LT_WIDTH+3)])
    _borrow_iter_for_tadd(
        qc, phase1=Phase1[0], iteration=Iter[0], l_q=l_q,
