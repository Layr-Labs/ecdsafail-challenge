...
pub(super) fn guarded(
    ...
    let rank4_capture=super::q792_rank4_lower_r01::pair_begin(circ,others);
    ...
    if others.len()==2 {
        // scratch*guard=0 after normalization. The middle gate therefore
        // consumes only guard*a*b; both outer gates return scratch exactly.
        if known {circ.x(scratch);}
        circ.ccx(others[0].0,others[1].0,scratch);
        circ.ccx(guard,scratch,target);
        circ.ccx(others[0].0,others[1].0,scratch);
        if known {circ.x(scratch);}
    ...
    super::q792_rank4_lower_r01::conditional_end(circ,rank4_capture,others,target,scratch,guard,known,dirty);
}
...
pub mod verification {
    ...
    pub fn run() {
        ...
        for n in [0usize,1,2,3,4,5,6,7,8,12,24,32] {for known in [false,true] {for mixed in [false,true] {
            ...
            let expected_t=match n {0|1=>0usize.max(n),2=>3,_=>2*n};assert_eq!(t,expected_t);
