...
pub(super) fn guarded(
    ...
    // With the step's dedicated clean scratch the guarded predicate is an exact (k+1)-control
    // Toffoli in 2k-1 gates via toggle; measured slightly below the 2k-2 guarded form overall.
    if let Some(clean)=super::length_recompute::clean_scratch() {
        let mut cs=vec![(guard,true)];cs.extend(others.iter().copied());
        super::paired_clean_mcx::toggle(circ,&cs,target,&clean);return;
    }
