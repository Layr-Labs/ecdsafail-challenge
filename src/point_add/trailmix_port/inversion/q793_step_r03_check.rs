...
pub fn run(){
    ...
    for block in 0..super::shared_step::SCHEDULE_BLOCKS{for j in 0..4{
        ...
        let t=b.ops.iter().filter(|op|op.kind==K::CCX).count();let span=super::shared_step::SCHEDULE_BLOCK;let copies=((block*span+span).min(1616)-block*span)/4;weighted_ops+=b.ops.len()*copies;weighted_t+=t*copies;
