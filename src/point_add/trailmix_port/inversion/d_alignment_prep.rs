...
/// PREP on borrowed clamped relative-position caches. Both cache words are
/// restored and remain live; beta describes logical B, not physical B<<phi.
/// Caller guarantees their common floor and the proved logical-B lower bound.
#[allow(clippy::too_many_arguments)]
pub(super) fn apply_cached(
    ...
    let carry = c.alloc_qreg("cache.prep.carry");
    // PB no longer needs a 5-to-9 extension. Only delta6 extends phi/srot5.
    let zeros = c.alloc_qreg_bits("cache.prep.pad", 1);
    let car: Vec<_> = ca.iter().collect();
    let cbr: Vec<_> = cb.iter().collect();
    if !inverse {
        clamp_xor(c, pa, pb, srot, &carry);
        let delta = c.alloc_qreg_bits("cache.prep.delta", 6);
        c.x(&delta[5]);
        add(c, &delta, srot, &zeros, &carry, false);
        add(c, &delta, phi, &zeros, &carry, true);
        rotate_delta(c, b, &delta, false);
