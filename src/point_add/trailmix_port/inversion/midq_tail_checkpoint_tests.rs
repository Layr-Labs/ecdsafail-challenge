...
pub(super) fn checkpoint_endpoint_sign_in_place() {
    for which in 0..2 {
        let mut c = Circuit::new();
        let a = c.alloc_qreg_bits("a", 3);
        let b = c.alloc_qreg_bits("b", 3);
        let output = c.alloc_qreg_bits("output", 1);
        with_endpoint_sign(&mut c, &a, &b, which, |c, sign| c.cx(sign, &output[0]));
        let midpoint = c.b.ops.len();
        with_endpoint_sign(&mut c, &a, &b, which, |c, sign| c.cx(sign, &output[0]));
        let regs = [ids(&a), ids(&b), ids(&output)];
        let (nq, nb, _, _) = analyze_ops(c.b.ops.iter());
        assert_eq!(nq, 7, "endpoint sign must not allocate scratch");
        let mut seed = Shake256::default();
        seed.update(b"checkpoint-endpoint-in-place-v1");
        let mut rng = seed.finalize_xof();
        let mut sim = Simulator::new(nq as usize, nb as usize, &mut rng);
        for old in 0..2 {
            sim.clear_for_shot();
            for input in 0..STATES {
                write(&mut sim, &regs[0], input, U256::from(input & 7));
                write(&mut sim, &regs[1], input, U256::from(input >> 3));
                write(&mut sim, &regs[2], input, U256::from(old));
            }
            for (step, range) in [(0, 0..midpoint), (1, midpoint..c.b.ops.len())] {
                sim.apply_iter(c.b.ops[range].iter());
                assert_eq!(sim.phase, 0);
                check_scratch(&sim, &[&regs[0], &regs[1], &regs[2]]);
                for input in 0..STATES {
                    let sign = usize::from(trajectory(input).1[which] < 0);
                    assert_eq!(read(&sim, &regs[0], input), U256::from(input & 7));
                    assert_eq!(read(&sim, &regs[1], input), U256::from(input >> 3));
                    assert_eq!(read(&sim, &regs[2], input), U256::from(old ^ if step == 0 { sign } else { 0 }));
                }
            }
        }
    }
    eprintln!("CHECKPOINT_ENDPOINT_INPLACE PASS: all 64 inputs x 2 signs x 2 target values, forward/roundtrip/phase, no scratch");
}
...
fn component(checkpoint: bool, encoded: bool) -> Component {
    let mut c = Circuit::new();
    let mut a = c.alloc_qreg_bits("a", WIDTH);
    let mut b = c.alloc_qreg_bits("b", WIDTH);
    ...
    let mut tape = Vec::new();
    if encoded {
        super::super::midq_odd_values::compress(&mut c, &mut a);
        super::super::midq_odd_values::compress(&mut c, &mut b);
        encoded_rounds(&mut c, &a, &b, &mut ca, &mut cb, false);
        super::super::midq_restore_endpoint_widths(&mut c, &mut ca, &mut cb);
    } else if checkpoint {
    ...
    if encoded {
        ...
        super::super::midq_odd_values::expand(&mut c, &mut a);
        super::super::midq_odd_values::expand(&mut c, &mut b);
        super::super::midq_restore_endpoint_widths(&mut c, &mut ca, &mut cb);
    } else if checkpoint {
...
pub(super) fn checkpoint_full_width_coefficients_match_reference() {
    let mut cases = vec![component(false, false), component(true, false)];
    if six_bits_enabled() { cases.push(component(true, true)); }
    ...
    for (checkpoint, case) in cases.iter().enumerate() {
        ...
        for batch in 0..8 {
            ...
            for input in 0..STATES {
                ...
                for reg in 0..2 {
                    if checkpoint == 2 {
                        let expected = if reg == 0 { input & 7 } else { input >> 3 };
                        assert_eq!(read(&sim, &case.terminal[reg], input), U256::from(expected));
                        continue;
                    }
    ...
    for observed in &observations[1..] { assert_eq!(&observations[0], observed); }
...
pub(super) fn checkpoint_schedule_storage_accounting() {
    assert_eq!(MIDQ_TAIL_VALUE_WIDTH.len(), MIDQ_TAIL_ROUNDS+1);
    assert!(MIDQ_TAIL_VALUE_WIDTH[START..].iter().all(|&w|w==4));
    if super::super::hybrid_profile::legacy_margin_allowed() {assert_eq!(MIDQ_TAIL_VALUE_WIDTH[START-1],5);}
    let raw_endpoint = MIDQ_TAIL_ROUNDS + 2 * WIDTH;
    let best_copied = (0..MIDQ_TAIL_ROUNDS)
        .map(|r| r + 4 * MIDQ_TAIL_VALUE_WIDTH[r] as usize - 2)
        .min()
        .unwrap();
    if super::super::hybrid_profile::legacy_margin_allowed() {assert_eq!(raw_endpoint,232);assert_eq!(best_copied,234);}
    let mut boundaries=vec![0,32,92,100,140,160,180,198,202,211,START];
    boundaries.retain(|&r|r<=START);boundaries.sort_unstable();boundaries.dedup();
    for r in boundaries {
