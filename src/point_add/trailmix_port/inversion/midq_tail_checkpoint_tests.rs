...
pub(super) fn checkpoint_schedule_storage_accounting() {
    ...
    assert_eq!(MIDQ_TAIL_VALUE_WIDTH[START - 1], 5);
    ...
    let best_copied = (0..MIDQ_TAIL_ROUNDS)
        ...
        .min()
        .unwrap();
    assert_eq!(raw_endpoint, MIDQ_TAIL_ROUNDS + 8);
    assert!(best_copied > raw_endpoint);
    for r in [0, 32, 92, 100, 140, 160, 180, 198, 202, START] {
