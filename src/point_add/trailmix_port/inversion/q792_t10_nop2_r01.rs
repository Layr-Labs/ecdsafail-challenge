...
pub(super) fn branch(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],n:usize,j:usize,last:bool,low:bool){
    ...
    // Direct target-pad carry lease preserves the global-A passenger in place.
    if last{
        ...
        else{let carry=&rest[0];let tail=&rest[1..];top_lease(circ,rank,a,g,carry,w2,tail);super::q793_t10_routes_v12::exchanges(circ,rank,a,c,g,&[p1,mask],&source,tail,carry,None);}
        ...
        else{let carry=&rest[0];let tail=&rest[1..];super::q793_t10_routes_v12::exchanges(circ,rank,a,c,g,&[mask],&source,tail,carry,None);top_lease(circ,rank,a,g,carry,w2,tail);}
        circ.cx(g,p1);
    }
    // Direct target-pad carry lease preserves the global-A passenger in place.
...
fn top_lease(circ:&mut Circuit,rank:&[QReg],a:&[QReg],g:&QReg,carry:&QReg,source:&[QReg],d:&[QReg]){let(root,ops)=super::q794_handoffs::gather_a(circ,rank,a,source,2,d);circ.cswap(g,root,carry);circ.b.ops.extend(ops.into_iter().rev());}
