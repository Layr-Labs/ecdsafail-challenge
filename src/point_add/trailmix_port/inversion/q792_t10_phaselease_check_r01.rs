...
pub fn run(){
    ...
    for block in 0..1{for j in 0..4{
        ...
        if export.is_some(){eprintln!("PHASE_ROUTING_EXPORTED_CLOCK j={j} ops={} T={t}",b.ops.len());}else{eprintln!("FOLD20_T10_PHASELEASE_BLOCK block={block} j={j} ops={} T={t} PASS",b.ops.len());}
    }}
    ...
    eprintln!("FOLD20_T10_PHASELEASE_PASS lanes={total} active={active} C1={c1_count} M255={m255} M256={m256} shortA={short} A253+={extreme} terminal={terminal} C256={c256} weighted_forward_ops={weighted_ops} weighted_forward_T={weighted_t}; all saved geometries/all4passengerpairs+2random/packed3holes/dirty/phase/literalinverse; NOT fullstep or wholeQ793");
}
