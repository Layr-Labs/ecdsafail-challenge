...
pub fn run(){
    ...
    for block in 0..1{for j in 0..4{
        ...
        for pattern in 0..6{for batch in 0..rows.len().div_ceil(64){
            ...
            let before=pack(&before);let after=pack(&after);sim.qubits.copy_from_slice(&before);sim.phase=0x937c1546a123b9ff;sim.apply_iter(b.ops.iter());
        ...
        eprintln!("FOLD20_T10_PHASELEASE_BLOCK block={block} j={j} ops={} T={t} PASS",b.ops.len());
    }}
