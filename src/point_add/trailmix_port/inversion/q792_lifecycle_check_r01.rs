...
// Independent fixed-boundary SWAP oracle. Production selects the same donor
// through the encoded A address; these two scalar lifecycle boundaries have
// known A=0 and A=255, so the reference uses only three explicit CNOTs.
fn boundary_swap(host:usize,pass:usize)->Vec<Op>{
 let mut out=Vec::new();for(a,t)in[(host,pass),(pass,host),(host,pass)]{let mut o=Op::empty();o.kind=OperationType::CX;o.q_control1=QubitId(a as u64);o.q_target=QubitId(t as u64);o.validate();out.push(o);}out
}
fn check_held_passenger<R:XofReader>(sim:&Simulator<R>,mapping:&[usize],passenger:&[QReg],initial:&[u64],inverse:bool,block:usize){
 let pass=passenger[2].id()as usize;assert_eq!(sim.qubits[pass],0,"held Sign must be zero inverse={inverse} block={block}");
 for q in &passenger[3..]{assert_eq!(sim.qubits[q.id()as usize],initial[q.id()as usize],"other passenger inverse={inverse} block={block}");}
 let ts=super::super::q792_fold20_rank_r01::triples();
 for lane in 0..64{let code=(0..20).fold(0usize,|v,i|v|((((sim.qubits[mapping[i]]>>lane)&1)as usize)<<i));
  let av=if code&15==15&&code>>5&1!=0{255}else{let(r,a,_,_)=super::super::q792_fold20_rank_r01::decode(code,&ts).expect("valid metadata at held boundary");64*ts[r][0]+a};
  let site=if av==255{mapping[282+258]}else{mapping[23+av+1]};assert_ne!(site,u32::MAX as usize);assert_eq!((sim.qubits[site]>>lane)&1,(initial[pass]>>lane)&1,"parked passenger inverse={inverse} block={block} lane={lane} A={av}");
 }
}
fn run_inner(boundary_only:bool){
 ...
 for inverse in [false,true]{let mapping=if inverse{&rebuilt_map}else{&initial_map};
  if !boundary_only{let host=if inverse{mapping[282+258]}else{mapping[23+1]};let acquire=boundary_swap(host,passenger[2].id()as usize);for sim in &mut sims{checked(sim,&acquire,"independent traversal acquire");}}
  ...
  if !boundary_only{for z in 0..super::super::shared_step::SCHEDULE_BLOCKS{let block=if inverse{super::super::shared_step::SCHEDULE_BLOCKS-1-z}else{z};let mut logical:Vec<_>=(0..4).map(|j|cache.get((block,j))).collect();let missing:Vec<_>=(0..4).filter(|&j|logical[j].is_none()).collect();let built=std::thread::scope(|scope|{let handles:Vec<_>=missing.iter().map(|&j|(j,scope.spawn(move||template(block,j)))).collect();handles.into_iter().map(|(j,h)|(j,h.join().unwrap())).collect::<Vec<_>>()});for(j,ops)in built{cache.insert((block,j),&ops);logical[j]=Some(ops);}let templates:Vec<_>=logical.into_iter().map(|ops|remap(ops.unwrap(),mapping,&passenger,inverse)).collect();let first=block*super::super::shared_step::SCHEDULE_BLOCK;let end=(first+super::super::shared_step::SCHEDULE_BLOCK).min(1616);
   ...
   for(batch,sim)in sims.iter().enumerate(){check_held_passenger(sim,mapping,&passenger,&initial[batch],inverse,block);}eprintln!("Q792_LIFECYCLE_BLOCK inverse={inverse} block={block} PASS");
  }
  }else{for batch in 0..batches{sims[batch].qubits=expected(&rows,batch,mapping,if inverse{64}else{132},&passenger,&initial[batch]);}}
  if !boundary_only{let host=if inverse{mapping[23+1]}else{mapping[282+258]};let release_held=boundary_swap(host,passenger[2].id()as usize);for sim in &mut sims{checked(sim,&release_held,"independent traversal return");}}
