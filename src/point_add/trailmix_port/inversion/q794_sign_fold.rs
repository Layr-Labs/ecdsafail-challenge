...
fn gather_sum<'a>(circ:&mut Circuit,rank:&[QReg],c:&[QReg],cache:&QReg,word:&'a[QReg],dirty:&[QReg],base:usize)->(&'a QReg,Vec<crate::circuit::Op>){
    ...
    // The selected leaf is source[k], k=257-(C+S), the top of the T word, and the block's
    // analytic T upper support is tend (the same bound that windows T10 and Sign), so
    // k<=tend and every address value C+S<257-tend is unreachable on support. Those leaves
    // are omitted (two-leaf margin); the tree over the remaining leaves is unchanged.
    let min_value=super::length_recompute::t_top().map(|tend|257usize.saturating_sub(tend+2)).unwrap_or(0);
    for value in 1..=257{if value>=min_value{nodes[value]=Some(&word[base-value]);}}
    let ts=triples();
...
fn gather_c<'a>(circ:&mut Circuit,rank:&[QReg],c:&[QReg],w1:&'a[QReg],dirty:&[QReg])->(&'a QReg,Vec<crate::circuit::Op>){
    // Address v is the residual length C; on support C<=259-rfirst (R occupies [rfirst,259)),
    // so leaves above c_max() (two-value margin) are unreachable and omitted.
    let c_max=super::length_recompute::c_max();
    let nodes:Vec<_>=(0..256).map(|v|if v>=2&&v<=c_max{Some(&w1[258-v])}else{None}).collect();
