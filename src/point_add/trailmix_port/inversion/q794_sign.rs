...
pub(super) fn emit(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,sign:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,n:usize){
    if super::metadata_muxlease::active("Q794_SIGN_RANK_FOLD"){super::q794_sign_fold::emit(circ,rank,a,c,sm,p1,p2,sign,w1,w2,helpers,j,n);return;}
    ...
    if maj {super::q794_sign_maj_history::emit(circ,rank,c,sm,p1,p2,mask,sign,w1,w2,dirty,j,n);}
