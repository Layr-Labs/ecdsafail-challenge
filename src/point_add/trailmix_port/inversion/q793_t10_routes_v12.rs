...
// Ported from welttowelt's public Q793 W47 submission, commit
// 4e6322f775db282dc8b22f700eb0c263e6e665fc. At this T10 consumer,
// M=A+C<=4b+3 bounds the shipped address above while the existing analytic
// A_SUPPORTS interval bounds it below. The routing unitary is immediately
// inverted, so leaves outside this live interval can be omitted exactly.
fn leaf_window(circ:&Circuit,hi:usize)->(usize,usize){
    let mut w=(1usize,hi);
    if let Some((lo,a_hi))=super::q792_t_narrow_support_r01::quotient_support(circ){
...
pub(super) fn exchanges(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],g:&QReg,passengers:&[&QReg],source:&[QReg],helpers:&[QReg],carry:&QReg,endpoint:Option<&QReg>){
    ...
    // A dummy is needed only for an empty window. Keeping it on nonempty
    // windows changes the selected rail and was rejected by the upstream
    // differential test.
