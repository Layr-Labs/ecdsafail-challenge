...
#[path="q806_prefix.rs"] pub(crate) mod prefix_cache;
#[path="q807_consumer_cache.rs"] pub(crate) mod consumer_cache;
...
pub(super) fn fused(s:&Scan<'_>,c:&mut Circuit,w1:&[QReg],w2:&[QReg],decision:&QReg){
 ...
 s.lower(c,0);s.lower(c,1);let low=c.b.ops[start..].to_vec();
    arithmetic::add(c,s.a,s.c,None,true);chart_seed(c,s.rank,s.a,s.c,s.g,s.mask,s.ha,s.hs,w1,w2,s.dirty,shift);
 ...
 let clean=prefix_cache::current();
 let consumer=consumer_cache::current();
 if let Some(q)=&consumer {
  assert!(!aliases.contains(&q.id())&&s.dirty.iter().chain(w1).chain(w2).all(|r|r.id()!=q.id()));
  assert!(clean.as_ref().is_none_or(|p|p.iter().all(|r|r.id()!=q.id())));
  assert!(super::super::length_recompute::clean_scratch().is_none_or(|r|r.id()!=q.id()));
 }
 if let Some(pool)=&clean {
  assert!(pool.iter().all(|q|!aliases.contains(&q.id())&&s.dirty.iter().chain(w1).chain(w2).all(|r|r.id()!=q.id())));
 }
 let mut upper_cache=clean.as_ref().map(|p|prefix_cache::Prefix::new(cc.iter().rev().collect(),&p[..6]));
 let mut lower_controls:Vec<_>=word[4..].iter().rev().collect();lower_controls.extend(s.sm.iter().rev());lower_controls.push(&s.c[0]);
 let mut lower_cache=clean.as_ref().map(|p|prefix_cache::Prefix::new(lower_controls,&p[6..]));
 let mut updates=Vec::new();let mut prefix=if clean.is_none(){a7_prefix::Prefix::new(n)}else{None};
 for i in 2..n{let at=c.b.ops.len();
  if let Some(p)=lower_cache.as_mut(){
   if i<=255&&(i+1)%2==s.j%2 {
    let value=(i+1)%256;let key=(value>>1)^(s.j>>1);
    c.cx(&s.a[0],&s.c[0]);p.toggle(c,&(0..7).rev().map(|b|key>>b&1!=0).collect::<Vec<_>>(),s.mask);c.cx(&s.a[0],&s.c[0]);
   }
  }else{lower(s,c,&word[4..],i);}
  let value=256-i;let cs:Vec<_>=cc.iter().enumerate().map(|(b,q)|(q,value>>b&1!=0)).collect();
  if let Some(p)=upper_cache.as_mut(){p.toggle(c,&(0..8).rev().map(|b|value>>b&1!=0).collect::<Vec<_>>(),s.mask);}
  else if !prefix.as_mut().is_some_and(|p|p.upper(c,&cc,&aa[7],s.g,s.mask,value)){c.x(s.g);super::super::paired_clean_mcx::toggle(c,&cs,s.mask,s.g);c.x(s.g);}updates.push(c.b.ops[at..].to_vec());s.carry(c,&w2[258-i],&w1[258-i],false);
 }
 c.cx(s.g,decision);c.ccx(s.g,s.ha,decision);
 if let Some(q)=&consumer{c.ccx(s.g,decision,q);}
 for i in (2..n).rev(){s.carry(c,&w2[258-i],&w1[258-i],true);c.cx(s.ha,&w2[258-i]);
  if let Some(q)=&consumer{super::gate(c,&[(q,true),(s.mask,true),(&w2[258-i],true)],&w1[258-i],s.dirty);}
  else{super::gate(c,&[(s.g,true),(s.mask,true),(&w2[258-i],true),(decision,true)],&w1[258-i],s.dirty);}
  c.cx(s.ha,&w2[258-i]);c.b.ops.extend(updates.pop().unwrap().into_iter().rev());}
 if let Some(q)=&consumer{c.ccx(s.g,decision,q);}
    arithmetic::add(c,&aa,&cc,None,true);c.b.ops.extend(converter.into_iter().rev());
    chart_seed(c,s.rank,s.a,s.c,s.g,s.mask,s.ha,s.hs,w1,w2,s.dirty,shift);s.low_update(c,w1,w2,decision,shift);arithmetic::add(c,s.a,s.c,None,false);c.b.ops.extend(low.into_iter().rev());assert_eq!(c.b.next_qubit,owned);
}
