...
pub fn run(){
    ...
    let mut total=0usize;let mut active=0usize;let mut c1_count=0usize;let mut endpoint=0usize;let mut extreme=0usize;let mut weighted_t=0usize;let mut weighted_ops=0usize;let mut prefix_active=0usize;let mut known_mask_counts=[0usize;2];let mut rank_prefix_active=0usize;
    for block in 0..26{for j in 0..4{
        ...
        let rows:Vec<_>=data[12..].chunks_exact(138).filter(|r|{let time=u16::from_le_bytes(r[..2].try_into().unwrap())as usize;(time-1)/super::shared_step::SCHEDULE_BLOCK==block&&(time-1)%4==j}).collect();assert!(!rows.is_empty());
        ...
        for pattern in 0..6{for batch in 0..rows.len().div_ceil(64){
            ...
            if super::q794_t10_fused::c1_prefix::checking(){prefix_active+=super::q794_t10_fused::c1_prefix::check(&before,&b.ops);}
            if super::q794_t10_fused::mask_one::checking(){let counts=super::q794_t10_fused::mask_one::check(&before,&b.ops);for i in 0..2{known_mask_counts[i]+=counts[i];}}
            if super::q794_t10_fused::rank_prefix::checking(){rank_prefix_active+=super::q794_t10_fused::rank_prefix::check(&before,&b.ops);}
    ...
    if super::q794_t10_fused::c1_prefix::checking(){assert_eq!(prefix_active,c1_count,"actual C1 guard matches scalar corpus");assert!(prefix_active>0);eprintln!("Q794_T10_C1_PREFIX_PASS entry_lanes={total} active={prefix_active} branch_variants=4; actual bank-zero entry, both decision values, arbitrary offguard bank, old/new allwire, exact phase, inverse");}
    if super::q794_t10_fused::mask_one::checking(){assert_eq!(known_mask_counts,[c1_count,active-c1_count]);assert!(c1_count>0&&active>c1_count);eprintln!("Q794_T10_KNOWN_MASK_PASS entry_lanes={total} c1={} general={} decisions=2; actual mask-zero/A-support entry, paired reference/allwire/phase/inverse, all clocks and low seam",known_mask_counts[0],known_mask_counts[1]);}
    if super::q794_t10_fused::rank_prefix::checking(){assert_eq!(rank_prefix_active,active-c1_count);assert!(rank_prefix_active>0);eprintln!("Q794_T10_RANK_PREFIX_PASS entry_lanes={total} active_general={rank_prefix_active} branch_variants=2; actual old/new general allwire, rank/A restore, both decision values, phase and inverse");}
