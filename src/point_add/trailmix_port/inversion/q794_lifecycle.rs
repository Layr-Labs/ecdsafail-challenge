...
fn template(block:usize,j:usize)->Vec<Op> {
    ...
    assert!(dual_phase());super::q794_step::step(&mut circ,&rank,&a,&c,&sm,&p1,&p2,&iter,&w1,&w2,&helpers,j,block);assert_eq!(circ.b.next_qubit,566);
...
fn remap(mut ops:Vec<Op>,core_ids:&[usize],passenger:&[QReg],inverse:bool)->Vec<Op> {
    ...
    let mut unique:Vec<_>=mapping.iter().copied().filter(|&q|q!=u32::MAX as usize).collect();assert_eq!(unique.len(),564);unique.sort_unstable();unique.dedup();assert_eq!(unique.len(),564);
