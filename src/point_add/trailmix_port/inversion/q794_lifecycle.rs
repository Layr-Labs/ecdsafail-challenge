...
// First11 lanes retain equality prefixes; final lane caches g*decision.
const PREFIX_QUBITS:usize=12;
const HALF_PLUS_ONE_LE:[u8;33]={let mut b=[0xff;33];b[0]=0x18;b[1]=0xfe;b[3]=0x7f;b[31]=0x7f;b[32]=0;b};
struct Core {rank:Vec<QReg>,a:Vec<QReg>,c:Vec<QReg>,sm:Vec<QReg>,phase1:QReg,phase2:QReg,iteration:QReg,work1:Vec<QReg>,work2:Vec<QReg>,scratch:QReg}
...
fn ids(core:&Core)->Vec<usize> {
    ...
    out.extend(core.work1.iter().chain(&core.work2).map(|q|q.id()as usize));assert_eq!(out.len(),542);out.push(core.scratch.id()as usize);out
}
...
fn initialize(circ:&mut Circuit,mut dx:Vec<QReg>,phase_passenger:&QReg,second_passenger:&QReg)->Core {
    ...
    Core {rank:circ.alloc_qreg_bits("rank5.rank",5),a:circ.alloc_qreg_bits("rank5.a",6),c:circ.alloc_qreg_bits("rank5.c",6),sm:circ.alloc_qreg_bits("rank5.sm",4),phase1,phase2,iteration,work1,work2:dx,scratch:circ.alloc_qreg("q795.clean-scratch")}
}
fn release_terminal(circ:&mut Circuit,mut core:Core)->Terminal {
    ...
    circ.zero_and_free(core.scratch);
...
fn rebuild_terminal(circ:&mut Circuit,mut terminal:Terminal,phase_passenger:&QReg,second_passenger:&QReg)->Core {
    ...
    Core {rank,a,c:terminal.history,sm,phase1,phase2,iteration:terminal.iteration,work1,work2:terminal.work2,scratch:circ.alloc_qreg("q795.clean-scratch.rebuilt")}
}
fn finish(circ:&mut Circuit,mut core:Core)->Vec<QReg> {
    ...
    circ.zero_and_free(core.scratch);toggle_initial_work1(circ,&core.work1);free_work1(circ,core.work1);free_clean(circ,core.rank);free_clean(circ,core.a);free_clean(circ,core.c);free_clean(circ,core.sm);
...
fn template(block:usize,j:usize)->Vec<Op> {
    template_inner(block,j,false)
}
fn template_with_prefix(block:usize,j:usize)->Vec<Op> {
    template_inner(block,j,true)
}
fn template_inner(block:usize,j:usize,prefix_enabled:bool)->Vec<Op> {
    ...
    let scratch=circ.alloc_qreg("clean_scratch");assert_eq!(scratch.id(),565);let _scratch_guard=super::length_recompute::CleanScratchGuard::set(&scratch);
    let prefix=if prefix_enabled{circ.alloc_qreg_bits("q807.logical-scan-bank",PREFIX_QUBITS)}else{Vec::new()};
    let scope=prefix_enabled.then(||super::q794_r01::numeric::prefix_cache::Scope::set(&prefix[..11]));
    let consumer_scope=prefix_enabled.then(||{assert_eq!(prefix[11].id(),577);super::q794_r01::numeric::consumer_cache::Scope::set(&prefix[11])});
    assert!(dual_phase());super::q794_step::step(&mut circ,&rank,&a,&c,&sm,&p1,&p2,&iter,&w1,&w2,&helpers,j,block);
    drop(consumer_scope);drop(scope);assert_eq!(circ.b.next_qubit,566+prefix.len()as u32);
...
pub fn sprint_census(){
 ...
 for block in 0..super::shared_step::SCHEDULE_BLOCKS {if only.is_some_and(|b|b!=block){continue;}
  for j in 0..4 {let ops=template(block,j);let t=ops.iter().filter(|o|o.kind==OperationType::CCX).count();let copies=super::shared_step::SCHEDULE_BLOCK/4;total_ops+=copies*ops.len();total_t+=copies*t;eprintln!("Q794_TEMPLATE block={block} j={j} ops={} T={t} copies={copies}",ops.len());if sample{compressed+=copies as u64*compressed_sample(&ops);}}
 }
...
fn remap(mut ops:Vec<Op>,core_ids:&[usize],passenger:&[QReg],inverse:bool)->Vec<Op> {
    remap_with_prefix(std::mem::take(&mut ops),core_ids,passenger,inverse,&[])
}
fn remap_with_prefix(mut ops:Vec<Op>,core_ids:&[usize],passenger:&[QReg],inverse:bool,prefix:&[QReg])->Vec<Op> {
    assert!(prefix.is_empty()||prefix.len()==PREFIX_QUBITS);
    assert_eq!(core_ids.len(),543);let first=if dual_phase(){assert_eq!(core_ids[21],passenger[1].id()as usize);2}else{1};assert!(passenger.len()>=first+23);assert_eq!(core_ids[22],passenger[0].id()as usize);let mut mapping=core_ids[..542].to_vec();mapping.extend(passenger[first..first+23].iter().map(|q|q.id()as usize));mapping.push(core_ids[542]);
    mapping.extend(prefix.iter().map(|q|q.id()as usize));assert_eq!(mapping.len(),566+prefix.len());
    assert_eq!(mapping.iter().filter(|&&q|q==u32::MAX as usize).count(),2);
    let mut unique:Vec<_>=mapping.iter().copied().filter(|&q|q!=u32::MAX as usize).collect();assert_eq!(unique.len(),564+prefix.len());unique.sort_unstable();unique.dedup();assert_eq!(unique.len(),564+prefix.len());
...
pub(crate) const CANDIDATE_OPS:usize=1_242_524_952;
pub(crate) const CANDIDATE_TOFFOLI:usize=779_005_524;
pub(crate) fn candidate_configuration()->bool{
    enabled()&&super::rank5_lifecycle::candidate_configuration()
        &&super::metadata_muxlease::active("Q794_R01_SEED_POLARITY")
        &&std::env::var("Q794_R01_NUMERIC").ok().as_deref()==Some("1")
        &&std::env::var("Q794_R01_NUMERIC_PARTIAL").ok().as_deref()==Some("1")
        &&std::env::var("Q794_SIGN_PREPARE_RIPPLE").ok().as_deref()==Some("1")
        &&std::env::var("Q794_SIGN_SUM_MAJ").ok().as_deref()==Some("1")
        &&std::env::var("Q794_SIGN_PREDICATES").ok().as_deref()==Some("1")
        &&std::env::var("Q794_SIGN_PREPARE_CLEAN").ok().as_deref()==Some("1")
        &&std::env::var("Q794_SIGN_SUM_PARITY").ok().as_deref()==Some("1")
        &&std::env::var("Q794_ENTRY_NUMERIC").ok().as_deref()==Some("1")
        &&std::env::var("Q794_R01_A7_PREFIX").ok().as_deref()==Some("1")
        &&std::env::var("Q794_COUNTER_REFLECTION").ok().as_deref()==Some("1")
        &&std::env::var("Q794_T10_NUMERIC_QUOTIENT").ok().as_deref()==Some("1")
        &&std::env::var("Q794_T10_C1_DECISION").ok().as_deref()==Some("1")
        &&std::env::var("Q794_T10_POLISH").ok().as_deref()==Some("1")
        &&std::env::var("Q794_R01_ENDPOINT_FACTOR").ok().as_deref()==Some("1")
        &&std::env::var("Q794_R01_NUMERIC_ROUTE").ok().as_deref()==Some("2")
        &&std::env::var("Q794_R01_NUMERIC_SEED").ok().as_deref()==Some("1")
        &&super::metadata_muxlease::active("Q794_T10_C1_PREFIX")
        &&super::q794_r01_odd_cache::mode()==1
        &&super::metadata_muxlease::active("Q794_T10_KNOWN_MASK")
        &&std::env::var("Q794_R00_ENDPOINTS").ok().as_deref()==Some("1")
        &&super::metadata_muxlease::active("Q794_R00_S_DOMAIN")
        &&super::metadata_muxlease::active("Q794_T10_RANK_PREFIX")
        &&super::metadata_muxlease::active("Q794_AUPDATE_ACTIVE_AFFINE")
        &&super::metadata_muxlease::active("Q794_TRANSFER_ACTIVE_AFFINE")
        &&super::metadata_muxlease::active("Q794_AUPDATE_PREFIX_AFFINE")
        &&super::metadata_muxlease::active("Q794_TRANSFER_PREFIX_AFFINE")
        &&super::q794_mbu::forward()&&super::q794_mbu::reverse()
        &&super::metadata_muxlease::active("Q794_SIGN_RANK_FOLD")
        &&super::metadata_muxlease::active("Q794_R01_SUM_XOR")
        &&super::paper2607_eea::shared_length_enabled()&&super::paper2607_eea::step_optimizer_enabled()
        &&super::metadata_muxlease::active("Q794_R00_PARTIAL_MAJ")
        &&super::metadata_muxlease::active("Q794_SIGN_MAJ_HISTORY")
        &&super::metadata_muxlease::active("Q794_R01_S_AFFINE")
        &&["Q794_COUNTER_AFFINE","Q794_ENTRY_UNPACK_AFFINE","Q794_ENTRY_PACK_AFFINE","Q794_EXIT_UNPACK_AFFINE","Q794_EXIT_PACK_AFFINE"].iter().all(|s|super::metadata_muxlease::active(s))
        &&std::env::var_os("Q795_TRACE").is_none()
        &&std::env::var("Q794_EXPERIMENTAL").ok().as_deref()!=Some("1")
        &&["Q794_T10_COMBINED_LOAN","Q794_T10_PAIRED_LOW","Q794_T10_ENDPOINT_LOCAL","Q794_T10_ENDPOINT_CACHE","Q794_T10_CACHE_PAIRED","Q794_SIGN_COMBINED_TOP","Q794_SIGN_FACTOR_LOW","Q794_SIGN_PAIRED_LOW","Q794_R01_SEED_FACTOR","Q794_TFACTOR"].iter().all(|s|super::metadata_muxlease::active(s))
}
fn emit_schedule(circ:&mut Circuit,core:&Core,passenger:&[QReg],inverse:bool) {
    ...
    // Dedicated clean state, never a passenger or an existing borrowed rail.
    // Each complete template restores the bank before the next template.
    let live_before=circ.b.active_qubits;
    let prefix=circ.alloc_qreg_bits("q807.schedule-scan-bank",PREFIX_QUBITS);
    assert_eq!(circ.b.active_qubits,live_before+PREFIX_QUBITS as u32);
    let mbu=if inverse{super::q794_mbu::reverse()}else{super::q794_mbu::forward()};
    let measurement=if mbu{Some(circ.b.alloc_bit())}else{None};
    for z in 0..super::shared_step::SCHEDULE_BLOCKS {let block=if inverse{super::shared_step::SCHEDULE_BLOCKS-1-z}else{z};let mut cached=Vec::new();
        for j in 0..4 {let ops=if let Some(bit)=measurement{remap_with_prefix(super::q794_mbu::template_with_prefix(block,j,inverse,bit),&mapping,passenger,false,&prefix)}else{remap_with_prefix(template_with_prefix(block,j),&mapping,passenger,inverse,&prefix)};let mut kinds=[0usize;18];let mut ct=0u64;for op in &ops{kinds[op.kind as usize]+=1;if matches!(op.kind,OperationType::CCX|OperationType::CCZ)&&op.c_condition!=crate::circuit::NO_BIT{assert_eq!(Some(op.c_condition),measurement);ct+=1;}}cached.push((std::sync::Arc::new(ops),kinds,ct));}
        let first=block*super::shared_step::SCHEDULE_BLOCK;let end=(first+super::shared_step::SCHEDULE_BLOCK).min(1616);
        for i in 0..end-first {let step=if inverse{end-1-i}else{first+i};let (ops,kinds,ct)=&cached[(step+1)%4];
            if mbu{super::q794_mbu::note(*ct);}
            if circ.b.compact_blocks.is_some(){assert!(circ.b.ops.is_empty());circ.b.compact_blocks.as_mut().unwrap().push(ops.clone());for kind in super::q794_mbu::KINDS{circ.b.add_counted_kind(kind,kinds[kind as usize]);}}
            else if circ.b.count_only&&circ.b.fiat_hash.is_none(){if let Some(check)=&mut circ.b.sprint_sim{check.apply(ops);}for kind in super::q794_mbu::KINDS{circ.b.add_counted_kind(kind,kinds[kind as usize]);}}
            else{for &op in ops.iter(){circ.b.push_op(op);}}
        }
    }
    circ.b.flush_compact_block();
    for lane in prefix{circ.zero_and_free(lane);}
    assert_eq!(circ.b.active_qubits,live_before);
}
