...
/// A16+A12 consumer-window leaf cut (research-lane packet; default ON, =0 kills).
/// At the block-b T10 consumer the proved bound M=A+C<=4b+3 caps the tree
/// address, and the analytic A support floors it. circ.q797_a_support carries
/// the block's (A_lo,A_hi); cap=min(2*A_hi-7,255)==min(4b+3,255) where it
/// bites. Dropped leaves are off the live address, where the routing unitary
/// cancels with its literal inverse, so the cut is an exact identity there.
