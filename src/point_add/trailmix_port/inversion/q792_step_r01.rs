...
pub(super) fn step(c:&mut Circuit,m:&[QReg],p1:&QReg,p2:&QReg,it:&QReg,w1:&[QReg],w2:&[QReg],h:&[QReg],post_j:usize,block:usize){
    super::q792_shared_capture_r01::begin();
    ...
    let bank:Vec<_>=(0..256).map(|a|if a==255{&w2[258]}else{&w1[a+1]}).collect();// The complete traversal acquired this A-pad loan once. sign starts zero;
    // the unrelated passenger remains in the A-addressed donor between steps.
    if full || super::q792_t_unified_selection_r01::CHOOSE_UNIFIED[block][post_j]{super::q792_t_only_full_r04::emit_with_loan(c,m,p1,p2,w1,w2,h,tend,entry_j,true);}else if super::q792_t_only_selection_r01::CHOOSE_NEW[block][post_j]{super::q792_t_only_full_r03::emit_with_loan(c,m,p1,p2,w1,w2,h,tend,entry_j,true);}else{super::q792_t10_phaselease_r01::emit_with_loan(c,m,p1,p2,w1,w2,h,tend,entry_j,true);}mark(c,"T10");
    ...
    super::q792_terminal_r01::emit(c,m,dirty,post_j==0,false);super::q792_counter_sign_loan_r02::begin(c,p1,p2,sign);super::q792_fold20_counter_r02::emit(c,m,p1,p2,dirty,entry_j);super::q792_counter_sign_loan_r02::end();mark(c,"counter");
    ...
    // Retain the clean sign and parked passenger for the next step.mark(c,"final");
    super::q792_shared_capture_r01::end();
    ...
    if std::env::var_os("Q795_TRACE").is_none(){let mut tail=c.b.ops.split_off(start);super::shared_optimize::cancel_nct(&mut tail,2048,8);super::shared_optimize::cancel_nct_live(&mut tail,2048);super::q792_trace_cancel_r01::reduce(&mut tail);let old_t=tail.iter().filter(|o|o.kind==crate::circuit::OperationType::CCX).count();let old_n=tail.len();super::q793_nct_frame_r03::apply(&mut tail);let star=super::q792_star_factor_r01::apply(&mut tail).0;super::q792_trace_cancel_r01::reduce(&mut tail);super::q793_nct_frame_r03::apply(&mut tail);let affine_saved=super::q792_affine_memory_r01::apply(&mut tail);super::q792_trace_cancel_r01::reduce(&mut tail);eprintln!("Q792_AFFINE_MEMORY saved_T={affine_saved}");let new_t=tail.iter().filter(|o|o.kind==crate::circuit::OperationType::CCX).count();assert!(new_t<=old_t&&tail.len()<=old_n+old_t);eprintln!("Q792_WHOLE_FACTOR old_T={old_t} new_T={new_t} star_saved={star} old_N={old_n} new_N={}",tail.len());let(library_saved,library_count)=super::q792_proved_library_r01::apply(&mut tail);eprintln!("Q792_PROVED_LIBRARY saved_T={library_saved} occurrences={library_count} unconditional=true");let(incremental_saved,incremental_count)=super::q792_incremental_library_r01::apply(&mut tail);eprintln!("Q792_INCREMENTAL_LIBRARY saved_T={incremental_saved} occurrences={incremental_count} unconditional=true");c.b.ops.extend(tail);}c.q797_a_support=oldsupport;
}
