...
pub(super) fn step(c:&mut Circuit,m:&[QReg],p1:&QReg,p2:&QReg,it:&QReg,w1:&[QReg],w2:&[QReg],h:&[QReg],post_j:usize,block:usize){
    ...
    let bank:Vec<_>=(0..256).map(|a|if a==255{&w2[258]}else{&w1[a+1]}).collect();super::q792_fold20_address_r01::exchange(c,m,sign,&bank,dirty);
    ...
    super::q792_fold20_address_r01::exchange(c,m,sign,&bank,dirty);mark(c,"final");
