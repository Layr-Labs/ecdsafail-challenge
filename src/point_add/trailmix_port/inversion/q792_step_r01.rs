...
pub(super) fn step(c:&mut Circuit,m:&[QReg],p1:&QReg,p2:&QReg,it:&QReg,w1:&[QReg],w2:&[QReg],h:&[QReg],post_j:usize,block:usize){
    ...
    if std::env::var_os("Q795_TRACE").is_none(){let mut tail=c.b.ops.split_off(start);super::shared_optimize::cancel_nct(&mut tail,2048,8);super::shared_optimize::cancel_nct_live(&mut tail,2048);super::q792_trace_cancel_r01::reduce(&mut tail);let old_t=tail.iter().filter(|o|o.kind==crate::circuit::OperationType::CCX).count();let old_n=tail.len();super::q793_nct_frame_r03::apply(&mut tail);let star=super::q792_star_factor_r01::apply(&mut tail).0;super::q792_trace_cancel_r01::reduce(&mut tail);super::q793_nct_frame_r03::apply(&mut tail);let new_t=tail.iter().filter(|o|o.kind==crate::circuit::OperationType::CCX).count();assert!(new_t<=old_t&&tail.len()<=old_n+old_t);eprintln!("Q792_WHOLE_FACTOR old_T={old_t} new_T={new_t} star_saved={star} old_N={old_n} new_N={}",tail.len());c.b.ops.extend(tail);}c.q797_a_support=oldsupport;
}
