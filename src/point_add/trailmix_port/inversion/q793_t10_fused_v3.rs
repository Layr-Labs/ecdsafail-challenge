...
struct Range<'a>{rank:&'a[QReg],a:&'a[QReg],g:&'a QReg,cache:&'a QReg,mask:&'a QReg,dirty:&'a[QReg],group:isize,prefix:Option<Prefix<'a>>}
/// A19-sm0 prefix rail state: the live level-2 key (bit,polarity) parked on d.
struct Prefix<'a>{rail:&'a QReg,key:Vec<(usize,bool)>,live:bool}
/// A19-sm0 (W47 research-lane packet; default ON, =0 kills): on non-last low
/// branches of cache-using blocks, hold the level-2 mask-ladder prefix
/// cache&[a_hi==v_hi] on the idle v12 carry sm[0] (zero on g between
/// pop_and_mask and mask_return); the leaf becomes d&[a_lo==v_lo]. Every
/// prefix/leaf op sits inside the recorded per-cell updates, so the literal
/// reverse replay unwinds the rail to its entry value before mask_return.
fn new_prefix<'a>(circ:&Circuit,sm:&'a[QReg],c1:bool,low:bool,helpers:&[QReg])->Option<Prefix<'a>>{
    if c1||!low||std::env::var("Q793_A19_SM0").ok().as_deref()==Some("0"){return None;}
    let (lo,hi)=circ.q797_a_support?;
    if lo/64==(hi-1)/64{return None;}
    assert!(helpers.iter().all(|q|q.id()!=sm[0].id()));
    Some(Prefix{rail:&sm[0],key:Vec::new(),live:false})
}
impl Range<'_>{
    fn emit_toggle(&self,circ:&mut Circuit,cs:&[(&QReg,bool)],target:&QReg){
        if super::metadata_muxlease::active("Q795_T10_TOP_MASK_CLEAN") {
            circ.x(self.g);super::paired_clean_mcx::toggle(circ,cs,target,self.g);circ.x(self.g);
        } else {let mut full=vec![(self.g,true)];full.extend_from_slice(cs);mixed_mcx(circ,&full,target,self.dirty);}
    }
    ...
    fn equality(&mut self,circ:&mut Circuit,value:usize){
        ...
        if factor&&lo/64!=(hi-1)/64&&self.prefix.is_some(){
            // A19-sm0 level-2 ladder: d holds cache&[a_hi==v_hi] over the
            // discriminating bits above the lowest three; refreshed whenever
            // the group or the high part changes (off before the cache group
            // switch, on after it). Leaf is mask ^= d&[a_lo==v_lo].
            let mut prefix=self.prefix.take().unwrap();
            let left=lo.max(h*64);let right=hi.min((h+1)*64);
            let disc:Vec<usize>=(0..6).filter(|&i|(left>>i)!=(right-1)>>i).collect();
            let split=disc.len().min(3);
            let key:Vec<(usize,bool)>=disc[split..].iter().map(|&b|(b,value>>b&1!=0)).collect();
            if self.group!=h as isize||prefix.key!=key{
                if prefix.live{let cs:Vec<_>=std::iter::once((self.cache,true)).chain(prefix.key.iter().map(|&(b,v)|(&self.a[b],v))).collect();self.emit_toggle(circ,&cs,prefix.rail);}
                if self.group!=h as isize{self.high(circ,self.group);self.high(circ,h as isize);self.group=h as isize;}
                let cs:Vec<_>=std::iter::once((self.cache,true)).chain(key.iter().map(|&(b,v)|(&self.a[b],v))).collect();self.emit_toggle(circ,&cs,prefix.rail);
                prefix.key=key;prefix.live=true;
            }
            let cs:Vec<_>=std::iter::once((prefix.rail,true)).chain(disc[..split].iter().map(|&b|(&self.a[b],value>>b&1!=0))).collect();
            self.emit_toggle(circ,&cs,self.mask);
...
pub(super) fn add_and_clear(circ:&mut Circuit,rank:&[QReg],source:&[QReg],target:&[QReg],a:&[QReg],g:&QReg,cache:&QReg,mask:&QReg,p1:&QReg,helpers:&[QReg],n:usize,c:&[QReg],sm:&[QReg],j:usize,c1:bool,low:bool){
    ...
    let mut range=Range{rank,a,g,cache,mask,dirty,group:-1,prefix:new_prefix(circ,sm,c1,low,helpers)};let mut updates=Vec::new();
    for i in 0..n{
