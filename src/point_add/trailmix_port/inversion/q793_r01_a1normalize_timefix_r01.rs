...
pub(super) fn q0_xor(circ:&mut Circuit,rank:&[QReg],c:&[QReg],w1:&[QReg],base:&[(&QReg,bool)],out:&QReg,dirty:&[QReg],support_end:usize){
    // A6b slack-4: A=1 on this branch so C >= rfirst-4. Do not enable A5 upper cap.
    let rfirst=259usize.saturating_sub(support_end);
    let lowcut=rfirst.saturating_sub(4);
    let start=circ.b.ops.len();let mut nodes:Vec<_>=(0..256).map(|cv|if cv>=lowcut&&cv<=252{Some(&w1[cv+2])}else{None}).collect();
...
pub(super) fn normalize(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],g:&QReg,mask:&QReg,w1:&[QReg],w2:&[QReg],dirty:&[QReg],j:usize,inverse:bool,support_end:usize){
    ...
    q0_xor(circ,rank,c,w1,&base,&w2[256],dirty,support_end);
