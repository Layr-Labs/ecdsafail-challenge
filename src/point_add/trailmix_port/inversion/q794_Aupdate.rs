...
fn permute_for_update(circ:&mut Circuit,rank:&[QReg],guard:&QReg,helpers:&[QReg],inverse:bool) {
    ...
    let start=circ.b.ops.len();for &(left,right) in programs::SWAPS{
        if super::metadata_muxlease::active("Q794_AUPDATE_ACTIVE_AFFINE"){
            super::q794_exit_frames::unguarded_swap(circ,rank,helpers,left,right);continue;
        }
    ...
    let update=|circ:&mut Circuit,i:usize| {
        ...
        if cache_bits==0{
            ...
            let key=value>>(8-cache_bits);if cache_key.get()!=Some(key){
                if let Some(old)=cache_key.get().filter(|&old|super::metadata_muxlease::active("Q794_AUPDATE_PREFIX_AFFINE")||super::metadata_muxlease::active("Q795_AUPDATE_PREFIX_TRANSITION")&&(old^key).is_power_of_two()){
                    if super::metadata_muxlease::active("Q794_AUPDATE_PREFIX_AFFINE"){
                        super::q794_exit_prefix::transition(circ,&boundary[8-cache_bits..],old,key,scratch,None,helpers);
                    }else{
