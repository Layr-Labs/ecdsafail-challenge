...
/// Clear a known comparison flag by X-basis measurement and its exact
/// comparison phase oracle. Both operand registers are restored.
pub(crate) fn clear_borrow_compare_refs(
    circ: &mut Circuit, v: &[&QReg], u: &[&QReg], out: &QReg,
) {
    if std::env::var("MIDQ_MEASURE_COMPARE").ok().as_deref() != Some("1") {
        borrow_compare_refs(circ, v, u, out);
        return;
    }
    let n = v.len();
    assert_eq!(u.len(), n);
    ...
    circ.with_condition(phase, |circ| {
        let section = circ.push_section("p.cmp.phase");
        for q in v { circ.x(q); }
        let carry = circ.alloc_qreg("bc.phase.c");
        circ.cx(v[0], u[0]);
        circ.cx(v[0], &carry);
        ...
        // At the MAJ-chain endpoint v[n-1] is exactly [v_original<u].
        circ.z(v[n - 1]);
        for i in (1..n).rev() {
            circ.ccx(v[i - 1], u[i], v[i]);
            circ.cx(v[i], v[i - 1]);
...
pub(crate) fn measured_compare_selftest() {
    ...
    for n in 1..=8 {
        ...
        let ids: Vec<_> = a.iter().chain(b.iter()).chain([&witness])
        ...
        seed.update(b"midq-measured-compare-test-v1");
        ...
        for first in (0..1usize << ids.len()).step_by(64) {
            ...
            assert_eq!(sim.phase & mask, 0, "comparison phase error n={n}");
            for shot in 0..valid {
                let initial = first + shot;
                let a_value = initial & ((1 << n) - 1);
                let b_value = (initial >> n) & ((1 << n) - 1);
                let expected = initial ^ (usize::from(a_value < b_value) << (2 * n));
                for (index, &id) in ids.iter().enumerate() {
                    assert_eq!((sim.qubit(id) >> shot) & 1, ((expected >> index) & 1) as u64);
                }
                checked += 1;
            }
            for &id in &ids { *sim.qubit_mut(id) = 0; }
            assert!(sim.qubits.iter().all(|&q| q & mask == 0), "dirty comparison scratch");
        }
    }
    eprintln!("MIDQ_MEASURE_COMPARE_SELFTEST PASS: {checked} exhaustive inputs, value/phase/ancilla");
}
