...
pub(super) fn branch(c:&mut Circuit,m:&[QReg],p1:&QReg,p2:&QReg,w1:&[QReg],w2:&[QReg],h:&[QReg],n:usize,j:usize,last:bool,low:bool){
 impl_branch(c,m,p1,p2,w1,w2,h,n,j,last,low,false);
}
pub(super) fn unified(c:&mut Circuit,m:&[QReg],p1:&QReg,p2:&QReg,w1:&[QReg],w2:&[QReg],h:&[QReg],n:usize,j:usize){impl_branch(c,m,p1,p2,w1,w2,h,n,j,true,true,true);}
fn impl_branch(c:&mut Circuit,m:&[QReg],p1:&QReg,p2:&QReg,w1:&[QReg],w2:&[QReg],h:&[QReg],n:usize,j:usize,last:bool,low:bool,unified:bool){
 ...
 super::q792_rank4_lower_r01::begin_capture();if unified{super::q792_t10_c1_unified_r01::branch(&mut v,&r,&a,&cc,&sm,&vp1,&vp2,&vw1,&vw2,&vh,n,j);}else{super::q793_t10_full_v13::branch(&mut v,&r,&a,&cc,&sm,&vp1,&vp2,&vw1,&vw2,&vh,n,j,last,low);}super::q792_rank4_lower_r01::end_capture();
