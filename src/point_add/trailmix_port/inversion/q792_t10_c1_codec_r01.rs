//! Unified C1 codec with live consumed quotient, reversible on off-domain data.
...
#[path="q792_c1_codec_plans_r01.rs"] mod codec_plans;
use codec_plans::POLICIES;
...
fn gates(circ:&mut Circuit,p:Poly<'_>,target:&QReg,dirty:&[QReg],g:&QReg,scratch:&QReg,metadata:&[u32],cache:&QReg,clean:&QReg){
    ...
    for(_, (meta,data))in groups{let data=tidy(data);let data=if super::q792_esop_r01::active(){super::q792_poly_r01::terms(data,1)}else{data};let mut others=Vec::new();let mut has_g=false;for&(q,v)in &meta{if q.id()==g.id(){assert!(v);has_g=true;}else{others.push((q,v));}}assert!(has_g);
        // Diagnostic census only: no change to emitted operations.
        static CENSUS:std::sync::OnceLock<bool>=std::sync::OnceLock::new();
        if *CENSUS.get_or_init(||std::env::var("LOWQ_Q793_NATIVE_MODE").ok().as_deref()==Some("fold20-c1-codec-census")){
            thread_local!{static SEEN:std::cell::RefCell<std::collections::BTreeSet<Vec<usize>>>=Default::default();}
            let mut widths:Vec<_>=data.iter().map(Vec::len).collect();widths.sort_unstable();let mut key=vec![others.len()];key.extend(&widths);
            SEEN.with(|s|{if s.borrow_mut().insert(key){eprintln!("C1_CODEC_CENSUS meta={} widths={:?}",others.len(),widths);}});
        }
        // C4/C5 are both zero on the OUTER C1 guard, including low-S=0.
        // Every codec call restores both rails. Off the outer guard this
        // reversible U is paired with its literal inverse around identity
        // arithmetic; live C0 changes only inside that arithmetic guard.
        let mut widths:Vec<_>=data.iter().map(Vec::len).collect();widths.sort_unstable();
        if let Some(&(_,_,policy))=POLICIES.iter().find(|(m,ds,_)|*m==others.len()&&*ds==widths.as_slice()){
            assert_ne!(cache.id(),clean.id());
            assert!([cache,clean].iter().all(|r|r.id()!=target.id()&&meta.iter().chain(data.iter().flatten()).all(|(q,_)|q.id()!=r.id())));
            if policy==1{
                super::paired_clean_mcx::toggle(circ,&meta,cache,clean);
                for term in &data{let mut cs=vec![(cache,true)];cs.extend(term);super::paired_clean_mcx::toggle(circ,&cs,target,clean);}
                super::paired_clean_mcx::toggle(circ,&meta,cache,clean);
            }else{assert_eq!(policy,2);for term in &data{let mut cs=meta.clone();cs.extend(term);super::paired_clean_mcx::toggle(circ,&cs,target,clean);}}
            continue;
        }
...
pub(super) fn flags(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],g:&QReg,m255:&QReg,m256:&QReg,dirty:&[QReg]){
    // C1 implies A<=254; M255 iff A254, and M256 is impossible.
    let _=(c,m256);
    for term in aeq_supported(circ,rank,a,254){let mut cs=vec![(g,true)];cs.extend(term);super::q794_t10_quotient::gate(circ,&cs,m255,dirty);}
    return;
...
pub(super) fn emit(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,g:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,inverse:bool,last:bool){
    assert!(last);assert!(helpers.len()>=22);let start=circ.b.ops.len();let h=&sm[3];let q=&helpers[..3];let dirty=&helpers[3..];let scratch=&sm[0];let metadata:Vec<_>=rank.iter().chain(a).chain(c).chain(&sm[1..3]).chain(std::iter::once(g)).map(QReg::id).collect();
    ...
    swap(circ,mul(&wire(g),&specs),h,&w2[255],dirty,g,scratch,&metadata,&c[4],&c[5]);
    ...
    if last{
        gates(circ,mul(&wire(g),&replace(wire(&w1[1]),aa0.clone(),false)),p1,dirty,g,scratch,&metadata,&c[4],&c[5]);
        gates(circ,mul(&wire(g),&replace(wire(&w1[2]),xor(aa0.clone(),aa1.clone()),false)),p2,dirty,g,scratch,&metadata,&c[4],&c[5]);
    }else{
        ...
        gates(circ,mul(&wire(g),&cc[2]),p2,dirty,g,scratch,&metadata,&c[4],&c[5]);
    ...
    let qq=if last{[wire(&c[0]),Vec::new(),Vec::new()]}else{[wire(p1),wire(p2),if j&1==1{Vec::new()}else{replace(replace(wire(&q[2]),cc[2].clone(),false),cc[3].clone(),true)}]};
    for shift in 0..=2{
        if j&1!=shift&1{continue;}
        let c0=((j>>1)^(j&1))^(shift>>1);if c0!=1{continue;}let sb=vec![vec![(g,true)]];
        ...
        for rwidth in [3usize,2,1]{
            ...
            gates(circ,mul(&even,&bb[2]),h,dirty,g,scratch,&metadata,&c[4],&c[5]);
            ...
            },&odd,h,dirty,g,scratch,&metadata,&c[4],&c[5]);
            let known_poly=if rwidth==1{one()}else if rwidth==2{wire(&c[0])}else{Vec::new()};let hp=xor(wire(h),mul(spec,&xor(wire(h),known_poly)));let all_even=mul(&base,&vec![vec![(&w1[0],false)]]);
            gates(circ,mul(&all_even,&hp),b2,dirty,g,scratch,&metadata,&c[4],&c[5]);
            ...
            anf(circ,&vars,|x|{let t=x&7;let r=x>>3&3;let v=x>>5&7;let qs=x>>8&3;(v.wrapping_mul(7usize.wrapping_sub(t*r)).wrapping_sub((qs<<shift)*t))>>2&1!=0},&all_even,b2,dirty,g,scratch,&metadata,&c[4],&c[5]);
        }
    }
