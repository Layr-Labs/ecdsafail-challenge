...
pub fn run(){
    ...
    for j in 0..4{
        ...
        let mut pc=Circuit::new();pc.b.count_only=false;pc.b.fiat_hash=None;pc.b.next_qubit=owned;
        super::q794_sign::measurement::emit(&mut pc,&rank,&a,&c,&sm,&p1,&p2,&sign,&w1,&w2,&dirty,j,258);
        assert_eq!(pc.b.next_qubit,owned);let phase_ops=pc.into_builder().ops;
        for pattern in 0..11{for batch in 0..rows.len().div_ceil(64){let mut rng=0xa7945100ce1u64^(j as u64)<<48^(pattern as u64)<<32^batch as u64;
            ...
            super::q794_mbu_native::check(&before,&b.ops,&phase_ops,crate::circuit::QubitId(sign.id()as u64));
    ...
    eprintln!("Q794_SIGN_GENERAL_MBU_PASS lanes={total} active={active}; actual phasebody/HMR bothmeasurements, pureoriginalSign, allwire/phase/dirty restoration; NOT fullcaller lifecycle");
}
