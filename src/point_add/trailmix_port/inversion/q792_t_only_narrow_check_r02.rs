...
pub fn run(){
    ...
    for block in 0..1{for j in 0..4{
        ...
        for pattern in 0..6{for batch in 0..rows.len().div_ceil(64){
            ...
            for lane in 0..64{
                ...
                let tag=phase==3&&bit(r,23)&&av==0&&cv==1&&sv==0;if tag{sv=256;}let on=phase==2&&av<=39;
                if on{
            ...
            if sim.qubits!=after{let diffs:Vec<_>=sim.qubits.iter().zip(&after).enumerate().filter(|(_, (x,y))|x!=y).map(|(i,(x,y))|(i,format!("{:016x}",x^y))).collect();panic!("T-only narrow block{block} j{j} pattern{pattern} batch{batch} diffs={diffs:?}");}
        ...
        if export.is_some(){eprintln!("PHASE_ROUTING_EXPORTED_CLOCK j={j} ops={} T={t}",b.ops.len());}else{eprintln!("T_ONLY_NARROW_R02_BLOCK block={block} j={j} ops={} T={t} PASS",b.ops.len());}
    }}
    ...
    eprintln!("T_ONLY_NARROW_R02_PASS lanes={total} active={active} C1={c1_count} M255={m255} M256={m256} shortA={short} A253+={extreme} terminal={terminal} C256={c256} weighted_forward_ops={weighted_ops} weighted_forward_T={weighted_t}; all saved geometries/all4passengerpairs+2random/packed3holes/dirty/phase/literalinverse; NOT fullstep or wholeQ793");
}
