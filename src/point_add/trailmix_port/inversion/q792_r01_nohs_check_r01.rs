...
pub(super) fn run(){
    ...
    for j in 0..4{
        ...
        super::signless_nohs(&mut circ,&rank,&a,&c,&sm,&p1,&p2,&w1,&w2,&dirty,j,259);assert_eq!(circ.b.next_qubit,owned);
        let builder=circ.into_builder();for op in &builder.ops{op.validate();assert!(matches!(op.kind,K::X|K::CX|K::CCX));}
        let nt=builder.ops.iter().filter(|op|op.kind==K::CCX).count();eprintln!("Q792_R01_NOHS_BUILT j={j} ops={} T={nt} interfaceQ={owned}",builder.ops.len());
        ...
        for pattern in 0..4{for batch in 0..rows.len().div_ceil(64){
            ...
            if sim.qubits!=after{let diffs:Vec<_>=sim.qubits.iter().zip(&after).enumerate().filter(|(_, (x,y))|x!=y).map(|(i,(x,y))|(i,format!("{:016x}",x^y))).collect();panic!("Q792_R01_NOHS active j={j} pattern={pattern} batch={batch} diffs={diffs:?}");}
        ...
        eprintln!("Q792_R01_NOHS_CLOCK_PASS j={j} active={active_here}");
    }
    ...
    eprintln!("Q792_R01_NOHS_PASS active={total} offguard={off}; actual rank5 ranges, A0/A1, two independent cargo bits, q1/q2 gather, three physical holes, packed width2/1 endpoints, normalized b2 HA restored, literal inverse/allphase; no whole-circuit claim");
}
