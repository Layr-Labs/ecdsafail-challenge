...
pub fn run() {
    ...
    for block in 0..26 {for post_j in 0..4 {
        ...
        let rows:Vec<_>=data[12..].chunks_exact(138).filter(|r|(u16::from_le_bytes(r[..2].try_into().unwrap())as usize-1)/super::shared_step::SCHEDULE_BLOCK==block && u16::from_le_bytes(r[..2].try_into().unwrap())as usize%4==post_j).collect();
