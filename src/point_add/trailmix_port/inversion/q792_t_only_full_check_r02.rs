...
pub fn run(){
    ...
    let support_hi=std::env::var("Q792_T_TEST_A_HI").ok().map(|v|v.parse::<usize>().unwrap()).unwrap_or(256);assert!((4..=256).contains(&support_hi));eprintln!("T_ONLY_FULL_R02_INTERFACE_SUPPORT A_hi={support_hi} M_max={}",(2*support_hi).saturating_sub(7));
    ...
    for block in 0..1{for j in 0..4{
        ...
        if export.is_some(){eprintln!("PHASE_ROUTING_EXPORTED_CLOCK j={j} ops={} T={t}",b.ops.len());}else{eprintln!("T_ONLY_FULL_R02_BLOCK block={block} j={j} ops={} T={t} PASS",b.ops.len());}
    }}
    ...
    eprintln!("T_ONLY_FULL_R02_PASS lanes={total} active={active} C1={c1_count} M255={m255} M256={m256} shortA={short} A253+={extreme} terminal={terminal} C256={c256} weighted_forward_ops={weighted_ops} weighted_forward_T={weighted_t}; all saved geometries/all4passengerpairs+2random/packed3holes/dirty/phase/literalinverse; NOT fullstep or wholeQ793");
}
