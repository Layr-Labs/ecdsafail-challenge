...
fn empty_q(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],w1:&[QReg],extra:&[(&QReg,bool)],out:&QReg,helpers:&[QReg],p1:&QReg) {
    if super::metadata_muxlease::active("Q795_ENTRY_FACTOR"){
        ...
        for &(m,v)in rank_programs::C_EQUAL[0]{let mut cs=extra.to_vec();cs.extend((0..5).filter(|&i|m>>i&1!=0).map(|i|(&rank[i],v>>i&1!=0)));cs.extend(c.iter().map(|q|(q,false)));mixed_mcx(circ,&cs,out,helpers);}
