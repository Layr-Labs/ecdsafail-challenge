...
fn static_contracts() {
    ...
    assert_eq!(
        ...
        SELECTED.rounds - match option_env!("HYBRID_CHECKPOINT_ROUNDS") { None | Some("4") => 4, Some("14") => 14, _ => panic!("invalid checkpoint test configuration") }
    );
    assert!(SELECTED.value_widths[super::super::midq_tail_checkpoint::START..].iter().all(|&w| w == 4));
