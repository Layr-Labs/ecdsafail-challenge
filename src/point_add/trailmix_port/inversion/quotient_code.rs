...
// Legacy fixture dimensions only. Production derives both dimensions below.
const Q_BITS: usize = 18;
const CODE_BITS: usize = 5;

pub(super) fn supports_width(width: usize) -> bool { (1..=257).contains(&width) }
pub(super) fn code_bits(width: usize) -> usize {
    assert!(supports_width(width)); usize::BITS as usize - width.leading_zeros() as usize
}
pub(super) fn legacy_rank_width(width: usize) -> bool { width == 18 }
...
pub(super) fn compress(c: &mut Circuit, ca: &[QReg], cb: &[QReg], q: &mut Vec<QReg>) -> Vec<QReg> {
    assert!(supports_width(q.len()), "quotient codec requires1..257 raw bits");
    assert_eq!(ca.len(), 257);
    assert_eq!(cb.len(), 257);
    let code = c.alloc_qreg_bits("qcode.k", code_bits(q.len()));
    xor_code(c, q, &code);
...
pub(super) fn restore(
    ...
    assert!(q.is_empty());
    assert_eq!(ca.len(),257);assert_eq!(cb.len(),257);
    assert_eq!(code.len(),code_bits(original_width));
    *q = c.alloc_qreg_bits("qcode.restored_q",original_width);
    xor_recovered(c, ca, cb, &code, q);
    xor_code(c, q, &code);
    for bit in code {
        c.zero_and_free(bit);
