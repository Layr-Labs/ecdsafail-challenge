...
// The codec is generic in the handoff quotient width; the code register only
// needs to hold the sentinel (= that width), so five bits cover widths < 32.
...
pub(super) fn compress(c: &mut Circuit, ca: &[QReg], cb: &[QReg], q: &mut Vec<QReg>) -> Vec<QReg> {
    assert!(
        (1..(1 << CODE_BITS)).contains(&q.len()),
        "quotient codec: handoff q width must be representable by the code register"
    );
...
pub(super) fn restore(
    ...
    assert!((1..(1 << CODE_BITS)).contains(&q_bits));
