...
fn paired_a_terms(circ:&mut Circuit,a:&[QReg],word:&[QReg],from:usize,to:usize,terms:&[Vec<(&QReg,bool)>],dirty:&[QReg],flip:bool) {
    ...
    for q in dirty {
        assert!(!word.iter().chain(a).any(|p|p.id()==q.id()));
...
pub(super) fn across_a_terms(circ:&mut Circuit,a:&[QReg],left_word:&[QReg],left_offset:usize,right_word:&[QReg],right_offset:usize,terms:&[Vec<(&QReg,bool)>],flip_left:bool,dirty:&[QReg]) {
    let(left,l)=gather_a(circ,a,left_word,left_offset,dirty);
    let(right,r)=gather_a(circ,a,right_word,right_offset,dirty);
    circ.cx(right,left);
...
pub(super) fn flip_a_terms(circ:&mut Circuit,a:&[QReg],word:&[QReg],offset:usize,terms:&[Vec<(&QReg,bool)>],dirty:&[QReg]) {
    let(root,ops)=gather_a(circ,a,word,offset,dirty);
    for term in terms{mixed_mcx(circ,term,root,dirty);}circ.b.ops.extend(ops.into_iter().rev());
}
pub(super) fn exchange_a_terms(circ:&mut Circuit,a:&[QReg],word:&[QReg],offset:usize,passenger:&QReg,terms:&[Vec<(&QReg,bool)>],dirty:&[QReg]) {
    exchange_a_terms_inner(circ,a,word,offset,passenger,terms,dirty,false);
}
fn exchange_a_terms_inner(circ:&mut Circuit,a:&[QReg],word:&[QReg],offset:usize,passenger:&QReg,terms:&[Vec<(&QReg,bool)>],dirty:&[QReg],flip_root:bool) {
    let(root,ops)=gather_a(circ,a,word,offset,dirty);
...
pub(super) fn adjacent_a_terms_flip(circ:&mut Circuit,a:&[QReg],word:&[QReg],from:usize,to:usize,terms:&[Vec<(&QReg,bool)>],helpers:&[QReg]) {
    if super::metadata_muxlease::active("Q795_CARGO_PAIR")&&!super::q792_passive_r01::active(){paired_a_terms(circ,a,word,from,to,terms,helpers,true);return;}
    assert_ne!(from,to);let shuttle=&helpers[0];let dirty=&helpers[1..];
    exchange_a_terms(circ,a,word,from,shuttle,terms,dirty);
    exchange_a_terms(circ,a,word,to,shuttle,terms,dirty);
    exchange_a_terms_inner(circ,a,word,from,shuttle,terms,dirty,true);
}
pub(super) fn adjacent_a_terms(circ:&mut Circuit,a:&[QReg],word:&[QReg],from:usize,to:usize,terms:&[Vec<(&QReg,bool)>],helpers:&[QReg]) {
    if super::metadata_muxlease::active("Q795_CARGO_PAIR")&&!super::q792_passive_r01::active(){paired_a_terms(circ,a,word,from,to,terms,helpers,false);return;}
    assert_ne!(from,to);let shuttle=&helpers[0];let dirty=&helpers[1..];
    exchange_a_terms(circ,a,word,from,shuttle,terms,dirty);
    exchange_a_terms(circ,a,word,to,shuttle,terms,dirty);
    exchange_a_terms(circ,a,word,from,shuttle,terms,dirty);
}
pub(super) fn exchange_a(circ:&mut Circuit,a:&[QReg],word:&[QReg],offset:usize,passenger:&QReg,controls:&[(&QReg,bool)],dirty:&[QReg]) {
    let(root,ops)=gather_a(circ,a,word,offset,dirty);
...
pub(super) fn adjacent_a(circ:&mut Circuit,a:&[QReg],word:&[QReg],from:usize,to:usize,controls:&[(&QReg,bool)],helpers:&[QReg]) {
    if super::metadata_muxlease::active("Q795_CARGO_PAIR")&&!super::q792_passive_r01::active(){paired_a_terms(circ,a,word,from,to,&[controls.to_vec()],helpers,false);return;}
    assert_ne!(from,to);let shuttle=&helpers[0];let dirty=&helpers[1..];
    exchange_a(circ,a,word,from,shuttle,controls,dirty);
    exchange_a(circ,a,word,to,shuttle,controls,dirty);
    exchange_a(circ,a,word,from,shuttle,controls,dirty);
}


pub(super) fn gather_a<'a>(circ:&mut Circuit,a:&[QReg],word:&'a[QReg],offset:usize,dirty:&[QReg])->(&'a QReg,Vec<crate::circuit::Op>){
    ...
    for level in 0..8{let mut next=Vec::new();for pair in nodes.chunks_exact(2){next.push(match(pair[0],pair[1]){
        (Some(left),Some(right))=>{circ.cswap(&a[level],left,right);Some(left)},
        (Some(q),None)|(None,Some(q))=>Some(q),(None,None)=>None,
    });}nodes=next;}
    let root=nodes[0].expect("nonempty physical selector");let ops=circ.b.ops[start..].to_vec();(root,ops)
}
