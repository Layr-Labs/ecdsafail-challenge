...
pub(super) fn mark(circ:&Circuit,name:&'static str){
    ...
    if census||std::env::var_os("Q795_TRACE").is_some()||super::q794_mbu::capturing(){TRACE.with(|t|{
...
fn newborn(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p2:&QReg,sign:&QReg,w1:&[QReg],w2:&[QReg],dirty:&[QReg],j:usize){
    ...
    // Raw C+S address (top of T, as in sign_fold::gather_sum): values below 257-tend are off support.
    let min_value=super::length_recompute::t_top().map(|tend|257usize.saturating_sub(tend+2)).unwrap_or(0);
    let start=circ.b.ops.len();let mut nodes=vec![None;512];for value in 1..=257{if value>=min_value{nodes[value]=Some(&w2[259-value]);}}
...
pub(super) fn step(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,iteration:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],post_j:usize,block:usize){
    ...
    let (rfirst,tend)=super::shared_step::SCHEDULE_SUPPORTS[block];let _t_top_guard=super::length_recompute::TTopGuard::set(tend);let _r_first_guard=super::length_recompute::RFirstGuard::set(rfirst);let (lo,hi)=super::metadata_entry_head5::A_SUPPORTS[block];let previous=circ.q797_a_support.replace((lo,hi));
    ...
    mark(circ,"mbu_before_loan");old::loan(circ,rank,a,w1,sign,dirty);mark(circ,"mbu_after_loan");circ.ccx(p1,p2,sign);
    ...
    super::q794_sign::emit(circ,rank,a,c,sm,p1,p2,sign,w1,w2,dirty,post_j,(tend+1).min(258));mark(circ,"mbu_after_sign");old::hide_birth(circ,rank,a,c,sm,p1,p2,dirty,post_j);old::loan(circ,rank,a,w1,sign,dirty);
    ...
    if std::env::var_os("Q795_TRACE").is_none()&&!super::q794_mbu::capturing(){let mut tail=circ.b.ops.split_off(start);super::shared_optimize::cancel_nct(&mut tail,window,8);super::shared_optimize::cancel_nct_live(&mut tail,window);
        if std::env::var("Q794_TFACTOR").ok().as_deref()==Some("1") {
            let removed=super::q794_tfactor::apply(&mut tail,4096);
