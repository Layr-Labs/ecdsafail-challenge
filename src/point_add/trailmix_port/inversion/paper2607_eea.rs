...
const STREAM_CX_PER_TRAVERSAL: usize = 12_821_303;
// Includes the two emitted CCX gates for every clean-C3X MBU marker.
const STREAM_CCX_PER_TRAVERSAL: usize = 288_673_687;
...
fn gap_correction(outer: GapGate, middle: GapGate) -> Option<u64> {
    // The exhaustive 577,629-window source census contains only an X blocker
    // on one outer control. Refuse all broader identities rather than rely on
    // a less-specific gate histogram than the production count invariants.
    if middle.kind != 1 {
        return None;
    }
    ...
    // The complete authenticated all-shard frontier has only CX corrections:
    // two outer CCX operations disappear while exactly one CX is introduced.
    if count != 1 || controls[0] == target {
        return None;
    }
    Some(0x22 | ((controls[0] as u64) << 8) | ((target as u64) << 18))
}

fn gap_conjugations(records: &[u64], expected: usize) -> Vec<GapWindow> {
                ...
                && position - previous - 1 <= GAP_MAX_INTERVENING
            {
                    ...
                    && last_selected.is_none_or(|last| previous > last)
                {
                    ...
                    if blockers == 1 {
                        ...
                        if let Some(correction) = gap_correction(current, middle) {
...
fn emit_conjugation_chunk(
    ...
    let transformed = materialize_r034_chunk(records, expected_r034);
    let windows = gap_conjugations(&transformed, expected_gap);
    if reverse {
