...
pub fn divide_forward(
    ...
    mod_mul_rfold_mbu(circ, &lambda, &terminal.work2[..FIELD_WIDTH], &dy);
    toggle_inverse_sign(circ, &terminal);
    ...
    mod_mul_rfold_mbu(circ, &dy, &lambda, &dx);
...
pub fn divide_cancel(
    ...
    use crate::point_add::trailmix_port::arith::rfold_mbu::{
        mod_mul_rfold_mbu, mod_mul_rfold_mbu_undo,
    };
    ...
    mod_mul_rfold_mbu(circ, &quotient, &terminal.work2[..FIELD_WIDTH], &dy);
    for (ghost, lane) in lambda_ghosts.into_iter().zip(&quotient) {
        circ.resolve_ghost(ghost, lane);
    }
    mod_mul_rfold_mbu_undo(circ, &quotient, &terminal.work2[..FIELD_WIDTH], &dy);
