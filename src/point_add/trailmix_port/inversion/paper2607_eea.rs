...
const STREAM_CX_PER_TRAVERSAL: usize = 12_243_674;
// Includes the two emitted CCX gates for every clean-C3X MBU marker.
const STREAM_CCX_PER_TRAVERSAL: usize = 289_841_846;
const STREAM_HMR_PER_TRAVERSAL: usize = 0;
const STREAM_CZ_PER_TRAVERSAL: usize = 0;

// Each source-bound A B A window is an exact local conjugation B C.  A and C
// are CCX gates; B is CX; B and C share their target and commute.  Replacing
// the three pinned windows saves three Toffolis per complete EEA traversal.
const STREAM_CCX_CONJUGATIONS: [(usize, u64, u64, u64); 3] = [
    (
        456_940,
        0x0000_0021_1866_1033,
        0x0000_0000_086a_1122,
        0x0000_0021_a866_1033,
    ),
    (
        457_143,
        0x0000_0021_1866_1033,
    ...
    (
        461_620,
        0x0000_0022_d41c_0133,
        0x0000_0000_0012_2d22,
        0x0000_0000_441c_0133,
    ),
];
...
fn read_primitive(records: &[u8], index: usize) -> u64 {
    let start = index * 8;
    u64::from_le_bytes(
...
fn emit_conjugation_chunk(circ: &mut Circuit, wires: &[&QReg], records: &[u8], reverse: bool) {
    let record_count = records.len() / 8;
    assert_eq!(records.len() % 8, 0, "partial conjugation chunk record");

    if reverse {
        let mut previous = record_count;
        for &(index, outer, middle, correction) in STREAM_CCX_CONJUGATIONS.iter().rev() {
            assert!(index + 3 <= previous, "overlapping source-bound reverse windows");
            assert_eq!(read_primitive(records, index), outer, "conjugation first CCX drift");
            assert_eq!(read_primitive(records, index + 1), middle, "conjugation CX drift");
            assert_eq!(read_primitive(records, index + 2), outer, "conjugation last CCX drift");
            emit_primitive_slice(circ, wires, &records[(index + 3) * 8..previous * 8], true);
            emit_record(circ, wires, correction);
            emit_record(circ, wires, middle);
            previous = index;
        }
        emit_primitive_slice(circ, wires, &records[..previous * 8], true);
    } else {
        let mut previous = 0;
        for &(index, outer, middle, correction) in &STREAM_CCX_CONJUGATIONS {
            assert!(previous <= index && index + 3 <= record_count, "invalid source-bound window");
            assert_eq!(read_primitive(records, index), outer, "conjugation first CCX drift");
            assert_eq!(read_primitive(records, index + 1), middle, "conjugation CX drift");
            assert_eq!(read_primitive(records, index + 2), outer, "conjugation last CCX drift");
            emit_primitive_slice(circ, wires, &records[previous * 8..index * 8], false);
            emit_record(circ, wires, middle);
            emit_record(circ, wires, correction);
            previous = index + 3;
        }
        emit_primitive_slice(circ, wires, &records[previous * 8..], false);
    }
}
...
fn emit_forward(circ: &mut Circuit, core: &Core, passenger: &[QReg]) {
    ...
    for compressed in STREAM_CHUNKS {
        ...
        if start == 1 {
            emit_conjugation_chunk(circ, &wires, &decoded[24..], false);
        } else {
            emit_primitive_slice(circ, &wires, &decoded[24..], false);
        }
...
fn emit_reverse(circ: &mut Circuit, core: &Core, passenger: &[QReg]) {
    ...
    for compressed in STREAM_CHUNKS.iter().rev() {
        ...
        if start == 1 {
            emit_conjugation_chunk(circ, &wires, &decoded[24..], true);
        } else {
            emit_primitive_slice(circ, &wires, &decoded[24..], true);
        }
