...
const STREAM_X_PER_TRAVERSAL: usize = 26_878_536;
const STREAM_CX_PER_TRAVERSAL: usize = 26_688_440;
// Includes the two emitted CCX gates for every clean-C3X MBU marker.
const STREAM_CCX_PER_TRAVERSAL: usize = 40_328_305;
const STREAM_HMR_PER_TRAVERSAL: usize = 4_699_696;
const STREAM_CZ_PER_TRAVERSAL: usize = 4_699_696;
...
fn decode_chunk(compressed: &[u8]) -> Vec<u8> {
    ...
    assert_eq!(&decoded[..8], b"P26EEA3\0");
...
// INERT production/native integration draft; not compiled or inserted.
// The same function must be extracted into the future native capsule.
#[derive(Clone, Copy)]
enum PairedDirectionV1 { Forward, Reverse }

fn emit_record_paired_v1(
    circ: &mut Circuit,
    local: &[&QReg],
    word: u64,
    direction: PairedDirectionV1,
) {
    assert_eq!(local.len(), 577);
    ...
    let expected = match kind {
        ...
        3 | 8 | 9 => 3,
        7 => 5,
        _ => panic!("unknown paired primitive"),
    };
    assert_eq!(arity, expected);
    assert_eq!(word >> (8 + 10 * arity), 0, "paired record padding");
    let mut q = [0_usize; 5];
    for i in 0..arity {
        q[i] = ((word >> (8 + 10 * i)) & 0x3ff) as usize;
        assert!(q[i] < local.len());
        for j in 0..i { assert_ne!(q[i], q[j], "paired operand alias"); }
    }
    match kind {
        1 => circ.x(local[q[0]]),
        2 => circ.cx(local[q[0]], local[q[1]]),
        3 => circ.ccx(local[q[0]], local[q[1]], local[q[2]]),
        4 => circ.z(local[q[0]]),
        5 => circ.cz(local[q[0]], local[q[1]]),
        6 => circ.swap(local[q[0]], local[q[1]]),
        7 => {
            // Existing self-inverse clean-C3X macro: unchanged in either
            // logical direction. It has its own previously qualified promise.
            circ.ccx(local[q[0]], local[q[1]], local[q[4]]);
            circ.ccx(local[q[2]], local[q[4]], local[q[3]]);
            circ.clear_and(local[q[4]], local[q[0]], local[q[1]]);
        }
        8 | 9 => {
            let compute = matches!((kind, direction),
                (8, PairedDirectionV1::Forward) | (9, PairedDirectionV1::Reverse));
            if compute {
                circ.ccx(local[q[0]], local[q[1]], local[q[2]]);
            } else {
                // Exactly HMR(target) + CZ(controls) under that fresh bit.
                // clear_and's declare_and_of is a no-op, not a state proof.
                circ.clear_and(local[q[2]], local[q[0]], local[q[1]]);
            }
        }
        _ => unreachable!(),
    }
...
// REQUIRED integration, not an inverse placeholder:
// decode_chunk must accept ONLY new magic b"P26EEA3\0" for this active stream.
// emit_forward: chunks ascending, words ascending, PairedDirectionV1::Forward.
// emit_reverse: chunks descending, words descending, PairedDirectionV1::Reverse.
// The stored words are always the FORWARD tags; do not also transform kind8/9
// while reversing. A Qiskit-generated logical inverse already swapped tags;
// its native equivalence test must therefore use Forward, not Reverse.
// Every boundary/shard must have closed C/U lifetimes and equal C/U counts.
// Native testing must use these exact arms plus pinned production B passes,
// not a count-only surrogate, and preserve both actual output directions.
...
fn count_stub_enabled(_circ: &Circuit) -> bool {
    // Disabled until real paired streams and all-kind counts exist.
    false
}
