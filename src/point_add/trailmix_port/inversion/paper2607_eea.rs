...
const STREAM_CCX_PER_TRAVERSAL: usize = 289_828_945;
...
fn emit_conjugation_chunk(
    ...
    if reverse {
        let mut previous = records.len() / 8;
        for &(index, middle, correction) in windows.iter().rev() {
            assert!(index + 3 <= previous, "overlapping reverse conjugations");
        ...
        for &(index, middle, correction) in &windows {
            assert!(previous <= index, "overlapping forward conjugations");
