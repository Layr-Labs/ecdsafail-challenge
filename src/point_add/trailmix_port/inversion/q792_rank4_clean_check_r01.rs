...
use super::super::q792_fold20_rank_r01::triples;
...
pub(super) fn run(){
    std::env::set_var("Q793_R01_NUMERIC_SEED","0");
    ...
    for class in 0..2{for j in 0..4{
        ...
        super::begin_capture();super::super::q793_r01_normal_timefix_r01::emit(&mut circ,&rank,&a,&c,&sm,&g,&mask,&hs,&ha,&decision,&w1,&w2,&dirty,j,259,false);super::end_capture();
        assert_eq!(circ.b.next_qubit,nq);let ops=super::lower_clean(&circ.b.ops,nq as usize,&dirty.iter().map(|q|q.id()as usize).collect::<Vec<_>>(),class);for op in &ops{op.validate();assert!(matches!(op.kind,K::X|K::CX|K::CCX));}
        let nt=ops.iter().filter(|o|o.kind==K::CCX).count();eprintln!("RANK4_R01_CLEAN_BUILT class={class} j={j} ops={} T={nt}",ops.len());
        let rows:Vec<_>=data[12..].chunks_exact(138).filter(|row|{
            ...
            (time+3)&3==j&&bits(r,21,2)==2&&av>=2&&av+cv<=253&&(if class==0{ts[rk].iter().sum::<usize>()<=2}else{ts[rk].iter().sum::<usize>()==3})
        }).collect();
        let mapping=super::chart(class);let pack=|old:&Vec<u64>|{
            let mut out=vec![0u64;nq as usize-1];out[4..].copy_from_slice(&old[5..]);for lane in 0..64{let r=(0..5).fold(0usize,|v,i|v|(((old[i]>>lane)&1)as usize)<<i);let code=mapping.iter().position(|&v|v==r).unwrap();for i in 0..4{out[i]|=(((code>>i)&1)as u64)<<lane;}}out
        };
        let mut f=Fixed;let mut sim=Simulator::new(nq as usize-1,0,&mut f);let mut here=0;
        for pattern in 0..6{for batch in 0..rows.len().div_ceil(64){
            ...
            let before=pack(&before);let after=pack(&after);sim.qubits.copy_from_slice(&before);sim.phase=0;sim.apply_iter(ops.iter());
        ...
        for batch in 0..16{let mut seed=0x793f010ffu64^((j as u64)<<40)^batch as u64;let mut before:Vec<_>=(0..nq).map(|_|rnd(&mut seed)).collect();
            for lane in 0..64{set(&mut before,&rank,lane,mapping[batch]);set(&mut before,&a,lane,lane);put(&mut before,&g,lane,false);}
            let before=pack(&before);sim.qubits.copy_from_slice(&before);sim.phase=0;sim.apply_iter(ops.iter());assert_eq!(sim.qubits,before,"rank4 clean normal g0 j={j} class={class} batch={batch}");assert_eq!(sim.phase,0);
    ...
    eprintln!("RANK4_R01_CLEAN_NATIVE_PASS active={total} offguard={off}; native phase/inverse,three physical holes, low3 qpre/borrow, A2head/two-passenger/highA, external leases only; no wholeQ793 claim");
}
