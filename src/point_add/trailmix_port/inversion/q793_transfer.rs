...
/// S0 residual-length erasure directly into literal C8. P1 lends the clean
/// eligibility mask on the exit guard; the four returned SM rails fund cache.
pub(super) fn erase_c_numeric(circ:&mut Circuit,aa:&[QReg],cc:&[QReg],sm:&[QReg],mask:&QReg,guard:&QReg,source:&[QReg],prefix:&[QReg],helpers:&[QReg]){
    assert_eq!(aa.len(),8);assert_eq!(cc.len(),8);
    let rank:Vec<_>=[&aa[6],&aa[7],&sm[2],&sm[3],mask].into_iter().map(QReg::borrowed_alias).collect();
    let out:Vec<_>=cc.iter().collect();let bits=std::env::var("Q795_EXIT_PREFIX").ok().map(|v|v.parse::<usize>().unwrap()).unwrap_or(0);
    assert!(bits==0||bits==4||bits==5);
    length_xor(circ,&rank,&aa[..6],source,&out,guard,prefix,helpers,if bits==0{None}else{Some((sm,bits))});
}
