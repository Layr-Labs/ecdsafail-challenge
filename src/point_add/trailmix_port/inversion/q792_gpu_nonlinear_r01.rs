//! Exact reversible CX/CCX coordinate frames and ESOP selected on both B50 GPUs.
//! Ordinary deterministic NCT emitter; no OpenCL dependency at runtime.
use crate::point_add::trailmix_port::circuit::{Circuit,QReg};
use crate::circuit::{Op,OperationType as K};
struct Plan{n:usize,k:usize,truth:u64,pol:usize,gates:&'static[u16],cubes:&'static[(usize,usize)]}
include!("q792_gpu_nonlinear_plans_r01.rs");
fn truth(p:&Plan){
 for x in 0..1usize<<p.n{let mut y=x;
  for &g in p.gates{let target=(g&7)as usize;let bit=if g&512!=0{((y>>((g>>6)&7))&(y>>((g>>3)&7)))&1}else{(y>>(g>>3))&1};y^=bit<<target;}
  y^=p.pol;let got=p.cubes.iter().fold(false,|v,&(m,b)|v^((y&m)==b));assert_eq!(got,p.truth>>x&1!=0,"nonlinear frame truth");
 }
}
fn gate(c:&mut Circuit,word:&[&QReg],g:u16){if g&512!=0{c.ccx(word[((g>>6)&7)as usize],word[((g>>3)&7)as usize],word[(g&7)as usize]);}else{c.cx(word[(g>>3)as usize],word[(g&7)as usize]);}}
fn emit(c:&mut Circuit,word:&[&QReg],prefix:&[(&QReg,bool)],out:&QReg,dirty:&[QReg],p:&Plan){
 for &g in p.gates{gate(c,word,g);}for(i,q)in word.iter().enumerate(){if p.pol>>i&1!=0{c.x(q);}}
 for &(q,v)in prefix{if !v{c.x(q);}}let positive:Vec<_>=prefix.iter().map(|&(q,_)|(q,true)).collect();
 for &(m,v)in p.cubes{let mut cs=positive.clone();cs.extend(word.iter().enumerate().filter(|(i,_)|m>>i&1!=0).map(|(i,q)|(*q,v>>i&1!=0)));super::length_recompute::mixed_mcx(c,&cs,out,dirty);}
 for(i,q)in word.iter().enumerate().rev(){if p.pol>>i&1!=0{c.x(q);}}for &g in p.gates.iter().rev(){gate(c,word,g);}
 // Mixed prefix X frame is restored by the caller, on disjoint wires.
}
...
pub fn run(){
 ...
 eprintln!("FOLD20_GPU_NONLINEAR_PASS plans={} lanes={lanes} aggregate_native_T={total_t} all_truths_exact=true arbitrary_dirty=true phase=0 inverse=true no_extra_qubits=true",PLANS.len());
}
