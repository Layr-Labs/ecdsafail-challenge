...
#[path="q793_lifecycle_minq_check_r01.rs"]pub mod check;
...
// Exact structural count from own minimum-Q reduction stage03. Executed T
// includes measurement-conditioned gates; canonical9024 determines its average.
// Official validation remains a separate release requirement.
pub(crate) const CANDIDATE_OPS:usize=1_716_803_375;
pub(crate) const CANDIDATE_TOFFOLI:usize=969_740_628;
pub(crate) fn candidate_configuration()->bool{
    enabled() && CANDIDATE_OPS>0 && super::q794_lifecycle::candidate_configuration()
        // Release pins require both measured reductions at the inherited16-step schedule.
        && super::metadata_muxlease::active("Q793_RESIDUAL_HOLD") && super::metadata_muxlease::active("Q793_CLOCK_WINDOWS")
        && super::shared_step::SCHEDULE_BLOCK==16 && super::shared_step::SCHEDULE_BLOCKS==101
...
fn emit_schedule(circ:&mut Circuit,core:&Core,passenger:&[QReg],inverse:bool) {
    ...
    let mut unique_ops=0usize;let mut unique_capacity=0usize;
    for z in 0..super::shared_step::SCHEDULE_BLOCKS {let block=if inverse{super::shared_step::SCHEDULE_BLOCKS-1-z}else{z};let mut cached=Vec::new();
        for j in 0..4 {let ops=if let Some(bit)=measurement{remap(super::q793_mbu::template(block,j,bit),&mapping,passenger,false)}else{let logical=if let Some(ops)=circ.b.q793_templates.get((block,j)){ops}else{let ops=template(block,j);circ.b.q793_templates.insert((block,j),&ops);ops};remap(logical,&mapping,passenger,inverse)};let mut kinds=[0usize;18];let mut ct=0u64;for op in &ops{kinds[op.kind as usize]+=1;if matches!(op.kind,OperationType::CCX|OperationType::CCZ)&&op.c_condition!=crate::circuit::NO_BIT{assert_eq!(Some(op.c_condition),measurement);ct+=1;}}// Diagnostic compact storage keeps the same operations and order, with no
        // spare Vec capacity retained across404 templates. Ordinary emission is unchanged.
        let ops=if circ.b.compact_blocks.is_some(){ops.into_boxed_slice().into_vec()}else{ops};
        unique_ops+=ops.len();unique_capacity+=ops.capacity();cached.push((std::sync::Arc::new(ops),kinds,ct));}
    ...
    if circ.b.count_only||circ.b.compact_blocks.is_some(){eprintln!("Q793_SCHEDULE_STORAGE inverse={inverse} templates={} unique_ops={unique_ops} unique_capacity={unique_capacity}",4*super::shared_step::SCHEDULE_BLOCKS);}
}
