...
// Exact count from own timefix stage07 integrated stream. Canonical9024 and
// official validation remain separate release requirements.
pub(crate) const CANDIDATE_OPS:usize=1_780_610_799;
pub(crate) const CANDIDATE_TOFFOLI:usize=1_009_473_940;
pub(crate) fn candidate_configuration()->bool{
    enabled() && CANDIDATE_OPS>0 && super::q794_lifecycle::candidate_configuration()
        && ["Q793_AFFINE_ENTRY_HEAD","Q793_AFFINE_R01_TRANSPOSE","Q793_AFFINE_SIGN_CODE","Q793_RANK_ECHO_R01","Q793_RANK_ECHO_T10","Q793_RANK_ECHO_STEP","Q793_RANK_ECHO_MOVES","Q793_RANK_ECHO_CARGO"].iter().all(|s|super::metadata_muxlease::active(s))
}
fn emit_schedule(circ:&mut Circuit,core:&Core,passenger:&[QReg],inverse:bool) {
    ...
    for z in 0..26 {let block=if inverse{25-z}else{z};let mut cached=Vec::new();
        for j in 0..4 {let logical=if let Some(ops)=circ.b.q793_templates.get((block,j)){ops}else{let ops=template(block,j);circ.b.q793_templates.insert((block,j),&ops);ops};let ops=remap(logical,&mapping,passenger,inverse);let mut kinds=[0usize;18];for op in &ops{kinds[op.kind as usize]+=1;}cached.push((std::sync::Arc::new(ops),kinds));}
