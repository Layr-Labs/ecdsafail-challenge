...
pub(super) fn run(){
    ...
    for j in 0..4{
        ...
        let nt=builder.ops.iter().filter(|op|op.kind==K::CCX).count();eprintln!("FOLD20_DYNAMIC_R01_BUILT j={j} ops={} T={nt} interfaceQ={owned}",builder.ops.len());
        ...
        for pattern in 0..4{for batch in 0..rows.len().div_ceil(64){
            ...
            let before=pack(&before);let after=pack(&after);let mut f=Fixed;let mut sim=Simulator::new(owned as usize-1,0,&mut f);sim.qubits.copy_from_slice(&before);sim.apply_iter(builder.ops.iter());
            if sim.qubits!=after{let diffs:Vec<_>=sim.qubits.iter().zip(&after).enumerate().filter(|(_, (x,y))|x!=y).map(|(i,(x,y))|(i,format!("{:016x}",x^y))).collect();panic!("FOLD20_DYNAMIC_R01 active j={j} pattern={pattern} batch={batch} diffs={diffs:?}");}
        ...
        eprintln!("FOLD20_DYNAMIC_R01_CLOCK_PASS j={j} active={active_here}");
    }
    ...
    eprintln!("FOLD20_DYNAMIC_R01_PASS active={total} offguard={off}; actual folded20 ranges, A0/A1, two independent cargo bits, q1/q2 gather, three physical holes, packed width2/1 endpoints, normalized b2 HA restored, literal inverse/allphase; no whole-circuit claim");
}
