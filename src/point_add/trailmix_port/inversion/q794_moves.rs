...
fn paired_a_terms(circ:&mut Circuit,rank:&[QReg],a:&[QReg],word:&[QReg],from:usize,to:usize,terms:&[Vec<(&QReg,bool)>],dirty:&[QReg],flip:bool) {
    ...
    for residue in 0..stride {
        ...
        if let (Some(left),Some(right))=(roots[0],roots[1]) {
            ...
            circ.cx(right,left);
            let with_left:Vec<Vec<(&QReg,bool)>>=controls.iter().map(|t|{let mut cs=t.clone();cs.push((left,true));cs}).collect();
            super::q793_rank_echo::grouped(circ,"Q793_RANK_ECHO_MOVES",&with_left,rank,right,dirty);
            circ.cx(right,left);
            if flip{super::q793_rank_echo::grouped(circ,"Q793_RANK_ECHO_MOVES",&controls,rank,left,dirty);}
        }
...
pub(super) fn across_a_terms(circ:&mut Circuit,rank:&[QReg],a:&[QReg],left_word:&[QReg],left_offset:usize,right_word:&[QReg],right_offset:usize,terms:&[Vec<(&QReg,bool)>],flip_left:bool,dirty:&[QReg]) {
    ...
    circ.cx(right,left);
    let with_left:Vec<Vec<(&QReg,bool)>>=terms.iter().map(|t|{let mut cs=t.clone();cs.push((left,true));cs}).collect();
    super::q793_rank_echo::grouped(circ,"Q793_RANK_ECHO_MOVES",&with_left,rank,right,dirty);
    circ.cx(right,left);
    if flip_left{super::q793_rank_echo::grouped(circ,"Q793_RANK_ECHO_MOVES",terms,rank,left,dirty);}
...
pub(super) fn flip_a_terms(circ:&mut Circuit,rank:&[QReg],a:&[QReg],word:&[QReg],offset:usize,terms:&[Vec<(&QReg,bool)>],dirty:&[QReg]) {
    ...
    super::q793_rank_echo::grouped(circ,"Q793_RANK_ECHO_MOVES",terms,rank,root,dirty);circ.b.ops.extend(ops.into_iter().rev());
}
...
fn exchange_a_terms_inner(circ:&mut Circuit,rank:&[QReg],a:&[QReg],word:&[QReg],offset:usize,passenger:&QReg,terms:&[Vec<(&QReg,bool)>],dirty:&[QReg],flip_root:bool) {
    ...
    let with_root:Vec<Vec<(&QReg,bool)>>=terms.iter().map(|t|{let mut cs=t.clone();cs.push((root,true));cs}).collect();
    super::q793_rank_echo::grouped(circ,"Q793_RANK_ECHO_MOVES",&with_root,rank,passenger,dirty);
    circ.cx(passenger,root);if flip_root{super::q793_rank_echo::grouped(circ,"Q793_RANK_ECHO_MOVES",terms,rank,root,dirty);}circ.b.ops.extend(ops.into_iter().rev());
}
