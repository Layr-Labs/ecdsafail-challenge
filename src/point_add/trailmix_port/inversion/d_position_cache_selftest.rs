...
fn raw_step(
    ...
    let mut cache =
        cached.then(|| Cache::begin_raw(c, a, b, if inverse { i + 1 } else { i }, false));
    if !inverse {
    ...
    d_alignment_iteration::enter(c, b, ca, cb, q, &phi, &role);
    ...
    d_alignment_iteration::exit(c, b, ca, cb, q, &phi, &role);
...
fn build(i: usize, direction: usize, nested: bool, cached: bool) -> Built {
    use crate::point_add::trailmix_port::inversion::shrunken_pz_schedule::reg_widths;
    ...
    assert!(
        ...
        "whole iteration peak={} step={i} cached={cached}",
        c.b.peak_qubits
    );
    Built {
        ops: c.b.ops.clone(),
...
fn check(i: usize, cases: &[Case], direction: usize, nested: bool, cached: bool) -> usize {
    let b = build(i, direction, nested, cached);
    let mut checked = 0;
    for mode in 0..4 {
    ...
    if i % 32 == 0 || i + 1 == MIDQ_PZ_CUT {
        eprintln!("D_POSITION_CACHE step={i} dir={direction} nested={nested} cached={cached} peak={} checked={checked}",b.peak);
    }
    checked
}

fn primitive_checks() -> usize {
    let mut checked = 0;
    for lo in 0usize..5 {
        for next in 0..=lo {
            for nested in [false, true] {
                let mut c = Circuit::new();
                let a = c.alloc_qreg_bits("cache.test.A", 5);
                let b = c.alloc_qreg_bits("cache.test.B", 5);
                let g = c.alloc_qreg("cache.test.swap");
                let carry = c.alloc_qreg("cache.test.carry");
                let data: Vec<_> = a
                    .iter()
                    .chain(&b)
                    .chain([&g])
                ...
                let inner = c.alloc_input_bit();
                let mut cache = None;
                let first = |c: &mut Circuit| {
                    let mut p = Cache {
                        alpha: Some(clz_deposit_a(c, &a, 9, lo)),
                        beta: clz_deposit_a(c, &b, 9, lo),
                        index: 0,
                    };
                    p.erase_a(c, &a, lo);
                    retune(c, &p.beta, lo - next, &carry, false);
                    p.create_a(c, &a, next);
                    for (a, b) in a.iter().zip(&b) {
                        c.cswap(&g, a, b);
                    }
                    swap_positions(c, &g, &p);
                    p
                };
                cache = Some(if nested {
                    c.with_conditions(&[outer, inner], first)
                } else {
                    first(&mut c)
                });
                c.flush_pending_frees();
                let split = c.b.ops.len();
                let p = cache.as_ref().unwrap();
                let mid: Vec<_> = p
                    .alpha
                    .as_ref()
                    .unwrap()
                    ...
                    .map(|q| QubitId(q.id().into()))
                    .collect();
                let mut second = |c: &mut Circuit| {
                    let mut p = cache.take().unwrap();
                    for (a, b) in a.iter().zip(&b) {
                        c.cswap(&g, a, b);
                    }
                    swap_positions(c, &g, &p);
                    p.erase_a(c, &a, next);
                    retune(c, &p.beta, lo - next, &carry, true);
                    p.create_a(c, &a, lo);
                    clz_undeposit_a(c, p.alpha.take().unwrap(), &a, lo);
                    clz_undeposit_a(c, p.beta, &b, lo);
                };
                if nested {
                    c.with_conditions(&[outer, inner], second);
                } else {
                    second(&mut c);
                }
                c.flush_pending_frees();
                ...
                let inputs: Vec<usize> = (0..32)
                    .flat_map(|a| {
                        (1usize << lo..32)
                            .flat_map(move |b| (0..2).map(move |g| a | (b << 5) | (g << 10)))
                    })
                    .collect();
                for mode in 0..4 {
                    let mut rng = Random::new(mode);
                    ...
                    for batch in inputs.chunks(64) {
                        sim.clear_for_shot();
                        let live = u64::MAX >> (64 - batch.len());
                        let m1 = if nested { 0xeeeeeeeeeeeeeeee } else { u64::MAX };
                        let m2 = if nested { 0xbbbbbbbbbbbbbbbb } else { u64::MAX };
                        let applied = m1 & m2;
                        *sim.bit_mut(BitId(outer.raw() as u64)) = m1;
                        *sim.bit_mut(BitId(inner.raw() as u64)) = m2;
                        for (j, &id) in data.iter().enumerate() {
                            *sim.qubit_mut(id) = batch
                                .iter()
                                .enumerate()
                                .fold(0, |v, (s, &x)| v | ((((x >> j) & 1) as u64) << s));
                        }
                        super::super::predicate_clear_selftest::checked_apply(
                            &mut sim,
                            &c.b.ops[..split],
                        ...
                        for (s, &x) in batch.iter().enumerate() {
                            let aa = x & 31;
                            let bb = (x >> 5) & 31;
                            let swapped = (x >> 10) & 1 != 0 && applied >> s & 1 != 0;
                            let (av, bv) = if swapped { (bb, aa) } else { (aa, bb) };
                            let got = data.iter().enumerate().fold(0usize, |v, (j, &id)| {
                                v | ((((sim.qubit(id) >> s) & 1) as usize) << j)
                            });
                            assert_eq!(got, av | (bv << 5) | (x & (1 << 10)));
                            ...
                            let want = if applied >> s & 1 != 0 {
                                position(av) | (position(bv) << 9)
                            } else {
                                0
                            };
                            let got = mid.iter().enumerate().fold(0usize, |v, (j, &id)| {
                                v | ((((sim.qubit(id) >> s) & 1) as usize) << j)
                            });
                            assert_eq!(got, want, "cache transition lo={lo} next={next} x={x}");
                        }
                        super::super::predicate_clear_selftest::checked_apply(
                            &mut sim,
                            &c.b.ops[split..],
                        ...
                        assert_eq!(sim.phase & live, 0);
                        let mut marked = vec![false; sim.qubits.len()];
                        for (j, &id) in data.iter().enumerate() {
                            marked[id.0 as usize] = true;
                            let want = batch
                                .iter()
                                .enumerate()
                                .fold(0, |v, (s, &x)| v | ((((x >> j) & 1) as u64) << s));
                            assert_eq!(sim.qubit(id) & live, want);
                        }
                        for (i, &v) in sim.qubits.iter().enumerate() {
                            if !marked[i] {
                                assert_eq!(v & live, 0, "cache transition scratch{i}");
                            }
                        }
                        checked += batch.len();
                    }
...
pub(super) fn run() {
    ...
    let primitive = primitive_checks();
    let mut checked = 0;
    for (&i, cases) in &groups {
        for direction in 0..3 {
            for nested in [false, true] {
                for cached in [false, true] {
                    checked += check(i, cases, direction, nested, cached);
                }
            }
        }
    }
    eprintln!("D_POSITION_CACHE_SELFTEST PASS cases={checked} cache_transition_cases={primitive} old_framed_vs_cached=true raw_frame_raw_interfaces=true forward_inverse_roundtrip=true midpoint_phase_checks=true every_reset_checked=true passenger256_preserved=true actual_PZ_sources=37 qualified_contiguous_prefix_cases=11664");
}
