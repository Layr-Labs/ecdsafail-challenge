...
fn endpoint_prepared(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],base:&[(&QReg,bool)],out:&QReg,dirty:&[QReg]){
    for azero in [false,true]{
        ...
        for term in terms{
            ...
            gate(circ,&cs,out,dirty);
        }
    }
}
pub(super) fn endpoint(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],base:&[(&QReg,bool)],out:&QReg,dirty:&[QReg]){
    add(circ,a,c,None,false);endpoint_prepared(circ,rank,a,c,base,out,dirty);add(circ,a,c,None,true);
}
...
fn normal_exchange(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],g:&QReg,p:&QReg,w1:&[QReg],helpers:&[QReg]){
    ...
    circ.cx(p,root);gate(circ,&[(g,true),(root,true)],p,helpers);
    endpoint_prepared(circ,rank,a,c,&[(g,true),(root,true)],p,helpers);circ.cx(p,root);
