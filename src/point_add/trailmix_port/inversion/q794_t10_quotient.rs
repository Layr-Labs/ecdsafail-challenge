...
fn endpoint_prepared(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],base:&[(&QReg,bool)],out:&QReg,dirty:&[QReg],paired_guard:Option<&QReg>){
    ...
    for azero in [false,true]{
        ...
        let swap=mux::swap_offer(truth,5,extra);
        // Site-exact by_t offer: every emitted cube/term wraps the shared
        // negative prefix literals per mixed_mcx, so they enter the ops count.
        let shared=6+if azero{6}else{0}+base.iter().filter(|&&(_,v)|!v).count();
        let esop=swap.esop.as_ref().filter(|e|{
            let ops:usize=e.cubes.iter().map(|&(m,v)|mux::mcx_t(extra+m.count_ones()as usize).max(1)+2*(shared+(m^v).count_ones()as usize)).sum::<usize>()+2*e.shear.is_some()as usize;
            let anf_ops:usize=swap.terms.iter().map(|&m|mux::mcx_t(extra+m.count_ones()as usize).max(1)+2*(shared+(m&!swap.polarity).count_ones()as usize)).sum();
            ops<=anf_ops&&(e.t,ops)<(swap.anf_t,anf_ops)
        });
        if let Some((i,j))=esop.and_then(|e|e.shear){circ.cx(&rank[j],&rank[i]);}
        if let Some(e)=esop{
            for &(m,v) in &e.cubes{
            ...
            for &term in &swap.terms{
                let mut cs=base.to_vec();cs.extend(c.iter().map(|q|(q,false)));
                if azero{cs.extend(a.iter().map(|q|(q,false)));}
                cs.extend((0..5).filter(|&i|term>>i&1!=0).map(|i|(&rank[i],swap.polarity>>i&1==0)));
                emit(circ,cs);
            }
        }
        if let Some((i,j))=esop.and_then(|e|e.shear){circ.cx(&rank[j],&rank[i]);}
