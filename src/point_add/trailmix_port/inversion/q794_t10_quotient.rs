...
fn endpoint_prepared(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],base:&[(&QReg,bool)],out:&QReg,dirty:&[QReg],paired_guard:Option<&QReg>){
    let emit=|circ:&mut Circuit,cs:Vec<(&QReg,bool)>|{
    ...
    for azero in [false,true]{
        ...
        let(polarity,terms)=mux::swap_terms(truth.clone(),5);
        // W45e joint arbitration at this bespoke site (same offer loop, same
        // accept rule as swap_plan: strictly smaller T actual, ops not up,
        // ties keep the ANF incumbent; Q793_TABLE_ESOP=0 drops the offer and
        // restores the W45 stream exactly). The emission shape differs from a
        // swap (no skeleton pair, shared negative-prefix wraps on every
        // emitted cube/term), so ops are re-scored site-exactly from the
        // challenger's (shear,cubes); the T model is the tree's 4k-8.
        let mut esop=None;
        {
            let extra=base.len()+6+if azero{6}else{0};
            let shared=6+if azero{6}else{0}+base.iter().filter(|&&(_,v)|!v).count();
            let anf_t:usize=terms.iter().map(|&m|mux::mcx_t(extra+m.count_ones()as usize)).sum();
            let anf_ops:usize=terms.iter().map(|&m|mux::mcx_t(extra+m.count_ones()as usize).max(1)+2*(shared+(m&!polarity).count_ones()as usize)).sum();
            if let Some((sh,cubes,t,_))=mux::esop_challenger(&truth,5,extra,mux::McxModel::Dirty){
                let ops:usize=cubes.iter().map(|&(m,v)|mux::mcx_t(extra+m.count_ones()as usize).max(1)+2*(shared+(m^v).count_ones()as usize)).sum::<usize>()+2*sh.is_some()as usize;
                if t<anf_t&&ops<=anf_ops{esop=Some((sh,cubes));}
            }
        }
        if let Some((sh,cubes))=esop{
