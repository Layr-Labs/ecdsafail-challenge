...
fn top_compare_eligible() -> bool {
    std::env::var("MIDQ_TAIL_TOP_COMPARE").ok().as_deref() == Some("1")
        ...
        && ((super::super::MIDQ_PZ_CUT == 384 && super::super::MIDQ_TAIL_ROUNDS == 186)
            || (super::super::MIDQ_PZ_CUT == 480 && super::super::MIDQ_TAIL_ROUNDS == 62)) // P0 shape: pin relaxed
...
fn clear_rotated_overflow(c: &mut Circuit, target: &[QReg], source: &[QReg], overflow: &QReg) {
    ...
    let low = if top_compare_eligible() { 136 } else { 0 };
    let section = (low != 0).then(|| c.push_section("midq.cell.fused_rotation.top120_compare"));
...
pub(super) fn try_apply(c: &mut Circuit, target: &[QReg], source: &[QReg], sign: &QReg) -> bool {
    if std::env::var("MIDQ_FUSED_ROTATION_CELL").ok().as_deref() != Some("1")
        || std::env::var("MIDQ_MEASURE_COMPARE").ok().as_deref() != Some("1")
        || !super::super::midq_rotated_halves()
    {
        return false;
