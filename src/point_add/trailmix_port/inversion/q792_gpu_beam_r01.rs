...
include!("q792_gpu_beam_plans_r01.rs");
fn truth(p:&Plan){
 for x in 0..1usize<<p.n{let mut y=x;
  ...
  y^=p.pol;let got=p.cubes.iter().fold(false,|v,&(m,b)|v^((y&m)==b));assert_eq!(got,p.truth>>x&1!=0,"beam frame truth");
 }
}
...
pub fn run(){
 ...
 eprintln!("FOLD20_GPU_BEAM_PASS plans={} lanes={lanes} aggregate_native_T={total_t} all_truths_exact=true arbitrary_dirty=true phase=0 inverse=true no_extra_qubits=true",PLANS.len());
}
