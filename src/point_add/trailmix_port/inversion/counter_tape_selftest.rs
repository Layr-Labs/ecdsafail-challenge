...
fn prefix_width_obligation() {
    ...
    let mut max_width = 0;
    let mut first_wide = None;
    for step in 0..MIDQ_PZ_CUT {
        let (_, _, ca, cb, _) = reg_widths(step);
        let width = trailmix_cacb_width(ca.max(cb));
        if width >= 256 && first_wide.is_none() {
            first_wide = Some(step);
        }
        max_width = max_width.max(width);
    }
    // Rows from `first_wide` on can hold ca=p. The prefix keeps the
    // no-terminal step there anyway (the counter stays a popcount cache and the
    // counter/tape codec sees counter=0), so a terminal input is a support miss
    // on this route rather than a circuit case; see tail_value_misses.
    eprintln!("COUNTER_TAPE_PREFIX PASS max coefficient width {max_width}; first 256-wide row {first_wide:?}; terminal inputs on those rows are support misses, done never increments counter");
}
