...
fn prefix_width_obligation() {
    ...
    assert_eq!(
        ...
        (super::super::hybrid_profile::SELECTED.pz_cut, super::super::hybrid_profile::SELECTED.rounds, 8)
    );
    let mut max_width = 0;
    ...
    eprintln!("COUNTER_TAPE_PREFIX PASS all {MIDQ_PZ_CUT} coefficient widths <= {max_width} < 256; ca=p cannot occur on the inherited exact-width support, hence done never increments counter and no wrap occurs there");
}
