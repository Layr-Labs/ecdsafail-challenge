...
//!
//! Ported from bulengerk's public 792 lineage (`q792_esop_r01.rs` +
//! `q792_esop4_r01.bin`, lane-kimi-792c) into the Q793 `swap_terms` offer loop.
//! Only the `plan`/`solve` pair and the 4-input cube ROM transfer; the 792
//! `paired` echo stays dead and is not ported.
use std::collections::HashMap;
pub(super) type Cubes=Vec<(usize,usize)>;
const TABLE:&[u8;3997696]=include_bytes!("q793_table_esop4_r01.bin");
...
pub fn run(){
 ...
 let mut seed=0x793e50fu64;let mut more=0;for n in [5,6]{for k in [0,1,2,6,12]{for _ in 0..64{seed^=seed<<13;seed^=seed>>7;seed^=seed>>17;let truth:Vec<_>=(0..1<<n).map(|x|seed>>x&1!=0).collect();plan(n,&truth,k).unwrap();more+=1;}}}
 eprintln!("Q793_TABLE_ESOP_TRUTH_PASS exhaustive4={cases} composed5and6={more} every_address=true no_sample_fit=true lineage=bulengerk-q792_esop_r01");
}
