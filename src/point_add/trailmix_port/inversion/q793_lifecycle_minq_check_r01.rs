...
fn run_inner(boundary_only:bool){
 ...
 let peak=793;assert!(dual_phase());assert_eq!(circ.b.active_qubits,513);assert_eq!(circ.b.peak_qubits,peak);assert_eq!(circ.b.next_qubit,peak);let owned=circ.b.next_qubit as usize;let mbu=super::super::q793_mbu::reverse()&&!boundary_only;let measurement=if mbu{Some(circ.b.alloc_bit())}else{None};let bits=circ.b.next_bit as usize;
 ...
 for inverse in [false,true]{let mapping=if inverse{&rebuilt_map}else{&initial_map};
  if !boundary_only{for z in 0..super::super::shared_step::SCHEDULE_BLOCKS{let block=if inverse{super::super::shared_step::SCHEDULE_BLOCKS-1-z}else{z};let templates:Vec<_>=(0..4).map(|j|if inverse&&mbu{remap(super::super::q793_mbu::template(block,j,measurement.unwrap()),mapping,&passenger,false)}else{remap(template(block,j),mapping,&passenger,inverse)}).collect();let first=block*super::super::shared_step::SCHEDULE_BLOCK;let end=(first+super::super::shared_step::SCHEDULE_BLOCK).min(1616);
 ...
 eprintln!("{marker} boundary_only={boundary_only} reverse_uses_MBU={mbu} inputs={count} lanes={} peak_qubits={peak} forward_ops={forward_ops} forward_T={forward_t} lifecycle_ops={lifecycle_ops}; actual init/release/independent inverse/sign/rebuild/finish, all256 passengers and every reset checked; full forward/reverse only when boundary_only=false, otherwise scalar boundary states injected explicitly; NOT whole point-add or official acceptance",64*batches);
}
