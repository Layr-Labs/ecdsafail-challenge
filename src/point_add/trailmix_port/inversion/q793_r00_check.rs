...
pub fn run() {
    ...
    for block in 0..blocks {for j in 0..4{
        ...
        let mut seed=|cc:&mut Circuit,mask:&QReg,carry:&QReg,helpers:&[QReg],clock:usize| {
            super::q793_r00_seed_dynamic::emit(cc,&rank,&a,&w1,&w2,mask,carry,helpers,clock);
        };
        ...
        for pattern in 0..8{for batch in 0..4{
            ...
            if sim.qubits!=after{let diffs:Vec<_>=sim.qubits.iter().zip(&after).enumerate().filter(|(_, (x,y))|x!=y).map(|(i,(x,y))|(i,format!("{:016x}",x^y))).collect();panic!("R00 mod8 j={j} pattern={pattern} batch={batch} diffs={diffs:?}");}
            assert_eq!(sim.phase,0);sim.apply_iter(builder.ops.iter().rev());assert_eq!(sim.qubits,before);assert_eq!(sim.phase,0);total+=64;
        }}
    }}
    eprintln!("Q793_R00_PASS lanes={total} templates={} helpers=23; production R00 dynamic endpoints, three omitted lanes, direct low carry; existing C5 scratch, A0/A1/A2 head cargo and A254 short-v, literal inverse and dirty restoration; full step not yet validated",4*blocks);
}
