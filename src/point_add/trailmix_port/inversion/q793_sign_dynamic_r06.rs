...
fn short_compare(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],g:&QReg,cache:&QReg,sign:&QReg,w1:&[QReg],w2:&[QReg],dirty:&[QReg],j:usize){
    ...
    for k in lower..=3{let value=257-k;
        ...
        if k>0{selected(circ,small,low_data_indexed(w1,w2,j,k,None,1,Some(a)),sign,dirty);}
    }
...
fn mask_loan(circ:&mut Circuit,rank:&[QReg],c:&[QReg],g:&QReg,cache:&QReg,mask:&QReg,w1:&[QReg],dirty:&[QReg]){
    c12_flag(circ,rank,c,g,cache,dirty);let start=circ.b.ops.len();let max_c=257-circ.q797_a_support.map(|(lo,_)|lo).unwrap_or(0);let mut nodes:Vec<_>=(0..256).map(|v|if v>=3&&v<=max_c{Some(&w1[258-v])}else{None}).collect();
