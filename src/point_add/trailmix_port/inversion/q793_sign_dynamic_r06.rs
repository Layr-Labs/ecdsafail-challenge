...
fn short_flag(circ:&mut Circuit,rank:&[QReg],c:&[QReg],sm:&[QReg],g:&QReg,out:&QReg,dirty:&[QReg],j:usize){
    ...
    for (h,values) in [(0usize,vec![1usize]),(3,vec![254usize,255]),(4,vec![256,257])]{
        let max_sum=257-circ.q797_a_support.map(|(lo,_)|lo).unwrap_or(0);let floor=if super::metadata_muxlease::active("Q793_SHORT_FLAG_LOW_CUT"){super::length_recompute::t_top().map(|t|257usize.saturating_sub(t+2)).unwrap_or(0)}else{0};let values:Vec<_>=values.into_iter().filter(|&v|v<=max_sum&&v>=floor).collect();if values.is_empty(){continue;}
        for _ in 0..2{
...
pub(super) fn sum_low_cut(circ:&Circuit)->usize{
    // A11: C+S >= 256-A_hi from the entry codec A+C+S in [255,257].
...
fn gather_span<'a>(circ:&mut Circuit,rank:&[QReg],c:&[QReg],cache:&QReg,word:&'a[QReg],base:usize,min:usize,max:usize,dirty:&[QReg],g:&QReg)->(&'a QReg,Vec<crate::circuit::Op>){
    let start=circ.b.ops.len();let max=max.min(257-circ.q797_a_support.map(|(lo,_)|lo).unwrap_or(0));let min=if super::metadata_muxlease::active("Q793_A11_SUMLOWCUT"){min.max(sum_low_cut(circ)).min(max)}else{assert!(min<=max);min};let mut nodes=vec![None;512];for value in min..=max{nodes[value]=Some(&word[base-value]);}let ts=triples();
