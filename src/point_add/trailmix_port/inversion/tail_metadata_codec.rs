...
// SPIKE (env-gated MIDQ_TERMINAL_A_ONLY=1): the terminal flag may carry a
// live quotient code (a draining handoff: A=0, q!=0 mapped onto the unit
// orbit). Rank = qtag + 19*(selector + 2*ctz) + 3268*terminal is then a
// bijection from {non-terminal} U {terminal, qtag 0..18, ctz 0, selector 0}
// onto 0..3286 (< 4096). Decode: t=[z>=3268]; z-=3268*t; divmod 19; z+=3268*t.
fn a_only() -> bool {
    std::env::var("MIDQ_TERMINAL_A_ONLY").ok().as_deref() == Some("1")
}
const TERMINAL_WEIGHT_A_ONLY: u16 = 3268;
...
fn xor_decode(c: &mut Circuit, z: &[QReg], out: &Raw) {
    if a_only() { return xor_decode_a_only(c, z, out); }
...
fn xor_decode_a_only(c: &mut Circuit, z: &[QReg], out: &Raw) {
    let weight = TERMINAL_WEIGHT_A_ONLY.to_le_bytes();
    let terminal = c.alloc_qreg("midq.metadata.is_terminal");
    compare_geq_const(c, z, &weight, &terminal);
    let remainder = c.alloc_qreg_bits("midq.metadata.remainder", 12);
    let quotient = c.alloc_qreg_bits("midq.metadata.quotient", 8);
    for (src, dst) in z.iter().zip(&remainder) { c.cx(src, dst); }
    controlled_sub_const(c, &terminal, &remainder, &weight);
    for i in (0..8).rev() {
        let divisor = (19u16 << i).to_le_bytes();
        compare_geq_const(c, &remainder, &divisor, &quotient[i]);
        controlled_sub_const(c, &quotient[i], &remainder, &divisor);
    ...
    controlled_add_const(c, &terminal, &remainder, &weight);
    for (src, dst) in z.iter().zip(&remainder) { c.cx(src, dst); }
    for bit in quotient.into_iter().chain(remainder) { c.zero_and_free(bit); }
    c.cx(&terminal, &out.terminal);
    compare_geq_const(c, z, &weight, &terminal);
...
fn rank(c: &mut Circuit, raw: &Raw, z: &[QReg], inverse: bool) {
    ...
    let weight = if a_only() { TERMINAL_WEIGHT_A_ONLY } else { 3250u16 };
    update(c, &raw.terminal, z, &weight.to_le_bytes());
    if inverse {
        for (src, dst) in raw.qtag.iter().zip(z) { c.cx(src, dst); }
    }
}
