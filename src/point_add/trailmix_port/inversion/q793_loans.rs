...
pub(super) fn global_a(
    ...
    for level in 0..8{let mut next=Vec::new();
        for pair in nodes.chunks_exact(2){
            if level<6{controlled_swap(circ,&[(&a[level],true)],pair[0],pair[1],dirty);}
            else{high_swap(circ,rank,level-6,pair[0],pair[1],dirty);}
            next.push(pair[0]);
        }nodes=next;
    }
    let route=circ.b.ops[all_start..].to_vec();let root=nodes[0];
