...
fn gather_span<'a>(circ:&mut Circuit,rank:&[QReg],c:&[QReg],sm:&[QReg],word:&'a[QReg],base:usize,min:usize,max:usize,dirty:&[QReg],g:&QReg,j:usize)->(&'a QReg,Vec<crate::circuit::Op>){
    ...
    let address=&dirty[..2];let carry=&dirty[2];let rest=&dirty[3..];let ts=triples();
    let delta=|circ:&mut Circuit|{
        for _ in 0..2{
            super::metadata_phase115_phased::carry_xor(circ,c,sm,g,carry,rest,j);
            for bit in 0..2{let truth=ts.iter().map(|t|((t[1]+t[2])^(t[1]+t[2]+1))>>bit&1!=0).collect();super::q792_fold20_r01::table(circ,&rank.iter().collect::<Vec<_>>(),truth,&[(carry,true)],&address[bit],rest);}
        }
        for bit in 0..2{let truth=ts.iter().map(|t|(t[1]+t[2])>>bit&1!=0).collect();super::q792_fold20_r01::table(circ,&rank.iter().collect::<Vec<_>>(),truth,&[],&address[bit],rest);}
    };
    for _ in 0..2{
        for bit in 0..2{for i in 0..4{if i>>bit&1==0{circ.cswap(&address[bit],roots[i],roots[i^(1<<bit)]);}}}
        delta(circ);
    }
...
fn cargo(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],g:&QReg,cache:&QReg,w1:&[QReg],w2:&[QReg],dirty:&[QReg],j:usize){
    ...
    let(l,lop)=gather_a(circ,rank,a,w1,2,253,dirty,g);let(r,rop)=gather_sum(circ,rank,c,sm,w2,257,2,dirty,g,j);
    ...
    let(l,lop)=gather_a(circ,rank,a,w1,1,254,dirty,g);let(r,rop)=gather_sum(circ,rank,c,sm,w2,258,2,dirty,g,j);circ.cswap(g,l,r);circ.b.ops.extend(rop.into_iter().rev());circ.b.ops.extend(lop.into_iter().rev());
...
fn mask_loan(circ:&mut Circuit,rank:&[QReg],c:&[QReg],g:&QReg,_cache:&QReg,mask:&QReg,w1:&[QReg],dirty:&[QReg]){
    ...
    let small=|circ:&mut Circuit,q:&QReg|{for cv in 1usize..=2{for mut cs in rank_terms(rank,|t|t[1]==0){cs.push((g,true));cs.extend(c.iter().enumerate().map(|(i,q)|(q,cv>>i&1!=0)));swap(circ,&cs,q,mask,dirty);}}};
    small(circ,root);circ.b.ops.extend(route.into_iter().rev());small(circ,&w1[255]);
}
fn top_flag(circ:&mut Circuit,rank:&[QReg],c:&[QReg],sm:&[QReg],g:&QReg,_cache:&QReg,mask:&QReg,w1:&[QReg],dirty:&[QReg],j:usize){
    super::metadata_phase115_phased::prepare(circ,c,sm,g,None,dirty,j,false);let(q,route)=gather_sum(circ,rank,c,sm,w1,257,3,dirty,g,j);
    circ.ccx(g,q,mask);
    ...
    circ.b.ops.extend(route.into_iter().rev());super::metadata_phase115_phased::prepare(circ,c,sm,g,None,dirty,j,true);
}
...
fn core(circ:&mut Circuit,rank:&[QReg],c:&[QReg],sm:&[QReg],g:&QReg,cache:&QReg,top:&QReg,sign:&QReg,w1:&[QReg],w2:&[QReg],dirty:&[QReg],j:usize){
    ...
    let(s,sop)=gather_span(circ,rank,c,sm,w1,256,2,253,dirty,g,j);
    let(d,dop)=gather_span(circ,rank,c,sm,w2,256,2,253,dirty,g,j);
...
pub(super) fn general(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,sign:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize){
    ...
    core(circ,rank,c,sm,p1,p2,mask,sign,w1,w2,dirty,j);;
    circ.b.ops.extend(top.into_iter().rev());circ.b.ops.extend(loan.into_iter().rev());circ.b.ops.extend(moves.into_iter().rev());
