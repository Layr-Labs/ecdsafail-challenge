...
fn emit_terms_with_scratch(circ:&mut Circuit,terms:Vec<Vec<(&QReg,bool)>>,out:&QReg,dirty:&[QReg],scratch:Option<&QReg>){
    let terms=simplify_terms(terms);if let Some(s)=scratch{if super::q792_bank_factor_r01::emit(circ,&terms,out,dirty,s){return;}}
    for term in terms{
...
fn span_carry(circ:&mut Circuit,c:&[QReg],sm:&[QReg],out:&QReg,dirty:&[QReg],j:usize){
    let low=(4-j)%4;
    for bit in 0..6{
        if bit<2&&low>>bit&1==0{continue;}
        for i in bit+1..6{if i>=2{circ.cx(&sm[i-2],&c[i]);}}
        let mut cs=vec![(&c[bit],false)];if bit>=2{cs.push((&sm[bit-2],true));}
        for i in bit+1..6{cs.push((&c[i],i<2&&low>>i&1!=0));}
        super::length_recompute::mixed_mcx(circ,&cs,out,dirty);
        for i in (bit+1..6).rev(){if i>=2{circ.cx(&sm[i-2],&c[i]);}}
    }
}
...
fn core(circ:&mut Circuit,rank:&[QReg],c:&[QReg],sm:&[QReg],g:&QReg,cache:&QReg,top:&QReg,sign:&QReg,w1:&[QReg],w2:&[QReg],dirty:&[QReg],j:usize){
    ...
    let([s,d],route)=gather_pair_span(circ,rank,c,sm,[w1,w2],256,2,253,dirty,g,j);
