...
//! Legacy P1 is arbitrary. Enabled C1 decision fusion is scoped to its actual
//! last-quotient caller (incoming1, post-midpoint0); general route unchanged.
use crate::point_add::trailmix_port::circuit::{Circuit,QReg};
#[path="q794_t10_c1_decision.rs"] pub(crate) mod c1_decision;
...
/// its COMPLETE circuit emits the old pair's map. General/legacy supports
/// arbitrary P1; enabled C1 decision fusion uses its actual last1 caller.
pub(super) fn add_and_clear(circ:&mut Circuit,rank:&[QReg],source:&[QReg],target:&[QReg],a:&[QReg],g:&QReg,cache:&QReg,mask:&QReg,p1:&QReg,helpers:&[QReg],n:usize,c:&[QReg],sm:&[QReg],j:usize,c1:bool){
    ...
    let carry=&helpers[0];let dirty=&helpers[1..];let start=circ.b.ops.len();let decision_one=c1&&c1_decision::enabled();
    ...
    for i in 0..n{
        ...
        else if decision_one{c1_decision::forward(circ,&source[i],&target[i],carry,mask,p1);}
    ...
    circ.cx(g,p1);
    if super::q794_t10::polish::enabled(){super::q794_t10::polish::threshold(circ,g,carry,source_top,target_top,p1,dirty);}else{
    ...
    if decision_one{circ.ccx(g,carry,target_top);}else{mixed_mcx(circ,&[(g,true),(p1,true),(carry,true)],target_top,dirty);}
    ...
    for i in (0..n).rev(){
        ...
        if decision_one&&!known_one{c1_decision::reverse_sum(circ,&source[i],&target[i],carry,mask,g,p1);}else{
            if known_one{mask_one::cell(circ,&source[i],&target[i],carry,true);}else{cell(circ,&source[i],&target[i],carry,mask,g,true);}
            circ.cx(carry,&source[i]);
            if known_one&&decision_one{circ.ccx(g,&source[i],&target[i]);}
            else if known_one{mixed_mcx(circ,&[(g,true),(&source[i],true),(p1,true)],&target[i],dirty);}
            else{mixed_mcx(circ,&[(g,true),(mask,true),(&source[i],true),(p1,true)],&target[i],dirty);}
        }
        if i==1{for term in &special{let mut cs=term.clone();cs.extend([(mask,true),(&source[1],true)]);if !decision_one{cs.push((p1,true));}super::q794_t10_quotient::gate(circ,&cs,&target[1],dirty);}}
...
fn low_correction(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],source:&[QReg],target:&[QReg],carry:&QReg,special:&[Vec<(&QReg,bool)>],g:&QReg,dirty:&[QReg],j:usize,c1:bool,inverse:bool){
    ...
    for term in special{if super::q794_t10::polish::enabled(){
        circ.cx(&target[257],&target[1]);circ.cx(&target[0],&target[1]);let mut cs=term.clone();cs.extend([(&source[1],true),(&target[1],true)]);correction_gate(circ,&cs,carry,g,dirty);circ.cx(&target[0],&target[1]);circ.cx(&target[257],&target[1]);
    }else{for q in [&target[1],&target[257],&target[0]]{let mut cs=term.clone();cs.extend([(&source[1],true),(q,true)]);correction_gate(circ,&cs,carry,g,dirty);}}}
