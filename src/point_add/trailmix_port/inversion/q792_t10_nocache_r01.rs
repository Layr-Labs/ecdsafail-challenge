...
fn top_loan(circ:&mut Circuit,rank:&[QReg],a:&[QReg],g:&QReg,carry:&QReg,source:&[QReg],dirty:&[QReg]) {
    let(root,gather)=super::q794_handoffs::gather_a(circ,rank,a,source,2,dirty);
...
// The memo contains an arbitrary preserved passenger. Echo its old contribution
// out of the carry, reuse memo XOR mask*source for the sum, then return memo.
fn echoed_cell(circ:&mut Circuit,s:&QReg,t:&QReg,carry:&QReg,mask:&QReg,memo:&QReg){
    circ.cx(s,t);circ.cx(carry,s);
    circ.ccx(memo,t,carry);circ.ccx(mask,s,memo);
    circ.ccx(memo,t,carry);circ.ccx(mask,s,memo);
}
fn inverse_echoed_cell(circ:&mut Circuit,s:&QReg,t:&QReg,carry:&QReg,mask:&QReg,g:&QReg,p1:&QReg,memo:&QReg,dirty:&[QReg]){
    circ.ccx(memo,t,carry);circ.ccx(mask,s,memo);
    circ.ccx(memo,t,carry);
    mixed_mcx(circ,&[(g,true),(memo,true),(p1,true)],t,dirty);
    circ.ccx(mask,s,memo);circ.cx(carry,s);circ.cx(s,t);
}
fn correct_group(circ:&mut Circuit,target:&[QReg],indices:&mut Vec<usize>,g:&QReg,p1:&QReg,memo:&QReg,dirty:&[QReg]){
    let Some(&first)=indices.first()else{return;};
    for &i in &indices[1..]{circ.cx(&target[first],&target[i]);}
    mixed_mcx(circ,&[(g,true),(memo,true),(p1,true)],&target[first],dirty);
    for &i in indices[1..].iter().rev(){circ.cx(&target[first],&target[i]);}
    indices.clear();
}
...
pub(super) fn add_and_clear(circ:&mut Circuit,rank:&[QReg],source:&[QReg],target:&[QReg],a:&[QReg],g:&QReg,cache:&QReg,mask:&QReg,p1:&QReg,helpers:&[QReg],n:usize,c:&[QReg],sm:&[QReg],j:usize,c1:bool,low:bool){
    ...
    let carry=&helpers[0];let memo=&helpers[1];let dirty=&helpers[2..];let start=circ.b.ops.len();
    ...
    // Borrow original Target[A+2]=0 directly. Source[A+1] may retain
    // the held global-A passenger; the masked top already restores arbitrary cargo.
    /* Caller holds the same carry zero through pop, arithmetic and mask return. */circ.cx(g,mask);
    ...
    for i in 0..n{
        ...
        echoed_cell(circ,&source[i],&target[i],carry,mask,memo);
    ...
    for i in (0..n).rev(){
        ...
        inverse_echoed_cell(circ,&source[i],&target[i],carry,mask,g,p1,memo,dirty);
        circ.b.ops.extend(updates.pop().unwrap().into_iter().rev());
    }
    // Every completed output contains the SAME g*p1*passenger offset.
    // The carrier has remained private through every cell and gather.
    // Correct all outputs once, after no carry cell can read them again.
    let mut correction:Vec<_>=(0..n).collect();
    correct_group(circ,target,&mut correction,g,p1,memo,dirty);
    if let Some(live)=lifetime.as_ref(){if live.known_value(){circ.x(live.rail());}}
    circ.b.ops.extend(chart.iter().rev().copied());
    assert!(updates.is_empty());circ.cx(g,mask);/* Caller holds the same carry zero through pop, arithmetic and mask return. */
