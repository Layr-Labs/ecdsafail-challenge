...
impl Range<'_>{
    ...
    fn equality(&mut self,circ:&mut Circuit,value:usize){
        ...
        // During this pure mask XOR, the live g=1 rail can hold the exact
        // high-A predicate. It restores before the next arithmetic cell.
        // Every off-g extension belongs to the literal carry U/U^-1 frame.
        let ts=super::q792_fold20_rank_r01::triples();circ.x(self.g);let at=circ.b.ops.len();super::q792_fold20_r01::table(circ,&self.rank.iter().collect::<Vec<_>>(),ts.iter().map(|t|t[0]==h).collect(),&[],self.g,self.dirty);let high=circ.b.ops[at..].to_vec();
        let mut cs=vec![(self.g,true)];cs.extend(low);mixed_mcx(circ,&cs,self.mask,self.dirty);circ.b.ops.extend(high.into_iter().rev());circ.x(self.g);
    }
