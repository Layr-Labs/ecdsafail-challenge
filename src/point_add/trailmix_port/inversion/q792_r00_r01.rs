...
fn high(circ:&mut Circuit,rank:&[QReg],guard:&QReg,target:&QReg,helpers:&[QReg],scratch:&QReg,axis:usize,h:isize){
    if !(0..4).contains(&h){return;}assert_eq!(rank.len(),4);let offset=if axis==0{0}else{2};
    // Exact full-domain two-bit selector. This cache is restored around the
    // only phase-controlled data center, so an unguarded producer is valid.
    let cs:Vec<_>=(0..2).map(|i|(&rank[offset+i],(h as usize)>>i&1!=0)).collect();
    super::length_recompute::mixed_mcx(circ,&cs,target,helpers);
}
struct Scan<'a> {rank:&'a[QReg],a:&'a[QReg],sm:&'a[QReg],mask:&'a QReg,guard:&'a QReg,hs:&'a QReg,ha:&'a QReg,helpers:&'a[QReg],output:&'a QReg,output_control:&'a QReg,scratch:&'a QReg,j:usize,support_end:usize,low_cache:&'a QReg,top_cache:&'a QReg,low_value:std::cell::Cell<Option<usize>>,top_value:std::cell::Cell<Option<usize>>,compare:bool,upper_cache:Option<std::cell::RefCell<super::q792_r00_lifetime_r01::Upper<'a>>>}
impl Scan<'_> {
    ...
    fn top(&self,circ:&mut Circuit,i:usize,data:&[&QReg],out:&QReg,singleton:bool) {
        ...
        if self.compare&&!singleton{if let Some(cache)=&self.upper_cache{cache.borrow_mut().equality(circ,value);return;}}
        if singleton {
    ...
    fn compare(&self,circ:&mut Circuit,source:&[QReg],target:&[QReg],carry:&QReg,seed:&mut dyn FnMut(&mut Circuit,&QReg,&QReg,&[QReg],usize)) {
        ...
        for i in 1..self.support_end {
            ...
            if next_a!=ah {
                if self.upper_cache.is_none(){high(circ,self.rank,self.guard,self.ha,self.helpers,self.scratch,0,ah);
        ...
        self.prefix_clear(circ,false);
        high(circ,self.rank,self.guard,self.hs,self.helpers,self.scratch,2,sh);
        if let Some(cache)=&self.upper_cache{cache.borrow_mut().clear(circ);}else{high(circ,self.rank,self.guard,self.ha,self.helpers,self.scratch,0,ah);}
...
pub(super) fn phase00_with_support(
    ...
    assert_eq!(rank.len(),4);assert_eq!(a.len(),6);assert_eq!(c.len(),6);
    ...
    let scan=Scan{rank,a,sm,output:sign,output_control:p2,mask:&c[0],guard,
        ...
        top_value:std::cell::Cell::new(None),compare:true,upper_cache:super::q792_r00_lifetime_r01::Upper::new(circ,rank,a,&c[2],&c[3],&c[0],support_end).map(std::cell::RefCell::new)};
...
pub(super) fn emit(circ:&mut Circuit,m:&[QReg],p1:&QReg,p2:&QReg,sign:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,end:usize){
    let n=circ.b.next_qubit;assert_eq!(m.len(),20);
    {let mut terminal:Vec<_>=m[..4].iter().map(|q|(q,true)).collect();terminal.push((&m[5],true));super::length_recompute::mixed_mcx(circ,&terminal,p1,helpers);}
    super::q792_czero_chart_r01::emit(circ,m,helpers,false);
    let(rank,a,cl,sm)=(&m[..4],&m[4..10],&m[10..16],&m[16..20]);
    let mut seed=|c:&mut Circuit,mask:&QReg,carry:&QReg,d:&[QReg],clock:usize|super::q792_r00_seed_r01::emit(c,rank,a,w1,w2,mask,carry,d,clock);
    phase00_with_support(circ,rank,a,cl,sm,p1,p2,sign,w1,w2,helpers,j,end,&mut seed);
    super::q792_czero_chart_r01::emit(circ,m,helpers,true);
    {let mut terminal:Vec<_>=m[..4].iter().map(|q|(q,true)).collect();terminal.push((&m[5],true));super::length_recompute::mixed_mcx(circ,&terminal,p1,helpers);}
    assert_eq!(n,circ.b.next_qubit);
}

#[path="q792_r00_check_r01.rs"]mod check;
