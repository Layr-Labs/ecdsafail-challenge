...
fn guard_delta(circ:&mut Circuit,rank:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,g:&QReg,dirty:&[QReg],j:usize,factors:u8){
    let phase=vec![(p1,true),(p2,false)];let mut gates=Vec::new();
    if factors&1!=0{gates.push(phase.clone());}
    for (bit,kind)in[(2,0),(4,1),(8,2)]{
        if factors&bit==0 || kind==2&&j==3{continue;}
        for term in rank_terms(rank,kind){let mut cs=phase.clone();cs.extend(term);
            if kind!=1{cs.extend(c.iter().enumerate().map(|(i,q)|(q,i==0)));}
            if kind!=0{cs.extend(sm.iter().map(|q|(q,false)));if j&1!=0{cs.push((&c[0],j==1));}}
            gates.push(cs);
        }
    }
    // Same normalization as q794_t10_quotient::gate; grouped exact dirty echo when cheaper.
    super::q793_rank_echo::grouped(circ,"Q793_RANK_ECHO_T10",&gates,rank,g,dirty);
}
