...
#[path="q794_r01_numeric_route.rs"] pub(crate) mod numeric_route;
#[path="q794_r01_numeric_seed.rs"] pub(crate) mod numeric_seed;
#[path="q794_r01_endpoint.rs"] pub(crate) mod endpoint_factor;
...
fn endpoint(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],p1:&QReg,p2:&QReg,g:&QReg,dirty:&[QReg]){
    if endpoint_factor::enabled(){endpoint_factor::emit(circ,rank,a,c,p1,p2,g,dirty);return;}
...
fn quotient(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],g:&QReg,decision:&QReg,cache:&QReg,w1:&[QReg],dirty:&[QReg]){
    if numeric_route::quotient_enabled(){numeric_route::quotient(circ,rank,a,c,g,decision,cache,w1,dirty);return;}
...
/// The seed's defining Boolean function, as an algebraic normal form over 11 variables.
/// SINGLE DEFINITION. The direct expansion and factor::emit must obtain the table here;
/// a second copy that drifts would change the emitted seed without changing any pin.
pub(super) fn seed_anf(shift:usize)->&'static [bool]{
    assert!(shift<2);
    static ANF:std::sync::OnceLock<[Vec<bool>;2]>=std::sync::OnceLock::new();
    &ANF.get_or_init(||std::array::from_fn(|shift|{
...
/// Admissibility of one ANF monomial. SINGLE DEFINITION: the direct expansion and
/// factor::emit's grouping must agree bit for bit or the two paths emit different seeds.
pub(super) fn seed_admissible(m:usize,shift:usize)->bool{
    (m&896).count_ones()<=1 && !(shift==1 && m&64!=0)
}

fn chart_seed(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],g:&QReg,mask:&QReg,ha:&QReg,cache:&QReg,w1:&[QReg],w2:&[QReg],dirty:&[QReg],shift:usize){
    if shift==0&&numeric_seed::enabled(){numeric_seed::emit(circ,rank,a,c,g,mask,ha,cache,w1,w2,dirty,shift);return;}
    ...
    let gathered=if shift==0{
        Some(if numeric_route::seed_enabled(){numeric_route::gather_seed(circ,rank,a,c,cache,w1,dirty)}else{gather_sum(circ,rank,a,c,w1,1,dirty)})
    }else{None};
    ...
    } else { for(m,&on)in anf.iter().enumerate(){if !on||!seed_admissible(m,shift){continue;}
...
impl Scan<'_>{
    ...
    fn fused(&self,circ:&mut Circuit,w1:&[QReg],w2:&[QReg],decision:&QReg){
        ...
        arithmetic::add(circ,self.a,self.c,None,true);chart_seed(circ,self.rank,self.a,self.c,self.g,self.mask,self.ha,self.hs,w1,w2,self.dirty,shift);arithmetic::add(circ,self.a,self.c,None,false);
        ...
        arithmetic::add(circ,self.a,self.c,None,true);chart_seed(circ,self.rank,self.a,self.c,self.g,self.mask,self.ha,self.hs,w1,w2,self.dirty,shift);
...
pub(super) fn signless(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,support_end:usize){
    ...
    borrow_word(circ,rank,a,Some(g),ha,w2,dirty);circ.cx(g,p2);quotient(circ,rank,a,c,g,decision,p2,w1,dirty);
    arithmetic::add(circ,a,c,None,false);
    ...
    arithmetic::add(circ,a,c,None,true);quotient(circ,rank,a,c,g,decision,p2,w1,dirty);circ.cx(g,p2);borrow_word(circ,rank,a,Some(g),ha,w2,dirty);
    // Exact identity E.P.E == P: endpoint (E) and phase_guard (P) both only XOR into g,
    // their controls exclude g, every lender and the a^=c conjugation are restored, and
    // E is an involution; so the endpoint pair around this phase_guard is omitted.
    // g here equals borrowed W1[A] XOR E (unchanged from the previous stream).
