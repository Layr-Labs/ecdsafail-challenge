...
pub(crate) fn template(block:usize,j:usize,inverse:bool,measurement:BitId)->Vec<Op>{
 template_inner(block,j,inverse,measurement,false)
}
pub(crate) fn template_with_prefix(block:usize,j:usize,inverse:bool,measurement:BitId)->Vec<Op>{
 template_inner(block,j,inverse,measurement,true)
}
fn template_inner(block:usize,j:usize,inverse:bool,measurement:BitId,prefix_enabled:bool)->Vec<Op>{
 ...
 let scratch=circ.alloc_qreg("clean_scratch");assert_eq!(circ.b.next_qubit,566);let _scratch_guard=super::length_recompute::CleanScratchGuard::set(&scratch);
 let prefix=if prefix_enabled{circ.alloc_qreg_bits("q807.logical-scan-bank",12)}else{Vec::new()};
 {let _scope=prefix_enabled.then(||super::q794_r01::numeric::prefix_cache::Scope::set(&prefix[..11]));let _consumer_scope=prefix_enabled.then(||{assert_eq!(prefix[11].id(),577);super::q794_r01::numeric::consumer_cache::Scope::set(&prefix[11])});let _capture=Capture::new();super::q794_step::step(&mut circ,&rank,&a,&c,&sm,&p1,&p2,&iteration,&w1,&w2,&helpers,j,block);}
 assert_eq!(circ.b.next_qubit,566+prefix.len()as u32);
 ...
 let(start,end,phase)=if inverse {
  ...
  let mut pc=Circuit::new();pc.b.count_only=false;pc.b.fiat_hash=None;pc.b.next_qubit=566;
  ...
  let(rfirst,tend)=super::shared_step::SCHEDULE_SUPPORTS[block];let _t_top_guard=super::length_recompute::TTopGuard::set(tend);let _r_first_guard=super::length_recompute::RFirstGuard::set(rfirst);
  super::q794_sign::measurement::emit(&mut pc,&rank,&a,&c,&sm,&p1,&p2,&helpers[0],&w1,&w2,&dirty,j,(tend+1).min(258));
  assert_eq!(pc.b.next_qubit,566);(at("before_sign"),at("mbu_after_sign"),pc.into_builder().ops)
 };
