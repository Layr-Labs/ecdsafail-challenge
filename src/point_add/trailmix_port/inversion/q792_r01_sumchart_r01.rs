...
pub(super) fn emit(c:&mut Circuit,rank:&[QReg],a:&[QReg],cl:&[QReg],sm:&[QReg],g:&QReg,d:&[QReg]){
 ...
 for cycle in dc{for &y in &cycle[1..]{transposition(c,&w,Some(g),d,cycle[0],y);}}
...
pub fn run(){
 ...
 emit(&mut c,&rank,&a,&cl,&sm,&g,&d);assert_eq!(n,c.b.next_qubit);let ops=c.into_builder().ops;let maps=[permutation(0),permutation(1)];let mut lanes=0u64;
