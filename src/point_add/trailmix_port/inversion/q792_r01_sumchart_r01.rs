...
fn permutation(carry:usize)->Vec<usize>{
 ...
 for(r,t)in ts.iter().enumerate(){let h=t[0]+t[1]+carry;let sh=t[2];if h>3||h+sh>4{continue;}
  for z in 0..2{if h+sh==4&&z!=0{continue;}let mut nz=z;let slot=match h{0=>sh,1=>t[0]+2*sh,2=>{if sh==2{if t[0]==2{assert_eq!(carry,0);nz=1;6}else{6+t[0]}}else{t[0]+3*sh}},3=>t[0]+4*sh,_=>unreachable!()};assert!(slot<8);let x=r+32*z;let y=h+4*slot+32*nz;assert!(!assigned[x]&&!used[y],"sum chart collision carry={carry} x={x} y={y}");p[x]=y;assigned[x]=true;used[y]=true;
  }
 }
...
fn transposition(c:&mut Circuit,w:&[&QReg],guard:&QReg,d:&[QReg],x:usize,y:usize){let delta=x^y;let pivot=delta.trailing_zeros()as usize;let base=if x>>pivot&1==0{x}else{y};for i in 0..6{if i!=pivot&&delta>>i&1!=0{c.cx(w[pivot],w[i]);}}let mut cs=vec![(guard,true)];cs.extend((0..6).filter(|&i|i!=pivot).map(|i|(w[i],base>>i&1!=0)));mixed_mcx(c,&cs,w[pivot],d);for i in (0..6).rev(){if i!=pivot&&delta>>i&1!=0{c.cx(w[pivot],w[i]);}}}
pub(super) fn emit(c:&mut Circuit,rank:&[QReg],a:&[QReg],cl:&[QReg],sm:&[QReg],d:&[QReg]){
 ...
 for bit in 0..2{let p=permutation(bit);let mut cycles=Vec::new();let mut seen=[false;64];for x in 0..64{if seen[x]{continue;}let mut cycle=Vec::new();let mut at=x;loop{assert!(!seen[at]);seen[at]=true;cycle.push(at);at=p[at];if at==x{break;}}cycles.push(cycle);}
  for reflect in 0..2{let involution=|c:&mut Circuit|{for cycle in &cycles{for i in 0..cycle.len(){let k=(reflect+cycle.len()-i)%cycle.len();if i<k{transposition(c,&w,flag,rest,cycle[i],cycle[k]);}}}};
   involution(c);carry(c,a,cl,flag,rest,bit!=0);involution(c);carry(c,a,cl,flag,rest,bit!=0);
  }
 }
}
pub(super) fn sh(code:usize)->usize{let h=code&3;let slot=code>>2;match h{0=>slot.min(3),1=>slot/2,2=>slot/3,3=>slot/4,_=>unreachable!()}}
