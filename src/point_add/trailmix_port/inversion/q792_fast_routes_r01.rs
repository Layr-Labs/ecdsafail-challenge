...
pub(super) fn final_cargo(c:&mut Circuit,m:&[QReg],p1:&QReg,p2:&QReg,it:&QReg,w1:&[QReg],w2:&[QReg],h:&[QReg],held:bool){
    ...
    for t10 in [false,true]{let flag=|c:&mut Circuit|{mixed_mcx(c,&[(p1,t10),(p2,!t10)],g,d);};let lease=if t10{p2}else{p1};flag(c);super::q792_unfold_lease_r01::emit(c,m,lease,g,d,false);let rank:Vec<_>=m[..4].iter().chain(std::iter::once(lease)).map(QReg::borrowed_alias).collect();
        super::q793_step_r03::final20_provided(c,&rank,&m[4..10],&m[10..16],g,w1,w2,d,t10);
        super::q792_unfold_lease_r01::emit(c,m,lease,g,d,true);flag(c);
