...
fn build(j:usize,numeric:bool)->(crate::point_add::B,usize){
    std::env::set_var("Q793_R01_NUMERIC_SEED",if numeric{"1"}else{"0"});
    ...
    super::seed(&mut c,&rank,&a,&cc,&sm,&g,&mask,&ha,&w1,&w2,&hs,&dirty,j,0);
...
pub(super) fn run(){
    ...
    for j in [1usize,3]{
        let(reference,owned)=build(j,false);let(candidate,owned2)=build(j,true);assert_eq!(owned,owned2);
        let rt=reference.ops.iter().filter(|o|o.kind==K::CCX).count();let ct=candidate.ops.iter().filter(|o|o.kind==K::CCX).count();
        eprintln!("Q793_R01_NUMERIC_SEED_BUILT j={j} reference_ops={} reference_T={rt} candidate_ops={} candidate_T={ct} delta_ops={} delta_T={}",reference.ops.len(),candidate.ops.len(),candidate.ops.len()as isize-reference.ops.len()as isize,ct as isize-rt as isize);
        ...
        for rk in 0..32{let mut seed=0x793_6015eedu64^((j as u64)<<48)^rk as u64;let mut before:Vec<_>=(0..owned).map(|_|rnd(&mut seed)).collect();
            for lane in 0..64{set(&mut before,0,5,lane,rk);set(&mut before,5,6,lane,lane);let c0=(j>>1)&1;let cv=(((lane*29+rk*11)&31)<<1)|c0;set(&mut before,11,6,lane,cv);
                set(&mut before,17,3,lane,0);put(&mut before,21,lane,true);put(&mut before,22,lane,true);put(&mut before,23,lane,false);put(&mut before,24,lane,false);}
    ...
    std::env::set_var("Q793_R01_NUMERIC_SEED","1");
