...
fn conditional_cost(others:usize)->usize{match others{0=>0,1=>1,2=>3,n=>2*n}}
fn data_cost(n:usize)->usize{if n<2{0}else{2*n-3}}
thread_local!{static JOINT:std::cell::RefCell<Option<Vec<(u32,Vec<Vec<(u32,bool)>>)>>> = const{std::cell::RefCell::new(None)};}
fn gates(circ:&mut Circuit,p:Poly<'_>,target:&QReg,dirty:&[QReg],g:&QReg,scratch:&QReg,metadata:&[u32]){
    if JOINT.with(|state|{let mut state=state.borrow_mut();if let Some(rows)=state.as_mut(){rows.push((target.id(),p.iter().map(|term|term.iter().map(|(q,b)|(q.id(),*b)).collect()).collect()));true}else{false}}){return;}
    ...
    for(_, (meta,data))in groups{let data=tidy(data);let data=if super::q792_esop_r01::active(){super::q792_poly_r01::terms(data,1)}else{data};let mut others=Vec::new();let mut has_g=false;for&(q,v)in &meta{if q.id()==g.id(){assert!(v);has_g=true;}else{others.push((q,v));}}assert!(has_g);
        ...
        if factored<plain{
            ...
            for _ in 0..2{
                ...
                let bank:Vec<_>=data.iter().map(|term|{let mut cs=vec![(d,true)];cs.extend(term);cs}).collect();if !super::q792_bank_factor_r01::emit(circ,&bank,target,&dirty[1..],scratch){for cs in &bank{super::paired_clean_mcx::toggle(circ,cs,target,scratch);}}
            }
...
fn joint_gates<'a>(circ:&mut Circuit,pending:Vec<(u32,Vec<Vec<(u32,bool)>>)>,refs:&[&'a QReg],dirty:&[QReg],g:&'a QReg,scratch:&QReg,metadata:&[u32]){
    use std::collections::BTreeMap;
    let lookup=|id:u32|->&'a QReg{*refs.iter().find(|q|q.id()==id).expect("joint codec operand")};
    let mut outputs:Vec<(u32,Poly<'a>)>=Vec::new();
    for(target,terms)in pending{let p=terms.into_iter().map(|t|t.into_iter().map(|(q,b)|(lookup(q),b)).collect()).collect::<Poly>();if let Some((_,old))=outputs.iter_mut().find(|(q,_)|*q==target){old.extend(p);}else{outputs.push((target,p));}}
    struct Group<'a>{target:&'a QReg,meta:Term<'a>,data:Poly<'a>,key:Vec<(u32,bool)>,base:usize,compute:usize,consume:usize}
    let mut groups=Vec::new();
    for(target,p)in outputs{
        let mut by_meta:BTreeMap<Vec<(u32,bool)>,(Term<'a>,Poly<'a>)>=BTreeMap::new();
        for term in tidy(p){let(meta,data):(Term,Term)=term.into_iter().partition(|(q,_)|metadata.contains(&q.id()));let key=meta.iter().map(|(q,b)|(q.id(),*b)).collect();let entry=by_meta.entry(key).or_insert((meta,Vec::new()));entry.1.push(data);}
        for(key,(meta,data))in by_meta{let data=tidy(data);let data=if super::q792_esop_r01::active(){super::q792_poly_r01::terms(data,1)}else{data};
            assert!(meta.iter().any(|(q,b)|q.id()==g.id()&&*b));let count=meta.len()-1;
            let compute=conditional_cost(count);let consume=data.iter().map(|d|data_cost(1+d.len())).sum::<usize>();
            let plain=data.iter().map(|d|conditional_cost(count+d.len())).sum::<usize>();let base=plain.min(2*compute+2*consume);
            groups.push(Group{target:lookup(target),meta,data,key,base,compute,consume});
        }
    }
    let mut occurrences:BTreeMap<Vec<(u32,bool)>,Vec<usize>>=BTreeMap::new();for(i,group)in groups.iter().enumerate(){occurrences.entry(group.key.clone()).or_default().push(i);}
    let mut eligible=Vec::new();for(indices_key,indices)in occurrences{if indices.len()!=2{continue;}let a=&groups[indices[0]];let b=&groups[indices[1]];let held=2*a.compute+2*a.consume+2*b.consume;let old=a.base+b.base;if held<old{eligible.push((old-held,indices_key,indices));}}
    eligible.sort_by(|a,b|b.0.cmp(&a.0).then(a.1.cmp(&b.1)));eligible.truncate(dirty.len().saturating_sub(3));
    let mut selected=BTreeMap::new();for(slot,(_,_,indices))in eligible.iter().enumerate(){selected.insert(indices[0],(slot,false));selected.insert(indices[1],(slot,true));}
    let tail=&dirty[eligible.len()..];assert!(tail.len()>=3);let mut live=vec![false;eligible.len()];
    for(i,group)in groups.into_iter().enumerate(){
        if let Some(&(slot,last))=selected.get(&i){assert_eq!(live[slot],last);let memo=&dirty[slot];
            assert!(group.meta.iter().chain(group.data.iter().flatten()).all(|(q,_)|q.id()!=memo.id()));assert_ne!(memo.id(),group.target.id());
            let consume=|c:&mut Circuit|{for term in &group.data{let mut cs=vec![(memo,true)];cs.extend(term);super::paired_clean_mcx::toggle(c,&cs,group.target,scratch);}};
            consume(circ);let others:Vec<_>=group.meta.iter().copied().filter(|(q,_)|q.id()!=g.id()).collect();
            super::conditional_mcx::guarded(circ,g,&others,memo,scratch,false,&tail[0]);consume(circ);live[slot]=!last;
        }else{
            let poly=group.data.into_iter().map(|term|{let mut all=group.meta.clone();all.extend(term);all}).collect();gates(circ,poly,group.target,tail,g,scratch,metadata);
        }
    }
    assert!(live.iter().all(|x|!*x));
    eprintln!("Q792_JOINT_CODEC held_predicates={} all_existing_dirty=true restored=true",eligible.len());
}
...
pub(super) fn emit(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,g:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,inverse:bool,last:bool){
    ...
    JOINT.with(|state|{assert!(state.borrow().is_none());*state.borrow_mut()=Some(Vec::new());});
    ...
    let pending=JOINT.with(|state|state.borrow_mut().take().unwrap());
    assert!(pending.first().is_none_or(|(target,_)|*target==h.id()));
    let refs:Vec<_>=rank.iter().chain(a).chain(c).chain(sm).chain(w1).chain(w2).chain(helpers).chain([p1,p2,g]).collect();
    joint_gates(circ,pending,&refs,dirty,g,scratch,&metadata);
