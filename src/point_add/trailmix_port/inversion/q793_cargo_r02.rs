...
fn swap(circ:&mut Circuit,rank:&[QReg],terms:&Terms<'_>,left:&QReg,right:&QReg,dirty:&[QReg]) {
    assert_ne!(left.id(),right.id());
    circ.cx(right,left);
    let with_left:Vec<Vec<(&QReg,bool)>>=terms.iter().map(|term|{assert!(term.iter().all(|&(q,_)|q.id()!=left.id()&&q.id()!=right.id()));let mut cs=term.clone();cs.push((left,true));cs}).collect();
    super::q793_rank_echo::grouped(circ,"Q793_RANK_ECHO_CARGO",&with_left,rank,right,dirty);
    circ.cx(right,left);
}
fn flip(circ:&mut Circuit,rank:&[QReg],terms:&Terms<'_>,target:&QReg,dirty:&[QReg]) {
    super::q793_rank_echo::grouped(circ,"Q793_RANK_ECHO_CARGO",terms,rank,target,dirty);
}
...
pub(super) fn inbound(circ:&mut Circuit,rank:&[QReg],a:&[QReg],w1:&[QReg],w2:&[QReg],from:usize,terms:&Terms<'_>,dirty:&[QReg]) {
    ...
    swap(circ,rank,&a253,&w1[255],&w2[255],dirty);
    if from==0 {swap(circ,rank,&a253,&w1[253],&w2[255],dirty);}
    if from==0 {swap(circ,rank,&a254,&w1[254],&w2[257],dirty);}
    else {swap(circ,rank,&a254,&w2[254],&w2[257],dirty);flip(circ,rank,&a254,&w2[254],dirty);}
}
...
pub(super) fn head_to_two(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],w1:&[QReg],w2:&[QReg],terms:&Terms<'_>,dirty:&[QReg]) {
    ...
    for av in [253,254] {
        ...
        if av==253 {swap(circ,rank,&cs,&w1[253],&w1[255],dirty);}
        else {swap(circ,rank,&cs,&w1[254],&w2[255],dirty);flip(circ,rank,&cs,&w1[254],dirty);}
    }
}
...
pub(super) fn before_entry(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,w2:&[QReg],dirty:&[QReg]) {
    ...
    swap(circ,rank,&terms,&w2[256],&w2[255],dirty);flip(circ,rank,&terms,&w2[256],dirty);
}
pub(super) fn after_entry(circ:&mut Circuit,rank:&[QReg],a:&[QReg],sign:&QReg,w2:&[QReg],dirty:&[QReg]) {
    ...
    flip(circ,rank,&terms,&w2[256],dirty);swap(circ,rank,&terms,&w2[256],&w2[255],dirty);
    swap(circ,rank,&terms,&w2[254],&w2[255],dirty);
}
...
pub(super) fn newborn_regular(circ:&mut Circuit,rank:&[QReg],a:&[QReg],sign:&QReg,left:&QReg,right:&QReg,dirty:&[QReg]) {
    ...
    swap(circ,rank,&terms,left,right,dirty);flip(circ,rank,&terms,left,dirty);
}
...
pub(super) fn finish_c1(circ:&mut Circuit,rank:&[QReg],a:&[QReg],w1:&[QReg],w2:&[QReg],terms:&Terms<'_>,dirty:&[QReg]) {
    ...
    let cs=at(rank,a,254,terms);swap(circ,rank,&cs,&w2[256],&w2[255],dirty);
}
...
pub(super) fn normalize_old11(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,w2:&[QReg],j:usize,dirty:&[QReg]) {
    ...
    let _=c;swap(circ,rank,&terms,&w2[256],&w2[255],dirty);
}
