...
fn grouped(circ:&mut Circuit,gates:&[Vec<(&QReg,bool)>],rank:&[QReg],g:&QReg,dirty:&[QReg]){
    if !mux::active("Q793_RANK_ECHO_R01")||dirty.len()<2{for cs in gates{gate(circ,cs,g,dirty);}return;}
