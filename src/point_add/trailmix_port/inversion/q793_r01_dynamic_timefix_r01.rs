...
pub(super) fn borrow_ha(circ:&mut Circuit,rank:&[QReg],a:&[QReg],g:&QReg,ha:&QReg,w2:&[QReg],dirty:&[QReg]){
...
pub(super) fn short_permutation(circ:&mut Circuit,b:[&QReg;3],base:&[(&QReg,bool)],x:usize,dirty:&[QReg]){
...
/// Caller supplies an exact phase01 normal-domain clean guard.
pub(super) fn provided_normal(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,support_end:usize){
    let g=&helpers[0];let ha=&helpers[1];let decision=&helpers[2];let dirty=&helpers[3..];
    circ.cx(g,p2);super::q793_r01_a1normalize_timefix_r01::normalize(circ,rank,a,c,sm,g,p1,w1,w2,dirty,j,false);
    borrow_ha(circ,rank,a,g,ha,w2,dirty);super::q793_r01_routes_v1::quotient(circ,rank,a,c,w1,g,p2,decision);
    super::q793_r01_normal_timefix_r01::emit(circ,rank,a,c,sm,g,p1,p2,ha,decision,w1,w2,dirty,j,support_end,false);
...
pub(super) fn provided_endpoint(circ:&mut Circuit,rank:&[QReg],a:&[QReg],p1:&QReg,g:&QReg,w1:&[QReg],w2:&[QReg],dirty:&[QReg],j:usize,value:usize){
if value==254{
    // Logical t=1 at A0: cancel its physical-head interpretation.
    for flag in aflags(rank,a,0){let mut cs=vec![(g,true)];cs.extend(flag);gate(circ,&cs,p1,dirty);}
    // p1 is a zero cache only under g. It marks the excluded A0 branch.
...
/// Own third-passenger hypothesis: the coefficient gap just below the
/// residual head when C>0. This diagnostic does not claim a whole-Q result.
pub(super) fn gap_probe(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],pass:&QReg,j:usize,end:usize){
    let exchange=|circ:&mut Circuit|{
        let at=circ.b.ops.len();let(root,_)=super::q794_handoffs::gather_a(circ,rank,a,w2,3,helpers);let route=circ.b.ops[at..].to_vec();
        circ.cx(root,pass);let mut base=vec![(p1,false),(p2,true),(pass,true)];gate(circ,&base,root,helpers);for term in [cflags(rank,c,0),cflags(rank,c,1)].concat(){let mut cs=base.clone();cs.extend(term);gate(circ,&cs,root,helpers);}circ.cx(root,pass);circ.b.ops.extend(route.into_iter().rev());
    };
    exchange(circ);signless(circ,rank,a,c,sm,p1,p2,w1,w2,helpers,j,end);exchange(circ);
}
#[path="q792_r01_gap_check_r01.rs"]mod gap_check;
pub fn run_gap(){gap_check::run();}

pub(super) fn signless_nohs(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,support_end:usize){
    assert!(helpers.len()>=23);let start=circ.b.ops.len();let owned=(circ.b.next_qubit,circ.b.active_qubits);
    let g=&helpers[0];let ha=&helpers[1];let decision=&helpers[2];let dirty=&helpers[3..];
    super::q793_loans::global_a(circ,rank,a,g,w1,&w2[258],None,dirty);
    normal_guard(circ,rank,a,c,sm,p1,p2,g,dirty,j);
    circ.cx(g,p2);super::q793_r01_a1normalize_timefix_r01::normalize(circ,rank,a,c,sm,g,p1,w1,w2,dirty,j,false);
    borrow_ha(circ,rank,a,g,ha,w2,dirty);super::q793_r01_routes_v1::quotient(circ,rank,a,c,w1,g,p2,decision);
    super::q792_r01_nohs_normal_r01::emit(circ,rank,a,c,sm,g,p1,p2,ha,decision,w1,w2,dirty,j,support_end,false);
    super::q793_r01_routes_v1::quotient(circ,rank,a,c,w1,g,p2,decision);borrow_ha(circ,rank,a,g,ha,w2,dirty);
    super::q793_r01_a1normalize_timefix_r01::normalize(circ,rank,a,c,sm,g,p1,w1,w2,dirty,j,true);circ.cx(g,p2);
    normal_guard(circ,rank,a,c,sm,p1,p2,g,dirty,j);
    endpoints(circ,rank,a,c,p1,p2,g,w1,w2,dirty,j);
...
#[path="q792_r01_nohs_check_r01.rs"]mod nohs_check;
pub fn run_nohs(){nohs_check::run();}
