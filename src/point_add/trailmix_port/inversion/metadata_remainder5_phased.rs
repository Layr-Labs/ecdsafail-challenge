...
struct Scan<'a> {rank:&'a[QReg],a:&'a[QReg],sm:&'a[QReg],mask:&'a QReg,guard:&'a QReg,hs:&'a QReg,ha:&'a QReg,helpers:&'a[QReg],output:&'a QReg,output_control:&'a QReg,scratch:&'a QReg,j:usize,support_end:usize,low_cache:&'a QReg,top_cache:&'a QReg,low_value:std::cell::Cell<Option<usize>>,top_value:std::cell::Cell<Option<usize>>}
impl Scan<'_> {
    ...
    fn prefix_mode(&self)->usize{static MODE:std::sync::OnceLock<usize>=std::sync::OnceLock::new();*MODE.get_or_init(||{let n=std::env::var("Q795_R00_PREFIX").ok().map(|v|v.parse().unwrap()).unwrap_or(0);assert!(n<=3);n})}
...
pub(super) fn phase00_with_support(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],guard:&QReg,p2:&QReg,sign:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,support_end:usize) {
    ...
    let scan=Scan{rank,a,sm,output:sign,output_control:p2,mask:&c[0],guard,hs:&c[1],ha:&c[2],scratch:&c[3],helpers,j,support_end,low_cache:&c[4],top_cache:&c[5],low_value:std::cell::Cell::new(None),top_value:std::cell::Cell::new(None)};
