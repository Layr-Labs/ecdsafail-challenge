...
//! On g: S<=2, A>=3, A<=252, M<=256, C>=1, SM[0..4]=0; rank5 is unfolded.
...
pub fn run(){
 ...
 for j in 0..4{let mut c=Circuit::new();c.b.count_only=false;c.b.fiat_hash=None;let q=c.alloc_qreg_bits("physical T low unpack r04",557);
  ...
  eprintln!("T_LOW_PHYSICAL_UNPACK_PASS j={j} T={} N={} cases={} inverse=true phase=0",ops.iter().filter(|o|o.kind==K::CCX).count(),ops.len(),data.len()/144);
 }eprintln!("T_LOW_PHYSICAL_UNPACK_TOTAL_PASS lanes={total} conditional_SM_loans=3 paid_q1_address=true whole_Q792=false");
}
