...
pub(crate) use quotient_code::selftest::run_generic as midq_generic_quotient_code_selftest;
...
#[path = "d_alignment_prep.rs"]
pub(crate) mod d_alignment_prep;
#[path = "d_alignment_ledger.rs"]
pub(crate) mod d_alignment_ledger;
#[path = "d_position_cache.rs"]
pub(crate) mod d_position_cache;

#[path = "d_alignment_iteration.rs"]
pub(crate) mod d_alignment_iteration;
#[path = "d_alignment_driver.rs"]
mod d_alignment_driver;
...
fn lowq_borrow_passenger_carry_enabled() -> bool {
    ...
    lowq_one_a_elim_enabled() && lowq_clz_diff_const_fold_enabled()
}
...
fn trailmix_q_width_step(wq: usize, wa: usize, wb: usize, wca: usize, wcb: usize) -> usize {
    if let Some(width) = qretain::fixed_width() { return width; }
...
fn prefix_no_terminal_eligible(step: usize) -> bool {
    ...
    (0..MIDQ_PZ_CUT).all(|i| {
        let (_, _, ca, cb, _) = reg_widths(i);
        trailmix_cacb_width(ca.max(cb)) < 256
    })
}
...
pub(crate) fn shrunken_pz_invert_forward(
    ...
    let cache_initial_t = c.b.counted_kind_ops[OperationType::CCX as usize] + c.b.counted_kind_ops[OperationType::CCZ as usize];
    let mut frame = d_alignment_driver::Frame::start(c, steps, aa, bb, qq, s_rot, counter);
    if trace_steps {
        let t = c.b.counted_kind_ops[OperationType::CCX as usize] + c.b.counted_kind_ops[OperationType::CCZ as usize] - cache_initial_t;
        eprintln!("MIDQ_PZ_CACHE_INITIAL forward cut={steps} emitted_toffoli={t} separate_from_iteration_sums=true");
    }
    for i in 0..steps {
    ...
    let frame_boundary_start = c.b.current_ops_len();
    if let Some(frame) = frame {
        frame.finish_forward(c, aa, bb, cca, ccb, qq);
    }
    if trace_steps {
        let boundary_t = c.b.ops[frame_boundary_start..c.b.current_ops_len()].iter()
            .filter(|op| matches!(op.kind, OperationType::CCX | OperationType::CCZ)).count();
        eprintln!("MIDQ_PZ_FRAME_BOUNDARY forward cut={steps} emitted_toffoli={boundary_t} separate_from_iteration_sums=true");
    }
    if trace_steps {
...
pub(crate) fn shrunken_pz_invert_backward(
    ...
    let frame_boundary_start = c.b.current_ops_len();
    let mut frame = d_alignment_driver::Frame::resume(c, steps, aa, bb, cca, ccb, qq, s_rot, counter);
    if trace_steps {
        let boundary_t = c.b.ops[frame_boundary_start..c.b.current_ops_len()].iter()
            .filter(|op| matches!(op.kind, OperationType::CCX | OperationType::CCZ)).count();
        eprintln!("MIDQ_PZ_FRAME_BOUNDARY backward cut={steps} emitted_toffoli={boundary_t} separate_from_iteration_sums=true");
    }
    ...
    let cache_final_t = c.b.counted_kind_ops[OperationType::CCX as usize] + c.b.counted_kind_ops[OperationType::CCZ as usize];
    if let Some(frame) = frame {
        frame.finish_backward(c, aa, bb);
    }
    if trace_steps {
        let t = c.b.counted_kind_ops[OperationType::CCX as usize] + c.b.counted_kind_ops[OperationType::CCZ as usize] - cache_final_t;
        eprintln!("MIDQ_PZ_CACHE_FINAL backward cut={steps} emitted_toffoli={t} separate_from_iteration_sums=true");
    }
    if trace_steps {
        for cut in [280usize, 320, 340, 360, 380, 400, 420, 440, 530]
            .into_iter()
...
#[path = "hybrid_profile.rs"]
pub(crate) mod hybrid_profile;
#[path = "hybrid_margin.rs"]
pub(crate) mod hybrid_margin;
#[path = "qretain.rs"]
pub(crate) mod qretain;
const MIDQ_PZ_CUT: usize = hybrid_profile::SELECTED.pz_cut;
const MIDQ_TAIL_ROUNDS: usize = hybrid_profile::SELECTED.rounds;
...
// The selected count width holds the initial value_widths[0] accumulator.
// The literal zero-input CTZ sentinel is width-1. Baseline keeps seven bits.
const MIDQ_CTZ_BITS: usize = hybrid_profile::SELECTED.ctz_bits;

// Each generated width includes the boundary pair and its pre-halve sum.
// Calibration provenance is recorded per profile. The baseline is copied
// verbatim from the submitted200,000-input empirical schedule.
const MIDQ_TAIL_VALUE_WIDTH: [u8; MIDQ_TAIL_ROUNDS + 1] = hybrid_profile::width_array();
...
fn midq_tail_forward_with_parity(
    ...
    let mut quotient_code = (quotient_code::enabled() && quotient_code::supports_width(q.len())).then(|| quotient_code::compress(c, ca, cb, q));
    ...
    // k=ctz(a)^ctz(b) plus one bit selecting a; this reversibly merges the two
    // count words on the full reachable Euclidean support.
    ...
    for round in 0..taped_rounds {
        ...
        if round + 1 == counter_tape::BITS && tail_metadata_codec::enabled()
            ...
            && quotient_code::legacy_rank_width(original_q_width)
        {
...
fn midq_tail_backward_with_parity(
    ...
    *cb = c.alloc_qreg_bits("midq.cb", ca.len());
    ...
    if let Some(code) = quotient_code {
        quotient_code::restore(c, ca, cb, q, code, original_q_width);
    }
...
pub fn shrunken_pz_divide_forward(
    ...
    let mut qq = c.alloc_qreg_bits("shpzdiv.q", qretain::initial_width(q0));
...
pub fn shrunken_pz_divide_cancel(
    ...
    let mut qq = c.alloc_qreg_bits("shpzcan.q", qretain::initial_width(q0));
