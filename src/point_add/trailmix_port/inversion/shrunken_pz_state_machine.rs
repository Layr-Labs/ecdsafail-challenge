...
pub(crate) fn shrunken_pz_invert_forward(
    ...
    midq_tail_enabled().then(|| midq_tail_forward(c, aa, bb, cca, ccb, qq, parity))
}
...
pub(crate) fn shrunken_pz_invert_backward(
    ...
    if let Some(state) = tail_state {
        midq_tail_backward(c, aa, bb, cca, ccb, qq, parity, state);
    }
...
fn midq_controlled_mod_halve(c: &mut Circuit, control: &QReg, value: &[QReg]) {
    use crate::point_add::trailmix_port::arith::const_add::controlled_add_const;
    ...
    controlled_add_const(c, &parity, value, &SECP256K1_P_LE);
...
fn midq_controlled_mod_halve_inverse(c: &mut Circuit, control: &QReg, value: &[QReg]) {
    use crate::point_add::trailmix_port::arith::const_add::controlled_sub_const;
    use crate::point_add::trailmix_port::mod_arith::SECP256K1_P_LE;
    assert_eq!(value.len(), 257);
    let parity = c.alloc_qreg("midq.chalve.parity");
    c.ccx(control, &value[255], &parity);
    midq_controlled_shift_right_inverse(c, control, value);
    controlled_sub_const(c, &parity, value, &SECP256K1_P_LE);
    c.ccx(control, &value[0], &parity);
    c.zero_and_free(parity);
}
...
fn midq_apply_inverse_power_of_two(
    ...
    if inverse {
        for shift in (0..max_shift).rev() {
            ...
            midq_controlled_mod_halve_inverse(c, &control, value);
            compare_geq_const(c, exponent, &[(shift + 1) as u8], &control);
        ...
        for shift in 0..max_shift {
            let control = c.alloc_qreg("midq.shift.active");
            compare_geq_const(c, exponent, &[(shift + 1) as u8], &control);
            midq_controlled_mod_halve(c, &control, value);
            compare_geq_const(c, exponent, &[(shift + 1) as u8], &control);
            c.zero_and_free(control);
        }
    }
...
fn midq_const_fold(
    ...
    f[4] = 0x01;
    if subtract {
        controlled_sub_const(c, control, &target[..256], &f);
    } else {
        controlled_add_const(c, control, &target[..256], &f);
    }
}

fn midq_signed_mod_add_fast(c: &mut Circuit, sign: &QReg, source: &[QReg], target: &[QReg]) {
    use crate::point_add::trailmix_port::arith::cuccaro::add_cuccaro_with_overflow;
    use crate::point_add::trailmix_port::inversion::shrunken_pz_primitives::borrow_compare_refs;
    for q in &target[..256] {
        c.cx(sign, q);
    }
    add_cuccaro_with_overflow(c, target, &source[..256]);

    // In the complemented subtraction frame, the unsigned carry selects the
    // exact pseudo-Mersenne correction +f modulo 2^256.
    midq_const_fold(c, target, source, &target[256], false);
    let lhs: Vec<&QReg> = target[..256].iter().collect();
    let rhs: Vec<&QReg> = source[..256].iter().collect();
    borrow_compare_refs(c, &lhs, &rhs, &target[256]);
    for q in &target[..256] {
        c.cx(sign, q);
    }
}
...
fn midq_value_round_forward(
    ...
    c.x(sign);
    ctrl_add(c, sign, &target_refs, &source_refs);
    c.x(sign);
    ctrl_sub(c, sign, &target_refs, &source_refs);
...
fn midq_tail_forward(
    ...
    let ctz_a = midq_compute_ctz(c, a, "midq.ctz_a");
    let ctz_b = midq_compute_ctz(c, b, "midq.ctz_b");
    rotate_right(c, a, &ctz_a);
    rotate_right(c, b, &ctz_b);
    ...
    controlled_field_neg(c, parity, ca);
    c.x(parity);
    controlled_field_neg(c, parity, cb);
    c.x(parity);
    midq_apply_inverse_power_of_two(c, ca, &ctz_a, false);
    midq_apply_inverse_power_of_two(c, cb, &ctz_b, false);

    let mut tape = Vec::with_capacity(MIDQ_TAIL_ROUNDS);
    for round in 0..MIDQ_TAIL_ROUNDS {
        ...
        let sign = c.alloc_qreg(&format!("midq.sign[{round}]"));
        if round % 2 == 0 {
            midq_value_round_forward(c, a, b, &sign, next_width);
            midq_mod_signed_add_halve(c, cb, ca, &sign, false);
        } else {
            midq_value_round_forward(c, b, a, &sign, next_width);
    ...
    controlled_field_neg(c, a.last().expect("tail a has a sign bit"), ca);
    controlled_field_neg(c, b.last().expect("tail b has a sign bit"), cb);
...
fn midq_tail_backward(
    ...
    *cb = c.alloc_qreg_bits("midq.cb", 257);
    for (keep, duplicate) in ca.iter().zip(cb.iter()) {
        c.cx(keep, duplicate);
    }
    controlled_field_neg(c, b.last().expect("tail b has a sign bit"), cb);
    ...
    for round in (0..MIDQ_TAIL_ROUNDS).rev() {
    ...
    midq_apply_inverse_power_of_two(c, cb, &ctz_b, true);
    midq_apply_inverse_power_of_two(c, ca, &ctz_a, true);
    c.x(parity);
    controlled_field_neg(c, parity, cb);
    c.x(parity);
    controlled_field_neg(c, parity, ca);
    rotate_left(c, b, &ctz_b);
    rotate_left(c, a, &ctz_a);
    midq_uncompute_ctz(c, b, ctz_b);
    midq_uncompute_ctz(c, a, ctz_a);
...
pub fn shrunken_pz_divide_forward(
    ...
    if let Some(tail_state) = tail_state {
        ...
        // retains its compact transcript beside the two signed-unit values.
        ...
        controlled_field_neg(c, &sgn, &lambda);
...
pub fn shrunken_pz_divide_cancel(
    ...
    if let Some(tail_state) = tail_state {
        ...
        controlled_field_neg(c, &sgn, &temp);
