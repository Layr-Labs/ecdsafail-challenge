...
fn midq_tail_forward(
    ...
    midq_field_neg(c, parity, ca, cb);
    c.x(parity);
    midq_field_neg(c, parity, cb, ca);
    c.x(parity);
    ...
    midq_field_neg(c, a.last().expect("tail a has a sign bit"), ca, cb);
    midq_field_neg(c, b.last().expect("tail b has a sign bit"), cb, ca);
