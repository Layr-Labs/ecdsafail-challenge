...
pub(crate) use packed::selftest_all as midq_packed_selftest;

#[path = "sign_storage.rs"]
pub(crate) mod sign_storage;

#[path = "payload_sign_loan.rs"]
mod payload_sign_loan;

#[path = "midq_odd_values.rs"]
pub(crate) mod midq_odd_values;
...
/// MIDQ_KG_ZERO_LAYER=1: emit the k = -1 (all-zero source) layer of the KG
/// bit-length ladder so `pos` ends at the all-ones sentinel on `src == 0`
/// (packed prefix, tools/spike/packed_design.md 2.4 / 9; required by
/// `packed::aligned_scan::aligned_scan_bottom`). Default off: the shipped
/// 974/999 circuits are unchanged.
fn kg_zero_layer_enabled() -> bool {
    std::env::var("MIDQ_KG_ZERO_LAYER").ok().as_deref() == Some("1")
}
...
fn bit_length_lean_middle(
    ...
    let pos_refs: Vec<&QReg> = pos.iter().collect();
    bit_length_lean_middle_refs(circ, src, &pos_refs, body);
}

// The packed scan can borrow an already clean metadata wire for its position.
fn bit_length_lean_middle_refs(
    circ: &mut Circuit,
    src: &[&QReg],
    pos: &[&QReg],
    body: impl FnOnce(&mut Circuit) -> bool,
    ...
    fn deposit_step(
        ...
        // i == n is the k = -1 layer: AND(ctrls) = "no 1 anywhere" (src == 0).
        // Skipped by default (bitlen(0) then reads as 1: pos = 0). With
        // MIDQ_KG_ZERO_LAYER=1 (packed prefix, tools/spike/packed_design.md 2.4)
        // it XORs the gray difference (-1) ^ 0 = all-ones into pos, so an
        // all-zero src leaves pos = 2^|pos| - 1 (the sentinel 63 for 6 bits,
        // never a valid index) instead of the ambiguous 0. 1 T per ladder pass.
        if i > n || (i == n && !zero_layer) {
            return;
        }
        let gd = if i == n { usize::MAX } else { let k = n - 1 - i; k ^ (k + 1) };
    ...
    let zero_layer = kg_zero_layer_enabled();
    ...
    let done = kg.forward(circ, |c, i, ctrls| {
        deposit_step(c, i, ctrls, pos, flag.as_ref(), n, zero_layer)
    }); // pos -> p
    let clean = body(circ);
    if clean {
...
/// Exhaustive value/phase/ancilla check of the windowed role compare (bit0 ++
/// top window) against its classical specification, with a forced small window
/// so the deep-tie boundary is exercised at every width.
pub(crate) fn midq_gate_compare_selftest() {
    use crate::circuit::analyze_ops;
    use crate::sim::Simulator;
    use sha3::{digest::{ExtendableOutput, Update}, Shake256};
    std::env::set_var("MIDQ_MEASURE_COMPARE", "1");
    std::env::set_var("MIDQ_GATE_COMPARE_WINDOW", "3");
    let mut checked = 0usize;
    for n in 2..=10usize {
        let mut circ = Circuit::new();
        let a = circ.alloc_qreg_bits("test.a", n);
        let b = circ.alloc_qreg_bits("test.b", n);
        let witness = circ.alloc_qreg("test.witness");
        let flag = circ.alloc_qreg("test.cmp");
        let ids: Vec<QubitId> = a
            .iter()
            .chain(b.iter())
            .chain([&witness])
            ...
            .collect();
        {
            let ar = gate_compare_refs(&a);
            let br = gate_compare_refs(&b);
            borrow_compare_refs(&mut circ, &ar, &br, &flag);
            circ.cx(&flag, &witness);
            clear_borrow_compare_refs(&mut circ, &ar, &br, &flag);
            circ.zero_and_free(flag);
        }
        let (nq, nb, _, _) = analyze_ops(circ.b.ops.iter());
        let mut seed = Shake256::default();
        seed.update(b"midq-gate-compare-test-v1");
        seed.update(&n.to_le_bytes());
        let mut rng = seed.finalize_xof();
        let mut sim = Simulator::new(nq as usize, nb as usize, &mut rng);
        let lo = n.saturating_sub(3);
        let windowed_lt = |av: usize, bv: usize| {
            if lo == 0 {
                return av < bv;
            }
            ((av & 1) | ((av >> lo) << 1)) < ((bv & 1) | ((bv >> lo) << 1))
        };
        for first in (0..1usize << ids.len()).step_by(64) {
            ...
            assert_eq!(sim.phase & mask, 0, "compare phase error n={n}");
            for shot in 0..valid {
                let initial = first + shot;
                let av = initial & ((1 << n) - 1);
                let bv = (initial >> n) & ((1 << n) - 1);
                let expected = initial ^ (usize::from(windowed_lt(av, bv)) << (2 * n));
                for (index, &id) in ids.iter().enumerate() {
                    assert_eq!(
                        (sim.qubit(id) >> shot) & 1,
                        ((expected >> index) & 1) as u64,
                        "value mismatch n={n} a={av} b={bv}"
                    );
                }
                checked += 1;
            }
            ...
            assert!(sim.qubits.iter().all(|&q| q & mask == 0), "dirty compare scratch n={n}");
        }
    }
    eprintln!("MIDQ_GATE_COMPARE_SELFTEST PASS: {checked} exhaustive inputs, value/phase/ancilla");
}
...
/// Window size for the hybrid role compare (MIDQ_GATE_COMPARE_WINDOW, default
/// 112; 0 disables windowing). The role compare feeds ONLY the branch select
/// and must round-trip exactly between its compute site (gcd pair, step start)
/// and its clear site (cofactor pair, post-multiply).
///
/// Safety argument (value-dependent discipline, same as the clz windows):
/// a windowed compare can only flip when the operands tie on every bit in
/// [lo, W) and differ on some bit in [1, lo). For the gcd pair, bitlens are
/// > W-78 on schedule-fitting inputs (the clz-window validity the nonce
/// search already enforces) and every fitting shift is <= 31 (s_rot is 5
/// bits), so the highest differing bit is > W-78-31 = W-109; window 112
/// (+2 margin +1 for the offset bit) makes a gcd-pair flip structurally
/// impossible on fitting inputs. The converged held state is exactly (0,1),
/// which the included bit0 decides exactly. For the cofactor pair a deep tie
/// below the window is a ~2^-112-class event per instance, far under the
/// 2^-78 tie residue the narrow_lt offset decisions already tolerate; the
/// nonce search rejects any nonce whose draws exhibit one (the support model
/// counts this compare's flips exactly).
fn gate_compare_window() -> usize {
    env_usize("MIDQ_GATE_COMPARE_WINDOW", 112)
}

/// Refs for the windowed role compare over `reg` (width W): bit 0 followed by
/// [W-win, W), i.e. the numeric compare of `(bit0, top window)` with the
/// window most significant. Exact unless the operands tie above bit 0 and
/// below the window (the deep-tie case bounded above). Full slice when
/// W <= win.
fn gate_compare_refs(reg: &[QReg]) -> Vec<&QReg> {
    let w = reg.len();
    let win = gate_compare_window();
    let lo = w.saturating_sub(win);
    if lo == 0 || win == 0 {
        return reg.iter().collect();
    }
    let mut v: Vec<&QReg> = Vec::with_capacity(w - lo + 1);
    v.push(&reg[0]);
    v.extend(reg[lo..].iter());
    v
}
...
// This is a specialization of the fixed production prefix, not a promise
// about arbitrary states accepted by the generic PZ step API. See the proof.
fn prefix_no_terminal_eligible(step: usize) -> bool {
    if std::env::var("MIDQ_PREFIX_NO_TERMINAL").ok().as_deref() != Some("1")
        || !midq_tail_enabled() || step >= MIDQ_PZ_CUT
    { return false; }
    // SPIKE (env-gated, default off): rows >= MIDQ_PREFIX_TERMINAL_FROM run the
    // exact terminal-aware step (counter/done). The popcount cache is erased
    // from the counter just before that row (see the drivers) so the counter
    // is the zero termination counter there, as the counter/tape codec expects.
    if step >= prefix_terminal_from() { return false; }
    // Applied on every prefix step. Below row 371 no coefficient row is 256
    // wide, so ca=p (a terminal) is impossible there by the row invariant; on
    // the later rows a terminal is possible and such an input is a support
    // miss (tail_value_misses counts it), not a circuit case: the
    // terminal-aware step variant shares the counter with the popcount cache
    // and the counter/tape codec and is not admitted on this route.
    true
}

pub(crate) fn prefix_terminal_from() -> usize {
    env_usize("MIDQ_PREFIX_TERMINAL_FROM", usize::MAX)
}

#[allow(clippy::too_many_arguments)]
fn swap_no_terminal(c: &mut Circuit, aa: &[QReg], bb: &[QReg],
    cca: &[QReg], ccb: &[QReg], qq: &[QReg], parity: &QReg) {
    let q_zero = c.alloc_qreg("sw.qz.no_terminal");
    or_is_zero(c, qq, &q_zero);
    // B>0, and q=0 implies A>0 because a true terminal would need ca=p.
    // Both row swaps preserve q and are self-inverse on this support.
    for (a, b) in aa.iter().zip(bb) { c.cswap(&q_zero, a, b); }
    for (a, b) in cca.iter().zip(ccb) { c.cswap(&q_zero, a, b); }
    c.cx(&q_zero, parity);
    clear_zero_predicate(c, qq, &q_zero, false);
    c.zero_and_free(q_zero);
}

#[path = "prefix_popcount.rs"]
mod prefix_popcount;
pub(crate) use prefix_popcount::selftest as midq_prefix_popcount_selftest;
pub(crate) use prefix_popcount::profile as midq_prefix_popcount_profile;

#[path = "prefix_no_terminal_selftest.rs"]
mod prefix_no_terminal_selftest;
pub(crate) use prefix_no_terminal_selftest::run as midq_prefix_no_terminal_selftest;
pub(crate) use prefix_no_terminal_selftest::profile as midq_prefix_no_terminal_profile;
pub(crate) use prefix_no_terminal_selftest::wide as midq_prefix_terminal_wide_selftest;
...
fn swap_and_done_forward(
    ...
    if no_terminal {
        let zero_word = if prefix_popcount::enabled(counter) {
            &counter[..prefix_popcount::BITS]
        } else { qq };
        swap_no_terminal(c, aa, bb, cca, ccb, zero_word, parity);
...
pub(crate) fn shrunken_pz_pass_step(
    ...
    let no_terminal = prefix_no_terminal_eligible(i);
    ...
    if inverse {
        let active = undo_done_and_swap(c, aa, bb, cca, ccb, qq, counter, parity, no_terminal);
        if inline_active {
            ...
            let car = gate_compare_refs(cca);
            let cbr = gate_compare_refs(ccb);
            ...
            if no_terminal && prefix_popcount::enabled(counter) {
                prefix_popcount::update(c, counter, &role, &active, true);
            }
            let aar = gate_compare_refs(aa);
            let bbr = gate_compare_refs(bb);
        ...
        swap_and_done_forward(c, aa, bb, cca, ccb, qq, counter, parity, active, no_terminal);
    } else if recompute_gate_predicate {
        let active = compute_active(c, active_counter);
...
pub(crate) fn shrunken_pz_invert_forward(
    ...
    s_rot: &mut Vec<QReg>,
    off: &mut Option<QReg>,
    borrowed_carry: &mut Option<QReg>,
) -> Option<MidqTailState> {
    if std::env::var_os("MIDQ_PAYLOAD_SIGN_LOAN_SELFTEST").is_some() {
        payload_sign_loan::selftest();
        std::process::exit(0);
    }
    ...
    for i in 0..steps {
        ...
        if i == prefix_terminal_from() && prefix_popcount::enabled(counter) {
            // SPIKE: counter := 0 (C -= popcount(q)) before the terminal-aware rows.
            prefix_popcount::from_quotient(c, counter, qq, true);
        }
    ...
    if midq_tail_enabled() {
        if prefix_popcount::enabled(counter) && prefix_terminal_from() >= MIDQ_PZ_CUT {
            // Restore the tail ABI's zero termination counter from actual q.
            prefix_popcount::from_quotient(c, counter, qq, true);
        }
        if payload_sign_loan::compact_padding() {
            if let Some(carry) = borrowed_carry.take() { c.zero_and_free(carry); }
        }
...
pub(crate) fn shrunken_pz_invert_backward(
    ...
    if let Some(state) = tail_state {
        ...
        if prefix_popcount::enabled(counter) && prefix_terminal_from() >= MIDQ_PZ_CUT {
            // Tail replay restores the same zero counter and quotient first.
            prefix_popcount::from_quotient(c, counter, qq, false);
        }
        if payload_sign_loan::compact_padding() && lowq_borrow_passenger_carry_enabled()
            && borrowed_carry.is_none() {
            *borrowed_carry = Some(c.alloc_qreg("midq.restored.passenger_carry"));
        }
        if off.is_none() {
    ...
    for i in (0..steps).rev() {
        ...
        if i == prefix_terminal_from() && prefix_popcount::enabled(counter) {
            // SPIKE: re-create the popcount cache (C += popcount(q)) for rows < FROM.
            prefix_popcount::from_quotient(c, counter, qq, false);
        }
        if i > 0 {
...
// Packed prefix: the cut is selected at COMPILE time by MIDQ_P0_CUT=384|480|530
// (cargo tracks option_env!, so a changed value rebuilds). 530 = no tail, THE
// point (866 q) and the branch default since the driver landed (run with
// MIDQ_PZ_PINGPONG_TAIL=0); 480 uses the regenerated 62-round table below; 384
// is the shipped 974 configuration (the cut-384 correctness milestone).
const fn p0_parse(s: &str) -> usize {
    let b = s.as_bytes();
    let mut i = 0;
    let mut v = 0;
    while i < b.len() {
...
const fn p0_cut() -> usize {
    match option_env!("MIDQ_P0_CUT") {
        Some(s) => p0_parse(s),
        None => 530,
    }
}
pub(crate) const MIDQ_PZ_CUT: usize = p0_cut();
pub(crate) const MIDQ_TAIL_ROUNDS: usize = if MIDQ_PZ_CUT == 480 { 62 } else { 186 };
#[path = "midq_tail_checkpoint.rs"]
pub(crate) mod midq_tail_checkpoint;

#[path = "checkpoint_cap_aware.rs"]
pub(crate) mod checkpoint_cap_aware;
#[path = "payload_value_replay.rs"]
mod payload_value_replay;
// The 73-bit handoff values have ctz in 0..=73, including the all-zero
// sentinel returned by bit_length_ctz_inplace, so seven bits are exact.
const MIDQ_CTZ_BITS: usize = 7;

// Maximal signed value width at each tail boundary for the 384-step prefix,
// made monotone, from a 1,000,000-input draw (seed 2) of the classical mirror
// tools/midq_tail_width_model.py. Rounds 0..39 carry no support-tuning bit:
// the draw's maximum there is a single ~1e-6 input, and one bit at those rounds
// is two qubits at the metadata-pack moment (tools/tail_width_tail_prob.py:
// exceedance 1-2e-6 per input). A 200,000-input draw (seed 1) contained one
// input needing 194 rounds; the support model prices it as a tail miss. The final floor leaves
// enough sign extension for the stable signed-unit orbit.
pub(crate) const MIDQ_TAIL_VALUE_WIDTH: [u8; MIDQ_TAIL_ROUNDS + 1] = p0_table();
const fn p0_table() -> [u8; MIDQ_TAIL_ROUNDS + 1] {
    let src: &[u8] = if MIDQ_PZ_CUT == 480 { &MIDQ_TAIL_VALUE_WIDTH_480 } else { &MIDQ_TAIL_VALUE_WIDTH_384 };
    let mut out = [0u8; MIDQ_TAIL_ROUNDS + 1];
    let mut i = 0;
    while i < out.len() {
        out[i] = src[i];
        i += 1;
...
// Cut 480 / 62 rounds: tools/midq_tail_width_model.py 480 auto 1000000 2 20
// (terminal_at_cut 977340/1M, draining 3449/1M, max unsigned bl at handoff 27).
// Same convention as the 384 table: no +1 tuning bit on rounds 0..39, then the
// monotone envelope (raw+1 from round 40 on; see the 384 comment above).
const MIDQ_TAIL_VALUE_WIDTH_480: [u8; 63] = [
    25,25,24,24,24,24,23,23,22,22,21,21,20,20,20,20,20,20,19,19,18,16,16,15,15,14,14,14,
    13,13,13,12,12,11,11,10,10,10,10,10,10,10,9,9,9,9,9,9,8,7,7,7,7,7,7,6,
    6,5,4,4,4,4,4,
];
const MIDQ_TAIL_VALUE_WIDTH_384: [u8; 187] = [
    73,73,72,72,72,71,71,71,71,70,70,70,70,70,69,69,68,68,68,67,67,66,66,66,66,65,65,64,
    64,63,63,63,61,61,61,60,60,59,59,59,59,58,58,58,57,57,57,57,56,56,56,56,56,55,55,54,
    53,53,52,52,52,52,51,51,51,51,50,50,50,50,49,49,48,48,48,48,48,48,48,48,47,47,47,47,
    46,46,45,45,44,44,44,43,43,42,42,42,42,41,41,40,39,38,37,37,36,36,35,35,35,34,34,33,
    32,32,32,32,31,31,31,31,31,29,29,29,29,29,28,28,27,27,26,26,26,26,26,25,25,24,23,23,
    23,23,22,22,21,21,21,19,19,18,18,17,17,16,15,15,15,15,14,14,13,13,13,12,12,11,11,10,
    10,10,9,9,8,8,7,7,7,6,6,5,5,5,4,4,4,4,4,
];

pub(crate) fn midq_handoff_q_bits() -> usize {
    std::env::var("MIDQ_HANDOFF_Q_BITS")
        .ok()
        .and_then(|s| s.parse::<usize>().ok())
        .unwrap_or(18)
        .max(1)
        .min(18) // tail_metadata_codec packs the quotient code as qtag=0..18
}
...
fn midq_toffoli_since(c: &Circuit, start: usize) -> usize {
    if c.b.ops.len() <= start {
        return 0; // count-only mode stores no ops; trace-only value
    }
    c.b.ops[start..]
        .iter()
        .filter(|op| matches!(op.kind, OperationType::CCX | OperationType::CCZ))
...
fn midq_constant_update(
    ...
    if checkpoint_cap_aware::try_update(c, control, target, value, dirty, subtract) { return; }
...
fn midq_rotated_halves() -> bool {
    std::env::var("MIDQ_ROTATED_HALVES").ok().as_deref() == Some("1")
}

fn midq_rotated_halve(c: &mut Circuit, target: &[QReg], dirty: &[QReg]) {
    for i in 0..255 {
        c.b.swap(QubitId(target[i].id().into()), QubitId(target[i + 1].id().into()));
    }
    // Rotating places the old parity in bit255. The correction stays below it.
    let half_f_minus_one = [0xe8, 0x01, 0x00, 0x80];
    if !cell_folds::try_constant_update(c, &target[255], &target[..255], &half_f_minus_one, true) {
        midq_constant_update(c, &target[255], &target[..255], &half_f_minus_one, dirty, true);
    }
}

fn midq_rotated_double(c: &mut Circuit, target: &[QReg], dirty: &[QReg]) {
    for i in (0..255).rev() {
        c.b.swap(QubitId(target[i].id().into()), QubitId(target[i + 1].id().into()));
    }
    // Rotation installs the carry's unit contribution; add only the even rest.
    let half_f_minus_one = [0xe8, 0x01, 0x00, 0x80];
    if !cell_folds::try_constant_update(c, &target[0], &target[1..256], &half_f_minus_one, false) {
        midq_constant_update(c, &target[0], &target[1..256], &half_f_minus_one, dirty, false);
    }
}

fn midq_mod_halve_fast(c: &mut Circuit, target: &[QReg], dirty: &[QReg]) {
    if midq_rotated_halves() { return midq_rotated_halve(c, target, dirty); }
...
fn midq_mod_double_fast(c: &mut Circuit, target: &[QReg], dirty: &[QReg]) {
    if midq_rotated_halves() { return midq_rotated_double(c, target, dirty); }
...
pub(crate) struct MidqTailState {
    ...
    inplace_endpoint_signs: bool,
    odd_values: bool,
    six_bit_checkpoint: bool,
    payload_replay_start: Option<usize>,
...
fn midq_tail_forward_with_parity(
    ...
    if trace {
        ...
        if std::env::var_os("MIDQ_DUMP_SCHED").is_some() {
            for i in 0..MIDQ_PZ_CUT {
                let (wa, wb, wca, wcb, wq) = super::shrunken_pz_schedule::reg_widths(i);
                eprintln!("MIDQ_SCHED step={i} raw=[{wa},{wb},{wca},{wcb},{wq}] q_eff={} work={}", trailmix_q_width_step(wq, wa, wb, wca, wcb), 2*trailmix_ab_width(wa.max(wb)) + 2*trailmix_cacb_width(wca.max(wcb)) + trailmix_q_width_step(wq, wa, wb, wca, wcb));
            }
        }
    }
    ...
    // The pending quotient at the cut is a rare-tail quantity: its schedule
    // envelope (25 bits at cut 388) is a max over samples, while a pending
    // quotient of k or more bits occurs with probability about 2^-k per
    // shot. The handoff truncates it to MIDQ_HANDOFF_Q_BITS (default 18):
    // the dropped wires are |0> unless the pending quotient is that large,
    // which the support model counts as a miss (tail_value_misses). The
    // codec recovers q at exactly this width on the reverse path.
    ...
    // SPIKE (env-gated MIDQ_TERMINAL_A_ONLY=1): after the flush, A==0 alone means
    // the EEA finished (terminal, q==0) OR is still draining (q!=0): in both
    // cases ca'=p and B=1, so the same terminal map yields the unit orbit. The
    // quotient code must then be extracted BEFORE the map (it reads ca'/cb).
    let a_only = std::env::var("MIDQ_TERMINAL_A_ONLY").ok().as_deref() == Some("1");
    let mut counter_terminal = (counter_tape::enabled() && !a_only).then(|| {
    ...
    if counter_tape::enabled() && a_only {
        assert_eq!(counter.len(), counter_tape::BITS, "counter/tape codec requires all eight counter bits");
        counter_terminal = Some(counter_tape::prepare(c, a, ca, cb, &[]));
    }
    ...
    let odd_values = midq_odd_values::enabled() && taped_rounds > counter_tape::BITS;
    let six_bit_checkpoint = checkpoint && odd_values && midq_narrow_coefficients()
        && midq_tail_checkpoint::inplace_endpoints() && midq_tail_checkpoint::six_bits_enabled();
    ...
    for round in 0..taped_rounds {
        ...
        if odd_values && round == counter_tape::BITS {
            midq_odd_values::compress(c, a);
            midq_odd_values::compress(c, b);
        }
        let compressed = odd_values && round >= counter_tape::BITS;
        if compressed {
        ...
        if round % 2 == 0 {
            if compressed {
                midq_odd_values::forward(c, a, b, logical, next_width, midq_value_vents());
            } else if shared {
            ...
            let odd_lows = (!compressed).then(|| midq_loan_odd_low_bits(c, a, b));
            if compressed && midq_odd_values::padding_enabled() {
                midq_odd_values::with_forward_padding(c, a, b, width, next_width,
                    Some(MIDQ_TAIL_VALUE_WIDTH[round - 1] as usize),
                    |c| midq_mod_signed_add_halve(c, cb, ca, logical, false));
            } else { midq_mod_signed_add_halve(c, cb, ca, logical, false); }
            if let Some(lows) = odd_lows { midq_restore_odd_low_bits(c, a, b, lows); }
            ...
            if compressed {
                midq_odd_values::forward(c, b, a, logical, next_width, midq_value_vents());
            } else if shared {
            ...
            let odd_lows = (!compressed).then(|| midq_loan_odd_low_bits(c, a, b));
            if compressed && midq_odd_values::padding_enabled() {
                midq_odd_values::with_forward_padding(c, b, a, width, next_width,
                    Some(MIDQ_TAIL_VALUE_WIDTH[round - 1] as usize),
                    |c| midq_mod_signed_add_halve(c, ca, cb, logical, false));
            } else { midq_mod_signed_add_halve(c, ca, cb, logical, false); }
            if let Some(lows) = odd_lows { midq_restore_odd_low_bits(c, a, b, lows); }
            if let Some(previous) = coefficient_section {
                c.pop_section(&previous);
            }
    ...
    if odd_values && !six_bit_checkpoint {
        midq_odd_values::expand(c, a);
        midq_odd_values::expand(c, b);
    }

    if checkpoint {
        let start = c.b.current_ops_len();
        let previous = c.push_section("midq.tail.forward.checkpoint");
        if six_bit_checkpoint { midq_tail_checkpoint::encoded_rounds(c, a, b, ca, cb, false); }
        else { midq_tail_checkpoint::forward(c, a, b, ca, cb); }
        c.pop_section(&previous);
    ...
    let inplace_endpoint_signs = checkpoint && midq_narrow_coefficients()
        && midq_tail_checkpoint::inplace_endpoints();
    if inplace_endpoint_signs && !six_bit_checkpoint { midq_tail_checkpoint::park_selectors(c, a, b); }
    ...
    if inplace_endpoint_signs {
        midq_tail_checkpoint::with_endpoint_sign(c, a, b, 0, |c, sign| midq_field_neg(c, sign, ca, cb));
        midq_tail_checkpoint::with_endpoint_sign(c, a, b, 1, |c, sign| midq_field_neg(c, sign, cb, ca));
    } else {
    ...
    let parked_checkpoint_selectors = inplace_endpoint_signs || (checkpoint
        && std::env::var("MIDQ_PARK_CHECKPOINT_SELECTORS").ok().as_deref() == Some("1"));
    if parked_checkpoint_selectors && !inplace_endpoint_signs {
    ...
    // Reconstruct an earlier value checkpoint only after the duplicate
    // coefficient has been discharged. This saves payload storage; it does
    // not reduce the original forward-tail live-set maximum.
    let payload_replay_start = payload_value_replay::configured_start();
    if let Some(start) = payload_replay_start {
        assert!(six_bit_checkpoint && inplace_endpoint_signs,
            "payload replay requires the six-bit odd checkpoint layout");
        payload_value_replay::pack(c, a, b, &mut tape, start);
    }
    ...
    MidqTailState {
        ...
        inplace_endpoint_signs,
        odd_values,
        six_bit_checkpoint,
        payload_replay_start,
        quotient_code,
        ctz,
        ctz_select_a,
        packed_metadata,
...
fn midq_tail_backward_with_parity(
    ...
    // Recreate the original checkpoint and sign tape before recreating the
    // second coefficient. The standard endpoint/cell inverse then sees its
    // original representation and retains its original operation ordering.
    if let Some(start) = payload_replay_start {
        payload_value_replay::unpack(c, a, b, &mut tape, start);
    }
    ...
    *cb = c.alloc_qreg_bits("midq.cb", ca.len());
    ...
    if inplace_endpoint_signs {
        midq_tail_checkpoint::with_endpoint_sign(c, a, b, 1, |c, sign| midq_field_neg(c, sign, cb, ca));
        midq_tail_checkpoint::with_endpoint_sign(c, a, b, 0, |c, sign| midq_field_neg(c, sign, ca, cb));
        // Payload preserves ca. Undoing its endpoint negation recovers the
        // exact zero high bit appended before that negation, even when the
        // normalized endpoint itself had a meaningful high bit.
        shrunken_pz_resize(c, ca, 256, "midq.ca");
        if !six_bit_checkpoint { midq_tail_checkpoint::restore_selectors(c, a, b); }
    } else {
        let a_sign = if checkpoint { &a[0] } else { a.last().expect("tail a has a sign bit") };
        let b_sign = if checkpoint { &b[0] } else { b.last().expect("tail b has a sign bit") };
        midq_field_neg(c, b_sign, cb, ca);
    ...
    let taped_rounds = if checkpoint {
        ...
        if six_bit_checkpoint { midq_tail_checkpoint::encoded_rounds(c, a, b, ca, cb, true); }
        else { midq_tail_checkpoint::backward(c, a, b, ca, cb); }
    ...
    for round in (0..taped_rounds).rev() {
        ...
        if compressed && midq_odd_values::padding_enabled()
            && MIDQ_TAIL_VALUE_WIDTH[round] == MIDQ_TAIL_VALUE_WIDTH[round + 1] {
            let target = if round % 2 == 0 { &mut *b } else { &mut *a };
            midq_odd_values::prepare_backward_target(c, target, MIDQ_TAIL_VALUE_WIDTH[round] as usize);
        }
        let odd_lows = (!compressed).then(|| midq_loan_odd_low_bits(c, a, b));
        if round % 2 == 0 {
            ...
            if compressed {
                midq_odd_values::backward(c, a, b, sign, MIDQ_TAIL_VALUE_WIDTH[round] as usize, midq_value_vents());
            } else { midq_value_round_backward(
                c,
                a,
                b,
                sign,
            ...
            let value_section = trace.then(|| c.push_section("midq.tail.backward.value"));
            if compressed {
                midq_odd_values::backward(c, b, a, sign, MIDQ_TAIL_VALUE_WIDTH[round] as usize, midq_value_vents());
            } else { midq_value_round_backward(
                c,
                b,
                a,
        ...
        if compressed && round == counter_tape::BITS {
            midq_odd_values::expand(c, a);
            midq_odd_values::expand(c, b);
        }
    ...
    let a_only = std::env::var("MIDQ_TERMINAL_A_ONLY").ok().as_deref() == Some("1");
    if a_only {
        if let Some(terminal) = counter_terminal.take() {
            counter_tape::restore(c, a, ca, cb, &[], terminal);
        }
    }
...
pub fn shrunken_pz_divide_forward(
    ...
    if packed::p0_shape::enabled() {
        // P0 shape build (tools/spike/packed_design.md section 10): packed rings.
        return packed::p0_shape::divide_forward(c, dx, dy);
    }
    ...
    if let Some(tail_state) = tail_state {
        ...
        midq_field_neg(c, payload_sign_loan::control(&dy, &payload_sign), &lambda, &cca);
        ...
        // A loaned high bit was known zero; it has no phase obligation. Never
        // ghost the sign tag: finish() has already removed it from dy.
...
pub fn shrunken_pz_divide_cancel(
    ...
    use crate::point_add::trailmix_port::inversion::shrunken_pz_schedule::reg_widths;
    if packed::p0_shape::enabled() {
        return packed::p0_shape::divide_cancel(c, dx, dy, lambda);
    }
    assert_eq!(dx.len(), 257);
    assert_eq!(dy.len(), 257);
    ...
    if let Some(tail_state) = tail_state {
        ...
        midq_field_neg(c, payload_sign_loan::control(&dy, &payload_sign), &temp, &cca);
        ...
        if sign_loaned && !compact_passenger {
            // Recreate canonical padding with a fresh allocation, never by
            // reacquiring the former high-bit ID (which may now hold payload).
