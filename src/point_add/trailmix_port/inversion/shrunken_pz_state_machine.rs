...
fn prefix_no_terminal_eligible(step: usize) -> bool {
    if std::env::var("MIDQ_PREFIX_NO_TERMINAL").ok().as_deref() != Some("1")
        || !midq_tail_enabled() || MIDQ_PZ_CUT != 370 || step >= MIDQ_PZ_CUT
    { return false; }
...
pub(crate) const MIDQ_PZ_CUT: usize = 370;
pub(crate) const MIDQ_TAIL_ROUNDS: usize = 213;
...
// The 81-bit handoff values have ctz in 0..=81, including the all-zero
// sentinel returned by bit_length_ctz_inplace, so seven bits are exact.
const MIDQ_CTZ_BITS: usize = 7;

// Maximal signed value width at each tail boundary for the 370-step prefix,
// widened by one support-tuning bit and made monotone: the elementwise max of
// two independent draws (200,000 inputs, seed 1; 1,000,000 inputs, seed 2) of
// the classical mirror tools/midq_tail_width_model.py. The final floor leaves
// enough sign extension for the stable signed-unit orbit.
pub(crate) const MIDQ_TAIL_VALUE_WIDTH: [u8; MIDQ_TAIL_ROUNDS + 1] = [
    81,81,80,80,79,79,79,79,78,78,77,77,77,77,76,76,75,75,75,74,74,74,73,73,73,73,72,71,
    71,71,71,71,71,70,70,69,68,68,68,67,67,67,66,66,66,66,64,64,64,64,63,62,62,61,61,61,
    61,61,61,60,60,59,59,58,58,58,58,57,57,56,56,56,55,55,55,55,55,53,53,53,53,53,52,52,
    51,51,51,50,50,49,49,48,48,47,47,46,46,46,46,46,45,45,44,43,43,43,42,42,41,41,41,41,
    41,40,40,40,40,40,39,39,38,38,37,37,36,36,36,35,35,34,33,33,33,32,32,32,32,31,31,31,
    31,30,29,29,29,29,29,29,29,28,28,27,26,25,25,25,25,25,25,24,24,23,23,22,22,21,21,21,
    20,20,18,18,18,17,17,17,17,16,15,15,15,14,14,13,13,13,13,13,13,12,12,11,11,10,10,10,
    10,10,10,9,9,8,7,7,6,6,5,5,5,4,4,4,4,4,
];
