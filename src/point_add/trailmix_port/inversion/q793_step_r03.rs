...
/// Rank-grouped exact dirty echo of a term list (all terms toggle `target`; product() already merged literals).
fn toggle_terms_echo(circ:&mut Circuit,terms:&Terms<'_>,rank:&[QReg],target:&QReg,dirty:&[QReg]){
    if !super::metadata_muxlease::active("Q793_RANK_ECHO_STEP"){toggle_terms(circ,terms,target,dirty);return;}
    super::q793_rank_echo::grouped(circ,"Q793_RANK_ECHO_STEP",terms,rank,target,dirty);
}
...
pub(super) fn step(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,iteration:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],post_j:usize,block:usize){
    ...
    toggle_terms_echo(circ,&c1_t10,rank,sign,dirty);
    ...
    if !to_c1.is_empty(){toggle_terms_echo(circ,&to_c1,rank,sign,dirty);super::q793_cargo_r02::inbound(circ,rank,a,w1,w2,0,&rflag,dirty);toggle_terms_echo(circ,&to_c1,rank,sign,dirty);}
    if !to_head.is_empty(){toggle_terms_echo(circ,&to_head,rank,sign,dirty);super::q793_cargo_r02::head_to_two(circ,rank,a,c,w1,w2,&rflag,dirty);toggle_terms_echo(circ,&to_head,rank,sign,dirty);}
