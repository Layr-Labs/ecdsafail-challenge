...
#[path="q793_residual_hold_r01.rs"] mod residual_hold;
#[path="q793_clock_windows_r01.rs"] mod clock_windows;
#[path="q793_residual_hold_check_r01.rs"] pub mod residual_hold_check;
...
pub(super) fn step(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,iteration:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],post_j:usize,block:usize){
    ...
    let (rfirst,tend,lo,hi)=clock_windows::get(block,post_j);let previous=circ.q797_a_support.replace((lo,hi));
    ...
    residual_hold::emit(circ,rank,a,c,sm,p1,p2,sign,w1,w2,dirty,post_j);
