...
where F:FnOnce(&mut Circuit,&[QReg]){
    ...
    let key=key.unwrap_or_default();let swap=super::metadata_muxlease::swap_offer(truth,5,0);
    let rank_cost=swap.chart_t();let consume_cost=mcx_cost(key.len()+1);let restored_pair=2*direct.min(2*rank_cost+2*consume_cost);let held=2*rank_cost+4*consume_cost;
