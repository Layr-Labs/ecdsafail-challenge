...
fn toggle_pair_around<F>(circ:&mut Circuit,terms:&Terms<'_>,rank:&[QReg],target:&QReg,dirty:&[QReg],body:F)
where F:FnOnce(&mut Circuit,&[QReg]){
    if !super::metadata_muxlease::active("Q793_C1_HOLD")||dirty.len()<3{
...
pub(super) fn step(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,iteration:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],post_j:usize,block:usize){
    ...
    toggle_pair_around(circ,&c1_t10,rank,sign,dirty,|circ,lenders|super::q793_cargo_r02::inbound(circ,rank,a,w1,w2,2,&tflag,lenders));
    ...
    if !to_c1.is_empty(){toggle_terms(circ,&to_c1,rank,sign,dirty);super::q793_cargo_r02::inbound(circ,rank,a,w1,w2,0,&rflag,dirty);toggle_terms(circ,&to_c1,rank,sign,dirty);}
