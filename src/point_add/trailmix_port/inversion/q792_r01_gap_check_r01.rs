...
pub(super) fn run(){
    ...
    for j in 0..4{
        ...
        let p1=circ.alloc_qreg("p1");let p2=circ.alloc_qreg("p2");let w1=circ.alloc_qreg_bits("w1",259);let w2=circ.alloc_qreg_bits("w2",259);let dirty=circ.alloc_qreg_bits("borrowed",23);let pass=circ.alloc_qreg("own-third-passenger");let owned=circ.b.next_qubit;
        super::gap_probe(&mut circ,&rank,&a,&c,&sm,&p1,&p2,&w1,&w2,&dirty,&pass,j,259);assert_eq!(circ.b.next_qubit,owned);
        let builder=circ.into_builder();for op in &builder.ops{op.validate();assert!(matches!(op.kind,K::X|K::CX|K::CCX));}
        let nt=builder.ops.iter().filter(|op|op.kind==K::CCX).count();eprintln!("Q792_R01_CGE2_THIRD_GAP_BUILT j={j} ops={} T={nt} interfaceQ={owned}",builder.ops.len());
        ...
        for pattern in 0..4{for batch in 0..rows.len().div_ceil(64){
            ...
            for lane in 0..64{
                ...
                if cv>1{assert!(!val(&before,&w2[av+3],lane),"gap premise A={av} C={cv} S={sv}");}
            ...
            if sim.qubits!=after{let diffs:Vec<_>=sim.qubits.iter().zip(&after).enumerate().filter(|(_, (x,y))|x!=y).map(|(i,(x,y))|(i,format!("{:016x}",x^y))).collect();panic!("Q792_R01_CGE2_THIRD_GAP active j={j} pattern={pattern} batch={batch} diffs={diffs:?}");}
        ...
        eprintln!("Q792_R01_CGE2_THIRD_GAP_CLOCK_PASS j={j} active={active_here}");
    }
    ...
    eprintln!("Q792_R01_CGE2_THIRD_GAP_PASS active={total} offguard={off}; actual rank5 ranges, A0/A1, two independent cargo bits, q1/q2 gather, three physical holes, packed width2/1 endpoints, normalized b2 HA restored, literal inverse/allphase; no whole-circuit claim");
}
