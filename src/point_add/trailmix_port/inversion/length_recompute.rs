...
/// Q795 route: one always-clean scratch qubit dedicated to multi-controlled predicates.
/// While set, every k>=3 predicate uses paired_clean_mcx::toggle (2k-3 Toffoli, scratch and all
/// controls restored, no relative phase) instead of the dirty-lender ladder (4k-8).
thread_local! { static CLEAN_SCRATCH: std::cell::RefCell<Option<QReg>> = const { std::cell::RefCell::new(None) }; }
/// Analytic T-word upper support of the current scheduled block (shared_step::SCHEDULE_SUPPORTS.1).
/// Installed per template build; consumers prune address leaves that the support makes unreachable.
...
pub(super) fn t_top() -> Option<usize> { T_TOP.with(|t| t.get()) }
/// Analytic R-word lower support of the current block (SCHEDULE_SUPPORTS.0): the residual occupies
/// positions [rfirst,259), so its length C is at most 259-rfirst on support.
thread_local! { static R_FIRST: std::cell::Cell<Option<usize>> = const { std::cell::Cell::new(None) }; }
pub(super) struct RFirstGuard;
impl RFirstGuard {
    pub(super) fn set(rfirst: usize) -> Self { R_FIRST.with(|t| { assert!(t.get().is_none(), "nested R-first support"); t.set(Some(rfirst)); }); RFirstGuard }
}
impl Drop for RFirstGuard { fn drop(&mut self) { R_FIRST.with(|t| t.set(None)); } }
pub(super) fn r_first() -> Option<usize> { R_FIRST.with(|t| t.get()) }
/// Largest residual length C reachable on support, with a two-value margin; usize::MAX when unset.
pub(super) fn c_max() -> usize { r_first().map(|r| 259usize.saturating_sub(r) + 2).unwrap_or(usize::MAX) }
pub(super) struct CleanScratchGuard;
impl CleanScratchGuard {
    pub(super) fn set(scratch: &QReg) -> Self {
        CLEAN_SCRATCH.with(|s| { assert!(s.borrow().is_none(), "nested clean scratch"); *s.borrow_mut() = Some(scratch.borrowed_alias()); });
        CleanScratchGuard
    }
}
impl Drop for CleanScratchGuard { fn drop(&mut self) { CLEAN_SCRATCH.with(|s| { *s.borrow_mut() = None; }); } }
pub(crate) fn clean_scratch() -> Option<QReg> { CLEAN_SCRATCH.with(|s| s.borrow().as_ref().map(QReg::borrowed_alias)) }
...
pub(super) fn mixed_mcx(circ: &mut Circuit, controls: &[(&QReg,bool)], target: &QReg, helpers: &[QReg]) {
    if controls.len() >= 3 { if let Some(scratch) = clean_scratch() {
