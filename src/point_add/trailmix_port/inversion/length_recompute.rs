...
pub(super) fn mixed_mcx(circ: &mut Circuit, controls: &[(&QReg,bool)], target: &QReg, helpers: &[QReg]) {
    let fold_capture=super::q792_passive_r01::begin_gate(circ,controls,target);
    let rank_capture=super::q792_rank4_lower_r01::exact_begin(circ,controls,target);
    ...
    super::q792_passive_r01::end_gate(circ,fold_capture,controls,target);
    super::q792_rank4_lower_r01::exact_end(circ,rank_capture,controls,target);
}
