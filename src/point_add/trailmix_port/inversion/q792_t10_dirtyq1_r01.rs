...
thread_local!{static DIRTY_Q1:std::cell::RefCell<Option<(u32,Vec<crate::circuit::Op>)>>=const{std::cell::RefCell::new(None)};}
thread_local!{static COLLECT:std::cell::RefCell<Option<Vec<(u32,Vec<Vec<(u32,bool)>>)>>> = const{std::cell::RefCell::new(None)};}
fn gates(circ:&mut Circuit,p:Poly<'_>,target:&QReg,dirty:&[QReg],g:&QReg,scratch:&QReg,metadata:&[u32]){
    if COLLECT.with(|v|{let mut v=v.borrow_mut();if let Some(v)=&mut *v{v.push((target.id(),p.iter().map(|term|term.iter().map(|(q,b)|(q.id(),*b)).collect()).collect()));true}else{false}}){return;}
    let state=DIRTY_Q1.with(|x|x.borrow().clone());
    let mut correction=Vec::new();if let Some((id,ops))=&state{
        assert_ne!(target.id(),*id);assert!(ops.iter().all(|o|o.q_target.0!=target.id()as u64),"q1 oracle depends on polynomial target");
        for term in &p{if term.iter().any(|(q,_)|q.id()==*id){let mut t:Vec<_>=term.iter().copied().filter(|(q,_)|q.id()!=*id).collect();let q=term.iter().find(|(q,_)|q.id()==*id).unwrap().0;t.push((q,true));correction.push(t);}}
    }
    gates_impl(circ,p,target,dirty,g,scratch,metadata);
    if !correction.is_empty(){let(_,ops)=state.unwrap();circ.b.ops.extend(ops.iter().rev().copied());gates_impl(circ,correction,target,dirty,g,scratch,metadata);circ.b.ops.extend(ops);}
}
fn gates_impl(circ:&mut Circuit,p:Poly<'_>,target:&QReg,dirty:&[QReg],g:&QReg,scratch:&QReg,metadata:&[u32]){
...
pub(super) fn emit(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,g:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,inverse:bool,last:bool){
    assert!(helpers.len()>=22);let start=circ.b.ops.len();let h=&sm[3];let q=&helpers[..3];let dirty=&helpers[3..];let scratch=&sm[0];let p2=&q[1];let metadata:Vec<_>=rank.iter().chain(a).chain(c).chain(&sm[1..3]).chain(std::iter::once(g)).map(QReg::id).collect();
    ...
    let q1_oracle;let readout_start=circ.b.ops.len();circ.cx(g,p1);
    if last{
        gates(circ,mul(&wire(g),&replace(wire(&w1[1]),aa0.clone(),false)),p1,dirty,g,scratch,&metadata);
        let q1_start=circ.b.ops.len();gates(circ,mul(&wire(g),&replace(wire(&w1[2]),xor(aa0.clone(),aa1.clone()),false)),p2,dirty,g,scratch,&metadata);q1_oracle=circ.b.ops[q1_start..].to_vec();
    }else{
        ...
        let q1_start=circ.b.ops.len();let q1cond=mul(&vec![vec![(g,true),(&sm[2],false)]],&xor(one(),cc[2].clone()));
        super::q793_t10_routes_v10::digit_xor(circ,rank,a,c,w1,p2,dirty,0,&q1cond,g,scratch);
        gates(circ,mul(&wire(g),&cc[2]),p2,dirty,g,scratch,&metadata);q1_oracle=circ.b.ops[q1_start..].to_vec();
    ...
    DIRTY_Q1.with(|v|{assert!(v.borrow().is_none());*v.borrow_mut()=Some((p2.id(),q1_oracle));});
    COLLECT.with(|v|{assert!(v.borrow().is_none());*v.borrow_mut()=Some(Vec::new());});
    ...
    // Regimes are disjoint: (shift,C0) and one-hot M255/M256/normal.
    // Inside a regime all h writes precede all b2 writes. Across regimes,
    // exact g-guarded target XORs commute because their selectors are stable
    // and disjoint. Gather all h polynomials, then each b2 polynomial; this
    // pays the stable W1-only q1 readout once per physical output target.
    let pending=COLLECT.with(|v|v.borrow_mut().take().unwrap());let mut grouped:Vec<(u32,Vec<Vec<(u32,bool)>>)>=Vec::new();
    for(target,terms)in pending{if let Some((_,p))=grouped.iter_mut().find(|(id,_)|*id==target){p.extend(terms);}else{grouped.push((target,terms));}}
    assert!(grouped.first().is_none_or(|(id,_)|*id==h.id()));
    let refs:Vec<_>=rank.iter().chain(a).chain(c).chain(sm).chain(w1).chain(w2).chain(helpers).chain([p1,p2,g]).collect();
    for(target,terms)in grouped{let lookup=|id:u32|->&QReg{*refs.iter().find(|q|q.id()==id).expect("codec collected operand")};let poly=terms.into_iter().map(|t|t.into_iter().map(|(id,b)|(lookup(id),b)).collect()).collect();gates(circ,poly,lookup(target),dirty,g,scratch,&metadata);}
    DIRTY_Q1.with(|v|*v.borrow_mut()=None);let poly_end=circ.b.ops.len();
