...
thread_local!{static T_PRIORITY:std::cell::Cell<bool>=const{std::cell::Cell::new(false)};}
pub(super) fn t_priority()->bool{T_PRIORITY.with(|x|x.get())}
pub(super) fn table(c:&mut Circuit,word:&[&QReg],truth:Vec<bool>,prefix:&[(&QReg,bool)],out:&QReg,dirty:&[QReg]){
 let at=c.b.ops.len();table_inner(c,word,truth.clone(),prefix,out,dirty);
 if super::q792_esop_r01::active()&&!super::q792_passive_r01::active()&&!super::q792_rank4_lower_r01::capture_active()&&!t_priority(){
  let old=c.b.ops.split_off(at);
  T_PRIORITY.with(|x|x.set(true));table_inner(c,word,truth.clone(),prefix,out,dirty);T_PRIORITY.with(|x|x.set(false));
  let ccx=|ops:&[crate::circuit::Op]|ops.iter().filter(|o|o.kind==crate::circuit::OperationType::CCX).count();
  if ccx(&c.b.ops[at..])>=ccx(&old)||c.b.ops.len()-at>old.len()+old.len()/10+8{c.b.ops.truncate(at);c.b.ops.extend(old);}
 }
 super::q792_shared_capture_r01::record(c,word,&truth,prefix,out,at);
}
fn table_inner(c:&mut Circuit,word:&[&QReg],truth:Vec<bool>,prefix:&[(&QReg,bool)],out:&QReg,dirty:&[QReg]){
    ...
    for producer in 0..2{
        ...
        if (t_priority()||candidate.len()<=best.len()) && score(&candidate)<score(&best){best=candidate;}
    }
    c.b.ops.extend(best);
}
fn table_direct(c:&mut Circuit,word:&[&QReg],truth:Vec<bool>,prefix:&[(&QReg,bool)],out:&QReg,dirty:&[QReg]){
    ...
    if let Some(mut cubes)=super::q792_esop_r01::plan(word.len(),&truth,prefix.len()){
        ...
        let keep=if t_priority(){
            let score=|ops:&[crate::circuit::Op]|(ops.iter().filter(|o|o.kind==crate::circuit::OperationType::CCX).count(),ops.len());
            score(&c.b.ops[start..])<score(&baseline)
        }else{c.b.ops.len()-start<baseline.len()};
        if !keep{c.b.ops.truncate(start);c.b.ops.extend(baseline);}
    ...
    super::q792_gpu_beam_r01::improve(c,word,&truth,prefix,out,dirty,start);
}
