...
pub(super) fn table(c:&mut Circuit,word:&[&QReg],truth:Vec<bool>,prefix:&[(&QReg,bool)],out:&QReg,dirty:&[QReg]){
 let at=c.b.ops.len();
 if super::q792_exact_decoder_r01::capture_calls(){
  super::q792_exact_decoder_r01::without(||super::q792_exact_esop_r01::without(||table_inner(c,word,truth.clone(),prefix,out,dirty)));let old=c.b.ops.split_off(at);
  table_inner(c,word,truth.clone(),prefix,out,dirty);super::q792_exact_decoder_r01::record_call(word.len(),prefix.len(),&truth,&old,&c.b.ops[at..]);
 }else if super::q792_exact_esop_r01::capture_calls(){
  super::q792_exact_esop_r01::without(||table_inner(c,word,truth.clone(),prefix,out,dirty));let old=c.b.ops.split_off(at);
  table_inner(c,word,truth.clone(),prefix,out,dirty);super::q792_exact_esop_r01::record_call(word.len(),prefix.len(),&truth,&old,&c.b.ops[at..]);
 }else{table_inner(c,word,truth.clone(),prefix,out,dirty);}
...
fn table_direct(c:&mut Circuit,word:&[&QReg],truth:Vec<bool>,prefix:&[(&QReg,bool)],out:&QReg,dirty:&[QReg]){
    ...
    super::q792_exact_esop_r01::improve(c,word,&truth,prefix,out,dirty,start);
    super::q792_exact_decoder_r01::improve(c,word,&truth,prefix,out,dirty,start);
}
